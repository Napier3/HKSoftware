 ...
 */
template<class Network, class Timer, int MAX_MQTT_PACKET_SIZE = 1050000, int MAX_MESSAGE_HANDLERS = 10>
class Client
{
...
public:
     ...
     */
    Client(Network& network, unsigned int command_timeout_ms = 5000);
...
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::readPacket(Timer& timer)
{
	...
	{
		//读取需要读取的长度
		nReadLen = ipstack.read(pBegin, rem_len-nTotalLen, 50);

		if (nReadLen > 0)
        {//读到数据，增加读取总长度，缓冲区向后移动
			nTotalLen += nReadLen;
			pBegin += nReadLen;
		}

		if (nTotalLen >= rem_len)
		{//全部数据读取完毕
			break;
		}
...
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::cycle(Timer& timer)
{
    ...
    switch (packet_type)
    {
        default:
            ...
            rc = SUCCESS; //if rcv==0  ,use success
            goto exit;
