...
CCfgDataMngrXmlRWKeys::CCfgDataMngrXmlRWKeys()
{
	...
	m_strCIecCfg92Version = L"cfg92-version";
}
...
void CCfgDataMngrConstGlobal::UpdataAppPortDataType_FT3Num(long nFT3Num)
{
	...
	{
		CString strFiberIndex, strFiberName, strFiberID, strPort = g_sLangTxt_Native_Port.GetString();
		for (int nIndex = pDataType->GetCount();nIndex<nFT3Num;nIndex++)
		{
			strFiberIndex.Format(_T("%d"),nIndex);
			strFiberName.Format(_T("FT3_%d"),(nIndex+1));
			strFiberName += strPort;//2024-9-10 wuxinyi 修改乱码
