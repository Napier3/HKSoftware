...
/* --------  ---  ------   -------------------------------------------	*/
/* 02/20/03  CRM     10    Fix printfs in readMmsOpEn, write_smemcfg	*/
/* 12/16/02  JRB     09    Chg mvl_defs.h to mvl_uca.h			*/
      ...
      if (strstr (lineBuf, mmsOpEnMap[i].str) != NULL)
        ...
        if (strstr (lineBuf, "REQ_RESP_EN") != NULL)
          ...
          *mmsOpEnMap[i].addr = RESP_EN;
          ...
          printf ("\n lineBuf = %s, mmsOpEnMap[i].str = %s", lineBuf, 
							    mmsOpEnMap[i].str);
          return (SD_FAILURE);
          }
        break;
