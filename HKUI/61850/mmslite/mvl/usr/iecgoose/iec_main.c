    ...
    for (scl_ln = (SCL_LN *) list_find_last ((DBL_LNK *) scl_ld->lnHead);
      ...
      for (scl_gcb = (SCL_GCB *) list_find_last ((DBL_LNK *) scl_ln->gcbHead);
        ...
        if (scl_gcb->isGoose)
          ...
          sprintf (gcRef, "%s/%s$GO$%s", scl_ld->domName, scl_ln->varName, scl_gcb->name);
          sprintf (datasetRef, "%s/%s$%s", scl_ld->domName, scl_ln->varName, scl_gcb->datSet);
          ...
          if (iecGoosePubCreate(
                ...
                scl_gcb->confRev,
                SD_TRUE,
                ETYPE_TCI_GOOSE,	/* TCI (fixed value for GOOSE)*/
                ETYPE_TYPE_GOOSE,	/* EtypeID (fixed value for GOOSE)*/
                (ST_UINT16) scl_gse->APPID)==NULL)
            {
