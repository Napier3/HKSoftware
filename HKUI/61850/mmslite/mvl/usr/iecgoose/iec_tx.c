...
ST_LONG retrans_goose (ST_LONG elapsed_msec)
{
  ...
  ST_CHAR goose_enc_buf[MAX_IEC_GOOSE_ENCODE_BUF];
  ...
  for (user_info = pub_ctrl.pub_list;
    ...
    if(user_info->goose_state == SD_GOOSE_IDLE)
      ...
      if( (user_info->retrans_timeout -= elapsed_msec)<=0) 
        ...
        if (user_info->retrans_curve_offset + 1 < user_info->curve_pointer->num_retrans)
          user_info->retrans_curve_offset++;
        user_info->retrans_timeout = user_info->curve_pointer->retrans[user_info->retrans_curve_offset];
...
ETYPE_INFO etype; /* Ethertype Info */
  ...
  user_info->curve_pointer = retrans_curve;
  user_info->retrans_timeout = retrans_curve->retrans[0];
...
IEC_GOOSE_SEND_USER_INFO *iecGoosePubCreate( ST_UCHAR *DstAddress,
          		 ...
          		 ST_CHAR *GooseRef,	/*must be persistent string*/
          		 ST_CHAR *DataSetRef,	/*must be persistent string*/

          		 ST_CHAR *AppID,	/*must be persistent string*/
          		 ST_ULONG ConfigRevNr,
  ...
  if((gptr = gse_iec_control_create (GooseRef, DataSetRef, AppID, numDataSetEntries))!=NULL)
    {
    gptr->confRev = ConfigRevNr;
...
GSE_IEC_CTRL *temp_gptr;
  ...
  start_trans_goose (goosehandle, &retrans_curve);
