...
ST_CHAR *token5;		/* 5th token	*/
  ...
  while (fgets (in_buf, sizeof(in_buf) - 1, in_fp) != NULL)
    ...
    else if (leaf && token3 && token4 && (token5==NULL))
      {		/* One parameter missing. Assume it is ItemName and adjust.*/
      strncpy_safe (map_entry->leaf,          leaf,   MAX_IDENT_LEN);
      strncpy_safe (map_entry->opcServerName, token3, MAX_IDENT_LEN);
      map_entry->opcItemName = "";	/* point to empty string.	*/
      strncpy_safe (map_entry->opcGroupName,  token4, MAX_IDENT_LEN);
      }
    else
      {
      SXLOG_ERR2 ("Invalid input at line %d in '%s'. Must contain 5 parameters.", line_num, datamap_cfg_filename);
...
RUNTIME_TYPE tmp_rt_type;
  ...
  if (strlen (comp_name))
    ...
    /* Allow extra char in these strncat_safe calls. If used, return error.*/
    strncat_safe (leaf_name, "$",       MAX_FLAT_LEN + 1);
    strncat_safe (leaf_name, comp_name, MAX_FLAT_LEN + 1);
  ...
  if (map_entry != NULL)
    ...
    if (fp_out_map)
#if defined (IEC_61850_SERVER_OPC_CLIENT)		/* customized	*/
      fprintf (fp_out_map, "%s\t%s\t\"%s\"\t\"%s\"\t\"%s\"\ttype=%s\n", 
               map_entry->ldevice,
               map_entry->leaf,
    ...
    if (strcmp (comp_name, "q") != 0			/* not "q"	*/
      ...
      if (fp_out_map)
        fprintf (fp_out_map, "%s\t%s\t%s\t%s\t%s\ttype=%s\n", 
		dom_name, leaf_name, "dummy_opcserver", "dummy_opcitem", "dummy_ioclass",
                tdl_buf);
      }
#else	/* !IEC_61850_SERVER_OPC_CLIENT	*/
      ...
      case RT_STR_START:
        ...
        if (strlen (comp_name))
          ...
          if (array_flag && rt_index == 0)
            ...
            {
            strncat_safe (branch_name, "$", MAX_FLAT_LEN);
            strncat_safe (branch_name, comp_name, MAX_FLAT_LEN);
            }
          }
#if defined(DATAMAP_USER_ENABLE)
...
ST_RTINT num_arr_elements;	/* num elements in array*/
