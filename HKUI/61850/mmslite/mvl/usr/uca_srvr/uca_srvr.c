...
/* goose_rx_mac: multicast addresses on which to receive packets.	*/
/* CRITICAL: the ALL-ES and ALL-IS addresses must be included for	*/
/* proper functioning of the ES-IS protocol of the OSI stack.		*/
static ST_UCHAR goose_rx_mac[] = {
		0x09, 0x00, 0x2B, 0x00, 0x00, 0x04,	/* ALL-ES	*/
		0x09, 0x00, 0x2B, 0x00, 0x00, 0x05,	/* ALL-IS	*/
		0x09, 0x09, 0x09, 0x09, 0x09, 0x09	/* my GOOSE MAC	*/
		};
...
#if defined (MVL_GOOSE_SUPPORT)
  clnp_read_thread_start ();		/* must be after mvl_start_acse	*/

  /* Filter on 3 multicast addresses stored in "goose_rx_mac".		*/
  if ((ret = clnp_snet_set_multicast_filter (goose_rx_mac, 3)) != SD_SUCCESS)
    {
