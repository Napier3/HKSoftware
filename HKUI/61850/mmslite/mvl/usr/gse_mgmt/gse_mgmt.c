...
ST_INT i;
  ...
  msg = gse_mgmt_msg_decode (sn_req, &etype);
  ...
  if (msg)
	...
	if (msg->msg.refReq.offset)
	  {
	  for (i = 0; i < msg->msg.refReq.numIntegers; i++)
	    SLOGCALWAYS2 ("Offset #%2d:  %2d", i, msg->msg.refReq.offset[i]);
	  }
	break;
	...
	if (msg->msg.offReq.references)
	  {
	  for (i = 0; i < msg->msg.offReq.numVStrings; i++)
	    SLOGCALWAYS2 ("Reference #%2d:  %s", i, msg->msg.offReq.references[i]);
	  }
       break;
	...
	if (msg->msg.refRsp.result)
	  {
	  for (i = 0; i < msg->msg.refRsp.numResults; i++)
	    {
	    if (*msg->msg.refRsp.result[i].reference)
	      SLOGCALWAYS2 ("Reference #%2d:  %s", i, msg->msg.refRsp.result[i].reference);
	    else
	      SLOGCALWAYS2 ("Error     #%2d:  %d", i, msg->msg.refRsp.result[i].error);
	    }
	  }
	break;
	...
	if (msg->msg.offRsp.result)
	  {
	  for (i = 0; i < msg->msg.offRsp.numResults; i++)
	    {
	    if (msg->msg.offRsp.result[i].offset)
	      SLOGCALWAYS2 ("Offset #%2d:  %d", i, msg->msg.offRsp.result[i].offset);
	    else
	      SLOGCALWAYS2 ("Error  #%2d:  %d", i, msg->msg.offRsp.result[i].error);
	    }
	  }
	break;
      }

    printf ("\n Received GSE Management Message.");
    gse_mgmt_msg_free (msg);
    retcode = SD_SUCCESS;
    }
  else    /* decode failed, log it    */
    SLOGALWAYS0 ("ERROR: GSE Management Message Decode Failed.");
...
ST_RET clnp_snet_etype_process (SN_UNITDATA *sn_req)
  ...
  switch (sn_req->lpdu_len)
    ...
    case ETYPE_TYPE_GSE:	/* IEC 61850 GSE Management msg. Process it.*/
      retcode = gse_mgmt_msg_process (sn_req);
      break;
    default:
...
ST_CHAR gse_enc_buf[MAX_GSE_MGMT_ENCODE_BUF];
ETYPE_INFO etype; 
...
#if 0
  /* If MVL services not needed, should be able to replace	*/
  /* "mvl_start_acse" call with this.				*/
  mvl_init_mms_objs ();
  ret = clnp_snet_init (&clnp_param);
#else
