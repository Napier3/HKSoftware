...
/************************************************************************/
/*			u_smpval_msg_ind					*/
/* User function to perform additional processing of SMPVAL msg indication.*/
/* DEBUG: customize as needed.						*/
/************************************************************************/
ST_VOID u_smpval_msg_ind (SMPVAL_MSG *smpvalMsg,
						  ETYPE_INFO *etypeInfo,	/* Ethertype header info		*/
						  SN_UNITDATA *sn_req)	/* includes MAC addresses		*/
{
	ST_UINT j;  
	...
	SLOGALWAYS1 ("SampledValue message received, numASDU = %d", smpvalMsg->numASDU);
}
/************************************************************************/
/*			smpval_msg_process					*/
/* Process received SMPVAL message.					*/
/* RETURNS:	SD_SUCCESS if msg processed here			*/
/*		SD_FAILURE if msg NOT processed here			*/
/************************************************************************/
ST_RET smpval_msg_process (SN_UNITDATA *sn_req)
{
	ST_RET retcode;
	...
	smpvalMsg = smpval_msg_decode (sn_req, &etypeInfo);
	if (smpvalMsg)
	{
		u_smpval_msg_ind (smpvalMsg, &etypeInfo, sn_req);	/* process decoded info	*/

		smpval_msg_free (smpvalMsg);	/* done with "smpvalMsg", so free it	*/
		retcode = SD_SUCCESS;	/* msg processed here			*/
	}
	else
		retcode = SD_FAILURE;	/* msg NOT processed here	*/
#else	/* !SMPVAL_SUPPORT	*/
	printf ("DEBUG: SampledValue msg received. Not supported.\n");
	retcode = SD_FAILURE;	/* msg NOT processed here	*/
...
/*		SD_FAILURE if msg NOT processed here			*/
/************************************************************************/
ST_RET goose_msg_process (SN_UNITDATA *sn_req)
{
	ST_RET retcode;
#ifdef MVL_GOOSE_SUPPORT
	/* Use "IEC GOOSE Framework" sample code to process this msg.	*/
	printf ("GOOSE msg received.\n");
	retcode = chk_for_goose_msg (sn_req);
#else
	printf ("DEBUG: GOOSE msg received. Not supported.\n");
	retcode = SD_FAILURE;	/* msg NOT processed here	*/
#endif
	return (retcode);
}
...
/************************************************************************/
/*			gse_manag_msg_process				*/
/* Process received GSE Management message.				*/
/* RETURNS:	SD_SUCCESS if msg processed here			*/
/*		SD_FAILURE if msg NOT processed here			*/
/************************************************************************/
ST_RET gse_manag_msg_process (SN_UNITDATA *sn_req)
{
	ST_RET retcode = SD_FAILURE;	/* msg NOT processed here	*/
	printf ("DEBUG: GSE Management msg received. Not supported.\n");
	return (retcode);
}
/************************************************************************/
/*			clnp_snet_etype_process				*/
/* Process received Ethertype packets (i.e. 802.3 packets where the	*/
/* "Length/Type" field contains the "Type" of the packet).		*/
/* Process any recognized packets.					*/
/* NOTE: This function may be used as a "Subnetwork read hook" function	*/
/*   by calling "clnp_snet_read_hook_add".				*/
/* RETURNS:	SD_SUCCESS if msg processed here			*/
/*		SD_FAILURE if msg NOT processed here			*/
/************************************************************************/
ST_RET clnp_snet_etype_process (SN_UNITDATA *sn_req)
{
	ST_RET retcode= SD_FAILURE;	/* assume msg NOT processed here	*/

	/* See if lpdu_len contains an Ethertype ID  we recognize.	*/
	/* If so, process the message.					*/
	switch (sn_req->lpdu_len)
	{
	case ETYPE_TYPE_SV:		/* IEC 61850 SMPVAL msg. Process it.	*/
		retcode = smpval_msg_process (sn_req);
		break;
	case ETYPE_TYPE_GOOSE:	/* IEC 61850 GOOSE msg. Process it.	*/
		retcode = goose_msg_process (sn_req);
		break;
	case ETYPE_TYPE_GSE:	/* IEC 61850 GSE Management msg. Process it.*/
		retcode = gse_manag_msg_process (sn_req);
		break;
	default:
		break;	/* param contains "len" not "type", OR "type" unrecognized*/
	}	/* end "switch"	*/
	return (retcode);
}
						  ...
						  ST_INT oper_type_id)	/* type_id for "oper_name" arg	*/
{
	...
	/* Don't know type of "Oper" so can't use struct. Use this buf for Oper data*/	
	ST_CHAR oper_data_buf [100];	/* "Oper" data	*/
...
#define TEST_MODE_IEC_REMOTE	4
...
#if defined(SMPVAL_SUPPORT) || defined(MVL_GOOSE_SUPPORT)

	/* CRITICAL: Add function to process received 802.3 Ethertype packets.*/
	/* Otherwise, Ethertype packets are ignored by the Subnetwork.	*/

	if (clnp_snet_read_hook_add (clnp_snet_etype_process) != SD_SUCCESS)
		printf("\nError setting up hook function to process received 802.3 Ethertype packets");
	...
	if (test_mode == TEST_MODE_IEC_REMOTE)
	{
		...
		remote_vmd_ctrl = config_iec_remote_vmd (
			"../scl_srvr/scl.xml",	/* matches config of "scl_srvr" sample	*/
			"E1Q1SB1",
			"S1");
	...
	else
	{
		...
		*/ 
		...
		case TEST_MODE_IEC_RPT:
			...
			if (rcb_info)
				list_add_last (&all_rcb_info->rcb_info_list, rcb_info);	/* add RCB to list*/
...
ST_VOID test_simple_requests (MVL_NET_INFO *clientNetInfo)
{
	...
	ret = getFile (clientNetInfo, "junk.jnk", "lean.cfg");
	if (ret != SD_SUCCESS)
		printf ("\n getFile () Error, ret = 0x%X.", ret);
	else
		printf ("\n getFile () OK");
#endif
	...
	{
		printf ("\n mvl_delvlist OK");
		printf ("\n   num_matched = %lu", reqCtrl->u.delvlist.resp_info->num_matched);
		printf ("\n   num_deleted = %lu", reqCtrl->u.delvlist.resp_info->num_deleted);
	}
	mvl_free_req_ctrl (reqCtrl);
#endif
	...
	{
		printf ("\n mvl_jinit OK");
		printf ("\n   del_entries = %lu", reqCtrl->u.jinit.resp_info->del_entries);
	}
	mvl_free_req_ctrl (reqCtrl);
#endif
	...
	{
		printf ("\n mvl_jstat OK");
		printf ("\n   cur_entries = %lu", reqCtrl->u.jstat.resp_info->cur_entries);
...
#endif
		ret = waitReqDone (reqCtrl, timeOut+60);  /* need to wait longer on connect */
	}
	if (ret != SD_SUCCESS)
		clientNetInfo = NULL;
...
ST_VOID u_mvl_info_rpt_ind (MVL_COMM_EVENT *event)
{
	...
	*/
	...
	else if (va_spec->var_acc_tag == VAR_ACC_VARLIST)
	{
		...
		/* Set up array of local variables (info_va).	*/
		...
		/* Decode data into array of local variables (info_va).	*/
		mvl_info_data_to_local (event, num_va, info_va);
		/* Loop again to examine decoded data	*/
		for (j = 0; j < num_va; ++j)
		{
			va = info_va[j];
			if (va != NULL)
