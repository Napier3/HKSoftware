...
/************************************************************************/
/*                          validQualField                              */
/* verify that a string contains a valid Quality field (qual=b,b,b,b,n) */
...
static ST_INT32             validQualField (
  ...
  strcpy (flagBuf, "000000");    /* create defaults */
  ...
  for (i=0; i < 24; i++)
    ...
    if (qual[i] <= ' ')
      ...
      {
      /* first 4 digits must be 0 or 1, rest is value 0 to 15 */
      digit++;
  ...
  if      (strcmp (work, "(QUAL=1,1,1)") == 0)
    ...
    return 3;
                       ...
                       /* 0123456789012 */
  else if (strcmp (work, "(QUAL=1,1,1,1)") == 0)
    {
    flagBuf[0] = qual[6];
    flagBuf[1] = qual[8];
    flagBuf[2] = qual[10];
    ...
    return 4;
                       ...
                       /* 012345678901234 */
  else if (strcmp (work, "(QUAL=1,1,1,1,9)") == 0)
    {
    flagBuf[0] = qual[6];
    flagBuf[1] = qual[8];
    flagBuf[2] = qual[10];
    ...
    flagBuf[4] = qual[14];    /* 1-digit 'n' value */
    flagBuf[5] = 0;           /* shorten return value */
    ...
    return 5;
                       ...
                       /* 0123456789012345 */
  else if (strcmp (work, "(QUAL=1,1,1,1,99)") == 0)
    {
    ST_INT32                num;
    ...
    flagBuf[4] = qual[14];    /* 2-digit 'n' value */
    flagBuf[5] = qual[15];

    /* when 'n' value is 1-digit, check for range 0 to 15 */
    /* the 'n' value is supposed to be a 4-bit value */
...
#if 0
  /* qualifier values no longer supported: */
  ST_INT                    b3;
  ST_INT                    bx;
#endif
...
ST_CHAR b0, b1, b2;    /* merge: 'b3' and 'rest' no longer used */
...
struct tm *pTm;
  ...
  /* merge: omit 'b3 >> 4' */
  /* merge: simpler sprintf, without 'b3' and 'rest' */ 

  sprintf (theQual, "Z(qual=%c,%c,%c)", b0, b1, b2);
