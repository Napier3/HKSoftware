...
/* We can log here if so desired */
  if ((mms_debug_sel & MMS_LOG_RESP) && ml_log_resp_info_fun != NULL)
    (*ml_log_resp_info_fun) (invoke_id, opcode, NULL);
...
/* We can log here if so desired */
  if ((mms_debug_sel & MMS_LOG_REQ) && ml_log_req_info_fun != NULL)
    (*ml_log_req_info_fun) (mmsl_invoke_id, opcode, NULL);
...
/* We can log here if so desired */
  if ((mms_debug_sel & MMS_LOG_RESP) && ml_log_resp_info_fun != NULL)
    (*ml_log_resp_info_fun) (invoke_id, opcode, info_ptr);
...
/* We can log here if so desired */
  if ((mms_debug_sel & MMS_LOG_REQ) && ml_log_req_info_fun != NULL)
    (*ml_log_req_info_fun) (mmsl_invoke_id, opcode, info_ptr);
...
/* We can log here if so desired */
  if ((mms_debug_sel & MMS_LOG_REQ) && ml_log_unsol_req_info_fun != NULL)
    (*ml_log_unsol_req_info_fun) (opcode, info_ptr);
