...
static ST_RET get_binary_time (RUNTIME_TYPE *rt_ptr, ST_INT numRt,
			       ST_CHAR *spaces,
			       ST_CHAR *dest, ST_INT destSize);

static ST_RET get_utcTime (RUNTIME_TYPE *rt_ptr, ST_INT numRt,
		           ST_CHAR *spaces,
		           ST_CHAR *dest, ST_INT destSize);
...
ST_CHAR str_buffer[5];	      /* variable names				*/
ST_INT i;
...
/* Now we parse the Runtime type and fill a C typedef statement	    */
  for (i = 0; i < numRt; ++i, ++rt_ptr)
	...
	break;
      ...
      case RT_BINARY_TIME:
	ret = get_binary_time (rt_ptr, numRt, spaces, dest, destSize);
	if (ret == SD_FAILURE)
	  return (SD_FAILURE);
   	break;

      case RT_UTC_TIME:
	ret = get_utcTime (rt_ptr, numRt, spaces, dest, destSize);
	if (ret == SD_FAILURE)
	  return (SD_FAILURE);
   	break;
...
RUNTIME_TYPE *rt_prev;		/* ptr to previous in RUNTIME_TYPE array*/
  ...
  if (struct_nest_level > 1)
	...
	++(*strnum);
      ...
      {
      strcpy(s->struct_name, rt_ptr->name);
      }
    }
  if (struct_nest_level == 1)
...
RUNTIME_TYPE *rt_prev;		/* ptr to previous in RUNTIME_TYPE array*/
  ...
  if (array_nest_level > 1 && rt_prev->el_tag == RT_ARR_START)
...
ST_RET ret;
  ...
  if (numRt == 1 || (numRt == 3 && (rt_ptr - 1)->el_tag == RT_ARR_START))
    ret = str_concat (dest, "typedef ", destSize, COPY) || ret;
  if (rt_ptr->u.p.el_len == 4)	/* This is Btime4		*/
    {
    ret = str_concat (dest, "ST_INT32 ", destSize, CAT) || ret;
    if (numRt != 1 && !((rt_ptr - 1)->el_tag == RT_ARR_START))
      {
      if (rt_ptr->name[0] == 0)
	ret = str_concat (dest, "btime4_ms", destSize, CAT) || ret;
      else
	ret = str_concat (dest, rt_ptr->name, destSize, CAT) || ret;
      if ((rt_ptr - 1)->el_tag != RT_ARR_START)
	{
	ret = str_concat (dest, ";  /* Btime4 is one ST_INT32 */\n", destSize, CAT) || ret;
	ret = str_concat (dest, spaces, destSize, CAT) || ret;
        }
      }
    }
  else if (rt_ptr->u.p.el_len == 6)	/* This is Btime6		*/
    {
    if (numRt == 1 || (numRt == 3 && (rt_ptr - 1)->el_tag == RT_ARR_START))
      {
      ret = str_concat (dest, "typedef struct\n", destSize, COPY) || ret;
      strcat (spaces, "  ");
      ret = str_concat (dest, spaces, destSize, CAT) || ret;
      ret = str_concat (dest, "{\n", destSize, CAT) || ret;
      ret = str_concat (dest, spaces, destSize, CAT) || ret;
    ...
    ret = str_concat (dest, "ST_INT32 ", destSize, CAT) || ret;
    if (rt_ptr->name[0] == 0)
      ret = str_concat (dest, "btime6_ms", destSize, CAT) || ret;
    else
      {
      ret = str_concat (dest, rt_ptr->name, destSize, CAT) || ret;
      ret = str_concat (dest, "_ms", destSize, CAT) || ret;
      }
    ret = str_concat (dest, ";  /* Btime6 is two ST_INT32 */\n", destSize, CAT) || ret;
    ret = str_concat (dest, spaces, destSize, CAT) || ret;
    ret = str_concat (dest, "ST_INT32 ", destSize, CAT) || ret;
    if (rt_ptr->name[0] == 0)
      ret = str_concat (dest, "btime6_days", destSize, CAT) || ret;
    else
      {
      ret = str_concat (dest, rt_ptr->name, destSize, CAT) || ret;
      ret = str_concat (dest, "_days", destSize, CAT) || ret;
      }
    ret = str_concat (dest, ";\n", destSize, CAT) || ret;
    ret = str_concat (dest, spaces, destSize, CAT) || ret;
    if (numRt == 1 || (numRt == 3 && (rt_ptr-1)->el_tag == RT_ARR_START))
...
ST_RET ret;
  ...
  ret = str_concat (dest, "ST_UINT32 ", destSize, CAT) || ret;
  if (rt_ptr->name[0] == 0)
    ret = str_concat (dest, "secs", destSize, CAT) || ret;
  else
    {
    ret = str_concat (dest, rt_ptr->name, destSize, CAT) || ret;
    ret = str_concat (dest, "_secs", destSize, CAT) || ret;
    }
  ret = str_concat (dest, ";  /* UTC Time is struct of three ST_UINT32 */\n", destSize, CAT) || ret;
  ret = str_concat (dest, spaces, destSize, CAT) || ret;
  ret = str_concat (dest, "ST_UINT32 ", destSize, CAT) || ret;
  if (rt_ptr->name[0] == 0)
    ret = str_concat (dest, "fraction", destSize, CAT) || ret;
  else
    {
    ret = str_concat (dest, rt_ptr->name, destSize, CAT) || ret;
    ret = str_concat (dest, "_fraction", destSize, CAT) || ret;
    }
  ret = str_concat (dest, ";\n", destSize, CAT) || ret;
  ret = str_concat (dest, spaces, destSize, CAT) || ret;
  ret = str_concat (dest, "ST_UINT32 ", destSize, CAT) || ret;
  if (rt_ptr->name[0] == 0)
    ret = str_concat (dest, "qflags", destSize, CAT) || ret;
  else
    {
    ret = str_concat (dest, rt_ptr->name, destSize, CAT) || ret;
    ret = str_concat (dest, "_qflags", destSize, CAT) || ret;
    }
  ret = str_concat (dest, ";\n", destSize, CAT) || ret;
  ret = str_concat (dest, spaces, destSize, CAT) || ret;
  if (numRt == 1 || (numRt == 3 && (rt_ptr-1)->el_tag == RT_ARR_START))
...
RUNTIME_TYPE *rt_next;		/* ptr to next     in RUNTIME_TYPE array*/
  ...
  if (numRt == 1 || (numRt == 3 && rt_prev->el_tag == RT_ARR_START))
    ret = str_concat (dest, "typedef ", destSize, COPY) || ret;
  if (rt_ptr->name[0] == 0)
    {
    ...
    if (numRt != 1 && !(numRt == 3 && rt_prev->el_tag == RT_ARR_START))
      {
      if (rt_prev->el_tag == RT_ARR_START && rt_prev->name[0] != 0)
	{  /* Array is named */
	ret = str_concat (dest, " ", destSize, CAT) || ret;
    ...
    {
    ret = str_concat (dest, "ST_CHAR", destSize, CAT) || ret;
    if (numRt != 1 && !(rt_next->el_tag == RT_ARR_END && rt_prev->el_tag == RT_ARR_START))
      {
      ret = str_concat (dest, " ", destSize, CAT) || ret;
      ret = str_concat (dest, rt_ptr->name, destSize, CAT) || ret;
      }
    /* Special case - named Vstring inside a named array, we carry over array name */
    else if (rt_prev->el_tag == RT_ARR_START && rt_next->el_tag == RT_ARR_END &&
...
RUNTIME_TYPE *rt_next;		/* ptr to next     in RUNTIME_TYPE array*/
  ...
  if (rt_ptr->u.p.el_len < 0)	    /* This is Variable-length	*/
    ...
    ret = str_concat (dest, spaces, destSize, CAT) || ret;
    ret = str_concat (dest, "} ", destSize, CAT) || ret;
    if (rt_ptr->name[0] == 0 && numRt != 1 && !(numRt == 3 && rt_prev->el_tag == RT_ARR_START))
      {
      if (rt_ptr->el_tag == RT_BIT_STRING)
	{
    ...
    else if ((numRt != 1) && !(rt_next->el_tag == RT_ARR_END && rt_prev->el_tag == RT_ARR_START))
      {
      ret = str_concat (dest, rt_ptr->name, destSize, CAT) || ret;
      ret = str_concat (dest, ";", destSize, CAT) || ret;
      }
