...
ST_VOID m_log_address (VAR_ACC_ADDR *addr_ptr)
  ...
  switch (addr_ptr->addr_tag)
    {
    case (0)  :
      MLOG_CALWAYS1 ("Numeric Address : %08lX",addr_ptr->addr.num_addr);
      break;
...
ST_RET mlogInt64 (ST_VOID *usr, ST_INT64   *data_ptr, RUNTIME_TYPE *rt)
  {
#ifdef _WIN32
  MLOG_CALWAYS1 ("INTEGER 64 value  : %I64d", *data_ptr);
#elif defined(_AIX) || defined(__hpux) || defined(linux) || defined(sun) || defined(__LYNX)
  MLOG_CALWAYS1 ("INTEGER 64 value  : %lld", *data_ptr);
#elif (defined(__alpha) && !defined(__VMS)) 
  MLOG_CALWAYS1 ("INTEGER 64 value  : %ld", *data_ptr);
#else
...
ST_RET mlogUint64 (ST_VOID *usr, ST_UINT64  *data_ptr, RUNTIME_TYPE *rt)
  {
#ifdef _WIN32
  MLOG_CALWAYS1 ("UNSIGNED 64 value  : %I64u", *data_ptr);
#elif defined(_AIX) || defined(__hpux) || defined(linux) || defined(sun) || defined(__LYNX)
  MLOG_CALWAYS1 ("UNSIGNED 64 value  : %llu", *data_ptr);
#elif (defined(__alpha) && !defined(__VMS)) 
  MLOG_CALWAYS1 ("UNSIGNED 64 value  : %lu", *data_ptr);
#else
  #error Missing INT64 code for this platform.
#endif
  return (SD_SUCCESS);
...
ST_RET mlogBcd4  (ST_VOID *usr, ST_INT32   *data_ptr, RUNTIME_TYPE *rt)
  {
  MLOG_CALWAYS1 ("BCD 32 value      : %ld", *((ST_INT32 *) data_ptr));
  return (SD_SUCCESS);
  }
...
STDTIME_STRING          ss;
  ...
  MLOG_CALWAYS1 ("BTOD Number ms since midnight: %ld", (ST_LONG) *data_ptr);
  MLOG_CALWAYS1 ("BTOD Number days since 1/1/1984: %ld", *(data_ptr+1));
  MmsTimeOfDay6ToStdTime ((STDTIME_MMS_TIMEOFDAY6 *)data_ptr, &st);
  StdTimeToStdTimeString (&st, &ss);
  MLOG_CALWAYS1 ("BTOD GMT Time : %s", ss.str);
...
ST_RET mlogUtc   (ST_VOID *usr, MMS_UTC_TIME *data_ptr, RUNTIME_TYPE *rt)
  {
  MLOG_CALWAYS2 ("\tUTC TIME seconds        : %lu (0x%8.8lX)", (ST_ULONG) data_ptr->secs, (ST_ULONG) data_ptr->secs);
  MLOG_CALWAYS2 ("\t         fraction       : %lu (0x%8.8lX)", (ST_ULONG) data_ptr->fraction, (ST_ULONG) data_ptr->fraction);
  MLOG_CALWAYS1 ("\t         quality flags  : 0x%2.2lX", (ST_ULONG) data_ptr->qflags);
  return (SD_SUCCESS);
  }
...
ST_RET mlogArrStartmydefine (ST_VOID *usr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	//MLOG_CALWAYS0 ("ArrayStart");
	strcpy(databuf, "<");
	return SD_SUCCESS;
...
ST_RET mlogArrEndmydefine   (ST_VOID *usr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	//MLOG_CALWAYS0 ("ArrayEnd");
	strcpy(databuf, ">");
...
ST_RET mlogStrStartmydefine (ST_VOID *usr, RUNTIME_TYPE *rt, SD_CONST RUNTIME_TYPE *rt_head, ST_CHAR *databuf)
{
	//MLOG_CALWAYS0 ("StructureStart");
	strcpy(databuf, "{");
...
ST_RET mlogStrEndmydefine   (ST_VOID *usr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	//MLOG_CALWAYS0 ("StructureEnd");
	strcpy(databuf, "}");
...
ST_RET mlogInt8mydefine  (ST_VOID *usr, ST_INT8    *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	ST_INT int_val;
	
	int_val = (ST_INT) (*data_ptr);
	sprintf(databuf, "%d", int_val);
	//MLOG_CALWAYS1 ("INTEGER 8 value   : %d",int_val);

	return SD_SUCCESS;
}

ST_RET mlogInt16mydefine (ST_VOID *usr, ST_INT16   *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	ST_INT i;

	i = (ST_INT) (*data_ptr);
	sprintf(databuf, "%d", (ST_INT)i);
	//MLOG_CALWAYS1 ("INTEGER 16 value  : %d", (ST_INT) i);

	return SD_SUCCESS;
}

ST_RET mlogInt32mydefine (ST_VOID *usr, ST_INT32   *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	//MLOG_CALWAYS1 ("INTEGER 32 value  : %ld", (ST_LONG) *data_ptr);
	sprintf(databuf, "%ld", (ST_LONG) *data_ptr);

	return SD_SUCCESS;
}

#ifdef INT64_SUPPORT
ST_RET mlogInt64mydefine (ST_VOID *usr, ST_INT64   *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
#ifdef _WIN32
	//MLOG_CALWAYS1 ("INTEGER 64 value  : %I64d", *data_ptr);
	sprintf(databuf, "%I64d", *data_ptr);
#elif defined(_AIX) || defined(__hpux) || defined(linux) || defined(sun) || defined(__LYNX)
	//MLOG_CALWAYS1 ("INTEGER 64 value  : %lld", *data_ptr);
	sprintf(databuf, "%lld", *data_ptr);
#elif (defined(__alpha) && !defined(__VMS)) 
	//MLOG_CALWAYS1 ("INTEGER 64 value  : %ld", *data_ptr);
	sprintf(databuf, "%ld", *data_ptr);
#else
#error Missing INT64 code for this platform.
#endif
	return SD_SUCCESS;
...
ST_RET mlogUint8mydefine (ST_VOID *usr, ST_UINT8   *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	ST_INT int_val;
...
ST_RET mlogUint16mydefine (ST_VOID *usr, ST_UINT16  *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	ST_INT i;
...
ST_RET mlogUint32mydefine (ST_VOID *usr, ST_UINT32  *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	//MLOG_CALWAYS1 ("UNSIGNED 32 value  : %lu", (ST_ULONG) *data_ptr);
	sprintf(databuf, "%lu", (ST_ULONG) *data_ptr);

	return SD_SUCCESS;
}

#ifdef INT64_SUPPORT
ST_RET mlogUint64mydefine (ST_VOID *usr, ST_UINT64  *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
#ifdef _WIN32
	//MLOG_CALWAYS1 ("UNSIGNED 64 value  : %I64u", *data_ptr);
	sprintf(databuf, "%I64u", (ST_ULONG) *data_ptr);
#elif defined(_AIX) || defined(__hpux) || defined(linux) || defined(sun) || defined(__LYNX)
	//MLOG_CALWAYS1 ("UNSIGNED 64 value  : %llu", *data_ptr);
	sprintf(databuf, "+%llu", *data_ptr);
#elif (defined(__alpha) && !defined(__VMS)) 
	//MLOG_CALWAYS1 ("UNSIGNED 64 value  : %lu", *data_ptr);
	sprintf(databuf, "+%lu", *data_ptr);
#else
#error Missing INT64 code for this platform.
#endif
	return SD_SUCCESS;
...
ST_RET mlogFltmydefine   (ST_VOID *usr, ST_FLOAT   *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	ST_DOUBLE  d;

	d =  (ST_DOUBLE) (*data_ptr);
	sprintf(databuf, "%e", d);
	//MLOG_CALWAYS1 ("ST_FLOAT value       : %f", d);
...
ST_RET mlogDblmydefine   (ST_VOID *usr, ST_DOUBLE  *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	//MLOG_CALWAYS1 ("ST_DOUBLE value      : %f", *((ST_DOUBLE *) data_ptr));
	sprintf(databuf, "%e", *((ST_DOUBLE *) data_ptr));

	return SD_SUCCESS;
}

ST_RET mlogOctmydefine   (ST_VOID *usr, ST_UCHAR   *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	ST_INT j, abs_len = 0;
	ST_INT16 *sp;
	ST_UCHAR ch, hex;
	...
	if (rt->u.p.el_len < 0)
	{
		...
		strcpy(databuf, tempbuf);
	}
	else
	{
		abs_len = rt->u.p.el_len;
...
ST_RET mlogBoolmydefine  (ST_VOID *usr, ST_BOOLEAN *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	ST_BOOLEAN *bptr;
	ST_INT int_val;
...
ST_RET mlogBcd1mydefine  (ST_VOID *usr, ST_INT8    *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	ST_UCHAR *ubyteptr;
	ST_INT int_val;
...
ST_RET mlogBcd2mydefine  (ST_VOID *usr, ST_INT16   *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	ST_INT i;
	ST_INT16 *sp;
...
ST_RET mlogBcd4mydefine  (ST_VOID *usr, ST_INT32   *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	//MLOG_CALWAYS1 ("BCD 32 value      : %ld", *((ST_INT32 *) data_ptr));
	sprintf(databuf, "%ld", *((ST_INT32 *) data_ptr));
	
	return SD_SUCCESS;
}

ST_RET mlogBsmydefine    (ST_VOID *usr, ST_UCHAR   *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	ST_INT i, j, k;
	ST_INT16 temp;
	ST_INT len = 0;
	ST_INT abs_len, temp_len, temp_bit;
	ST_INT16 *sp;
	ST_CHAR tempbuf[512];
	ST_CHAR temp_val[64];
	...
	if (rt->u.p.el_len < 0)
	{
		...
		{
			temp = *(data_ptr + j + 2);
			memset(temp_val, 0, 64);
			itoa(temp, temp_val, 2);

			len = strlen(temp_val);
			if(len < 8)
			{
				for(k = len - 1; k >= 0; k --)
					temp_val[k + 8 - len] = temp_val[k]; 
...
ST_RET mlogVismydefine   (ST_VOID *usr, ST_CHAR    *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	strcpy(databuf, data_ptr);
	//MLOG_CALWAYS1 ("VIS STRING value  : %s", data_ptr);	
	return SD_SUCCESS;
}

ST_RET mlogBt4mydefine   (ST_VOID *usr, ST_INT32   *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	sprintf(databuf, "%ld", (ST_LONG) *data_ptr);
	//MLOG_CALWAYS1 ("BTOD Number ms since midnight: %ld", (ST_LONG) *data_ptr);
	//MLOG_CALWAYS0 ("BTOD Number days since 1/1/1984: Not included");
...
ST_RET mlogBt6mydefine   (ST_VOID *usr, ST_INT32   *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	STDTIME                 st;
	STDTIME_STRING          ss;
...
ST_RET mlogGtmydefine   (ST_VOID *usr, time_t     *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	sprintf(databuf, "%s", ctime(data_ptr));
	//MLOG_CALWAYS1 ("TIME value        : %s", ctime (data_ptr));
	
	return SD_SUCCESS;
}

ST_RET mlogUtcmydefine   (ST_VOID *usr, MMS_UTC_TIME *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	STDTIME                 st;
	STDTIME_STRING          ss;

	if (data_ptr->fraction > 0 || data_ptr->qflags > 0 || data_ptr->secs > 0)
	{
		MmsUtcTimeToStdTime((STDTIME_MMS_UTC_TIME *)data_ptr, &st);
		StdTimeToStdTimeString(&st, &ss);
		sprintf(databuf, "%23s[%08lu]", ss.str, data_ptr->fraction);
	}
...
ST_RET mlogUtf8mydefine (ST_VOID *usr, ST_UCHAR *data_ptr, RUNTIME_TYPE *rt, ST_CHAR *databuf)
{
	ST_INT iTextLen;
	ST_CHAR tempbuf[512];
	unsigned short wszGBK[512];

	memset(tempbuf, 0, 512);
	memset(wszGBK, 0, 512);

	//hejian alter on 2010-7-26
	iTextLen = MultiByteToWideChar(CP_UTF8, 0, (LPCTSTR)data_ptr, -1, NULL,0); 
	memset(wszGBK, 0, iTextLen * 2 + 2); 
	MultiByteToWideChar(CP_UTF8, 0, (LPCTSTR)data_ptr, -1, wszGBK, iTextLen); 

	iTextLen = WideCharToMultiByte(CP_ACP, 0, wszGBK, -1, NULL, 0, NULL, NULL); 
	WideCharToMultiByte (CP_ACP, 0, wszGBK, -1, tempbuf, iTextLen, NULL,NULL); 
...
SXD_ARB_DATA_CTRL_mydefine mlog_arb_log_ctrl_mydefine =
  {
  mlogArrStartmydefine, /* arrStart 	*/
  mlogArrEndmydefine, 	 /* arrEnd    	*/
  mlogStrStartmydefine, /* strStart  	*/
  mlogStrEndmydefine, 	 /* strEnd    	*/
  mlogInt8mydefine,  	/* int8      	*/
  mlogInt16mydefine,	/* int16     	*/
  mlogInt32mydefine, 	/* int32     	*/
#ifdef INT64_SUPPORT
  mlogInt64mydefine, 	/* int64     	*/
#endif
  mlogUint8mydefine, 	/* uint8     	*/
  mlogUint16mydefine,	/* uint16    	*/
  mlogUint32mydefine,	/* uint32    	*/
#ifdef INT64_SUPPORT
  mlogUint64mydefine,	/* uint64    	*/
#endif
  mlogFltmydefine,   	/* flt       	*/
  mlogDblmydefine,   	/* dbl       	*/
  mlogOctmydefine, 	/* oct       	*/
  mlogBoolmydefine, 	/* bool      	*/
  mlogBcd1mydefine, 	/* bcd1      	*/
  mlogBcd2mydefine, 	/* bcd2      	*/
  mlogBcd4mydefine, 	/* bcd4      	*/
  mlogBsmydefine, 	/* bs	       	*/
  mlogVismydefine, 	/* vis       	*/
  mlogBt4mydefine, 	/* Bin Time    	*/
  mlogBt6mydefine, 	/* Bin Time    	*/
  mlogGtmydefine, 	/* Gen Time    	*/
  mlogUtcmydefine, 	/* UTC Time    	*/
  mlogUtf8mydefine, 	/* UTF8string  	*/
  };
#endif
...
ST_CHAR *asn1Copy;
  ...
  tdl = ms_asn1_to_tdl ((ST_UCHAR *) asn1Copy, asn1_len, 10000);
  if (tdl)
    {
