...
bool WheatyExceptionReport::m_bTerminate = true;
...
#else
   sLogCtrl = &m_logCtrl;
   sLogCtrl->fc.fileName = new char[sizeof (m_szLogFileName) + 1];
   strcpy (sLogCtrl->fc.fileName, m_szLogFileName);
#endif
    ...
    PEXCEPTION_POINTERS pExceptionInfo )
{
    ...
    HANDLE file = CreateFile( m_szDumpFileName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL,
        ...
        bool bWriteVariables )  // true if local/params should be output
{
    ...
    while ( 1 )
    {
        ...
        sprintf(buffer, _T("%08X  %08X  "), sf.AddrPC.Offset, sf.AddrFrame.Offset );
        ...
        {
            sprintf(buffer, _T("%hs+%I64X"), pSymbol->Name, symDisplacement );
            
            if ( SymGetLineFromAddr( m_hProcess, sf.AddrPC.Offset,
                                &dwLineDisplacement, &lineInfo ) )
               sprintf(buffer, _T("%hs+%I64X  %s line %u"),pSymbol->Name, symDisplacement, lineInfo.FileName,lineInfo.LineNumber); 
	    else
               sprintf(buffer, _T("%hs+%I64X"), pSymbol->Name, symDisplacement );
        }
