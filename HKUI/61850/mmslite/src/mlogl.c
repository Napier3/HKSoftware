...
static ST_VOID _ml_log_reqind_info (ST_CHAR *pduTypeText, 
			    ST_INT invokeId, ST_INT op, ST_VOID *info);
...
INIT_INFO *init_info;
  ...
  if (rslt->err_code == NO_DECODE_ERR)
	...
	init_info = (INIT_INFO *) rslt->data_ptr;
      ...
      case MMSERROR : 
        MLOG_CALWAYS1 ("Invoke ID = %lu", rslt->id);
...
ST_VOID ml_mlog_install ()
  {
  ml_log_req_info_fun = _ml_log_req_info;
  ml_log_ind_info_fun = _ml_log_ind_info;

  ml_log_resp_info_fun = _ml_log_resp_info;
  ml_log_conf_info_fun = _ml_log_conf_info;

  ml_log_unsol_req_info_fun = _ml_log_unsol_req_info;
  ml_log_unsol_ind_info_fun = _ml_log_unsol_ind_info;

  ml_log_error_conf_fun = _ml_log_error_conf;
  ml_log_error_resp_fun = _ml_log_error_resp;

  ml_log_reject_send_fun = _ml_log_reject_send;
  ml_log_reject_recv_fun = _ml_log_reject_recv;
  }
