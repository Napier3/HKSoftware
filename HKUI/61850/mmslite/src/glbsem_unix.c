...
ST_THREAD_ID         thisThreadId = GET_THREAD_ID();
  ...
  if ( result != 0 )
    {
    GLBSEM_LOG_ERR3 ("GLBSEM gs_mutex_create error: Thread 0x%08lx failed to initialize"
                     " mutex semaphore ms=0x%08X attributes (error=%d).",
  ...
  if ( result != 0 )
    {
    GLBSEM_LOG_ERR3 ("GLBSEM gs_mutex_create error: thread 0x%08lx failed to set mutex"
                     " semaphore ms=0x%08X attributes (error=%d).", 
                     thisThreadId, ms, result);
    switch ( result )
      {
      case EINVAL:
  ...
  if ( result != 0 )
    {
    GLBSEM_LOG_ERR3 ("GLBSEM gs_mutex_create error: thread 0x%08lx failed to initialize"
                     " mutex semaphore ms=0x%08X (error=%d).",
                     thisThreadId, ms, result);
    switch ( result )
      {
      case EAGAIN:
  ...
  GLBSEM_LOG_FLOW2 ("GLBSEM gs_mutex_create: thread 0x%08lx initialized "
                    " mutex semaphore ms=0x%08X.", thisThreadId, ms);
...
ST_THREAD_ID   thisThreadId = GET_THREAD_ID();
  ...
  if ( result != 0 )
    ...
    GLBSEM_LOG_ERR3 ("GLBSEM gs_mutex_destroy error: thread 0x%08lx failed to destroy mutex"
                     " semaphore ms=0x%08X (error=%d)",
                     thisThreadId, ms, result);
    switch ( result )
      {
      case EBUSY:
  ...
  GLBSEM_LOG_FLOW2 ("GLBSEM gs_mutex_destroy: thread 0x%08lx destroyed mutex"
                    " semaphore ms=0x%08X.", thisThreadId, ms);
...
ST_THREAD_ID  thisThreadId = GET_THREAD_ID();	/* Let's see just who we are	*/
  ...
  if ( es == NULL )
    {
    GLBSEM_LOG_ERR1 ("GLBSEM gs_get_event_sem error: thread 0x%08lx"
  ...
  if ( result != 0 )
    {
    GLBSEM_LOG_ERR2 ("GLBSEM gs_get_event_sem error: thread 0x%08lx failed"
                     " (pthread_cond_init failed result=%d).",
                     thisThreadId, result);
    switch ( result )
      {
      case EAGAIN:
  ...
  GLBSEM_LOG_FLOW2 ("GLBSEM gs_get_event_sem: thread 0x%08lx initialized event sem es=0x%08X",
                    thisThreadId, es);
...
struct timeval curtimeval;	/* for gettimeofday	*/
  ...
  if (!es)
    {
    GLBSEM_LOG_ERR1 ("GLBSEM gs_wait_event_sem error: thread 0x%08lx"
  ...
  if (es->predicate == 1)
    ...
    GLBSEM_LOG_FLOW2 ("GLBSEM gs_wait_event_sem: thread 0x%08lx returned from wait for es=0x%08lx (on entry p=1)",
                      thisThreadId, es);
    return (SD_SUCCESS);
    }
  ...
  if ( timeout >= 0 )
    ...
    if ( result != 0 )
      {
      GLBSEM_LOG_ERR3 ("GLBSEM gs_wait_event_sem error: thread 0x%08lx, es=0x%08lx"
  ...
  while ( !es->predicate )
    ...
    if ( result == 0 )
      ...
      if (es->predicate)
        ...
        GLBSEM_LOG_FLOW2 ("GLBSEM gs_wait_event_sem: thread 0x%08lx returning from wait for es=0x%08lx (success)",
                          thisThreadId, es);
        break;				/* exit while 			*/
        }
      else
        ...
        if (es->manualReset == SD_TRUE)
          ...
          ret = SD_SUCCESS;  		/* cond var was signaled	*/
          GLBSEM_LOG_FLOW2 ("GLBSEM gs_wait_event_sem: thread 0x%08lx returning from wait for es=0x%08lx",
                            thisThreadId, es);
          break;				/* exit while 		*/
          }
        else
          ...
          GLBSEM_LOG_FLOW2 ("GLBSEM gs_wait_event_sem: thread 0x%08lx spurious wake up for es=0x%08lx",
                            thisThreadId, es);
          continue;
          }
        }
      ...
      if (es->predicate)
        ...
        GLBSEM_LOG_FLOW2 ("GLBSEM gs_wait_event_sem: thread 0x%08lx returned from wait for es=0x%08lx (timeout but p=1)",
                          thisThreadId, es);
        }
      else
        {
        ret = SD_TIMEOUT; 		/* timeout			*/
        GLBSEM_LOG_FLOW3 ("GLBSEM gs_wait_event_sem: thread 0x%08lx, es=0x%08lx"
...
ST_THREAD_ID  thisThreadId = GET_THREAD_ID();	/* Let's see just who we are	*/
  ...
  if (!es)
    {
    GLBSEM_LOG_ERR1 ("GLBSEM _gs_sig_event_sem error: thread 0x%08lx"
                     " (invalid es=NULL)", thisThreadId);
    return (result);
    }
  ...
  if (result != 0)
    {
    GLBSEM_LOG_ERR3 ("GLBSEM _gs_sig_event_sem error: thread 0x%08lx, es=0x%08lx"
...
ST_VOID gs_signal_event_sem (ST_EVENT_SEM es)
  ...
  if (result == 0)
    {
    GLBSEM_LOG_FLOW2 ("GLBSEM gs_signal_event_sem: thread 0x%08lx signaled es=0x%08lx",
                      GET_THREAD_ID(), es);
    }
  }
...
ST_VOID gs_pulse_event_sem (ST_EVENT_SEM es)
  ...
  if (result == 0)
    {
    GLBSEM_LOG_FLOW2 ("GLBSEM gs_pulse_event_sem: thread 0x%08lx signaled es=0x%08lx",
                      GET_THREAD_ID(), es);
    }
  }
...
ST_THREAD_ID  thisThreadId = GET_THREAD_ID();	/* Let's see just who we are	*/
  ...
  if (!es)
    {
    GLBSEM_LOG_ERR1 ("GLBSEM gs_free_event_sem error: thread 0x%08lx"
                     " (invalid es=NULL)", thisThreadId);
    return;
    }
  ...
  if (result != 0)
    {
    GLBSEM_LOG_ERR3 ("GLBSEM gs_free_event_sem error: thread 0x%08lx, es=0x%08lx"
    ...
    {
    GLBSEM_LOG_FLOW2 ("GLBSEM gs_free_event_sem: thread 0x%08lx released event sem es=0x%08lx",
                      thisThreadId, es);
    }
...
pthread_attr_t   init_attr;
  ...
  if (result == 0 )
    ...
    GLBSEM_LOG_FLOW1 ("GLBSEM: gs_start_thread created thread threadHandle=0x%08lx",
                      threadHandle);
    }
  else
    {
...
ST_INT result;
  ...
  if ( result == 0 )
    ...
    ret = SD_SUCCESS;
    GLBSEM_LOG_FLOW1 ("GLBSEM: pthread_join SUCCESS for thread threadHandle=0x%08lx",
                      threadHandle);
    }
  else
    {
    ret = SD_FAILURE;
    GLBSEM_LOG_ERR2 ("GLBSEM error: pthread_join failed for thread 0x%08lx"
