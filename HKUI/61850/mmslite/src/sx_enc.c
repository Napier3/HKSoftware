...
FILE *fp;
  ...
  if ((fp = fopen (fileName, "w")) == NULL)
    {
    SXLOG_ERR1 ("XML File (%s) Open Error", fileName);
    return (SD_FAILURE);
    }
...
ST_CHAR strVal[100];
  ...
  if (sxEncCtrl->errCode == 0)
    ...
    if (!sxEncCtrl->useFp && (sx_debug_sel & SX_LOG_ENC))
      {
      SLOGCALWAYS1 ("sx_wr_ulong_hex: %s", sxEncCtrl->nextWritePos);
      }
    sxEncCtrl->nextWritePos = _writePos;
    }
  }
...
ST_BOOLEAN timeMemberFound = SD_FALSE;
  ...
  /* Use strncat_safe to avoid buffer overrun	      */
  ...
  if (sxDuration->years > 0)
    ...
    strncat_safe( buffer, temp, size - 1 );
    }
