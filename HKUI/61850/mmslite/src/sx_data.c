...
/************************************************************************/
/************************************************************************/
/*			sxd_process_arb_data_mydefine 				*/
/************************************************************************/
  
ST_RET sxd_process_arb_data_mydefine (ST_CHAR *datptr, SD_CONST RUNTIME_TYPE *rt_head, 
									ST_INT rt_num, ST_VOID *usr, SXD_ARB_DATA_CTRL_mydefine *ac, 
									ST_BOOLEAN *elPres, ST_CHAR *valbuf)
{
	ST_RET uDataRet = 0;
	RUNTIME_TYPE *rt_ptr;
	RUNTIME_TYPE *rt_end;
	...
	while (rt_ptr < rt_end )
	{
		...
		case RT_ARR_START :
			if (ac->arrStart)
				uDataRet = (*ac->arrStart) (usr, rt_ptr, char_val);
			break;

		case RT_STR_START :
			if (ac->strStart)
				uDataRet = (*ac->strStart) (usr, rt_ptr, rt_head, char_val);
			break;

		case RT_ARR_END :			/* array done			*/
			if (ac->arrEnd)
				uDataRet = (*ac->arrEnd) (usr, rt_ptr, char_val);
			break;

		case RT_STR_END :			/* structure done		*/
			if (ac->strEnd)
				uDataRet = (*ac->strEnd) (usr, rt_ptr, char_val);
			break;

		case RT_BOOL :
			if (ac->booln)
				uDataRet = (*ac->booln) (usr, (ST_BOOLEAN *) datptr, rt_ptr, char_val);
			break;

		case RT_BIT_STRING :
			{
				if (ac->bs)
					uDataRet = (*ac->bs) (usr, (ST_UCHAR *) datptr, rt_ptr, char_val);

				len = strlen(char_val);
				for(i = len; i >= 0; i --)
				{
					char_val[i+1] = char_val[i];
				}
				char_val[0] = '[';
				char_val[len+1] = ']';
				char_val[len+2] = '\0';
		...
		case RT_INTEGER :
			...
			case 1 :			/* one byte int 		*/
				if (ac->int8)
					uDataRet = (*ac->int8) (usr, (ST_INT8 *) datptr, rt_ptr, char_val);
				break;

			case 2 :			/* two byte int 		*/
				if (ac->int16)
					uDataRet = (*ac->int16) (usr, (ST_INT16 *) datptr, rt_ptr, char_val);
				break;

			case 4 :			/* four byte integer		*/
				if (ac->int32)
					uDataRet = (*ac->int32) (usr, (ST_INT32 *) datptr, rt_ptr, char_val);
				break;

#ifdef INT64_SUPPORT
			case 8 :			/* eight byte integer		*/
				if (ac->int64)
					uDataRet = (*ac->int64) (usr, (ST_INT64 *) datptr, rt_ptr, char_val);
#endif
				break;
			}
			break;
		...
		case RT_UNSIGNED :
			{
				...
				case 1 :			/* one byte int 		*/
					if (ac->uint8)
						uDataRet = (*ac->uint8) (usr, (ST_UCHAR *) datptr, rt_ptr, char_val);
					break;

				case 2 :			/* two byte int 		*/
					if (ac->uint16)
						uDataRet = (*ac->uint16) (usr, (ST_UINT16 *) datptr, rt_ptr, char_val);
					break;

				case 4 :			/* four byte integer		*/
					if (ac->uint32)
						uDataRet = (*ac->uint32) (usr, (ST_UINT32 *) datptr, rt_ptr, char_val);
					break;

#ifdef INT64_SUPPORT
				case 8 :			/* eight byte integer		*/
					if (ac->uint64)
						uDataRet = (*ac->uint64) (usr, (ST_UINT64 *) datptr, rt_ptr, char_val);
					break;
#endif 	/* INT64_SUPPORT	*/
				}
		...
		case RT_FLOATING_POINT : 
			{
				...
				{
					if (ac->dbl)
						uDataRet = (*ac->dbl) (usr, (ST_DOUBLE *) datptr, rt_ptr, char_val);
				}
				else
				{
					if (ac->flt)
						uDataRet = (*ac->flt) (usr, (ST_FLOAT *) datptr, rt_ptr, char_val);
				}
		...
		case RT_OCTET_STRING : 	
			{
				if (ac->oct)
					uDataRet = (*ac->oct) (usr, (ST_UCHAR *) datptr, rt_ptr, char_val);
		...
		case RT_VISIBLE_STRING : 	
			{
				if (ac->vis)
					uDataRet = (*ac->vis) (usr, (ST_CHAR *) datptr, rt_ptr, char_val);
		...
		case RT_GENERAL_TIME : 		
			if (ac->gt)
				uDataRet = (*ac->gt) (usr, (time_t *) datptr, rt_ptr, char_val);
			break;
#endif
		...
		case RT_BINARY_TIME : 		
			...
			case 4:
				if (ac->bt4)
					uDataRet = (*ac->bt4) (usr, (ST_INT32 *) datptr, rt_ptr, char_val);
				break;
			case 6:
				if (ac->bt6)
					uDataRet = (*ac->bt6) (usr, (ST_INT32 *) datptr, rt_ptr, char_val);
				break;
			}
			break;
#endif
		...
		case RT_BCD : 	
			{
				...
				{
					if (ac->bcd1)
						uDataRet = (*ac->bcd1) (usr, (ST_INT8 *) datptr, rt_ptr, char_val);
				}
				else if (rt_ptr->u.p.el_len <= 4)
				{
					if (ac->bcd2)
						uDataRet = (*ac->bcd2) (usr, (ST_INT16 *) datptr, rt_ptr, char_val);
				}
				else if (rt_ptr->u.p.el_len <= 8)
				{
					if (ac->bcd4)
						uDataRet = (*ac->bcd4) (usr, (ST_INT32 *) datptr, rt_ptr, char_val);
				}
				break;
			}
		...
		case RT_UTC_TIME :
			{
				if (ac->utc)
					uDataRet = (*ac->utc) (usr, (MMS_UTC_TIME *) datptr, rt_ptr, char_val);
		...
		case RT_UTF8_STRING :
			{
				if (ac->utf8)
					uDataRet = (*ac->utf8) (usr, (ST_UCHAR *) datptr, rt_ptr, char_val);
