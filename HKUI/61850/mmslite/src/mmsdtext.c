...
static ST_RET LocToTextUtc (MMS_UTC_TIME *pSrc, RUNTIME_TYPE *rt, ST_CHAR *text)
  {
	...
	pSrc->secs += 8 * 60 * 60;
	MmsUtcTimeToStdTime((STDTIME_MMS_UTC_TIME *)pSrc, &st);
	StdTimeToStdTimeString(&st, &ss);
	sprintf(text, "%23s", ss.str);
...
static ST_RET LocToTextUtf8 (ST_UCHAR *pSrc, RUNTIME_TYPE *rt, ST_CHAR *text)
  {
	ST_INT iTextLen = WideCharToMultiByte( CP_ACP, 0, (LPCWSTR)pSrc, -1, NULL, 0, NULL, NULL);
	WideCharToMultiByte(CP_ACP , 0, (LPCWSTR)pSrc, -1, text, iTextLen, NULL, NULL); 
