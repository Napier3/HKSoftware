...
LOGCFGX_VALUE_MAP sockLogMaskMaps[] =
  {
    {"SOCK_LOG_ERR",	SOCK_LOG_ERR,	&sock_debug_sel, _LOGCFG_DATATYPE_UINT_MASK, "Error"},
    {"SOCK_LOG_NERR",	SOCK_LOG_NERR,	&sock_debug_sel, _LOGCFG_DATATYPE_UINT_MASK, "Notice"},
    {"SOCK_LOG_FLOW",	SOCK_LOG_FLOW,	&sock_debug_sel, _LOGCFG_DATATYPE_UINT_MASK, "Flow"},
    {"SOCK_LOG_RX",	SOCK_LOG_RX,	&sock_debug_sel, _LOGCFG_DATATYPE_UINT_MASK, "Receive"},
    {"SOCK_LOG_TX",	SOCK_LOG_TX,	&sock_debug_sel, _LOGCFG_DATATYPE_UINT_MASK, "Transmit"}
  };
...
ST_RET         rc = SD_SUCCESS;
  ...
  if (ctxId == 0)
    ...
    /* do not put any logging above */
      ...
      if (wRet = WSAStartup(0x0101, &wsaData))
        {
        SOCK_LOG_ERR2 ("Can't initialize WinSock interface to use version 1.1 (error=%d) for context '%s'",
  ...
  */
  ...
  SOCK_LOG_FLOW1 ("%s: sockStart() initialization pending...", sockCtx->ctxName);
  ...
  SOCK_LOG_FLOW1 ("%s: sockStart() successful", sockCtx->ctxName);
  *sockCtxOut = sockCtx;
...
ST_RET rc;
  ...
  SOCK_LOG_FLOW1 ("%s: sockEnd() pending...", sockCtx->ctxName);
...
/* First let's terminate terminate all service threads and the free thread */
  ...
  SOCK_LOG_FLOW1 ("%s: sockEnd() complete", sockCtx->ctxName);
...
int err;
  ...
  if (sockCtx->ctxStatus == GEN_SOCK_CTX_TERMINATING)
    ...
    SOCK_LOG_NERR1 ("sockInitCalling failed in context '%s' is terminating", sockCtx->ctxName);
    return (SD_FAILURE);
    }
  ...
  if (hSock == INVALID_SOCKET)
    {
    SOCK_LOG_ERR0 ("Socket: socket failure");
    return (SOCKET_ERRORNO);
    }
#if !defined(_WIN32)
  if (hSock >= FD_SETSIZE)
    {	/* Can't use this socket because illegal to use in "select" call*/
    SOCK_LOG_ERR1 ("Socket: socket num %d > FD_SETSIZE", hSock);
    CLOSE_SOCKET (hSock);
    return (SD_FAILURE);
    }
#endif
  ...
  SOCK_LOG_FLOW1 ("%s: in sockInitCalling()", pSock->sockIdStr);
  *pSockOut = pSock;
...
/* Set up the socket address */
  if (strchr(ipAddr, '.') != NULL)      /* dotted address format */
    ...
    if (pHost == NULL)
      {
      err = SOCKET_ERRORNO;
      SOCK_LOG_NERR2 ("%s: gethostbyname() failed for host='%s'", pSock->sockIdStr, ipAddr);
...
/* Do non-blocking connect */
  ...
  if (sRet >= 0)
    ...
    SOCK_LOG_FLOW1 ("%s: socket connected", pSock->sockIdStr);
    ...
    if (rc == SD_SUCCESS)
      ...
      {
      SOCK_LOG_FLOW1 ("%s: closing socket connections", pSock->sockIdStr);
      CLOSE_SOCKET (pSock->hSock);
      chk_free (pSock);	
      }
    S_UNLOCK_UTIL_RESOURCES ();
	...
	/* since socket is not connected we can use the system socket close	*/
      ...
      {
      SOCK_LOG_NERR4 ("%s: connect() to host=%s failed, port=%d, errno=%d",
...
int err;
  ...
  if (sockCtx->ctxStatus == GEN_SOCK_CTX_TERMINATING)
    {
    /* prevent adding new sockets to serviceCtrl */ 
    SOCK_LOG_NERR1 ("%s: sockInitListen() failed, context is terminating", sockCtx->ctxName);
    return (SD_FAILURE);
    }
  ...
  if (hSock == INVALID_SOCKET)
    {
    err = SOCKET_ERRORNO;
    SOCK_LOG_ERR2 ("%s: socket() failed for listen socket, errno=%d", sockCtx->ctxName, err);
    return (err);
    }
#if !defined(_WIN32)
  if (hSock >= FD_SETSIZE)
    {	/* Can't use this socket because illegal to use in "select" call*/
    SOCK_LOG_ERR2 ("%s: socket() returned invalid num %d > FD_SETSIZE", sockCtx->ctxName, hSock);
    CLOSE_SOCKET (hSock);
    return (SD_FAILURE);
    }
#endif
  ...
  pListenSock = _sockAllocSock (sockCtx, GS_ROLE_LISTENING, GS_STATE_LISTENING, 
                                hSock, sockCfg);
  SOCK_LOG_FLOW1 ("%s: initializing listen socket", pListenSock->sockIdStr);
  *pListenSockOut = pListenSock;
  ...
  if (sRet != 0)
    {
    err = SOCKET_ERRORNO;
    SOCK_LOG_NERR4 ("%s: listen socket bind() error on port=%d, errno=%d %s", 
                    pListenSock->sockIdStr, portNo, err,
                    (err == SOCK_EADDRINUSE) ? "(port in use)" : " ");
    CLOSE_SOCKET (pListenSock->hSock);
    chk_free (pListenSock);
    return (err);
    }
  SOCK_LOG_FLOW1 ("%s: listen socket bind() OK", pListenSock->sockIdStr);
  ...
  if (sRet != 0)
    {
    err = SOCKET_ERRORNO;
    SOCK_LOG_NERR2 ("%s: listen() failed, errno=%d", pListenSock->sockIdStr, err);
    CLOSE_SOCKET (pListenSock->hSock);
    chk_free (pListenSock);
    return (err);
    }
  ...
  if (rc != SD_SUCCESS)
    ...
    SOCK_LOG_FLOW2 ("%s: listen started on port=%d", pListenSock->sockIdStr, portNo);
    ...
    if (rc == SD_SUCCESS)
        ...
        if (sockData->eot)
	  {
          if (pSock->sockTxQueueGroupCnt > 0 )
            ...
            SOCK_LOG_ERR1 ("%s: tx queue count invalid.", pSock->sockIdStr);
	  }
        (*pSock->sockCfg.uSockTxBufFree) (pSock, sockData);
        }
      else
      ...
      {
      SOCK_LOG_NERRC1 ("%s: sockTx() failed, emptying send queue.", pSock->sockIdStr);
...
ST_RET ret;
  ...
  if (pSock->sockState != GS_STATE_CONNECTED)
    {
    SOCK_LOG_NERR1 ("%s: sockTx() failed, socket not connected", pSock->sockIdStr);
    return (SD_FAILURE);
    }
  ...
  if (numSent >= 0)  /* Success ? */
    ...
    *numSentOut = numSent;
    ...
    switch (err)
      ...
      case SOCK_TIMEDOUT:
        SOCK_LOG_FLOWC2 ("%s: _sockTx sent 0, error (%d), no send buffers?",
                         pSock->sockIdStr, err);
        pSock->sockCfg.chkWritable = SD_TRUE;
        sockServiceWake (pSock->serviceCtrl);
        *numSentOut = 0;
      ...
      default:
        SOCK_LOG_NERR2 ("%s: socket send() error (%d)", pSock->sockIdStr, err);
      break;
      }
    }
...
GEN_SOCK_CTXT *sockCtx;
  ...
  SOCK_LOG_FLOW1 ("%s: in sockClose()", pSock->sockIdStr);
...
GEN_SOCK_CTXT *sockCtx;
  ...
  SOCK_LOG_FLOW1 ("%s: in sockFree()", pSock->sockIdStr);

  sockCtx = pSock->sockCtx;

  if (pSock->sockState != GS_STATE_CLOSED)
    {
    SOCK_LOG_NERR1 ("%s: socket state not closed, can't free", pSock->sockIdStr);
    return (SD_FAILURE);
    }
  ...
  if (pSock->role != GS_ROLE_LISTENING || pSock->numActive == 0)
    {
    SOCK_LOG_FLOW2 ("%s: freeing GEN_SOCK 0x%08lx", pSock->sockIdStr, pSock);
    chk_free (pSock);
    }
  else
    {	/* Should never fail. Just log if it does.	*/
    SOCK_LOG_ERR2 ("%s: ERROR freeing GEN_SOCK 0x%08lx", pSock->sockIdStr, pSock);
    }
  S_UNLOCK_UTIL_RESOURCES ();
  return SD_SUCCESS;
  }
...
/* OK, there is a wakeup port for this sock service */
  ...
  if (numSent == sizeof (wakeupData))
    {
    SOCK_LOG_FLOW2 ("%s: XSocket sent %d wakeup bytes", sockCtx->ctxName, numSent);
    }
  else
    {
    ...
    SOCK_LOG_FLOW3 ("%s: XSocket wakeup data send() error, errno=%d",
    ...
    if (pSock->sockState == GS_STATE_CONNECTED)
      ...
      if (FD_ISSET (pSock->hSock, &sockFds->readfds))
        ...
        if (_sockCheckRxData (pSock) != SD_SUCCESS)
          {	/* sockClose already called. Do nothing here.	*/
          }
        }

      /* See if we are checking for writability too */
      if (sockCfg->chkWritable && FD_ISSET (pSock->hSock, &sockFds->writefds))
	{
        SOCK_LOG_FLOW1 ("%s: socket is Writable", pSock->sockIdStr);
      ...
      if (FD_ISSET (pSock->hSock, &sockFds->writefds))
        ...
        if (sRet == 0)
          ...
          if (connError != 0)
            {
            SOCK_LOG_FLOW2 ("%s: socket Connect failed. Error = %d", pSock->sockIdStr, connError);
          ...
          {
          SOCK_LOG_ERR2 ("%s: getsockopt(...,SOL_SOCKET, SO_ERROR,...) failed, errno=%d.",
  ...
  if (nfds > 0)
    ...
    /* Non-blocking receive, dump data */
      ...
      if (memcmp (sockCtx->ctxName, SLOGIPC_NAME, strlen(SLOGIPC_NAME)) != 0)
        ...
        if (recvRet > 0)
          {
          SOCK_LOG_FLOW2 ("%s: XSocket received %d wakeup bytes", sockCtx->ctxName, recvRet);
          }
        else if (recvRet == 0)
          {
          SOCK_LOG_NERR1 ("%s: XSocket disconnected detected by recv()", sockCtx->ctxName);
          }
        }
      }
    }
...
static ST_VOID _sockServiceDiscardSock (GEN_SOCK_CTRL *serviceCtrl, 
                                        GEN_SOCK *pSock)
  {
  SOCK_LOG_FLOW2 ("%s: preparing to free GEN_SOCK 0x%08lx", pSock->sockIdStr, pSock);
...
/* See if we need to return a receive buffer to the user */
  ...
  if (pSock->usrCloseCalled == SD_FALSE)
    ...
    if (pSock->sockCfg.secDisconnectProc == NULL)
      {
      SOCK_LOG_FLOW1 ("%s: calling uSockDisconnect()",pSock->sockIdStr);
      (*pSock->sockCfg.uSockDisconnect)(pSock);
      }
    else
      {
      SOCK_LOG_FLOW1 ("%s: calling secDisconnectProc()", pSock->sockIdStr);
...
/* This accept should succeed immediately */
  ...
  if (hNewSock == INVALID_SOCKET)
    {
    SOCK_LOG_NERR1 ("%s: socket accept() error", pListenSock->sockIdStr);
  ...
  if (hNewSock >= FD_SETSIZE)
    {	/* Can't use this socket because illegal to use in "select" call*/
    SOCK_LOG_ERR2 ("%s: socket num %d > FD_SETSIZE", pListenSock->sockIdStr, hNewSock);
  ...
  if (sockCtx->ctxStatus == GEN_SOCK_CTX_TERMINATING)
    {
    /* prevent adding new sockets to serviceCtrl */ 
    SOCK_LOG_FLOW1 ("%s: can't accept socket connection, context is terminating", pListenSock->sockIdStr);
    CLOSE_SOCKET (hNewSock);
    return (SD_FAILURE);
    }

  /* Got a connection, see if we are still listening ... */
  SOCK_LOG_FLOW4 ("%s: accepted connection (socket=%d) from IP Address=%s Port=%u",
  ...
  if (okToAccept == SD_FALSE)
    {
    SOCK_LOG_FLOW2 ("%s: cannot process new socket connection, reached maxActive=%d socket connections",
  ...
  /* See if we need to pass it to a receive processing subsystem */
    ...
    if (rc == SD_SUCCESS)
      ...
      if (numRxd)
        ...
        printf (".");
#endif
        SOCK_LOG_FLOW2 ("%s: received %d bytes", pSock->sockIdStr, numRxd);
        ...
        /* We got all the data we wanted, see if it is the header or data */
          if (pSock->recvState == RECV_STATE_HUNT)
            ...
            /* OK, the hunt is over and we have the length of the buffer to read  */
              ...
              SOCK_LOG_FLOWC2 ("%s: hunt complete, expecting %d more bytes", pSock->sockIdStr, bodySize);
            ...
            {
            SOCK_LOG_FLOWC1 ("%s: got body", pSock->sockIdStr);
            ++pSock->sockStats.numRecv;
...
/* Not a clean receive, see if we have a disconnect */
  if (recvRet == 0)
    ...
    SOCK_LOG_FLOW1 ("%s: Socket disconnect detected by recv()", pSock->sockIdStr);
    sockClose (pSock);
    return (SD_FAILURE);
    }
...
/* Some kind of error */
  ...
  switch (err)			/* See if recoverable */
    ...
    default:
      SOCK_LOG_FLOW2 ("%s: Socket error detected by recv() errno=%d", pSock->sockIdStr, err);
...
int sRet;
  ...
  pSock = (GEN_SOCK *) chk_calloc (1, sizeof (GEN_SOCK)); 
...
/* Make the socket non-blocking */
  ...
  SOCK_LOG_FLOW3 ("%s: allocated GEN_SOCK 0x%08lx, socket=%d",
...
/* Find a service thread with capacity */
  ...
  if (serviceCtrl == NULL)
    ...
    SOCK_LOG_NERR2 ("%s: Can't add GEN_SOCK to list (limit reached %d sockets)", 
  ...
  if (rc == SD_SUCCESS)
    ...
    ++serviceCtrl->numSock;
    sockServiceWake (serviceCtrl);
    SOCK_LOG_FLOW1 ("%s: Added GEN_SOCK to service list", pSock->sockIdStr);
    }
  else
    SOCK_LOG_FLOW2 ("%s: Add GEN_SOCK to service list failed (error=%d)", pSock->sockIdStr, rc);
...
GEN_SOCK_CTXT *sockCtx = serviceCtrl->sockCtx;
  ...
  SOCK_LOG_FLOW1 ("%s: in sockAddServiceThread()", sockCtx->ctxName);

  serviceCtrl->serviceEvent = gs_get_event_sem (SD_FALSE);
  if (!serviceCtrl->serviceEvent)
    {
    SOCK_LOG_ERR1 ("%s: could not get service event", sockCtx->ctxName);
    return (SD_FAILURE);
    }
  ...
  if (rc != SD_SUCCESS)
    {
    SOCK_LOG_ERR2 ("%s: could not start Service thread error=%d", sockCtx->ctxName, rc);
    gs_free_event_sem (serviceCtrl->serviceEvent);
    serviceCtrl->serviceEvent = NULL;
    if (serviceCtrl->xCallingSock)
      {
...
ST_INT ret;
  ...
  SOCK_LOG_FLOW1 ("%s: sockServiceThread started", sockCtx->ctxName);
...
/* Close all sockets associated with the service control */
  ...
  SOCK_LOG_FLOW1 ("%s: sockServiceThread ended", sockCtx->ctxName);
...
/* Step 1: Make a non-blocking listen socket */
  ...
  for (i = 0; i < portRange; ++i)
    ...
    if (sRet == 0)
      {
      SOCK_LOG_FLOW2 ("%s: XSocket bind on port=%d", sockCtx->ctxName, (ST_UINT) wakeupPort);
      break;
      }

    SOCK_LOG_FLOW3 ("%s: XSocket bind() failure on port=%d, errno=%d",
                    sockCtx->ctxName, (ST_UINT) wakeupPort, SOCKET_ERRORNO);
    wakeupPort++;
    localAddr.sin_port = htons(wakeupPort);
  ...
  if (sRet != 0)
    {
    SOCK_LOG_ERR2 ("%s: XSocket bind() failed, errno=%d", sockCtx->ctxName, SOCKET_ERRORNO);
  ...
  if (sRet != 0)
    {
    SOCK_LOG_ERR2 ("%s: XSocket listen() failed, errno=%d", sockCtx->ctxName, SOCKET_ERRORNO);
    CLOSE_SOCKET (hListenSock);
    return (SD_FAILURE);
    }
...
/* Do non-blocking connect */
  ...
  if (sRet >= 0)
    {
    SOCK_LOG_FLOW2 ("%s: XSocket connected on port=%d", sockCtx->ctxName, (ST_UINT) wakeupPort);
    }
  else
    {
    sRet = SOCKET_ERRORNO;
    if (sRet == SOCK_WOULDBLOCK || sRet == SOCK_INPROGRESS)
      {
      SOCK_LOG_FLOW1 ("%s: XSocket connection pending", sockCtx->ctxName);
      }
    else
      {
      CLOSE_SOCKET (hListenSock);
      CLOSE_SOCKET (xCallingSock);
      SOCK_LOG_ERR2 ("%s: XSocket connect() failed, errno=%d", sockCtx->ctxName, sRet);
      return (SD_FAILURE);
      }
    }
...
/* Step 3: Do non-blocking accept */
  ...
  if (xCalledSock == INVALID_SOCKET)
    ...
    CLOSE_SOCKET (xCallingSock);
    CLOSE_SOCKET (hListenSock);
    SOCK_LOG_ERR2 ("%s: XSocket accept() failed, errno=%d", sockCtx->ctxName, sRet);
    return (SD_FAILURE);
    }

  SOCK_LOG_FLOW2 ("%s: XSocket accept() successful on port=%d", sockCtx->ctxName, wakeupPort);
  noDelay = 1;
  keepAlive = 0;
...
ST_RET ret;
  ...
  if (ret == SD_SUCCESS)
    ...
    if (portOut != NULL)
      *portOut =  (ST_INT) ntohs (pp->sin_port);
    }
  return (ret);
  }
  ...
  while (sockCtx)
    ...
    while (serviceCtrl != NULL)
      ...
      while (pSock)
        ...
        if (ret != SD_SUCCESS)
          {
          SOCK_LOG_ERR2 ("%s: %s, closing socket.", pSock->sockIdStr, errMsg);
          sockClose(pSock);
	  }
        pSock = (GEN_SOCK *)list_get_next (serviceCtrl->sockList, pSock);
        }
...
ST_INT  wRet;
  ...
  if (!bWSAStarted)
    ...
    if (wRet = WSAStartup(0x0101, &wsaData))
      {
      SLOGALWAYS1 ("convertIPAddr: unable to initialize WinSock interface to use version 1.1 (error=%d)", wRet);
      return (ipAddr);
      }
    bWSAStarted = SD_TRUE;
    }
  ...
  if (ipAddr == 0 || ipAddr == htonl(INADDR_NONE))
	...
	{
	ipAddr = 0;
        SLOGALWAYS2 ("convertIPAddr : gethostbyname IPAddr='%s'conversion error=0x%x",
