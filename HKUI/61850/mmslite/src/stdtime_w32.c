...
STDTIMEW32_STATIC STDTIME_WIN_BOOL  StdTimeW32FileTimeToLocalFileTime (
  ...
  rc = StdTimeToTimeTypeEx (&gmtStdTime, &gmtTimeType, &msec, &usec, &nsec);

  if (rc != STDTIME_OK)
    return STDTIME_WIN_FALSE;

  pStructTm = localtime (&gmtTimeType.t);
  ...
  rc = StructTmExToStdTime (&locStructTm, msec, usec, nsec, &locStdTime);
...
STDTIMEW32_STATIC STDTIME_WIN_BOOL  StdTimeW32LocalFileTimeToFileTime (
  ...
  rc = StdTimeToStructTmEx (&locStdTime, &locStructTm, &msec, &usec, &nsec);

  if (rc != STDTIME_OK)
    return STDTIME_WIN_FALSE;

  gmtTimeType.t = mktime (&locStructTm);

  if (gmtTimeType.t == (time_t) (-1))
    return STDTIME_WIN_FALSE;

  rc = TimeTypeExToStdTime (&gmtTimeType, msec, usec, nsec, &gmtStdTime);
...
STDTIMEW32_STATIC void StdTimeW32GetTimeOfDayToFileTime (FILETIME *pFileTime)
{
  QUADLIB_U64               qResult;
  QUADLIB_U64               qNum;
