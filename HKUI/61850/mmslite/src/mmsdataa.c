...
/* determine whether octect string is fixed or variable length		*/
  if (g_rt_ptr->u.p.el_len >= 0)
    {
    if (asn1r_get_octstr (aCtx, (ST_UCHAR *) DATA_DEST) || aCtx->asn1r_octetcount != abs_len)
      {
    ...
    {
    if (asn1r_get_octstr (aCtx, (ST_UCHAR *) (DATA_DEST + sizeof (ST_INT16))))
      asn1r_set_dec_err (aCtx, BAD_DATA);
    else
      {
      *((ST_INT16 *)DATA_DEST) = (ST_INT16) aCtx->asn1r_octetcount;
