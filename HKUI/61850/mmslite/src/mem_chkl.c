...
#ifdef DEBUG_SISCO
static ST_VOID *_nd_chk_realloc (ST_VOID *old, ST_UINT size)
#else
...
ST_VOID *x_chk_malloc (ST_UINT size, SD_CONST ST_CHAR *SD_CONST file, 
		       SD_CONST ST_INT line)
  {
  return (_nd_chk_malloc (size));	/* get the pointer			*/
  }

ST_VOID *x_chk_calloc (ST_UINT num, ST_UINT size, 
		       SD_CONST ST_CHAR *SD_CONST file, SD_CONST ST_INT line)
  {
  return (_nd_chk_calloc (num,size));
  }

ST_VOID *x_chk_realloc (ST_VOID *ptr, ST_UINT size, 
			SD_CONST ST_CHAR *SD_CONST file, SD_CONST ST_INT line)
  {
