...
SXD_CONTEXT *sxdContext;
  ...
  sxdContext = (SXD_CONTEXT *) usr;
#ifdef _WIN32
  sprintf (sxdContext->sTmpData,"%I64d",*pSrc);
#elif defined(_AIX) || defined(__hpux) || defined(linux) || defined(sun) || defined(__LYNX)
  sprintf (sxdContext->sTmpData,"%lld",*pSrc);
#elif (defined(__alpha) && !defined(__VMS)) 
  sprintf (sxdContext->sTmpData,"%ld",*pSrc);
#else
...
SXD_CONTEXT *sxdContext;
  ...
  sxdContext = (SXD_CONTEXT *) usr;
#ifdef _WIN32
  sprintf (sxdContext->sTmpData,"%I64u", *pSrc);
#elif defined(_AIX) || defined(__hpux) || defined(linux) || defined(sun) || defined(__LYNX)
  sprintf (sxdContext->sTmpData,"%llu", *pSrc);
#elif (defined(__alpha) && !defined(__VMS)) 
  sprintf (sxdContext->sTmpData,"%lu", *pSrc);
#else
  #error Missing INT64 code for this platform.
#endif
  sxaAddString (usr, sxdContext->sTmpData);
...
ST_CHAR timeString[MAX_TIME_STRING_LEN];
  ...
  if (Btime4ValsToString (timeString, *pSrc))
    {
...
ST_CHAR timeString[MAX_TIME_STRING_LEN];
  ...
  pMsec = pSrc;
  pDays = (pSrc+1);
  if (Btime6ValsToString (timeString, *pDays, *pMsec))
    {
...
ST_CHAR timeString[MAX_TIME_STRING_LEN];
  ...
  if (tstrTimeToString (*pSrc, timeString))
    {
...
/* Make a global temporary buffer available (easier than calloc..)	*/
  ...
  sxdContext.nStrLen = strlen (sxdContext.sxaTextWorkBuf);
  sxdContext.nDataIdx = 0;	     
  sxdContext.nStringCount = 0;
...
/* then convert to local format						*/
  ...
  if (sxaCheckIntString (sxdContext->sTmpData) == SD_SUCCESS && 
#ifdef _WIN32
      (sscanf (sxdContext->sTmpData, "%I64d", &nDest) == 1)
#elif defined(_AIX) || defined(__hpux) || defined(linux) || defined(sun) || defined(__LYNX)
      (sscanf (sxdContext->sTmpData, "%lld", &nDest) == 1)
#elif (defined(__alpha) && !defined(__VMS)) 
      (sscanf (sxdContext->sTmpData, "%ld", &nDest) == 1)
#else
  #error Missing INT64 code for this platform.
...
/* then convert to local format						*/
  ...
  if (sxaCheckUintString (sxdContext->sTmpData) == SD_SUCCESS && 
#ifdef _WIN32
      (sscanf (sxdContext->sTmpData, "%I64u", &nDest) == 1)
#elif defined(_AIX) || defined(__hpux) || defined(linux) || defined(sun) || defined(__LYNX)
      (sscanf (sxdContext->sTmpData, "%llu", &nDest) == 1)
#elif (defined(__alpha) && !defined(__VMS)) 
      (sscanf (sxdContext->sTmpData, "%lu", &nDest) == 1)
#else
  #error Missing INT64 code for this platform.
...
/* then convert to local format						*/
  ...
  bstringLength = strlen(sxdContext->sTmpData);
...
/* Look at each character in the string and verify it is valid		*/
  for (i = 0; i < slen; ++i) 
  {// hejian alter on 2010-11-11 isdigit(pC[i])
	  if ((pC[i] >= '0' && pC[1] <= '9')) 
		  continue;
	  if (i == 0  && (pC[i] == '-'))
		  continue;
