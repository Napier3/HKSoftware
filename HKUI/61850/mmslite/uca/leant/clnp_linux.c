...
ST_CHAR all_zero_mac [6] = {0, 0, 0, 0, 0, 0};
  ...
  if (recvLen == -1)
    ...
    {
    /* This should never happen, but somehow it does on SUSE Linux.	*/
    /* These should be caught by "all_zero_mac" check above. If not,	*/
    /* just log an error here.						*/
    CLNP_LOG_ERR1 ("Received Raw Ethernet frame (len = %d). FRAME IS TOO LONG. DISCARDING.", recvLen);
    }
  else
    {
