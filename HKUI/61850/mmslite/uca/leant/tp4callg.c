...
/* --------  ---  ------   -------------------------------------------  */
/* 09/17/04  JRB    13     Chg tp4_connect to return connection ID.	*/
/* 02/02/04  JRB    12     tp4_connect: fix tp_type, bind_id checks.	*/
/* 07/30/03  EJV    11     Added encrypt_ctrl to tp4_connect.		*/
...
TPDU_CX *tpdu_cr;
  ...
  if (rem_tp4_addr->tp_type == TP_TYPE_TP4)
    ...
    if (tp4_bind_id != only_tp4_bind_id)
      {
      TP_LOG_ERR1 ("tp4_connect: bind id %ld invalid. TP4 must be enabled in config.", tp4_bind_id);
      return (-1);
      }
    }    
  else		/* Transport type not TP4, must be TP0	*/
#if defined (TP0_ENABLED)	/* If TP0 enabled, call TP0 function	*/
    {
    return (tp0_connect (tp4_bind_id, user_conn_id, rem_tp4_addr,
            conndata_len, conndata, encrypt_ctrl));
    }
#else				/* If TP0 not enabled, this is error.	*/
    {
    TP_LOG_ERR0 ("tp4_connect: invalid transport type");
    return (-1);
    }
#endif
