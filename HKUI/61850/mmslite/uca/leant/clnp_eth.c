...
/* Custom Subnetwork read function pointer. Set by call to	*/
/* clnp_snet_read_hook_add.					*/
ST_RET (*clnp_snet_read_hook_ptr) (SN_UNITDATA *sn_req);
...
/************************************************************************/
/*			clnp_snet_read_hook_add				*/
/* Add subnetwork read hook function to do custom processing of a	*/
/* received packet.							*/
/* NOTE: this function really belongs in the Subnetwork interface	*/
/*   module, but they are customized for each platform.			*/
/************************************************************************/
ST_RET clnp_snet_read_hook_add (
		ST_RET (*usr_fun)(SN_UNITDATA *sn_req))	/* hook function*/
  {
ST_RET retcode;
  /* For now, only allow one hook function.	*/
  if (clnp_snet_read_hook_ptr==NULL)
    {
    clnp_snet_read_hook_ptr = usr_fun;
    retcode = SD_SUCCESS;
    }
  else
    retcode = SD_FAILURE;
...
/************************************************************************/
/*			clnp_snet_read_hook_remove			*/
/* Remove subnetwork read hook function that was previously added by	*/
/* call to clnp_snet_read_hook_add (i.e. stop custom processing).	*/
/************************************************************************/
...
ST_RET retcode;
  /* For now, only allow one hook function.	*/
  if (clnp_snet_read_hook_ptr)
    {
    clnp_snet_read_hook_ptr = NULL;
    retcode = SD_SUCCESS;
    }
  else
    retcode = SD_FAILURE;
...
/************************************************************************/
/*			clnp_snet_read_hook_process			*/
/* Call all necessary hook functions.					*/
/* RETURN: SD_SUCCESS if packet was processed.				*/
/*	   SD_FAILUE if not processed (main code should process it).	*/
/************************************************************************/
...
ST_RET retcode;
  /* For now, only allow one hook function.	*/
  if (clnp_snet_read_hook_ptr)
    retcode = (*clnp_snet_read_hook_ptr) (sn_req);
  else
    retcode = SD_FAILURE;
  return (retcode);
  }
