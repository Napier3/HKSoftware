...
ST_BOOLEAN macRetrieved;
  ...
  /* check all of the command line parameters for a mac address */
  for (i = 1; i < argc; i++)
    {
    macRetrieved = getMacAddress(argv[i]);
    if (macRetrieved == SD_TRUE)
      break;
    }    
...
ST_VOID printUsage ()
  ...
  printf ("\n\n [mac address]  =  a series of 12 hexadecimal digits");
...
/************************************************************************/
/*                            getMacAddress                             */
/* This function will convert a string into a unsigned character array  */
/* that holds the resulting mac address                                 */
/************************************************************************/
  
ST_BOOLEAN getMacAddress (ST_CHAR *mac)
  {
ST_CHAR *charPtr;
ST_CHAR hexChar;
ST_INT  intValue;
ST_INT  numHexVals;
  ...
  /* parse the string and derive the mac address */

  /* initialize character pointer to string input by user */  
  charPtr = mac;
  ...
  while (numHexVals != 6)
    ...
    hexChar = *charPtr++;
    /* check to see if the character is a legit hex char */
    if (isxdigit (hexChar))
      {
      if (hexChar >= '0' && hexChar <= '9')
        {
        /* the character falls within 0 - 9 */
        intValue = (atoi (&hexChar)) << 4;
        }
      else
        {
        /* the character falls within A - F */
        intValue = (hexChar - 55) << 4;
        }
      }
    else
      {
      /* this is not a valid remote mac address */
      break;
      }
