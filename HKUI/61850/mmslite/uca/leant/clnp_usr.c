...
/*----------------------------------------------------------------------*/
/* CLNP variables exposed to CLNP-user					*/
/*----------------------------------------------------------------------*/

	/* Struct below holds parameters needed	for the CLNP operations.*/
...
/* Function pointers default to "clnp_read". If threads started,	*/
/* may be changed to "clnp_read_main" or "clnp_read_goose".		*/
N_UNITDATA * (*clnp_read_fun_cl) (ST_VOID);
N_UNITDATA * (*clnp_read_fun_co) (ST_VOID);
ST_EVENT_SEM hMainEvent;
...
static ST_VOID	clnpl_log_nsdu (N_UNITDATA *req, ST_ULONG log_mask);
...
ST_RET ret, ret_param;
  ...
  if (!clnpl_initialized)
    {
    if (clnp_read_fun_cl == NULL)
      clnp_read_fun_cl = clnp_read;	/* Set funct ptr to default funct.*/
    if (clnp_read_fun_co == NULL)
      clnp_read_fun_co = clnp_read;	/* Set funct ptr to default funct.*/
...
/* from the CLNP.							*/
/* The CLNP-user should free the returned N_UNITDATA pointer after the	*/
/* indication has been processed.					*/
/*									*/
/* Parameters:								*/
/*	ST_VOID			none					*/
/*									*/
...
{
ST_RET		 ret;		/* used internally by the func */
N_UNITDATA	*req;
SN_UNITDATA	 sn_req;
ST_UCHAR	 dest_lsap;	/* From LLC decoding			*/

  req = NULL;		/* assume there is no indication received	*/

  if (clnpl_initialized)
    {
    /* read data from sub-network */

    if ((ret = clnp_snet_read (&sn_req)) == SD_SUCCESS)
      {
      assert (sn_req.lpdu != NULL);	/* If read successful, lpdu should NEVER be NULL*/

      /* Pass packet to "hook" functions.		*/
      /* If it fails, continue with default decoding.	*/
      if (clnp_snet_read_hook_process (&sn_req) == SD_SUCCESS)
        CLSNS_LOG_IND0 ("Received Subnetwork packet processed by user 'hook' function");
      else
        {
        /* Make sure len is legal before logging or decoding.	*/
        if (sn_req.lpdu_len <= ETHE_MAX_LEN_UDATA)
          {
          /* log the data received from sub-network */
          clnpl_log_snsdu (&sn_req, CLSNS_LOG_IND);

          /* First decode the LLC layer.						*/
          /* Note that the LLC decoding may return err code if the received	*/
          /* LPDU is not to be handled by this stack.  In this case we just	*/
          /* ignore the LPDU.							*/

          if ((ret = clnpl_llc_decode (sn_req.lpdu, &dest_lsap)) == SD_SUCCESS)
            {
            /* LLC decoding was OK, decode the NPDU.				*/
            switch (dest_lsap)
              {
              case LLC_LSAP:		/* Normal LSAP */
                /* We have received a LPDU, decode it */
                ret = clnpl_decode (&sn_req, &req);
                if (req != NULL)
                  {
                  /* N_UNITDATA.indication before we handle it to CLNP-user	*/
                  clnpl_log_nsdu (req, CLNP_LOG_IND);
                  }
              break;

#if defined(UCA_SMP)
              case LLC_LSAP_SMP:	/* UCA Station Management Protocol LSAP */
                {
                SMP_DATAUNIT smp_du;

                memset (&smp_du, 0, sizeof (SMP_DATAUNIT));
                smp_du.pdu_buf = sn_req.lpdu + LLC_HDR_LEN;
                smp_du.pdu_len = sn_req.lpdu_len - LLC_HDR_LEN;
                smp_du.loc_mac = sn_req.loc_mac;
                smp_du.rem_mac = sn_req.rem_mac;
                if (clnp_snet_get_type () == SUBNET_ADLC)
                  {
                  /* data has appended rec_time on the end */
              ...
              default:
                CLSNS_LOG_IND1 ("Discarding received NPDU (unsupported LLC LSAP=0x%02X)", dest_lsap);
              }
            }
          }	/* end if lpdu_len OK	*/
        else
          CLSNS_LOG_IND1 ("Discarding received NPDU (len = 0x%X is not legal for CLNP)", sn_req.lpdu_len);
	}
      /* we are done with the processing, free anything		*/
      /* allocated by sub-network					*/

      clnp_snet_free (&sn_req);
      }	/* end if clnp_snet_read returned SD_SUCCESS*/
    }
  else
    {
    CLNP_LOG_ERR0 ("CLNP-ERROR: read failed (CLNP not initialized)");
    }

  return (req);	/* if DT PDU received and decoding OK then req != NULL	*/
}
