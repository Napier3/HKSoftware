...
long CRioIMax::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CRioIMax::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
	CXrioCfgMngrXmlRWKeys *pXmlKeys = (CXrioCfgMngrXmlRWKeys*)pXmlRWKeys;
...
long CRioIMax::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
void CRioIMax::InitAfterRead()
{

}

BOOL CRioIMax::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
...
BOOL CRioIMax::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
...
CBaseObject* CRioIMax::Clone()
{
	CRioIMax *p = new CRioIMax();
	Copy(p);
	return p;
}

BOOL CRioIMax::CanPaste(UINT nClassID)
{
...
CExBaseObject* CRioIMax::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
...
CExBaseObject* CRioIMax::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
...
void CRioIMax::InsertOwnTreeCtrl(CTreeCtrl *pTreeCtrl, HTREEITEM htiParent, DWORD dwParam)
{
