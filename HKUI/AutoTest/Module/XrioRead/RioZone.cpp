// Zone.cpp : ??????
//

#include "stdafx.h"
#include "RioZone.h"


// CZone

CRioZone::CRioZone()
{
    //?????????
//    m_strName = _T("Zone???");
    m_strType = _T("");
    m_strBlockType = _T("");
//    m_strID = _T("");
    m_strEnabled = _T("");
}

CRioZone::~CRioZone()
{
}

long CRioZone::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
    CXrioCfgMngrXmlRWKeys *pXmlKeys = (CXrioCfgMngrXmlRWKeys*)pXmlRWKeys;
    xml_GetAttibuteValue(pXmlKeys->m_strTypeKey, oNode, m_strType);
    xml_GetAttibuteValue(pXmlKeys->m_strBlockTypeKey, oNode, m_strBlockType);
    xml_GetAttibuteValue(pXmlKeys->m_strEnabledKey, oNode, m_strEnabled);
    xml_GetAttibuteValue(pXmlKeys->m_strIdKey, oNode, m_strID);
    xml_GetElementText(pXmlKeys->m_strNameKey, oNode, m_strName);

    return 0;
}

long CRioZone::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
   CXrioCfgMngrXmlRWKeys *pXmlKeys = (CXrioCfgMngrXmlRWKeys*)pXmlRWKeys;
   xml_SetAttributeValue(pXmlKeys->m_strTypeKey, oElement, m_strType);
   xml_SetAttributeValue(pXmlKeys->m_strBlockTypeKey, oElement, m_strBlockType);
   xml_SetAttributeValue(pXmlKeys->m_strEnabledKey, oElement, m_strEnabled);
   xml_SetAttributeValue(pXmlKeys->m_strIdKey, oElement, m_strID);
   xml_SetElementText(pXmlKeys->m_strNameKey, oXMLDoc, oElement, m_strName);


    return 0;
}

long CRioZone::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
    if(oBinaryBuffer.IsCalSizeMode())
    {

        BinarySerializeCalLen(oBinaryBuffer, m_strType);
        BinarySerializeCalLen(oBinaryBuffer, m_strBlockType);
        BinarySerializeCalLen(oBinaryBuffer, m_strID);
    BinarySerializeCalLen(oBinaryBuffer, m_strEnabled);
    }
    else if(oBinaryBuffer.IsReadMode())
    {

        BinarySerializeRead(oBinaryBuffer, m_strType);
        BinarySerializeRead(oBinaryBuffer, m_strBlockType);
        BinarySerializeRead(oBinaryBuffer, m_strID);
    BinarySerializeRead(oBinaryBuffer, m_strEnabled);
    }
    else if(oBinaryBuffer.IsWriteMode())
    {

        BinarySerializeWrite(oBinaryBuffer, m_strType);
        BinarySerializeWrite(oBinaryBuffer, m_strBlockType);
        BinarySerializeWrite(oBinaryBuffer, m_strID);
    BinarySerializeWrite(oBinaryBuffer, m_strEnabled);
    }
    return 0;
}


BOOL CRioZone::IsEqualOwn(CBaseObject* pObj)
{
    if(this == pObj)
    {
        return TRUE;
    }

    CRioZone *p = (CRioZone*)pObj;


    if(m_strType != p->m_strType)
    {
        return FALSE;
    }
    if (m_strBlockType != p->m_strBlockType)
    {
        return FALSE;
    }
    if (m_strID != p->m_strID)
    {
        return FALSE;
    }
    if(m_strEnabled != p->m_strEnabled)
    {
        return FALSE;
    }

    return TRUE;
}

BOOL CRioZone::CopyOwn(CBaseObject* pDest)
{
    if(this == pDest)
    {
        return TRUE;
    }

    CRioZone *p = (CRioZone*)pDest;

    p->m_strType = m_strType;
    p->m_strBlockType = m_strBlockType;
    p->m_strID = m_strID;
    p->m_strEnabled = m_strEnabled;

    return TRUE;
}

CBaseObject* CRioZone::Clone()
{
    CRioZone *p = new CRioZone();
    Copy(p);
    return p;
}

BOOL CRioZone::CanPaste(UINT nClassID)
{
    if (nClassID == XRIOCLASSID_ZONE ||
        nClassID == XRIOCLASSID_GENERICSHAPE || nClassID == XRIOCLASSID_ACTIVE || nClassID == XRIOCLASSID_MHOSHAPE
     || nClassID == XRIOCLASSID_INDEX || nClassID == XRIOCLASSID_FAULTLOOP || nClassID == XRIOCLASSID_TOLERANCES
    )
    {
        return TRUE;
    }
    return FALSE;
}

CExBaseObject* CRioZone::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
    CExBaseObject *pNew  = NULL;
    CXrioCfgMngrXmlRWKeys *pXmlKeys = (CXrioCfgMngrXmlRWKeys*)pXmlRWKeys;

    if (strClassID == pXmlKeys->m_strGenricShapeKey)
    {
        pNew = new CRioGenricShape();
    }
    if (strClassID == pXmlKeys->m_strActiveKey)
    {
        pNew = new CRioActive();
    }
    if (strClassID == pXmlKeys->m_strMhoShapeKey)
    {
        pNew = new CRioMhoShape();
    }
    if (strClassID == pXmlKeys->m_strIndexKey)
    {
        pNew = new CRioIndex();
    }
    if (strClassID == pXmlKeys->m_strFaultLoopKey)
    {
        pNew = new CRioFaultLoop();
    }
    if (strClassID == pXmlKeys->m_strTolerancesKey)
    {
        pNew = new CRioTolerances();
    }

    return pNew;
}

CExBaseObject* CRioZone::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
    CExBaseObject *pNew  = NULL;

     if (nClassID == XRIOCLASSID_GENERICSHAPE)
    {
        pNew = new CRioGenricShape();
    }
     if (nClassID == XRIOCLASSID_ACTIVE)
     {
         pNew = new CRioActive();
     }
     if (nClassID == XRIOCLASSID_MHOSHAPE)
     {
         pNew = new CRioMhoShape();
     }
     if (nClassID == XRIOCLASSID_INDEX)
     {
         pNew = new CRioIndex();
     }
     if (nClassID == XRIOCLASSID_FAULTLOOP)
     {
         pNew = new CRioFaultLoop();
     }
     if (nClassID == XRIOCLASSID_TOLERANCES)
     {
         pNew = new CRioTolerances();
     }

    return pNew;
}

void CRioZone::InsertOwnTreeCtrl(CTreeCtrl *pTreeCtrl, HTREEITEM htiParent, DWORD dwParam)
{
    CString strZoneInfo;
    strZoneInfo.Format(_T("ZoneType = %s"),m_strType.GetString());
    m_dwItemData = (DWORD)pTreeCtrl->InsertItem(strZoneInfo, 0, 0, htiParent);
    pTreeCtrl->SetItemData((HTREEITEM)m_dwItemData,(DWORD)this);
}

