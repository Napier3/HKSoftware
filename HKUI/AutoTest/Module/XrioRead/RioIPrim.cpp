...
long CRioIPrim::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CRioIPrim::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
	CXrioCfgMngrXmlRWKeys *pXmlKeys = (CXrioCfgMngrXmlRWKeys*)pXmlRWKeys;
...
long CRioIPrim::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
void CRioIPrim::InitAfterRead()
{

}

BOOL CRioIPrim::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
...
BOOL CRioIPrim::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
...
CBaseObject* CRioIPrim::Clone()
{
	CRioIPrim *p = new CRioIPrim();
	Copy(p);
	return p;
}

BOOL CRioIPrim::CanPaste(UINT nClassID)
{
...
CExBaseObject* CRioIPrim::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
...
CExBaseObject* CRioIPrim::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
...
void CRioIPrim::InsertOwnTreeCtrl(CTreeCtrl *pTreeCtrl, HTREEITEM htiParent, DWORD dwParam)
{
