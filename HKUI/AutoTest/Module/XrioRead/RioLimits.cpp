...
long CRioLimits::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CRioLimits::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
	CXrioCfgMngrXmlRWKeys *pXmlKeys = (CXrioCfgMngrXmlRWKeys*)pXmlRWKeys;
...
long CRioLimits::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
BOOL CRioLimits::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
...
BOOL CRioLimits::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
...
CBaseObject* CRioLimits::Clone()
{
	CRioLimits *p = new CRioLimits();
	Copy(p);
	return p;
}

BOOL CRioLimits::CanPaste(UINT nClassID)
{
    if (nClassID == XRIOCLASSID_LIMITS ||
        nClassID == XRIOCLASSID_VMAX_LL ||
        nClassID == XRIOCLASSID_IMAX
	)
	{
		return TRUE;
	}
...
CExBaseObject* CRioLimits::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
	CExBaseObject *pNew  = NULL;
	CXrioCfgMngrXmlRWKeys *pXmlKeys = (CXrioCfgMngrXmlRWKeys*)pXmlRWKeys;

    if (strClassID == pXmlKeys->m_strVMaxLLKey)
	{
        pNew = new CRioVMaxLL();
	}
    else if (strClassID == pXmlKeys->m_strIMaxKey)
	{
        pNew = new CRioIMax();
	}
...
CExBaseObject* CRioLimits::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;

    if (nClassID == XRIOCLASSID_VMAX_LL)
	{
        pNew = new CRioVMaxLL();
	}
    else if (nClassID == XRIOCLASSID_IMAX)
	{
        pNew = new CRioIMax();
	}
...
void CRioLimits::InsertOwnTreeCtrl(CTreeCtrl *pTreeCtrl, HTREEITEM htiParent, DWORD dwParam)
{
    CString strLimits;
    strLimits.Format(_T("Limits Type = %s"),m_strType.GetString());
    m_dwItemData = (DWORD)pTreeCtrl->InsertItem(strLimits, 0, 0, htiParent);
