...
long CRioBay::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CRioBay::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
	CXrioCfgMngrXmlRWKeys *pXmlKeys = (CXrioCfgMngrXmlRWKeys*)pXmlRWKeys;
...
long CRioBay::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
void CRioBay::InitAfterRead()
{

}

BOOL CRioBay::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
...
BOOL CRioBay::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
...
CBaseObject* CRioBay::Clone()
{
	CRioBay *p = new CRioBay();
	Copy(p);
	return p;
}

BOOL CRioBay::CanPaste(UINT nClassID)
{
...
CExBaseObject* CRioBay::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
...
CExBaseObject* CRioBay::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
...
void CRioBay::InsertOwnTreeCtrl(CTreeCtrl *pTreeCtrl, HTREEITEM htiParent, DWORD dwParam)
{
