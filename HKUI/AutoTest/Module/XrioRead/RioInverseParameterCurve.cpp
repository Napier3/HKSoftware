...
CRioInverseParameterCurve::CRioInverseParameterCurve()
{
	//��ʼ������
//    m_strName = _T("");
	m_strType = _T("");
...
long CRioInverseParameterCurve::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CRioInverseParameterCurve::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
	CXrioCfgMngrXmlRWKeys *pXmlKeys = (CXrioCfgMngrXmlRWKeys*)pXmlRWKeys;
...
long CRioInverseParameterCurve::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
BOOL CRioInverseParameterCurve::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
    ...
    CRioInverseParameterCurve *p = (CRioInverseParameterCurve*)pObj;
...
BOOL CRioInverseParameterCurve::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
    ...
    CRioInverseParameterCurve *p = (CRioInverseParameterCurve*)pDest;
...
CBaseObject* CRioInverseParameterCurve::Clone()
{
    CRioInverseParameterCurve *p = new CRioInverseParameterCurve();
	Copy(p);
	return p;
}

BOOL CRioInverseParameterCurve::CanPaste(UINT nClassID)
{
    if (nClassID == XRIOCLASSID_INVERSEPARAMETERCURVE ||
        nClassID == XRIOCLASSID_TIMEDOVERCURRENTELEMENT_NAME ||
            ...
            nClassID == XRIOCLASSID_PARAMETERF
	)
	{
		return TRUE;
	}
...
CExBaseObject* CRioInverseParameterCurve::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
    ...
    if (strClassID == pXmlKeys->m_strParameterFKey)
    {
        pNew = new CRioParameterF();
    }
...
CExBaseObject* CRioInverseParameterCurve::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
     ...
     if (nClassID == XRIOCLASSID_PARAMETERA)
	{
     ...
     if (nClassID == XRIOCLASSID_PARAMETERF)
     {
         pNew = new CRioParameterF();
     }
...
void CRioInverseParameterCurve::InsertOwnTreeCtrl(CTreeCtrl *pTreeCtrl, HTREEITEM htiParent, DWORD dwParam)
{
