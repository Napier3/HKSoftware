...
long CXrioCustomParameter::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
    ...
    //获取Unit子对象中的属性，该属性应该在Unit节点中，但是该节点按正常处理不应该作为CreateNewChild的对象。
    CXmlRWNodeBase *UnitoNode = oNode.GetChildNode(pXmlKeys->m_strUnitKey);
    if(UnitoNode != NULL)
    {
        xml_GetAttibuteValue(pXmlKeys->m_strUnitDecimalPlacesKey, *oNode.GetChildNode(pXmlKeys->m_strUnitKey), m_strDecimalPlaces);
    }
...
BOOL CXrioCustomParameter::GetParameterFullPath(CString &strValue)
{
	...
	while(pParent != NULL)
	{
        ...
        strValue.Format(_T("%s$%s"),pParent->m_strID.GetString(),strTmp.GetString());
		strTmp = strValue;
