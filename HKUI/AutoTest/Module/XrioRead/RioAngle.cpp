...
long CRioAngle::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CRioAngle::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
	CXrioCfgMngrXmlRWKeys *pXmlKeys = (CXrioCfgMngrXmlRWKeys*)pXmlRWKeys;
...
long CRioAngle::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
void CRioAngle::InitAfterRead()
{

}

BOOL CRioAngle::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
...
BOOL CRioAngle::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
...
CBaseObject* CRioAngle::Clone()
{
    CRioAngle *p = new CRioAngle();
	Copy(p);
	return p;
}

BOOL CRioAngle::CanPaste(UINT nClassID)
{
...
CExBaseObject* CRioAngle::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
...
CExBaseObject* CRioAngle::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
...
void CRioAngle::InsertOwnTreeCtrl(CTreeCtrl *pTreeCtrl, HTREEITEM htiParent, DWORD dwParam)
{
