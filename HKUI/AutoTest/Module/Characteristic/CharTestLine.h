...
class CCharTestLine : public CExBaseObject
{
public:
    CCharTestLine();
    virtual ~CCharTestLine();
	...
	long m_nTestLineID;
    long m_nFaultType; //20240829 wanmj 故障类型
    CXDrawLine *m_pDrawLineRef;     //20240920 wanmj 最新添加的线段

//重载函数
public:
    virtual UINT GetClassID() {    return CHARCLASSID_CCHARTESTLINE;   }
    virtual BSTR GetXmlElementKey()  {      return CCharacteristicXmlRWKeys::CCharTestLineKey();     }
...
public:
	...
	void SetTestLineUserDef()	{	m_strType = CHAR_TEST_LINE_TYPE_USER;	}
	void SetTestLineSys()	{	m_strType = CHAR_TEST_LINE_TYPE_SYS;	}
};
