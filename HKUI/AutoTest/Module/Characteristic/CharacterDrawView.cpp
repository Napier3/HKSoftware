...
CCharacterDrawView::CCharacterDrawView()
{
	...
	m_bShowAllCharacts = TRUE;//默认显示全部曲线
}
...
DWORD CCharacterDrawView::XCreateElement(CExBaseObject *pData, CWnd *pParentWnd)
{
	...
	else if (nClassID == CHARCLASSID_CCHARACTERISTICS)
	{
        ...
        CExBaseList *pCharTemp = NULL;
		POS pos = m_pCharacteristics->GetHeadPosition();

		
		while (pos != NULL)
		{
            pCharTemp = (CExBaseList*)m_pCharacteristics->GetNext(pos);
            if(pCharTemp->GetClassID() != CHARCLASSID_CCHARACTERISTIC)
                continue;

			CCharacteristic *pChar = (CCharacteristic *)pCharTemp;
			if (m_bShowAllCharacts || (pChar->m_nUse != 0))
			{//显示全部曲线或显示所有勾选使用的曲线
				InitCharDrawView(pChar, pParentWnd);
			}
			//else
			//{
			//	if (pChar->m_nUse == 1)//不显示全部曲线时 只显示勾选使用的曲线 20240923 luozibing
			//	{
			//		InitCharDrawView(pChar, pParentWnd);
			//	}
			//}
...
// void CCharacterDrawView::UpdateCurCharact(CXDrawList *pDrawList)
// {
// }
...
//     if(pCharTemp == m_pCharacteristic)
//     {
//         UpdateCurCharact(pDrawList);
//     }
...
void CCharacterDrawView::xdraw_TestSolvingEquation()
{
	...
	xdraw_SolveEquation(pCircle1, pCircle2, listDatas, true);
	xdraw_SolveEquation(pCircle1, pLine, listDatas, true);
	xdraw_SolveEquation(pCircle2, pLine, listDatas, true);
...
//20240806 gongyiping 图形自动比例调整
BOOL CCharacterDrawView::SetAxisAdjust(double dX ,double dCoef)
{
	CAxisInterface *pAxisInterface = NULL;
	m_pXDrawMngr_Axis->GetAxisInterface(&pAxisInterface);
