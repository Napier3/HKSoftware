...
public:
    ...
    void SetCurCharact(CCharacteristic *pCharacterist){ m_pCharacteristic = pCharacterist;}

//    virtual void SetCurDrawList(CXDrawList *pDrawListRef){  }
//    virtual CXDrawList* GetCurDrawList(){ return NULL; }

	BOOL m_bShowAllCharacts;//显示所有曲线(TRUE)或根据曲线m_nuUse参数显示曲线(FALSE）luozibing 20240923
protected:
...
private:
    ...
    //CCharacteristics       *m_pCharacteristics;  //20240822 wanmj 改为保护成员
...
//    virtual void UpdateCurCharact(CXDrawList *pDrawList);
...
public:
	...
	//20240806 gongyiping 自动准同期图形调整
	BOOL SetAxisAdjust(double dX ,double dCoef = 2.0);
