...
#include "../../../../RelayTest/Module/UI/SttTestCntrCmdDefine.h"
...
void CSttCharacterDrawPng::InitCharDrawTest(const CString &strCharID, long nIndex)
{
	...
	while (pos)
	{
		pCurrObj = (CExBaseObject*)m_pCharacteristics->GetNext(pos);
...
void CSttCharacterDrawPng::InitCharDrawTest(const CString &strMacro_UI_ID, const CString &strCharID, long nIndex)
{
	InitCharDrawTest(strCharID, nIndex);

	if (strMacro_UI_ID == STT_ORG_MACRO_CurrInverseTimeTest || (strMacro_UI_ID == STT_ORG_MACRO_CurrInverseTimeTest_I))
	{
		InitCharDrawTestAxis(CSttCharacterDrawPng::CurrInverseTime);
	}
	else if (strMacro_UI_ID == STT_ORG_MACRO_VolInverseTimeTest || (strMacro_UI_ID == STT_ORG_MACRO_VolInverseTimeTest_I))
	{
		InitCharDrawTestAxis(CSttCharacterDrawPng::VolInverseTime);
	}
	else if (strMacro_UI_ID == STT_ORG_MACRO_UFInverseTimeTest)
	{
		InitCharDrawTestAxis(CSttCharacterDrawPng::VolInverseTime);
	}
	else if ((strMacro_UI_ID == STT_ORG_MACRO_DiffCurr6ITest) || (strMacro_UI_ID  == STT_ORG_MACRO_LineDiffCurr6ITest)
		|| (strMacro_UI_ID == STT_ORG_MACRO_BusDiffCurr6ITest) || (strMacro_UI_ID == STT_ORG_MACRO_ElecRailwayDiffTest)
		|| (strMacro_UI_ID == STT_ORG_MACRO_DiffTest_I))
	{
		InitCharDrawTestAxis(CSttCharacterDrawPng::DiffCurr6I);
	}
	else if ((strMacro_UI_ID == STT_ORG_MACRO_DistanceTest) || (strMacro_UI_ID == STT_ORG_MACRO_DistanceTest_I))
	{
		InitCharDrawTestAxis(CSttCharacterDrawPng::Distance);
	}
	else if ((strMacro_UI_ID == STT_ORG_MACRO_DistanceSearchTest) || (strMacro_UI_ID == STT_ORG_MACRO_DistSearchOneTest))
	{
		InitCharDrawTestAxis(CSttCharacterDrawPng::DistanceSearch);
	}
	else
	{
		InitCharDrawTestAxis(NULL);
	}
}

void CSttCharacterDrawPng::InitCharDrawTest_I(const CString &strMacro_UI_ID, const CString &strMacroID_Tester, long nFaultType)
{
	//每次初始化时,都需要清空测试点结果
	ClearSearchPoints_png();
	ClearPngCache();
	...
	m_pCharacteristic = NULL;//各模块根据需要查找该对应给其赋值,对于距离和阻抗搜索显示全部区段,此处初始化为空即可,反时限模块只有一个charact对象也初始化为空即可
	...
	if (strMacro_UI_ID == STT_ORG_MACRO_CurrInverseTimeTest_I)
	{
		InitCharDrawTestAxis(CSttCharacterDrawPng::CurrInverseTime);
	}
	else if (strMacro_UI_ID == STT_ORG_MACRO_VolInverseTimeTest_I)
	{
		InitCharDrawTestAxis(CSttCharacterDrawPng::VolInverseTime);
	}
	else if (strMacro_UI_ID == STT_ORG_MACRO_DiffTest_I)
	{
		InitCharDrawTestAxis(CSttCharacterDrawPng::DiffCurr6I);
	}
	else if (strMacro_UI_ID == STT_ORG_MACRO_DistanceTest_I)
	{
		InitCharDrawTestAxis(CSttCharacterDrawPng::Distance);
	}
	else if (strMacro_UI_ID == STT_ORG_MACRO_DistSearchOneTest)
	{
		InitCharDrawTestAxis(CSttCharacterDrawPng::DistanceSearch);
	}
	else
