...
CExBaseObject* CCharElement::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	...
	if (nClassID == CHARCLASSID_CCHARTESTLINE)
	{
		pNew = new CCharTestLine();
	}
...
// 	}
	...
	while (pos != NULL)
	{
		pLine = (CCharTestLine *)oListSys.GetNext(pos);
		pLine->m_fPos = nIndex;
		nIndex++;
	}
...
long CCharElement::GetTestLines_Sys()
{
	...
	while (pos != NULL)
	{
		...
		if (p->GetClassID() == CHARCLASSID_CCHARTESTLINE)
		{
			if ( ((CCharTestLine*)p)->IsTestLineSys())
			{
				nCount++;
			}
		}
