									...
									, const CString &strEngineMode, BOOL bUseOwn)
{
	...
	while (pos != NULL)
	{
		...
		if (bUseOwn)
		{
			...
			//CLogPrint::LogFormatString(XLOGLEVEL_ERROR, _T("++++for debug  CPpDeviceRefs::CreateAllDevice+++ bUseOwn++++%s"), pCpus->m_strPpEngineProgID.GetString());
		}
		else
		{
		...
		if (pPpDeviceRef->m_strDeviceModelFile.GetLength() == 0)
		{
			...
			//CLogPrint::LogFormatString(XLOGLEVEL_ERROR, _T("++++for debug  CPpDeviceRefs::CreateAllDevice+++ 2++++%s"), pCpus->m_strPpEngineProgID.GetString());
		}
