...
#ifdef _use_QSttFAParasSetDialog
...
void QSttTestCntrFrameLinux::CalRectArea()
{
	...
	{
        m_rcWorkArea = QRect(0
                             , pToolBarParas->m_nHeight + SttFrame_Interval + m_pMenuBar->height()
                             , m_pSttFrameConfig->m_nWidth
                             ,nHeight);
	}
	else
	{
        ...
        m_rcWorkArea = QRect(pSize->m_nWidth
                             , pToolBarParas->m_nHeight + SttFrame_Interval  + m_pMenuBar->height()
                             , m_pSttFrameConfig->m_nWidth - pSize->m_nWidth - SttFrame_Interval
                             , nHeight);
...
void QSttTestCntrFrameLinux::InitFrame( const CString &strFrameCfgFile )
{
...
// 	}
    ...
    long nLenSend =g_oSttTestAppConfigTool.m_pClientEngine->m_pSttClientSocket->XSendCmd(&oCmd);
    ...
    else
    {
        ...
        system("pkill -9 SttTestServer_PNS330");
		system("pkill -9 SttNativeMain");
 //	 	system("pkill -9 TestCtrlCntrLinuxD");
...
void QSttTestCntrFrameLinux::OnMacroTest(CSttMacroTestUI_TestMacroUI* pTestMacroUI)
{
...
void QSttTestCntrFrameLinux::EnableMenu(const CString& strID, BOOL bEnable)
{
...
void QSttTestCntrFrameLinux::EnableAction(const CString& strID, BOOL bEnable, BOOL bSelect)
{
    for (int i = 0; i < m_pMenuBar->m_listAct.size(); i++)
    {
...
void QSttTestCntrFrameLinux::SelectActionByID(const CString& strID, BOOL bSelect)
{
...
void QSttTestCntrFrameLinux::SelectActionByName(const CString& strName, BOOL bSelect)
{
    QAction* pAction =  NULL;
    for (int i = 0; i < m_pMenuBar->m_listAct.size(); i++)
    {
...
void QSttTestCntrFrameLinux::ShowButtonsByID(const CString& strButtonID, BOOL bShow)
{
...
void QSttTestCntrFrameLinux::InitToolButtons_AfterInit()
{
    //Menu
...
//    if (!IsAutoTest())
//    {
//        SelectActionByID(STT_CNTR_CMD_ItemList, FALSE);
//    }
//    else
//    {
...
//    EnableAction(STT_CNTR_CMD_OutputTabView, TRUE, TRUE);
//    SelectActionByID(STT_CNTR_CMD_OutputTabView, TRUE);
//    EnableAction(STT_CNTR_CMD_SettingLink, TRUE, TRUE);
//    SelectActionByID(STT_CNTR_CMD_SettingLink, TRUE);

    //ToolBar
    EnableStartStopButton(FALSE);
    EnableManualTriggerButton(FALSE);
    QSttTestCntrFrameBase::UpdateButtonsStateByID(STT_CNTR_CMD_StopTest, FALSE);
    //QSttTestCntrFrameBase::UpdateButtonsStateByID(STT_CNTR_CMD_AddMacro, FALSE);
    QSttTestCntrFrameBase::UpdateButtonsStateByID(STT_CNTR_CMD_SaveTemplate, FALSE);
    QSttTestCntrFrameBase::UpdateButtonsStateByID(STT_CNTR_CMD_SaveAsTemplate, FALSE);
    QSttTestCntrFrameBase::UpdateButtonsStateByID(STT_CNTR_CMD_TestThis, FALSE);
...
void QSttTestCntrFrameLinux::OnCmd_IecCap()
{
	...
	{
		g_theSttIecRecordMainWidget->InitIecRecordCbWidget(g_pTheSttTestApp->m_pTestMacroUI->m_strID,-1);
	}
}
