...
void QLongLineEdit::SetValue(long *pnValue)
{
	disconnect(this, SIGNAL(editingFinished()), this,SLOT(slot_UpateLongValue()));
	m_pnValue = pnValue;
	SetValue(*m_pnValue);
	connect(this, SIGNAL(editingFinished()), SLOT(slot_UpateLongValue()));
}

void QLongLineEdit::slot_UpateLongValue()
{
	if (m_pnValue != NULL)
	{
		*m_pnValue = GetValue();
