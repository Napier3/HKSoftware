...
// #endif
	...
	//m_cmbCb->SetBoxSize(16, 30);
	m_cmbCb->setFont(font);
	...
	//m_cmbFunc->SetBoxSize(16, 30);
	m_cmbFunc->setFont(font);
...
//     ui->verticalLayout->insertWidget(0,m_pGridIecCb);
	...
	//m_cmbHarmDisplay->SetBoxSize(20, 30); //chenling 2024.5.14  设置字体大小
	m_cmbHarmDisplay->setFont(font);
...
void QSttIecRecordCbWidget::BtnBack()
{
	...
	{
		//disconnect(this, SIGNAL(sig_UpdateMUTimeAccurRlt(CDvmData*)), g_TestWinBase, SLOT(slot_UpdateMUTimeAccurRlt(CDvmData*)));// suyang g_theTestCntrFrame --g_TestWinBase
		m_pSVDelayData = NULL;
	}
...
void QSttIecRecordCbWidget::Connect_UpdateMUTimeAccurRlt(BOOL bCoonect)
{
	...
	if ((m_pSVDelayData != NULL)&&(pXSttCap_61850!= NULL))
	{
		if (bCoonect)
		{
			//connect(this, SIGNAL(sig_UpdateMUTimeAccurRlt(CDvmData*)), g_theTestCntrFrame, SLOT(slot_UpdateMUTimeAccurRlt(CDvmData*)));//suyang 
		}
	}
}
...
void QSttIecRecordCbWidget::Ats_IecRecord(BOOL bIsFromIecfgFile)
{
	...
	g_TestWinBase->SendIecRecord(&oMacroParas);//suyang

// 	CSttTestCtrlCntrNative *pNative = (CSttTestCtrlCntrNative*)g_pTheSttTestApp->m_pTestCtrlCntr;
//     pNative->Ats_IecRecord(&oMacroParas);suyang
...
QScrollComboBox *QSttIecRecordCbWidget::CombHarmDisplayType()
{
	...
	pcmbHarmDisplay->addItem(_T("有效值"));
	pcmbHarmDisplay->addItem(_T("含有率"));
	pcmbHarmDisplay->addItem(_T("柱状图"));
...
{	
	...
	{
		ui->m_layButtons->insertWidget(9, m_cmbHarmDisplay);
