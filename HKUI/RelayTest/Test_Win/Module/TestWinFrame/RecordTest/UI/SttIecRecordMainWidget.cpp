...
void QSttIecRecordMainWidget::InitIecRecordCbWidget(const CString &strFuncID)
{
#ifndef _PSX_QT_LINUX_//在LINUX下还使用原有方式
// 	if ((strFuncID == STT_ORG_MACRO_MUAccuracyTest)||(strFuncID == STT_ORG_MACRO_MUZeroDriftTest)
// 		||(strFuncID == STT_ORG_MACRO_MUAccurAutoTest))
// 	{
// 		if (!g_pSttIecRecordCbWidget->IsTabUI_Mode())//合并单元准确度等测试模式需要为Tab方式显示
// 		{
// 			QDialog *pOldSttIecRecordCbWidget = m_pSttIecRecordCbWidget;
// 			m_pSttIecRecordCbWidget = new QSttMUTestRecordCbWidget(font(), this);
// 			m_pSttIecRecordDetectWidget->hide();
...
// 	}
// 	else 
	if (g_pSttIecRecordCbWidget->IsTabUI_Mode())//非合并单元准确度等测试模式需要为原有方式显示
	{
		QDialog *pOldSttIecRecordCbWidget = m_pSttIecRecordCbWidget;
		m_pSttIecRecordCbWidget = new QSttIecRecordCbWidget(font(), this);
		m_pSttIecRecordCbWidget->hide();
...
void QSttIecRecordMainWidget::ClearAll_Detect()
{
	((QSttIecRecordDetectWidget *)m_pSttIecRecordDetectWidget)->ClearAll_Detect();
}
