...
void CSttIecCbGrid::InitGridTitle_61850()
{
	...
	xlang_GetLangStrByFile(astrGridTitle[1],_T("Native_Type"));
...
//	xlang_GetLangStrByFile(astrGridTitle[2],_T("Native_APPID"));
	...
	{
		int iGridWidth[6]={50*g_dUIShowCoef, 80*g_dUIShowCoef, 100*g_dUIShowCoef, 300*g_dUIShowCoef, 100*g_dUIShowCoef, 120*g_dUIShowCoef};
		QExBaseListGridBase::InitGridTitle(astrGridTitle, iGridWidth, 6);
	}
	else
	{
		int iGridWidth[6]={50, 80, 100, 300, 100, 120};
	QExBaseListGridBase::InitGridTitle(astrGridTitle, iGridWidth, 6);
	}
...
//2022-6-11 lijunqing 此处修改考虑不周，应该计算CRC或者是通过其他方式解决这个问题
// 	if (pCapDevice->m_strCRC.IsEmpty())//20220609 zhouhj 对应CRC为空的情况,不显示控制块,进入探测界面后,自动导入配置
// 	{
// 		return;
// 	}
