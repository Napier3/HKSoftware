...
void QLuminousPowerDlg::InitUI()
{
	...
	strTitle = /*"光功率"*/g_sLangTxt_Gradient_OpticalPower;
	this->setWindowTitle(strTitle);

	m_pMainLayout = new QVBoxLayout(this);
	CString strText=/*_T("关闭")*/g_sLangTxt_Close;
...
//	xlang_SetLangStrToWidget(m_pbtnClose,"sClose", XLang_Ctrls_QPushButton);
	...
	m_pbtnClose->setFont(oFont);
	m_pbtnClose->setText(strText);
	m_pbtnClose->setFixedWidth(80);
	
	m_pOddLuminousPowerGrid =new QLuminousPowerGrid(this);
	m_pOddLuminousPowerGrid->setFont(oFont);
	m_pOddLuminousPowerGrid->horizontalHeader()->setFont(oFont);
	m_pOddLuminousPowerGrid->InitGrid();

	m_pEvenLuminousPowerGrid =new QLuminousPowerGrid(this);
	m_pEvenLuminousPowerGrid->setFont(oFont);
	m_pEvenLuminousPowerGrid->horizontalHeader()->setFont(oFont);
...
void QLuminousPowerDlg::InitData(CDataGroup *pDataGroup)
{
	...
	while(pos)
	{
		...
		if (nIndex%2==0)
		{	
			strName.Format(/*_T("光口%d")*/g_sLangTxt_Gradient_POpticalPort.GetString(),nIndex+1);
