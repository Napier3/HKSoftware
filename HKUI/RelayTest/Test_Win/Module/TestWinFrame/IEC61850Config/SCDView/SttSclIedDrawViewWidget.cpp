...
// 			//m_pGraphicsScene->setSceneRect(-80, -10, /*width()*/m_pGraphicsScene->width(), nHeight+10);
...
// 	OutPutDataPairs(pSclIed ,m_strAppIdSmvOut,  m_pSmvOutBlockRect, nHeightOut);
// 	OutPutDataPairs(pSclIed ,m_strAppIdGsOut,  m_pGooseoutBlockRect, nHeightOut);
	...
	//int nHeightLeft = 240 ,nHeightRight = 240;
	long nCount = 0;
	CString strAppID;
	m_pInputRect.clear();
	...
	//for (int nIndex = 0; nIndex < m_strAppIdSmvIn.size(); ++nIndex)
	{
		...
		while(posSmvIn)
		{
			...
			{
				//if(dataPair.second == /*pSclCtrlSmvIn->m_strName*/strExtName)
				{
					nCount ++;
...
// 					strName.replace("--", "\n");
	...
	//for (int nIndex = 0; nIndex < m_strAppIdGsIn.size(); ++nIndex)
	{
		...
		while(posGsIn)
		{
			...
			//if (dataPair.first == pSclIed->m_strID)
			{
				//if(dataPair.second == /*pSclCtrlGsIn->m_strName*/strExtName)
				{
					nCount ++;
...
// 		for (int i = 0; i < m_strAppIdGsOut.size(); ++i)
// 		{
// 			const QPair<CString, CString>& pair = m_strAppIdGsOut.at(i);
// 			if (pair.second == strAppID)
// 			{
// 				isDuplicate = true;
// 				break;
// 			}
...
// 		{
// 			QPair<CString, CString> dataPairGsOut(strID, strAppID);
// 			m_strAppIdGsOut.append(dataPairGsOut); // 添加到m_strAppIdGsOut列表中
// 		}
		long totalChs = 0;
...
// 		bool isDuplicate = false;
// 		for (int i = 0; i < m_strAppIdSmvOut.size(); ++i)
// 		{
// 			const QPair<CString, CString>& pair = m_strAppIdSmvOut.at(i);
// 			if (pair.second == strAppID)
// 			{
// 				isDuplicate = true;
// 				break;
...
// 		{
// 			QPair<CString, CString> dataPairSmvOut(strID, strAppID);
// 			m_strAppIdSmvOut.append(dataPairSmvOut); 
// 		}
		long totalChs = 0;
		...
		if (dataPairs.first == dataPair.first && dataPairs.second == dataPair.second)
		{
			for (int i =0 ; i < m_strRecvAppIdIedSmv.size() ; ++i)
			{
				...
				if (str  == dataPairs.second)
				{
					...
					QSttCustomRect *pReceiveRect = new QSttCustomRect(QPointF(100, m_nHeightLeft+15), 130, 40, "接收", NULL);
		...
		if (dataPairs.first == dataPair.first && dataPairs.second == dataPair.second)
		{
			for (int i =0 ; i < m_strRecvAppIdIedGs.size() ; ++i)
			{
				...
				if (str  == dataPairs.second)
				{
					 ...
					 QSttCustomRect *pReceiveRect = new QSttCustomRect(QPointF(770, m_nHeightRight+15), 130, 40, "接收", NULL);
					 pReceiveRect->setFillColor(QColor(255, 255, 255));
					 pReceiveRect->setLineStyle(Qt::DashLine);
					 m_pReceiveRect.append(pReceiveRect);
