...
#include "../Module/CommonMethod/commonMethod.h" //2023/10/25 wjs
...
// 	this->setFixedHeight(500);
	...
	xlang_GetLangStrByFile(strTemp,"sClose");
...
void QSttIecChsEditWidget::InitData(CIecCfgChsBase *pIecCfgChs)
{
	...
	{
		g_oSttTestResourceMngr.m_oIecDatasMngr.UpdateFT3ChsTypeChanged(m_pIecCfgChs,g_oSystemParas.m_nIecFormat);//20230118 suyang 在添加的时候也需要更新FT3类型
...
// 		m_pVoltageMapComboBox->addItem(SMV_RATEID_STR_UABCZ6);
	...
	{
		slot_SetInitValueEnabled(false);
	}
...
void QSttIecChsEditWidget::InitUI_SMVFT3InMap()/////////
{
	...
	connect(m_pIecCfgChsGrid, SIGNAL(sig_OpenBinMapDialog( )), this, SLOT(slot_DIMapClicked()));
...
void QSttIecChsEditWidget::slot_DIMapClicked()
{
	...
	//m_pIecCfgChsGrid->ShowDatas(m_pIecCfgChs);
	m_pIecCfgChsGrid->ShowCommonBinMap_StaticString(pIecCfg6044CommonCh,nRow);
