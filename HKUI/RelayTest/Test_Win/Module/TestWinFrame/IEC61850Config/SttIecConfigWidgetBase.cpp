...
void QSttIecConfigWidgetBase::InitData(CIecCfgDatasBase* pIecCfgDatasBase)
{
	...
	OnDataSelChanged(m_pIecCfgDataGridBase, 0, 0);//dingxy 20240402 打开配置文件，行不变槽函数不响应，数据不刷新，强制再次刷新
	EnableAllButtons();
}
...
CIecCfgDataBase* QSttIecConfigWidgetBase::AddNewIecCfgData()
{
    ...
    //add wangtao 20240515 根据光口数改变最大总控制块数量
	if (m_pIecCfgDatasBase->GetClassID() == CFGCLASSID_CIECCFGDATASSMV)//包含了SMVTYPE_91 SMVTYPE_92 SMVTYPE_6044，需要在下面再筛选一下
	{ 
		int nMax = g_nIecCfgSmvOut_91_92MaxNum *IecCfgAddMax;
		if (m_pIecCfgDatasBase->GetDatasCount(CFGCLASSID_CIECCFG92DATA)>nMax)
		{
			CLogPrint::LogFormatString(XLOGLEVEL_ASSIST,_T("当前总控制块数量大于%d个,无法继续添加."),nMax);
			//CLogPrint::LogFormatString(XLOGLEVEL_ASSIST,g_sLangTxt_block.GetString());	
			return NULL;
		}
	}
	else if (m_pIecCfgDatasBase->GetClassID() == CFGCLASSID_CIECCFGDATASGOUT)
	{ 
        int nMax = g_nIecCfgGoutMaxNum *IecCfgAddMax;
		if (m_pIecCfgDatasBase->GetCount()>nMax)
		{
			CLogPrint::LogFormatString(XLOGLEVEL_ASSIST,_T("当前总控制块数量大于%d个,无法继续添加."),nMax);
			//CLogPrint::LogFormatString(XLOGLEVEL_ASSIST,g_sLangTxt_block.GetString());	
			return NULL;
    ...
    else if (m_pIecCfgDatasBase->GetClassID() == CFGCLASSID_CIECCFGDATASGIN)
    { 
		int nMax = g_nIecCfgGinMaxNum *IecCfgAddMax;
		if (m_pIecCfgDatasBase->GetCount()>nMax)
		{
			CLogPrint::LogFormatString(XLOGLEVEL_ASSIST,_T("当前总控制块数量大于%d个,无法继续添加."),nMax);
			//CLogPrint::LogFormatString(XLOGLEVEL_ASSIST,g_sLangTxt_block.GetString());	
	...
	{
	if (m_pIecCfgDatasBase->GetCount()>/*50*/32)
	{
...
// 	}
	...
	//2042-5-21 wuxinyi FT3光口添加无限制，默认添加不选择
	if(pCfgData->GetClassID() != CFGCLASSID_CIECCFG6044COMMONDATA)
	{
...
void QSttIecConfigWidgetBase::OnDeleteN_1Clicked()
{
	...
	if (pIecCfgData != NULL)
	{
		...
		emit sig_UpdateCB_ChDatas(pIecCfgData); //chenling 2024.4.26
	} 
	else
	{
		emit sig_UpdateCB_ChDatas(NULL);
