...
void CIecCfgSmvFT3OutChsGrid::InitGrid()
{
	...
	connect(this,SIGNAL(cellClicked(int,int)),this,SLOT(slot_SetInitValueBtn_Clicked(int,int)),Qt::UniqueConnection);
}
...
void CIecCfgSmvFT3OutChsGrid::UpdateData_ByDataType(CIecCfg6044CommonCh *pIecCfgFT3Ch,int nRow)
{
	...
	SetItemEditable(nRow,5,FALSE);//suyang 20240418 双击初始值表格里不让修改
	CString strText;
	strText = Global_GetCodeValueMapString(pIecCfgFT3Ch);
...
{
// 	slot_SetInitValueBtn_Clicked(nRow,nCol);//suyang 20240418 此处与330不同，双击作为修改了，单击的时候调用
