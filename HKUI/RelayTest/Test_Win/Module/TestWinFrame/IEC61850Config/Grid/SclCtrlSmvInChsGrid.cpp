...
void QSclCtrlSmvInChsGrid::InitGridTitle()
{
	...
	//CString astrGridTitle[SCLCTRLS_SMVIN_CHS_GRID_COLS] = {"引用", "描述", "订阅索引", "订阅引用", "订阅描述"};
	CString astrGridTitle[SCLCTRLS_SMVIN_CHS_GRID_COLS] = {g_sLangTxt_Native_quote, g_sLangTxt_State_Description,g_sLangTxt_Native_subscriptionIndex, 
		g_sLangTxt_Native_subscriptionReference, g_sLangTxt_Native_subscriptionDescription};
	int iGridWidth[SCLCTRLS_SMVIN_CHS_GRID_COLS]={200, 200,100,200,200};
...
void QSclCtrlSmvInChsGrid::ShowData(CExBaseObject *pData, int& nRow, BOOL bInsertRow)
{
	...
	Show_StaticString(pData,nRow,4,&pSclChSmvIn->inDODesc); //没找到
	nRow++;
