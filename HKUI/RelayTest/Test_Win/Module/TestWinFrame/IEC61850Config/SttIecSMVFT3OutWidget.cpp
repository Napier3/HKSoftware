...
CIecCfgDataBase* QSttIecSMVFT3OutWidget::AddNewIecCfgData()
{
	...
	int nMax = g_nIecCfgSmvOut_60044MaxNum *IecCfgAddMax;
	if (m_pIecCfgDatasBase->GetDatasCount(CFGCLASSID_CIECCFG6044COMMONDATA)>nMax)//wangtao 20240516 将SMV发布FT3和9-2分别计数
	{
		CLogPrint::LogFormatString(XLOGLEVEL_ASSIST,_T("当前总控制块数量大于%d个,无法继续添加."),nMax);
		return NULL;
	}

	//2024-5-20 wuxinyi 现修改需求，不限制光口添加
