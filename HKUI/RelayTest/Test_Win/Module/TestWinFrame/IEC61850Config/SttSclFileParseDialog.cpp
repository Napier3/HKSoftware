...
void QSttSclFileParseDialog::SetSclFileParseFont()
{
	if (g_pSttGlobalFont!= NULL)
	{
		...
		m_pSaveAsTscl_PushButton->setFont(*g_pSttGlobalFont);
...
void QSttSclFileParseDialog::InitUI()
{
	...
	CString strTitle;
	//strTitle = _T("SCL解析界面");
	strTitle = g_sLangTxt_Gradient_SCLPars; 
	...
	m_pIedSearch_LineEdit = new QLineEdit(this);
...
// 	}
	...
	CString strSaveAsTscd;
	strSaveAsTscd = _T("另存为TSCD文件");
	m_pSclFileBottom_HBoxLayout = new QHBoxLayout;
	m_pSCLSelect_PushButton = new QPushButton(strSclSelect);
	m_pSaveAsTscl_PushButton = new QPushButton(strSaveAsTscd);
	...
	m_pSclFileBottom_HBoxLayout->addWidget(m_pSaveAsTscl_PushButton);
	...
	connect(m_pSaveAsTscl_PushButton, SIGNAL(clicked()),SLOT(slot_SaveAsTscdBtnClicked()));
...
// 	}
	...
	if (m_pSttSclParseProgDlg == NULL)
	{
		...
		m_pSttSclParseProgDlg->initUI(_T("SCL加载进度条"),_T("正在加载SCL文件,请稍后..."));
	}
...
#endif
{
	...
	//20230103chenling 重新写了示意图界面，不需要的代码注释
// 	g_oGloalIEDNameList.clear();
// 	g_oIED_Hash.clear();
...
//	g_oTotalIEDList.clear();
	...
	while (posFirst)
	{
		...
		CString strName = pIedFirst->m_strID + "\n" + pIedFirst->m_strName;//chneling20231228
...
// 	}
	...
	{
		m_pSttSclSkectMapWidget->ShowIED(pSclIed);
	}
}
...
void QSttSclFileParseDialog::slot_OneKeyCfgBtnClicked()
{
	...
	{
		m_pOneKeyCfg_PushButton->setText("取消一键配置");
	}
	else
	{
...
void QSttSclFileParseDialog::slot_SCLSelectBtnClicked()
{
	...
	{
		CString strMsg;
		strMsg = /*_T("解析文件(")*/g_sLangTxt_Gradient_ParseLFile;
		strMsg += strCurrTscdFilePath;
		strMsg += /*_T(")不存在,无法进行SCL选择切换.")*/g_sLangTxt_Gradient_SCLNoSel; 
...
void QSttSclFileParseDialog::slot_SaveAsTscdBtnClicked()
{
	if (m_pSclStation->GetCount()<=0)
	{
		CString strMsgText;
		strMsgText.Format(_T("未选择SCL文件,或当前SCL文件内容为空."));
		CXMessageBox::information(this,g_sLangTxt_Message,strMsgText);
		return;
	}

	CString strText;
	strText = _T("TSCD文件另存为");
	QFileDialog dlg(this,strText);
	dlg.setFileMode(QFileDialog::AnyFile);
	strText = _T("TSCD文件(*.tscd)");
	...
	if (dlg.exec() == QFileDialog::Accepted)
	{
		...
		strFileName = ChangeFilePostfix(strFileName,_T("tscd"));
		m_pSclStation->SaveXmlFile(strFileName, CCfgDataMngrXmlRWKeys::g_pXmlKeys);
	}
}
...
void QSttSclFileParseDialog::slot_ImportIecCfgBtnClicked()
{
	...
	emit sig_InterIecCfgDataMngrUpdate();
	close();
}
