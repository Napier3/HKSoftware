...
void QSttIecConfigDialogWin::InitUI()
{
	...
	CString strTitle;
	//strTitle = _T("IEC配置");
	strTitle = g_sLangTxt_Gradient_IECConfiguration;
	setWindowTitle(strTitle);


	m_pSttIecConfigMainLayout = new QGridLayout(this);
	m_pIecConfigRightTabWidget = new QTabWidget(this);
...
void QSttIecConfigDialogWin::CreateNavigationTree(long nTotalFiberNum_LC , long nTotalFiberNum_STSend)
{
	...
	strStyleSheet.Format("QTreeView::indicator:checked {image: url(%schecked.png);} QTreeView::indicator:unchecked {image: url(%sunchecked.png);}",
...
//	xlang_GetLangStrByFile(strTemp,"IEC_SysParasSet");
	...
	strTmpString = /*_T("系统参数")*/g_sLangTxt_SystemPara;
	astrItemDesc<<strTmpString;
...
//	xlang_GetLangStrByFile(strTemp,"IEC_Message");
	...
	pChildItem->setFlags(pChildItem->flags() & ~Qt::ItemIsUserCheckable);//2024-01-31 suyang 取消树形控件勾选框
...
// 	m_pIecGinWidget->setFont(m_oIecFont);
	...
	connect(m_pIecSMV92OutWidget,SIGNAL(sig_UpdateCB_ChDatas(CExBaseList *)),m_pSttIecChsEdit92OutWidget,SLOT(slot_UpdateCB_ChDatas(CExBaseList *)));
	connect(m_pIecSMVFT3OutWidget,SIGNAL(sig_UpdateCB_ChDatas(CExBaseList *)),m_pSttIecChsEditFT3OutWidget,SLOT(slot_UpdateCB_ChDatas(CExBaseList *)));
	connect(m_pIecSMV92InWidget,SIGNAL(sig_UpdateCB_ChDatas(CExBaseList *)),m_pSttIecChsEdit92InWidget,SLOT(slot_UpdateCB_ChDatas(CExBaseList *)));
	connect(m_pIecSMVFT3InWidget,SIGNAL(sig_UpdateCB_ChDatas(CExBaseList *)),m_pSttIecChsEditFT3InWidget,SLOT(slot_UpdateCB_ChDatas(CExBaseList *)));
//	connect(m_pIecSMV92OutWidget,SIGNAL(sig_UpdateCB_ChDatas(CExBaseList *)),SLOT(slot_UpdateData()));
	connect(m_pIecGinWidget,SIGNAL(sig_UpdateCB_ChDatas(CExBaseList *)),m_pSttIecChsEditGinWidget,SLOT(slot_UpdateCB_ChDatas(CExBaseList *)));
	connect(m_pIecGoutWidget,SIGNAL(sig_UpdateCB_ChDatas(CExBaseList *)),m_pSttIecChsEditGoutWidget,SLOT(slot_UpdateCB_ChDatas(CExBaseList *)));
}
