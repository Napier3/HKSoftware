...
long g_nComBoxVertScrollWidth = 50;
...
void QSttIecConfigDialogBase::slot_OpenCfgBtnClicked()
{
	...
	strFolderPath = g_oFileMngrTool.GetFolderPath("IEC61850Config") ;
...
//		if (g_oXSclFileMngr.GetCount() == 0)//如果当前SCL历史记录为空,则直接弹出选择SCL界面
		{
			...
			xlang_GetLangStrByFile(strCancel, "sCancel");
 			...
 			CString strText;
			strText = _T("SCL文件(*.scd *.icd *.cid *.ccd)");
...
//		g_oXSclFileMngr.SaveSclFileMngrCfg();
	...
	connect(&oSttSclFileDlg, SIGNAL(sig_InterIecCfgDataMngrUpdate()),SLOT(slot_InterIecCfgDataMngrUpdate()));
//	Global_SoftKeyBoardAttachObj(&oSttSclFileDlg,STT_DefaultKeyboardType_English);
	oSttSclFileDlg.exec();
#ifdef _USE_SoftKeyBoard_ 
	QSoftKeyBoard::ReAttachObj();
#endif
	disconnect(&oSttSclFileDlg, SIGNAL(sig_InterIecCfgDataMngrUpdate()),this,SLOT(slot_InterIecCfgDataMngrUpdate()));
}

void QSttIecConfigDialogBase::slot_InterIecCfgDataMngrUpdate()
{
	CIecCfgDatasSMV* pIecCfgDatasSMV = m_pIecCfgDatasMngr_Inter->GetSmvMngr();
