...
QFT3GridComBoxDelegBase::QFT3GridComBoxDelegBase( QObject *parent/*=NULL*/ )
:QExBaseListComBoxDelegBase(parent)
{

}

QFT3GridComBoxDelegBase::~QFT3GridComBoxDelegBase()
{

}

void QFT3GridComBoxDelegBase::setModelData( QWidget *editor, QAbstractItemModel *model, const QModelIndex &index ) const
{
	if (!m_bEditable)
	{
		return;
	...
	if (pVCellData->pExBaseList != NULL)
	{
		...
		CDataTypeValue* pDataTypeValue = (CDataTypeValue*)pDataType->FindByID(strItem);
...
void QFT3GridComBoxDelegBase::setEditorData( QWidget * editor, const QModelIndex &index ) const
{
	QGV_ITEM* pCurItem = GetCurrItem(index);
...
///////////////////////////////////////////////////////
FT3Grid::FT3Grid(QWidget* pParent) : QExBaseListGridBase(pParent)
{
...
void FT3Grid::InitGridTitle()
{
	CString astrGridTitle[FT3CHS_GRID_COLS]={/*_T("描述")*/g_sLangTxt_State_Description,/*_T("通道类型")*/g_sLangTxt_IEC_ChType,/*_T("通道映射")*/g_sLangTxt_IEC_ChMaps,/*_T("字节长度")*/g_sLangTxt_IEC_ChByteLen,/*_T("值")*/g_sLangTxt_Goose_Value,/*_T("设置")*/g_sLangTxt_Gradient_Setting};
...
void FT3Grid::ShowData(CExBaseObject *pData, int& nRow, BOOL bInsertRow)
{
	ASSERT(pData);
...
//	Show_DataType_Val(pData,nRow,1,iecfg_SmvCommon6044ChDataType,&pIecCfg6044CommonCh->m_strChType,EndEditCell_FT3ChSelChanged);//通道类型 
...
void FT3Grid::UpdateData_ByDataType(CIecCfg6044CommonCh *pCommonCh,int nRow)
{
	...
	//	Show_Long(pCommonCh,nRow,4,&pCommonCh->m_nValue,TRUE,_T(""),EndEditCell_AppCh_FT3Value);
		Show_StaticString(pCommonCh,nRow,4,_T("--"));//20240228 suyang 只有在状态值 、其他情况下可以进行修改
	...
	else if (pCommonCh->m_strChType == iecfg_SmvChDataType_Val_Other)
	{
		...
		//Show_StaticString(pCommonCh,nRow,4,_T("--"));
		//Show_Long(pCommonCh,nRow,4,&pCommonCh->m_nValue,TRUE,_T(""),EndEditCell_AppCh_FT3Value);//dingxy 20240428 采样率值初始显示更改
		//Show_StaticString(pCommonCh,nRow,5,_T(""));
		if ((pCommonCh->m_strAppChID==g_strAppChID_SmpCnt) || (pCommonCh->m_strAppChID == g_strAppChID_TDelay))
		{
			Show_StaticString(pCommonCh,nRow,4,_T("--"));
		}
		else
	...
	//ShowSettingCol(pCommonCh,nRow);
}

void FT3Grid::UpdateData_ByChMap(CIecCfg6044CommonCh *pCommonCh,int nRow)
{
...
//	SetItemEditable(nRow,4,FALSE); //dingxy 20240429 按照测试需求，FT3发布界面的值和设置的二进制和十进制可以编辑相互转换
...
CDataType* FT3Grid::FindDataType(const CString &strChType)
{
...
void FT3Grid::EndEditCell_FT3ChSelChanged(int nRow, int nCol,QGV_ITEM *pCell, QExBaseListGridBase *pGrid)
{
	...
	if (nCol=1)
	{
		((FT3Grid*)pGrid)->UpdateData_ByDataType(pCommonCh,nRow);
		emit ((FT3Grid*)pGrid)->sig_Ft3Value_Changed();
	}
	if (nCol=2)
	{
		((FT3Grid*)pGrid)->UpdateData_ByChMap(pCommonCh,nRow);
		emit ((FT3Grid*)pGrid)->sig_Ft3Value_Changed();
	}
}


void FT3Grid::EndEditCell_AppCh_FT3Value(int nRow, int nCol,QGV_ITEM *pCell, QExBaseListGridBase *pGrid)
{
...
void FT3Grid::Slot_Setting_Clicked(int nRow,int nCol)
{
	if (nCol !=5)
	{
		return;
	...
	if (pIecCfg6044CommonCh->m_strChType == iecfg_SmvChDataType_Val_StateValue)
	{
		...
		QFT3SetDialog oFT3SetDialog(pIecCfg6044CommonCh->m_nValue,this);

		if (oFT3SetDialog.exec() ==  QDialog::Accepted)
		{
			pIecCfg6044CommonCh->m_nValue=oFT3SetDialog.m_dwValue;	
			UpdateDatas_Setting();
...
void FT3Grid::UpdateDatas_Setting()
{
	if (m_pDatas == NULL)
	{
		return;
...
void FT3Grid::SetRunState(long nRunState)
{
...
void FT3Grid::Show_DataType_Val( CExBaseObject *pData, const int& nRow, const int& nCol, CDataType *pDataType, CString *pstrValue, GRID_CELL_EDIT_FUNC pFunc )
{
