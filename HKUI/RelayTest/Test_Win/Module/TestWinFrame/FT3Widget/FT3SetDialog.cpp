...
QFT3SetDialog::QFT3SetDialog(long dwValue,QWidget *parent)
: QDialog(parent)
{
...
QFT3SetDialog::~QFT3SetDialog()
{

}


void QFT3SetDialog::InitUI()
{
	resize(440,440);
	this->setFixedWidth(440);
	this->setFixedHeight(440);

	CString strTemp	=/*_T("FT3发布参数设置")*/g_sLangTxt_IEC_FT3PublishParameterSet;
	setWindowTitle(strTemp);

	CString strText;
	strText	= /*_T("确定")*/g_sLangTxt_OK;
...
void QFT3SetDialog::slot_CancelClicked()
{
	close();
}

void QFT3SetDialog::slot_OKClicked()
{
	SaveData();
	this->accept();
}