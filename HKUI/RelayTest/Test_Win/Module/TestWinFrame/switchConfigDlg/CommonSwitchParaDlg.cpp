...
CommonSwitchParaDlg::CommonSwitchParaDlg(QWidget *parent): QDialog(parent)
{
	...
	m_pLogicOr = NULL;
	m_pLogicAnd = NULL;
	setWindowIcon (QPixmap( ":/images/setSwitch.png"));
...
void CommonSwitchParaDlg::setData(tmt_BinaryConfig *pBinaryCfg,int nInNum,int nOutNum,int nFaultOutNum)
{
	...
	if (nInNum > 0)
	{
		...
		for (int i=0;i<nGroupNum;i++)
		{
			for(int j=0;j<5;j++)
			{
				...
				if (nBinIndex < m_nInNum)
				{
					...
					m_pChIns[nBinIndex] = new QCheckBox(strBinName,groupInBox);
		...
		pHLayAndOr->addStretch();  //2023-12-14 wuxinyi 修改逻辑或、与控件居中
		pHLayAndOr ->addWidget(m_pLogicAnd);
		pHLayAndOr->addStretch();
