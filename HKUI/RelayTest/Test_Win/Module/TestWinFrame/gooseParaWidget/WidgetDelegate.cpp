	...
	case GOPUB_CHAN:
		{
			...
			else if (index.column() == 2)
			{
				...
				if (strType == /*tr("单点")*/g_sLangTxt_GOOSE_Single)
				{
				...
				else if (strType == /*tr("双点")*/g_sLangTxt_GOOSE_Double)
				{
	...
	const QModelIndex &index) const
{
	if(m_type == GOPUB_CHAN)
	{
		if (index.column() == 3)
		{
			...
			if(strType == /*tr("单点")*/g_sLangTxt_GOOSE_Single || strType == /*tr("双点")*/g_sLangTxt_GOOSE_Double)
			{
				QComboBox *comboBox = new QComboBox(parent);
				return comboBox;
			}
			else if(strType == /*tr("字符串")*/g_sLangTxt_CharacterString)
			{
				QLineEdit *lineEdit = new QLineEdit(parent);
				return lineEdit;
			}
			else if(strType == /*tr("品质")*/g_sLangTxt_IEC_Quality)
			{
				QString str = index.model()->data(index, Qt::EditRole).toString();
				CLineEdit *lineEdit = new CLineEdit(CLineEdit::APPID_TEXT, str, parent);
				return lineEdit;
			}
			else if(strType == /*tr("浮点")*/g_sLangTxt_GOOSE_Float)
			{
				QLineEdit *lineEdit = new QLineEdit(parent);
...
void CComplexDelegate::setEditorData(QWidget *editor, const QModelIndex &index) const
{
	...
	if (m_type == GOPUB_CHAN)
	{
		if (index.column() == 3)
		{
			...
			else if(strType == tr("INT32") || strType == tr("INT32U"))
			{
				QLineEdit *lineEdit = dynamic_cast<QLineEdit *>(editor);
				lineEdit->setText(strItem);
			}
