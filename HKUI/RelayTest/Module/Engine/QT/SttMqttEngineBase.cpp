...
void CSttMqttEngineBase::SubscribeTopic(const char* pszTopic,int nQos)
{
	m_oMqttClient.SubscribeTopic(pszTopic, nQos);
}

void CSttMqttEngineBase::UnSubscribeTopic(const char* pszTopic)
{
	m_oMqttClient.UnSubscribeTopic(pszTopic);
}
...
void CSttMqttEngineBase::OnMqttConnected()
{
...
void CSttMqttEngineBase::PublicTopic(char *strTopic, char *pBuf, long nLen)
{
	m_oMqttClient.PublicTopic(strTopic, pBuf, nLen);
}

void CSttMqttEngineBase::PublicTopic(CSttMqttTopicCmbn &oTopicCmbn)
{
	m_oMqttClient.PublicTopic(oTopicCmbn.CmbnTopic()
			, oTopicCmbn.m_pszBuffer, oTopicCmbn.m_nBufferLength);
}

void CSttMqttEngineBase::Connect(const CString &strIP,long nPort, const CString &strClientIDKey,const CString &strUser,const CString &strPwd,BOOL bAuthority)
{
	...
	m_oMqttClient.InitMqttClient(strIP,nPort,m_strClientID,strUser,strPwd,bAuthority);
}
