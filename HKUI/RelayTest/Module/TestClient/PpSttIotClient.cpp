...
BOOL CPpSttIotClient::ConnectSttServer()
{
	...
	CString strIP = m_oTestDeviceCfg.m_strRemoteIP;
	...
	//设置LocalIP
	m_pXClientEngine->m_pSttClientSocket->SttXmdChAddr(m_oTestDeviceCfg.m_strLocalIP);
	...
	m_pXClientEngine->m_oCurrUser.IP_User(m_pXClientEngine->m_pSttClientSocket->SttXmdChAddr());
	m_pXClientEngine->m_oCurrUser.Name_User(m_pXClientEngine->m_pSttClientSocket->SttXmdChAddr());
