...
CSttParaObject* CSttParaObjects::RegisterPara(const char *pszName,const char *pszID,const char *pszUnit,const char *pszDataTypeID,const char *pszDefaultValue, VAR_DOUBLE_LIST *val)
{
    CSttParaObject *pParaObj = new CSttParaObject(pszName,pszID,pszUnit,pszDataTypeID,pszDefaultValue, val);
    AddTail(pParaObj);
    pParaObj->SetParent(this);
...
CSttParaObject* CSttParaObjects::RegisterPara(const char *pszName,const char *pszID,const char *pszUnit,const char *pszDataTypeID,const char *pszDefaultValue, VAR_FLOAT_LIST *val)
{
    CSttParaObject *pParaObj = new CSttParaObject(pszName,pszID,pszUnit,pszDataTypeID,pszDefaultValue, val);
    AddTail(pParaObj);
    pParaObj->SetParent(this);
...
CSttParaObject* CSttParaObjects::RegisterPara(const char *pszName,const char *pszID,const char *pszUnit,const char *pszDataTypeID,const char *pszDefaultValue, VAR_INT_LIST *val)
{
    CSttParaObject *pParaObj = new CSttParaObject(pszName,pszID,pszUnit,pszDataTypeID,pszDefaultValue, val);
    AddTail(pParaObj);
    pParaObj->SetParent(this);
...
void CSttParaObjects::GetParameter(CString &strParameter, BOOL bOnlyGetChange)
{
	...
	if (bOnlyGetChange)
	{
		...
		while (pos != NULL)
		{
			...
			{
				strParameter.AppendFormat(_T("%s=%s;"),  pPara->m_strID, pPara->m_varValue.ToString());
			}
		}
	}
	else
...
*/
		strReport.AppendFormat(_T("%s=%s;"), pObject->m_strID, strValue);
	}
}
...
void CSttParaObjects::Parser(CEquationBuffer *pEquation)
{
	...
	while (pos != NULL)
	{
		...
		if (pObject != NULL)
		{
			...
			{
/*
				strValue = CSttParaTranConfig::DataTypeValueTrans(pObject->m_strDataTypeID,strValue);
*/
