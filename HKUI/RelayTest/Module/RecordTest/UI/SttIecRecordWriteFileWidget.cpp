...
void QSttIecRecordWriteFileGrid::Show_Fiber(CExBaseObject *pStData, const int& nRow, const int& nCol, UINT nFiberIndex)
{
	...
	strFiber = g_sLangTxt_IEC_FI.GetString();
	strFiber.AppendFormat(_T("%d"), nFiberIndex + 1);
...
//	CString strFilePathName = CXSmartCapFileWrite::GetPcapFilePath();
...
#else
	CString strTip = _T("Pcap文件另存为");
	strPcapFilePath = QFileDialog::getSaveFileName(0, strTip,
	...
	if (bRet)
	{
		...
		strText = g_sLangTxt_Native_FileSaveSuccess.GetString();
		strText += _T("(");
		strText += strPcapFilePath;
		strText += _T(")");
		strMsgText = strText;
#else
