...
long CSttDisTanceTest_I::CalReport(UINT nState)
{
	if(m_oImpBaseResults.m_resultState[1].m_nAct)
    {
        ...
        {
			m_oDistanceResults.m_fFirstTripTime[nIndex] = m_oImpBaseResults.m_resultState[1].m_frTimeBinAct[nIndex][0];
        }
...
void CSttDisTanceTest_I::TranslateToStateParas()
{
    ...
    pState1->m_nTriggerCondition = TMT_MDSTATE_TIMEBINTRIGGER;
    for (int i = 0;i<8;i++)
    {
        pState1->m_binIn[i].nSelect = m_nBinState[i];
    }
    pState1->m_nBinLogic = m_nBinLogic;
    //pState1->m_fTimeState += m_fMinTripTime;
