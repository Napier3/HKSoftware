...
{
	memset(&pParas, 0, sizeof(tmt_LinevoltManuParas));
	pParas->init();
}

void stt_init_results(tmt_LinevoltManuResult *pResults)
{
	memset(&pResults, 0, sizeof(tmt_LinevoltManuResult));
	pResults->init();
}
void stt_xml_serialize(tmt_LinevoltManuParas *pParas, CSttXmlSerializeBase *pXmlSierialize)
{
	stt_xml_serialize(pParas,pXmlSierialize,6,6);
}

void stt_xml_serialize(tmt_LinevoltManuParas *pParas, CSttXmlSerializeBase *pXmlSierialize,long nVoGroupNum,long nCurGroupNum)
{
	CString strName,strID;

	for (int nGroupIndex = 0;nGroupIndex<nVoGroupNum;nGroupIndex++)
	{
		strID.Format(_T("Uab%d"),nGroupIndex+1);
		strName = strID;
		pXmlSierialize->xml_serialize(strName.GetString(), strID.GetString(),"V","number",pParas->m_uiVOL[nGroupIndex*2].Harm[1].fAmp);
		strID.Format(_T("Uab%dPh"),nGroupIndex+1);
		strName = strID; 
		pXmlSierialize->xml_serialize(strName.GetString(), strID.GetString(), "°", "number",pParas->m_uiVOL[nGroupIndex*2].Harm[1].fAngle);
		strID.Format(_T("3U0_%d"),nGroupIndex+1);
		strName = strID;
		pXmlSierialize->xml_serialize(strName.GetString(), strID.GetString(), "V", "number",pParas->m_uiVOL[nGroupIndex*2+1].Harm[1].fAmp);
		strID.Format(_T("3U0_%dPh"),nGroupIndex+1);
		strName = strID;
		pXmlSierialize->xml_serialize(strName.GetString(), strID.GetString(), "°", "number",pParas->m_uiVOL[nGroupIndex*2+1].Harm[1].fAngle);
	}
	
// 	pXmlSierialize->xml_serialize("", "Uab2", "V", "number",pParas->m_uiVOL[2].Harm[1].fAmp);
// 	pXmlSierialize->xml_serialize("", "Uab2Ph", "°", "number",pParas->m_uiVOL[2].Harm[1].fAngle);
// 	pXmlSierialize->xml_serialize("", "3U0_2", "V", "number",pParas->m_uiVOL[3].Harm[1].fAmp);
// 	pXmlSierialize->xml_serialize("", "3U0_2Ph", "°", "number",pParas->m_uiVOL[3].Harm[1].fAngle);

	//long nChNum = nCurGroupNum*3;
	for(int nIndex=0;nIndex<nCurGroupNum;nIndex++)   //20240508    xueyangfan   电流相位默认改为0 -120 120
	{
		strID.Format(_T("I%d"),nIndex+1);
		strName = strID;
		pXmlSierialize->xml_serialize(strName.GetString(),strID.GetString(),"A","number",pParas->m_uiCUR[nIndex].Harm[1].fAmp);
		strID.Format(_T("I%dPh"),nIndex+1);
		strName = strID;
		pXmlSierialize->xml_serialize(strName.GetString(),strID.GetString(),"°","number",pParas->m_uiCUR[nIndex].Harm[1].fAngle);

// 		strID.Format(_T("Ib%d"),nIndex+1);
// 		pXmlSierialize->xml_serialize("",strID.GetString(),"A","number",pParas->m_uiCUR[nIndex*3+1].Harm[1].fAmp);
// 		strID.Format(_T("Ib%dPh"),nIndex+1);
// 		pXmlSierialize->xml_serialize("",strID.GetString(),"°","number",pParas->m_uiCUR[nIndex*3+1].Harm[1].fAngle);
// 
// 		strID.Format(_T("Ic%d"),nIndex+1);
// 		pXmlSierialize->xml_serialize("",strID.GetString(),"A","number",pParas->m_uiCUR[nIndex*3+2].Harm[1].fAmp);
// 		strID.Format(_T("Ic%dPh"),nIndex+1);
// 		pXmlSierialize->xml_serialize("",strID.GetString(),"°","number",pParas->m_uiCUR[nIndex*3+2].Harm[1].fAngle);
	...
	pXmlSierialize->xml_serialize("变化模式","Mode","","UIVariateMode",pParas->m_oLinevoltGradient.nMode);
	pXmlSierialize->xml_serialize("变化始值","Start","","number",pParas->m_oLinevoltGradient.fStart);
	pXmlSierialize->xml_serialize("变化终值","End","","number",pParas->m_oLinevoltGradient.fEnd);
	pXmlSierialize->xml_serialize("变化步长","Step","","number",pParas->m_oLinevoltGradient.fStep);
	pXmlSierialize->xml_serialize("每步时间","StepTime","","number",pParas->m_oLinevoltGradient.fStepTime);
	pXmlSierialize->xml_serialize("触发延时","TrigDelay","","number",pParas->m_oLinevoltGradient.fTrigDelay);
	...
	pXmlSierialize->xml_serialize(/* "变化模式" */g_sLangTxt_Native_change_mode.GetString(),"Mode","","UIVariateMode",pParas->m_oLinevoltGradient.nMode);
	pXmlSierialize->xml_serialize(/* "变化始值" */g_sLangTxt_Gradient_Init.GetString(),"Start","","number",pParas->m_oLinevoltGradient.fStart);
	pXmlSierialize->xml_serialize(/* "变化终值" */g_sLangTxt_Gradient_Finish.GetString(),"End","","number",pParas->m_oLinevoltGradient.fEnd);
	pXmlSierialize->xml_serialize(/* "变化步长" */g_sLangTxt_Gradient_Step.GetString(),"Step","","number",pParas->m_oLinevoltGradient.fStep);
	pXmlSierialize->xml_serialize(/* "每步时间" */g_sLangTxt_Native_step_time.GetString(),"StepTime","","number",pParas->m_oLinevoltGradient.fStepTime);
	pXmlSierialize->xml_serialize(/* "触发延时" */g_sLangTxt_Native_trigger_delay.GetString(),"TrigDelay","","number",pParas->m_oLinevoltGradient.fTrigDelay);
...
void stt_xml_serialize_linevolt(tmt_ManualParas *pParas, CSttXmlSerializeBase *pXmlSierialize, tmt_channel *puiVOL, tmt_channel *puiCUR)
{
    ...
    {
        strID.Format(_T("Uab%d"), nIndex + 1);
        pXmlSierialize->xml_serialize("", strID.GetString(), "V", "number", puiVOL[nIndex * 2].Harm[1].fAmp);
        strID.Format(_T("Uab%dPh"), nIndex + 1);
        pXmlSierialize->xml_serialize("", strID.GetString(), "°", "number", puiVOL[nIndex * 2].Harm[1].fAngle);
        puiVOL[nIndex * 2].Harm[1].fFreq = fFreq;
        strID.Format(_T("3U0_%d"), nIndex + 1);
        pXmlSierialize->xml_serialize("", strID.GetString(), "V", "number", puiVOL[nIndex * 2 + 1].Harm[1].fAmp);
        strID.Format(_T("3U0_%dPh"), nIndex + 1);
        pXmlSierialize->xml_serialize("", strID.GetString(), "°", "number", puiVOL[nIndex * 2 + 1].Harm[1].fAngle);
        puiVOL[nIndex * 2 + 1].Harm[1].fFreq = fFreq;
    }

    for(int nIndex = 0; nIndex < 18; nIndex++)
    {
        strID.Format(_T("I%d"), nIndex + 1);
        pXmlSierialize->xml_serialize("", strID.GetString(), "A", "number", puiCUR[nIndex].Harm[1].fAmp);
        strID.Format(_T("I%dPh"), nIndex + 1);
        pXmlSierialize->xml_serialize("", strID.GetString(), "°", "number", puiCUR[nIndex].Harm[1].fAngle);
        puiCUR[nIndex].Harm[1].fFreq = fFreq;
    }
...
void stt_xml_serialize_act_rcd(tmt_LinevoltManuResult *pResults, CSttXmlSerializeBase *pXmlSierialize)//动作记录,最大64次,根据实际动作情况,上送总的动作信息
{
...
{
	tmt_LinevoltManuTest oManualTest;
	oManualTest.init();
	...
	stt_xml_serialize(&oManualTest.m_oLinevoltManuParas, pMacroParas);
	CString strXMLPath;
	strXMLPath = _P_GetConfigPath();
	strXMLPath += ("LinevoltManuTestDemo.xml");
	oSttXmlSerializeTool.Stt_WriteFile(strXMLPath);
}

CSttXmlSerializeBase* stt_xml_serialize(tmt_LinevoltManuTest *pParas, CSttXmlSerializeBase *pXmlSierialize,long nVolGroupNum,long nCurGroupNum)
{
	CSttXmlSerializeBase *pXmlParas = pXmlSierialize->xml_serialize("paras", "paras", "paras", stt_ParasKey());

	if (pXmlParas  != NULL)
	{
		stt_xml_serialize(&pParas->m_oLinevoltManuParas, pXmlParas,nVolGroupNum,nCurGroupNum);
	}
	...
	if (pXmlResults  != NULL)
	{
		stt_xml_serialize(&pParas->m_oLinevoltManuResult, pXmlResults);
	}
