...
// 	float	m_fVnom;	//额定电压
    ...
    int		m_nAngleMode;	//校正选择	0-无校正 1-△侧校正 2-Y侧校正  新增3-Z侧校正(等同△侧校正)  2	用以计算KPH、KPL AngleMode
    ...
    // 谐波参数
    int		m_nHarmOutSide;	//谐波输出侧
    int		m_nHarmIndex;	//谐波制动次数选择，0-2值分别对应2次、3次和5次//国际版中为0-3对应2次，3次，4次，5次
...
typedef struct tmt_diff_curr_harm_test_I : public tmt_DiffCurrThrTest // 谐波制动
{
public:
    virtual void init()
    {
        tmt_DiffCurrThrTest::init();
        strcpy(m_pszMacroID, STT_MACRO_ID_PsuDiffHarmTest_I);
    }

    tmt_diff_curr_harm_test_I()	{init();}
    virtual ~tmt_diff_curr_harm_test_I(){}
}tmt_DiffCurrHarmTest_I;
