...
// 	pXmlSierialize->xml_serialize("额定电压","Vnom","","number",pParas->m_fVnom);
    pXmlSierialize->xml_serialize("额定频率","Freq","Hz","number",pParas->m_fFnom);
    ...
    if(pParas->m_nAngleMode == 3)//3-Z侧校正(等同1-△侧校正)
    {
        pParas->m_nAngleMode = 1;
    }
...
void stt_xml_serialize_harm(tmt_DiffCurrParas *pParas, CSttXmlSerializeBase *pXmlSierialize)
{
    ...
    pXmlSierialize->xml_serialize("谐波输出侧","HarmOutSide","","DiffHarmIndex",pParas->m_nHarmOutSide);
...
//	stt_xml_serialize((PTMT_PARAS_HEAD)pTests, pXmlSerialize);
	...
	if (pXmlParas != NULL)
	{
      ...
      pXmlParas->xml_serialize(/* "类型" */ g_sLangTxt_Type.GetString(), "RateType", "", "number", ((tmt_DiffCurrRateTest *)pTests)->m_nRateType);
        ...
        else if (strcmp(pTests->m_pszMacroID,STT_MACRO_ID_PsuDiffHarmTest) == 0 || strcmp(pTests->m_pszMacroID,STT_MACRO_ID_PsuDiffHarmTest_I) == 0 || strcmp(pTests->m_pszMacroID,STT_MACRO_ID_PsuManuDiffHarmTest) == 0)
		{
