...
void CSttBP2CBOperateTest::TranslateToStateParas()
{
	...
	else if (m_nULockType == BP2_DIFF_U_LOCK_TYPE_U2)
	{
        ...
        float fPhi=(float)(atan2(3.0*fStdLN_Vol-6.0*m_fULockValue,sqrt(3.0)*fStdLN_Vol)*180.0f/XPI);
        oUa = p1.polar((float)((3.0f*fStdLN_Vol-6.0f*m_fULockValue)/sqrt(3.0f)/2.0f/sin(fPhi*XPI/180.0f)),-90.0+fPhi);
        oUb = p1.polar((float)((3.0f*fStdLN_Vol-6.0f*m_fULockValue)/sqrt(3.0f)/2.0f/sin(fPhi*XPI/180.0f)),-90.0-fPhi);
