...
// 		}
	...
	float GetPTRate(int nGroup)
	{
		...
		{
			fPTRate = m_fVSecondary[0] / (1000 * m_fVPrimary[0]);
		}
		return fPTRate;
	}

	float GetCTRate(int nGroup)
	{
		float fCTRate = 1;
		if(nGroup < MAX_DIGITAL_GROUP_NUM)
		{
		...
		{
			fCTRate = m_fISecondary[0] / m_fIPrimary[0];
		}
		return fCTRate;
	}
