...
void stt_xml_serialize(CString strName, CString strID, int nIndex, int nType, tmt_HarmParas *pParas, CSttXmlSerializeBase *pXmlSierialize, int nHarmCount)
{
	...
	for(int i  =  2;  i  < nHarmCount ;  i++)
	{
		...
		bFindSelect = pHarmSerialize->xml_serialize("是否使能", chEnable, "", "BOOL", pHarm->Harm[i].m_bSelect);
#ifndef _STT_NOT_IN_TEST_SERVER_
		if(!bFindSelect)
		{
			pHarm->Harm[i].m_bSelect = 1;
		}
#endif
	...
	if(pHarm->InterHarm.m_bSelect)
	{
		...
		bFindSelect = pHarmSerialize->xml_serialize("", "InterHarmAmp",   chUnit,   "number",   pHarm->InterHarm.fAmp);
		if(!bFindSelect)
		{
			pHarm->InterHarm.fAmp = 0.0;
		}
		pHarmSerialize->xml_serialize("", "InterHarmAng",   "°",   "number",   pHarm->InterHarm.fAngle);
		pHarmSerialize->xml_serialize("", "InterHarmFre",   "Hz",   "number",   pHarm->InterHarm.fFreq);
	}
}
