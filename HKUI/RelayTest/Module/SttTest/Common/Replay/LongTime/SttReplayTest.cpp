...
void CurrentMergeCode(unsigned int nPos, Drv_ReplayCoefItems *pCoefItems, long *nData1, long *nData2)
{
	PSTT_MODULE pModule = stt_adj_find_module_byModulePos(g_theDeviceSystemParameter,
														nPos, STT_MODULE_TYPE_CURRENT);
		...
		if(nChannelNum == 6)
		{
			if(nChannelNum == nDefChannelNum)
			{
				...
				if(nMergeCurTerminal == STT_LARGE_CURRENT_MERGE_ONE_CH)
				{
					for(int k = 0; k < 5;k++)
					{
						nData1[pCoefItems->Item[k + 1].nCh] =
								nData1[pCoefItems->Item[0].nCh];
						nData2[pCoefItems->Item[k + 1].nCh] =
								nData2[pCoefItems->Item[0].nCh];
					}
				}
				else if(nMergeCurTerminal == STT_LARGE_CURRENT_MERGE_THREE_CH)
				{
					for(int k = 0; k < 3;k++)
					{
						nData1[pCoefItems->Item[k + 3].nCh] =
								nData1[pCoefItems->Item[k].nCh];
						nData2[pCoefItems->Item[k + 3].nCh] =
								nData2[pCoefItems->Item[k].nCh];
					}
				}
			}
			else
...
void CSttReplayTest::OnceWriteProcessDataToDrive1(BYTE *pDriveBuf, int &nWritePos, int nPointNum)
{
	...
	for(int nModuleIndex = 0; nModuleIndex < MAX_MODULE_COUNT; nModuleIndex++)
    {
		...
		else
		{
			...
			for(int i = 0; i < nPointNum; i++)
			{
				...
				CurrentMergeCode(m_oModule[nModuleIndex].m_nIndex, pCoefItems, nChReplayData1, nChReplayData2);
...
{
	stt_xml_serialize_replay(this, pXmlSerialize);
