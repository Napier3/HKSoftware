...
CSttChMapsConfig::CSttChMapsConfig()
{
...
void ChMapConfig_Module_Module(CSttChMaps &oChMaps, PSTT_MODULE pModule)
{
	...
	if(pModuleAttr->m_nModuleType == STT_MODULE_TYPE_CURRENT)
	{
		long nDefChannelNum = pModuleAttr->m_nDefChannelNum;
		if(nChCount == 6 && nDefChannelNum != nChCount)
		{
			if(nDefChannelNum == 3)
			{
				for(nIndex = 0; nIndex < 3; nIndex++)
				{
					pChDefs->m_oChDef[nIndex + 3].m_nChRsIndex = pChDefs->m_oChDef[nIndex].m_nChRsIndex;
					pModule->m_fChTmtCoef[nIndex] = pModule->m_fChTmtCoef[nIndex]*0.5;
					pModule->m_fChTmtCoef[nIndex + 3] = pModule->m_fChTmtCoef[nIndex];
				}
			}
			else if(nDefChannelNum == 1)
			{
				...
				{
					pChDefs->m_oChDef[nIndex].m_nChRsIndex = pChDefs->m_oChDef[0].m_nChRsIndex;
					pModule->m_fChTmtCoef[0] = pModule->m_fChTmtCoef[0]*0.166667;
					pModule->m_fChTmtCoef[nIndex] = pModule->m_fChTmtCoef[0];
				}
			}
		}
	}
