...
//    BOOL	     m_bRepeat;				//是否循环输出
    ...
    struct
    {
		...
		float		m_fActT_AbsErr;//绝对正误差限
		float		m_fActT_AbsErrNeg;//绝对负误差限
...
//		m_bRepeat = FALSE;
		...
		for (int i = 0; i < 10; i++)
        {
			...
			m_paraEstimates[i].m_fActT_AbsErrNeg = 0;
...
BOOL stt_xml_serialize(tmt_StatePara &oStatePara, long nIndex, CSttXmlSerializeBase *pXmlSierialize,int nHarmCount = MAX_HARM_COUNT);
