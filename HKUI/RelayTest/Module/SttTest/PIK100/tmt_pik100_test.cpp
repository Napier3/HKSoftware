...
void stt_xml_serialize(tmt_Pik100Para *pPara, CSttXmlSerializeBase *pXmlSierialize)
{
    ...
    pSerializePara->xml_serialize("","DelayTime","","long",pPara->m_oDA.nDelayTime);
    pSerializePara->xml_serialize("","StopTestAfterCheck","","long",pPara->m_oDA.nStopTestAfterCheck);
}

void stt_xml_serialize(tmt_Pik100Result *pResult, CSttXmlSerializeBase *pXmlSierialize)
{
    pXmlSierialize->xml_serialize("","ResUseful","","number",pResult->m_nResUseful);
    CString strID;
    for(int i = 0;i<12;i++)
    {
        strID.Format(_T("KR[%d]"),i);
        pXmlSierialize->xml_serialize("",strID.GetString(),"","long",pResult->m_nKR[i]);
        strID.Format(_T("KC[%d]"),i);
        pXmlSierialize->xml_serialize("",strID.GetString(),"","long",pResult->m_nKC[i]);
    }
}
