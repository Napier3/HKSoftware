...
void CSttManuTest::TtmToDrv_L336D()
{
#ifdef L336D
	//手动试验
	m_oDrvManuPara.oAnalogPara.nCount = 2;
	m_oDrvManuPara.oAnalogPara.oAnalogHarm[0].nModule=2;
	m_oDrvManuPara.oAnalogPara.oAnalogHarm[0].nWave=1;
	m_oDrvManuPara.oAnalogPara.oAnalogHarm[1].nModule=4;
	m_oDrvManuPara.oAnalogPara.oAnalogHarm[1].nWave=1;
	for (int i=0; i<8; i++)//bo
	{
		m_oDrvManuPara.oAnalogPara.nAnalogBO += ((m_oManuParas.m_binOut[i].nState)<<i);
	}

	tmt_channel *pUData,*pIData;
	for(int k=0;k<6;k++)
	{
		pUData=&m_oManuParas.m_uiVOL[k];
		pIData=&m_oManuParas.m_uiCUR[k];

		CSttDevice::g_pSttDevice->TranslateACVoltBuffer(pUData->Harm[1].fAmp
				,pUData->Harm[1].fAngle,pUData->Harm[1].fFreq,
				k,m_oDrvManuPara.oAnalogPara.oAnalogHarm[0].nData+6);

		CSttDevice::g_pSttDevice->TranslateACCurrentBuffer(pIData->Harm[1].fAmp
				,pIData->Harm[1].fAngle,pIData->Harm[1].fFreq,
				k,m_oDrvManuPara.oAnalogPara.oAnalogHarm[1].nData+6);

		CSttDevice::g_pSttDevice->TranslateACVoltBufferDC(pUData->Harm[0].fAmp,k,m_oDrvManuPara.oAnalogPara.oAnalogHarm[0].nData);
		CSttDevice::g_pSttDevice->TranslateACCurrentBufferDC(pIData->Harm[0].fAmp,k,m_oDrvManuPara.oAnalogPara.oAnalogHarm[1].nData);
	}
...
void CSttManuTest::TtmToDrv_PTU()
{
#ifndef L336D
