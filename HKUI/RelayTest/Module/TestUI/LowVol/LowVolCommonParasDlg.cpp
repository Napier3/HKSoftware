...
#include "ui_LowVolCommonParasDlg.h"
#include <cmath>
#include "../../XLangResource_Native.h"
extern CFont *g_pSttGlobalFont;

QLowVolCommonParasDlg::QLowVolCommonParasDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::QLowVolCommonParasDlg)
{
    ui->setupUi(this);
	m_pParas = NULL;
    m_nModify = 0;
...
QLowVolCommonParasDlg::~QLowVolCommonParasDlg()
{
    delete ui;
}
void QLowVolCommonParasDlg::InitUI()
...
void QLowVolCommonParasDlg::SetDatas(CExBaseList *pParas)
{
...
void QLowVolCommonParasDlg::SetValueToPage(CSttMacroTestParaData *pData)
{

	if(pData->m_strID == STT_MULTI_MACRO_LOWVOL_PARAS_ID_PREFAULTTIME)
	{
	...
	else if (pData->m_strID == STT_MULTI_MACRO_LOWVOL_PARAS_ID_TIMEZERO)
	{
...
void QLowVolCommonParasDlg::GetDatas(CExBaseList *pParas)
{
...
void QLowVolCommonParasDlg::GetPageToValue(CSttMacroTestParaData *pData)
{
	if(pData->m_strID == STT_MULTI_MACRO_LOWVOL_PARAS_ID_PREFAULTTIME)
	{
...
void QLowVolCommonParasDlg::on_m_txt_SteadyTime_editingFinished()
{
	ui->m_txt_SteadyTime->setText(CheckDataMinMax(ui->m_txt_SteadyTime->text(),999.999,0,3));
}

void QLowVolCommonParasDlg::on_m_cmb_VBlock_currentIndexChanged(int index)
{

}
void QLowVolCommonParasDlg::on_m_rBtn_FreqChange_toggled(bool checked)
{
    
}

void QLowVolCommonParasDlg::on_m_btnOK_clicked()
{
	this->done(Accepted);
}

void QLowVolCommonParasDlg::on_m_btnCancel_clicked()
{
	this->close();
}
void QLowVolCommonParasDlg::CheckAllPresence(CExBaseList *pParas)
{
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWVOL_PARAS_ID_PREFAULTTIME);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWVOL_PARAS_ID_TIMEZERO);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWVOL_PARAS_ID_USETMODE);
}
void QLowVolCommonParasDlg::UpdataAllCommonParas(CExBaseList *pAllItems)
...
BOOL QLowVolCommonParasDlg::CheckItemsModify(CExBaseList *pItem)
{
    if (pItem == NULL)
    {
        return FALSE;
    ...
    while(pos != NULL)
    {
        ...
        if(pData->m_strID == STT_MULTI_MACRO_LOWVOL_PARAS_ID_TIMEZERO||
            pData->m_strID == STT_MULTI_MACRO_LOWVOL_PARAS_ID_USETMODE)
        {
