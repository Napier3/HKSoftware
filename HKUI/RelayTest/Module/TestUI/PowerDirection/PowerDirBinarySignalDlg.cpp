...
#include "ui_PowerDirBinarySignalDlg.h"
...
QPowerDirBinarySignalDlg::QPowerDirBinarySignalDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::QPowerDirBinarySignalDlg)
{
    ui->setupUi(this);
    InitUI();
    m_nModify = 0;
...
QPowerDirBinarySignalDlg::~QPowerDirBinarySignalDlg()
{
    delete ui;
}

void QPowerDirBinarySignalDlg::InitUI()
{
	CString strPngPath;
    ...
    ui->m_gbBIn->setTitle(g_sLangTxt_Native_InputSet);
    ui->m_gbNormalBOut->setTitle(g_sLangTxt_PowerDir_NormalBout);
    ui->m_gbFaultBOut->setTitle(g_sLangTxt_PowerDir_FaultBout);
...
void QPowerDirBinarySignalDlg::SetBinBoutName()
{
    //开入开出映射修改
    //开入量
...
void QPowerDirBinarySignalDlg::SetValueToPage(CSttMacroTestParaData *pData)
{
	if(pData->m_strID == STT_MULTI_MACRO_POWERDIR_PARAS_ID_A)
	{
...
void QPowerDirBinarySignalDlg::GetPageToValue(CSttMacroTestParaData *pData)
{
	if(pData->m_strID == STT_MULTI_MACRO_POWERDIR_PARAS_ID_A)
	{
		CString strTmp;
...
void QPowerDirBinarySignalDlg::CheckAllPresence(CExBaseList *pParas)
{
...
void QPowerDirBinarySignalDlg::GetDatas(CExBaseList *pParas)
{
...
void QPowerDirBinarySignalDlg::SetDatas(CExBaseList *pParas)
{
	if(pParas == NULL)
		return;
	m_pParas = pParas;
...
void QPowerDirBinarySignalDlg::UpdateCurrentBinarys(CExBaseList *pItem)
{
...
void QPowerDirBinarySignalDlg::UpdateAllBinarys(CExBaseList *pAllItems)
{
	CSttMacroTest *pSttMacroTest = NULL;
...
BOOL QPowerDirBinarySignalDlg::CheckItemsModify(CExBaseList *pItem)
{
    if (pItem == NULL)
    {
        return FALSE;
...
//确认按钮
void QPowerDirBinarySignalDlg::on_m_btnOk_clicked()
{
	this->done(Accepted);
}
...
//取消按钮
void QPowerDirBinarySignalDlg::on_m_btnCancel_clicked()
{
	this->close();
}
