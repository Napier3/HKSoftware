...
#include "ui_DiffBinarysDlg.h"
#include "../../XLangResource_Native.h"
#include "../../UI/SttTestCntrFrameBase.h"
extern CFont *g_pSttGlobalFont;

QDiffBinarysDlg::QDiffBinarysDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::QDiffBinarysDlg)
{
    ui->setupUi(this);
	m_pParas = NULL;
    m_nModify= 0;
...
QDiffBinarysDlg::~QDiffBinarysDlg()
{
    delete ui;
}
void QDiffBinarysDlg::InitUI()
...
void QDiffBinarysDlg::SetBinBoutName()
{
	//开入开出映射修改
	//开入量
...
void QDiffBinarysDlg::SetDatas(CExBaseList *pParas)
{
...
void QDiffBinarysDlg::SetValueToPage(CSttMacroTestParaData *pData)
{
	if(pData->m_strID == STT_MULTI_MACRO_DIFF_PARAS_ID_A)
	{
...
void QDiffBinarysDlg::GetDatas(CExBaseList *pParas)
{
...
void QDiffBinarysDlg::GetPageToValue(CSttMacroTestParaData *pData)
{
	if(pData->m_strID == STT_MULTI_MACRO_DIFF_PARAS_ID_A)
	{
...
void QDiffBinarysDlg::on_m_btnOK_clicked()
{
	this->done(Accepted);
}

void QDiffBinarysDlg::on_m_btnCancel_clicked()
{
	this->close();
}
void QDiffBinarysDlg::CheckAllPresence(CExBaseList *pParas)
{
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID_A);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID_B);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID_C);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID_D);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID_E);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID_F);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID_G);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID_H);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID_AndOr);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID__B01);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID__B02);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID__B03);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID__B04);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID__B05);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID__B06);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID__B07);
	CheckForPresence(pParas,STT_MULTI_MACRO_DIFF_PARAS_ID__B08);
}
void QDiffBinarysDlg::UpdataAllBinarys(CExBaseList *pAllParas)
{
...
BOOL QDiffBinarysDlg::CheckItemsModify(CExBaseList *pItem)
{
    if (pItem == NULL)
    {
        return FALSE;
