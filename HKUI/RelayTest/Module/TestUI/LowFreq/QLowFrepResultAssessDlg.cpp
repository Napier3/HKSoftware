...
QLowFrepResultAssessDlg::QLowFrepResultAssessDlg(CDataGroup *m_oLowFrepDataGroup, QWidget *parent) : QDialog(parent)
{
	m_oLowFrepResultAssessDataGroup = m_oLowFrepDataGroup;
	// 在2k分辨率150%缩放下显示不全
...
QLowFrepResultAssessDlg::~QLowFrepResultAssessDlg()
{

}

void QLowFrepResultAssessDlg::InitUI()
{
	setWindowFlags(Qt::CustomizeWindowHint|Qt::WindowTitleHint| Qt::Dialog);
	m_pResultTableWidget = new QTableWidget(5, 5, this);
...
//	m_pResultTableWidget->setColumnWidth(1, 240);

//	m_pResultTableWidget->setColumnWidth(2, 120);
//	m_pResultTableWidget->setColumnWidth(3, 120);
...
//Sections
    ...
    m_pLabTripFre = new QLabel(/*"频率动作值(Hz)"*/g_sLangTxt_Gradient_FreAValue);
	m_pResultTableWidget->setCellWidget(1, 0, m_pLabTripFre);

	m_pLabDvdtClose = new QLabel(/*频率滑差动作值(Hz/s)*/g_sLangTxt_LowFreq_dvdtSetting);
    ...
    m_pLabTripULock = new QLabel(/*"低电压闭锁值(V)"*/g_sLangTxt_LowFreq_ILock + "(V)");
	m_pResultTableWidget->setCellWidget(4, 0, m_pLabTripULock);

    //m_pLabTrip = new QLabel(/*"频率动作值绝对误差限(Hz)"*/g_sLangTxt_Native_FreqPickAbsErr + "(Hz)");
	//m_pLabTrip = new QLabel(/*"频率动作值绝对误差限(Hz)"*/g_sLangTxt_StateEstimate_AbsError);
	//m_pResultTableWidget->setCellWidget(0, 1, m_pLabTrip);
    //m_pLabDvdt = new QLabel(/*"滑差闭锁值绝对误差限(Hz)"*/g_sLangTxt_Native_sliptripValue + g_sLangTxt_Native_AbsErrorLimit + "(Hz)");
	//m_pLabDvdt = new QLabel(/*"滑差闭锁值绝对误差限(Hz)"*/g_sLangTxt_StateEstimate_AbsError);
	//m_pResultTableWidget->setCellWidget(2, 1, m_pLabDvdt);

	m_pLabTripFreNeg = new QLabel("--");
	m_pLabDvdtCloseNeg = new QLabel("--");
	m_pLabTripILockNeg = new QLabel("--");
	m_pLabTripULockNeg = new QLabel("--");

	m_pResultTableWidget->setCellWidget(1, 4, m_pLabTripFreNeg);
	m_pResultTableWidget->setCellWidget(2, 4, m_pLabDvdtCloseNeg);
	m_pResultTableWidget->setCellWidget(3, 4, m_pLabTripILockNeg);
	m_pResultTableWidget->setCellWidget(4, 4, m_pLabTripULockNeg);


	m_pLabActionTime->setAlignment(Qt::AlignCenter);
	m_pLabTripFre->setAlignment(Qt::AlignCenter);
	m_pLabDvdtClose->setAlignment(Qt::AlignCenter);
	m_pLabTripILock->setAlignment(Qt::AlignCenter);
	m_pLabTripULock->setAlignment(Qt::AlignCenter);

	m_pLabTripFreNeg->setAlignment(Qt::AlignCenter);
	m_pLabDvdtCloseNeg->setAlignment(Qt::AlignCenter);
	m_pLabTripILockNeg->setAlignment(Qt::AlignCenter);
	m_pLabTripULockNeg->setAlignment(Qt::AlignCenter);

	m_pCmbActionTime = new QComboBox(m_pResultTableWidget);
	m_pCmbTripFre = new QComboBox(m_pResultTableWidget);
	m_pCmbDvdtClose = new QComboBox(m_pResultTableWidget);
	m_pCmbTripILock = new QComboBox(m_pResultTableWidget);
	m_pCmbTripULock = new QComboBox(m_pResultTableWidget);

	m_pCmbActionTime->setObjectName("0");
	m_pCmbTripFre->setObjectName("1");
	m_pCmbDvdtClose->setObjectName("2");
	m_pCmbTripILock->setObjectName("3");
	m_pCmbTripULock->setObjectName("4");
	...
	CbbErrorType(1, m_pCmbTripFre->currentText());
	CbbErrorType(2, m_pCmbDvdtClose->currentText());
	CbbErrorType(3, m_pCmbTripILock->currentText());
	CbbErrorType(4, m_pCmbTripULock->currentText());

	
}
void QLowFrepResultAssessDlg::slot_cmbErrorChanged(const QString& strText)
{
...
void QLowFrepResultAssessDlg::CbbErrorType(int nIndex, CString strText)
{
...
void QLowFrepResultAssessDlg::SetItemEnable(int nRow, int nCol, BOOL bEnable)
{
...
void QLowFrepResultAssessDlg::ComboxModelStyle(QComboBox *pCombox)
{
...
void QLowFrepResultAssessDlg::InitFont()
{
	...
	m_pLabTripFre->setFont(*g_pSttGlobalFont);
	...
	m_pLabTripULock->setFont(*g_pSttGlobalFont);

// 	m_pLabAct->setFont(*g_pSttGlobalFont);
// 	m_pLabLock->setFont(*g_pSttGlobalFont);
}

void QLowFrepResultAssessDlg::slot_btnOK_Clicked()
{
	long nActionTime = m_pCmbActionTime->currentIndex();
	long nTripFre = m_pCmbTripFre->currentIndex();
	long nDvdtClose = m_pCmbDvdtClose->currentIndex();
	long nTripILock = m_pCmbTripILock->currentIndex();
	long nTripULock = m_pCmbTripULock->currentIndex();
	...
	float fHzActValRelErr = m_pResultTableWidget->item(1, 2)->text().toFloat();
	float fHzActValAbsErr = m_pResultTableWidget->item(1, 3)->text().toFloat();

	float fDfdtRelErr = m_pResultTableWidget->item(2, 2)->text().toFloat();
	float fDfdtAbsErr = m_pResultTableWidget->item(2, 3)->text().toFloat();

	float fIlowRelErr = m_pResultTableWidget->item(3, 2)->text().toFloat();
	float fIlowAbsErr = m_pResultTableWidget->item(3, 3)->text().toFloat();

	float fUlowRelErr = m_pResultTableWidget->item(4, 2)->text().toFloat();
	float fUlowAbsErr = m_pResultTableWidget->item(4, 3)->text().toFloat();

	CDvmData *pData = NULL;

	if(m_oLowFrepResultAssessDataGroup == NULL)
	{
		return;
	}


	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_ACTTIME_RELERR);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(fTtripRelErr, 'f',3);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_ACTTIME_ABSERRPOS);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(fTtripAbsErrPos, 'f', 3);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_ACTTIME_ABSERRNEG);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(fTtripAbsErrNeg, 'f', 3);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_ACTVALUE_RELERR);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(fHzActValRelErr, 'f', 3);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_ACTVALUE_ABSERR);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(fHzActValAbsErr, 'f', 3);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_SLIPTRIP_RELERR);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(fDfdtRelErr, 'f', 3);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_SLIPTRIP_ABSERR);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(fDfdtAbsErr, 'f',3);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_IBLOCK_RELERR);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(fIlowRelErr, 'f',3);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_IBLOCK_ABSERR);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(fIlowAbsErr, 'f',3);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_VBLOCK_RELERR);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(fUlowRelErr, 'f',3);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_VBLOCK_ABSERR);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(fUlowAbsErr, 'f',3);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_ACTTIME_RSLTEVAL);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(nActionTime);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_ACTVALUE_RSLTEVAL);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(nTripFre);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_SLIPTRIP_RSLTEVAL);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(nDvdtClose);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_IBLOCK_RSLTEVAL);
	if (pData != NULL)
	{
		pData->m_strValue = QString::number(nTripILock);
	}

	pData = (CDvmData *)m_oLowFrepResultAssessDataGroup->FindByID(STT_MULTI_MACRO_LOWFREQ_PARAS_ID_VBLOCK_RSLTEVAL);
	if (pData != NULL)
	{
...
void QLowFrepResultAssessDlg::InitConnections()
{
	...
	connect(m_pCmbTripFre, SIGNAL(currentIndexChanged(const QString &)), this, SLOT(slot_cmbErrorChanged(const QString &)));
	...
	connect(m_pCmbTripULock, SIGNAL(currentIndexChanged(const QString &)), this, SLOT(slot_cmbErrorChanged(const QString &)));
}
void QLowFrepResultAssessDlg::slot_btnCancel_Clicked()
{
	reject();
}

void QLowFrepResultAssessDlg::UpdataResultAssess()
{
	CDvmData *pData = NULL;
	long nValue;
	float fValue;
	...
	{
		fValue = CString_To_double(pData->m_strValue);
		m_pItem10 = new QTableWidgetItem(QString::number(fValue, 'f', 3));
		m_pItem10->setTextAlignment(Qt::AlignCenter);
		m_pResultTableWidget->setItem(4, 2, m_pItem10);
	}
	...
	{
		fValue = CString_To_double(pData->m_strValue);
		m_pItem11 = new QTableWidgetItem(QString::number(fValue, 'f', 3));
		m_pItem11->setTextAlignment(Qt::AlignCenter);
		m_pResultTableWidget->setItem(4, 3, m_pItem11);
	}
	...
	{
		nValue = CString_To_long(pData->m_strValue);
		m_pCmbTripFre->setCurrentIndex(nValue);
	}
	...
	{
		nValue = CString_To_long(pData->m_strValue);
		m_pCmbTripULock->setCurrentIndex(nValue);
	}
}

void QLowFrepResultAssessDlg::slot_cellClicked(int row ,int col)
{
