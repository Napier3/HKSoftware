...
#include "ui_LowFreqBinarysDlg.h"
#include "../../XLangResource_Native.h"
#include "../../UI/SttTestCntrFrameBase.h"
extern CFont *g_pSttGlobalFont;

QLowFreqBinarysDlg::QLowFreqBinarysDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::QLowFreqBinarysDlg)
{
    ui->setupUi(this);
	m_pParas = NULL;
    m_nModify= 0;
...
QLowFreqBinarysDlg::~QLowFreqBinarysDlg()
{
    delete ui;
}
void QLowFreqBinarysDlg::InitUI()
...
void QLowFreqBinarysDlg::SetBinBoutName()
{
	//开入开出映射修改
	//开入量
...
void QLowFreqBinarysDlg::SetDatas(CExBaseList *pParas)
{
...
void QLowFreqBinarysDlg::SetValueToPage(CSttMacroTestParaData *pData)
{
	if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID_A)
	{
		ui->m_check_A->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID_B)
	{
		ui->m_check_B->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID_C)
	{
		ui->m_check_C->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID_D)
	{
		ui->m_check_D->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID_E)
	{
		ui->m_check_E->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID_F)
	{
		ui->m_check_F->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID_G)
	{
		ui->m_check_G->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID_H)
	{
		ui->m_check_H->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID_AndOr)
	{	
	...
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B01)
	{
		ui->m_check_B01->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B02)
	{
		ui->m_check_B02->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B03)
	{
		ui->m_check_B03->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B04)
	{
		ui->m_check_B04->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B05)
	{
		ui->m_check_B05->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B06)
	{
		ui->m_check_B06->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B07)
	{
		ui->m_check_B07->setChecked((bool)pData->m_strValue.toInt());
	}
	else if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B08)
	{
		ui->m_check_B08->setChecked((bool)pData->m_strValue.toInt());
	}
}
void QLowFreqBinarysDlg::GetDatas(CExBaseList *pParas)
{
...
void QLowFreqBinarysDlg::GetPageToValue(CSttMacroTestParaData *pData)
{
	if(pData->m_strID == STT_MULTI_MACRO_LOWFREQ_PARAS_ID_A)
	{
...
void QLowFreqBinarysDlg::on_m_btnOK_clicked()
{
	this->done(Accepted);
}

void QLowFreqBinarysDlg::on_m_btnCancel_clicked()
{
	this->close();
}
void QLowFreqBinarysDlg::CheckAllPresence(CExBaseList *pParas)
{
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID_A);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID_B);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID_C);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID_D);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID_E);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID_F);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID_G);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID_H);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID_AndOr);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B01);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B02);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B03);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B04);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B05);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B06);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B07);
	CheckForPresence(pParas,STT_MULTI_MACRO_LOWFREQ_PARAS_ID__B08);
}
void QLowFreqBinarysDlg::UpdataAllBinarys(CExBaseList *pAllParas)
{
...
BOOL QLowFreqBinarysDlg::CheckItemsModify(CExBaseList *pItem)
{
    if (pItem == NULL)
    {
        return FALSE;
