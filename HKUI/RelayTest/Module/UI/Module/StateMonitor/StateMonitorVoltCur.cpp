...
extern void Stt_Global_GetBinBoutMaxWidth(const CString &sSrcValue);	//20241017 huangliang 记录U/I中最大宽度
...
QStateMonitorVoltCur::QStateMonitorVoltCur(QWidget* parent)
{
	...
	m_labelValue.setGeometry(2 + 120, 1, nWidth, nHeight);//mod wangtao 20240818 修改间距由60变为120，防止文字阻挡
}
...
void QStateMonitorVoltCur::InitCurve(CExBaseList* pChlUDatasList, CExBaseList* pChlIDatasList)
{
	...
	if(g_pStateMonitorWidget->IsSingle())
	{
		...
		UpdateYTtile(tr(""), 9, TRUE);//wangtao 20240817 单通道Y轴左边默认不显示文本
...
void QStateMonitorVoltCur::InitYTick(double fUMin, double fUMax, double fIMin, double fIMax)
{
	...
	Stt_Global_GetBinBoutMaxWidth(QString::number(nUMax));	//20241017 huangliang 记录电压电流的最大宽度
	Stt_Global_GetBinBoutMaxWidth(QString::number(nIMax));
...
void QStateMonitorVoltCur::AddPoint(CSttMacroChannels* pMacroChs)
{
		...
		while (pos)
	{
		CSttMacroChannel* pMacroCh = (CSttMacroChannel*)pMacroChs->GetNext(pos);
			...
			{
				ManagePointData(pMacroCh, nIndex);
				nIndex++;
			}
		}
	} 
	else
	{
		...
		{
		case uf_type:
			pMacroCh = &pMacroChs->m_oUfCh; break;
		case if_type:
			pMacroCh = &pMacroChs->m_oIfCh; break;
		case z_type:
			pMacroCh = &pMacroChs->m_oZ_Fault; break;
		default:
			break;
		}
		if (pMacroCh)
		{
			ManagePointData(pMacroCh, 0);
		}
		}
}

void QStateMonitorVoltCur::ManagePointData(CSttMacroChannel* pMacroCh, int nIndex)
{
		QPointF pt;

	QStateMonitorData* pLastPoint = (QStateMonitorData*)m_pLastPoints->FindByID(pMacroCh->m_strID);
	//if ((m_dLastTime < EPSINON) && (pLastPoint))//add wangtao 20240904 递变相关模块，底层会在结束时发送一包绝对时为0的报文，剔除掉
	//{
	//	return;
	//}
		pt.setX(m_dLastTime);
	...
	switch (m_ChangeType)
		...
		case phasor_type:
		fVal = pMacroCh->m_oRtDataCh.m_fAng < 0 ? pMacroCh->m_oRtDataCh.m_fAng + 360 : pMacroCh->m_oRtDataCh.m_fAng; break;
		case fre_type:
...
CSttMacroChannel* QStateMonitorVoltCur::GetUfMacroChannelClone()
{
	CSttMacroChannel* pChannel = (CSttMacroChannel*)&g_oSttTestResourceMngr.m_oRtDataMngr.m_pMacroChannels->m_oUfCh;
	return (CSttMacroChannel*)pChannel->Clone();
}

CSttMacroChannel* QStateMonitorVoltCur::GetIfMacroChannelClone()
{
	CSttMacroChannel* pChannel = (CSttMacroChannel*)&g_oSttTestResourceMngr.m_oRtDataMngr.m_pMacroChannels->m_oIfCh;
	return (CSttMacroChannel*)pChannel->Clone();
}

CSttMacroChannel* QStateMonitorVoltCur::GetImpMacroChannelClone()
{
	CSttMacroChannel* pChannel = (CSttMacroChannel*)&g_oSttTestResourceMngr.m_oRtDataMngr.m_pMacroChannels->m_oZ_Fault;
	return (CSttMacroChannel*)pChannel->Clone();
}
...
void QStateMonitorVoltCur::UpdateValueText()
{
	...
	while (pos)
	{
		...
		if(pCurvePoints->size())
		{
			...
			//add wangtao 20240818 判断当前变化类型 幅值 相位 频率
			if (m_ChangeType == 0)  
			{
				if (pMacroCh->m_strID.contains("I"))
			{
				...
				else if (pMacroCh->m_strID.contains("U"))
			{
				strText += "V]";
			}
				else if (pMacroCh->m_strID.contains("Z"))
				{
					strText += CString("Ω]");
				}
			}
...
void QStateMonitorVoltCur::GetMacroChannel(int nChannel)
{
	...
	else if (nChannel == uf_type)
	{
		pMacroChannel = GetUfMacroChannelClone();
		m_oMacroChannels.AddTail(pMacroChannel);
	}
	else if (nChannel == if_type)
	{
		pMacroChannel = GetIfMacroChannelClone();
		m_oMacroChannels.AddTail(pMacroChannel);
	}
	else if (nChannel == z_type)
	{
		pMacroChannel = GetImpMacroChannelClone();
		m_oMacroChannels.AddTail(pMacroChannel);
	}
