...
QImpCharactWidget::QImpCharactWidget(CCharacteristics *pCharacteristics,QWidget *parent, bool bUseOptrButtons, BOOL bUseMouseMoveChar) :
    QWidget(parent)
{
	m_bStart = false;
...
void QImpCharactWidget::UpdateDrawOptrBtns()
{
	...
	if(nHeight > 600)
	{
		...
		{
            m_listDrawOptrBtn[i]->setGeometry(i * (50 + nWidth), rc.height() - STT_Charact_ToolBtn_Width , STT_Charact_ToolBtn_Width, STT_Charact_ToolBtn_Width);
		}
	}
	else
	{
		...
		for (int i = 0; i < m_listDrawOptrBtn.size(); i++)
		{
            m_listDrawOptrBtn[i]->setGeometry(rc.right() - STT_Charact_ToolBtn_Width, 30 + i * (50 + nHeight), STT_Charact_ToolBtn_Width, STT_Charact_ToolBtn_Width);
		}
	}
}
...
void QImpCharactWidget::ResizeUI(int nWidth,int nHeight)
{
	...
	if (m_bUseOptrButtons)
	{
		if(nHeight > 600)
		{
			...
			for (int i = 0; i < m_listDrawOptrBtn.size(); i++)
			{
                m_listDrawOptrBtn[i]->setGeometry(i * (50 + nWidth), nHeight - STT_Charact_ToolBtn_Width , STT_Charact_ToolBtn_Width, STT_Charact_ToolBtn_Width);
			}
		}
		else
		{
			...
			for (int i = 0; i < m_listDrawOptrBtn.size(); i++)
			{
                m_listDrawOptrBtn[i]->setGeometry(nWidth - STT_Charact_ToolBtn_Width, 30 + i * (50 + nHeight), STT_Charact_ToolBtn_Width, STT_Charact_ToolBtn_Width);
			}
		}
	}
...
void QImpCharactWidget::SetShowCurrCharactColor(BOOL bUpdate)
{ 
	m_pCharacterDrawView->m_bShowCurrCharactColor = bUpdate;
}

void QImpCharactWidget::SetCurCharact(CCharacteristic *pCharacterist)
{
    m_pCharacteristic = pCharacterist;
    m_pCharacterDrawView->SetCurCharact(pCharacterist);
}
...
{
	BOOL bShowChars = FALSE;//是否显示全部曲线
    //20241025 wanmj 如果入参为NULL m_pCharacteristic才赋值NULL;
	if (pCharacteristic == NULL)//如果传入的为空,则显示全部曲线
	{
		m_pCharacteristic = NULL;
		bShowChars = TRUE;
	}
	else if (pCharacteristic->GetClassID() == CHARCLASSID_CCHARACTERISTIC)
	{
		m_pCharacteristic = (CCharacteristic*)pCharacteristic;
	}
	else if (pCharacteristic->GetClassID() == CHARCLASSID_CCHARACTERISTICS)
	{
		m_pCharacteristics = (CCharacteristics*)pCharacteristic;
		bShowChars = TRUE;
	}

	ASSERT(m_pCharacteristics);
	//m_pCharacteristic应该为m_pCharacteristics的子对象,如果不是,则给其初始化为空
	if (m_pCharacteristics->Find(m_pCharacteristic) == NULL)
	{
		m_pCharacteristic = NULL;
	}
	...
	if (m_pCharacteristic != NULL && (!bShowChars))
	{
		m_pCharacteristic->CalCharElement(NULL);
		m_pCharacteristic->CalCharErrBandLen(m_pCharacteristics->m_dMaxSensAng);
