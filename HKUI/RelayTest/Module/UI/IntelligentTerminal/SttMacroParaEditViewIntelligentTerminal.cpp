...
void QSttMacroParaEditViewIntelligentTerminal::SendUpdateParameter()
{
	...
	if (!g_oSystemParas.m_nHasDigital&&m_pIntellTerminalParas->m_nFuncType==0)
	{
		...
		if (nIndex>=0)
		{
			CString strItemPathNodes=stt_Frame_TestMacroUI()->m_strID.Left(nIndex);
		strItemPath+= "$" + strItemPathNodes;
		}
		
		strMacroID=STT_ORG_MACRO_BinaryManuTest;//开关量手动测试MarcoID
        stt_Frame_Ats_SetItemPara(strItemPath, strMacroID);
	}
}
...
void QSttMacroParaEditViewIntelligentTerminal::CopyBinaryConfig(BOOL b)
{
	if(b)
	{
		stt_GetBinaryConfig()->m_nBinLogic = m_pIntellTerminalParas->m_nBinLogic;
		for (int i=0;i<MAX_BINARYIN_COUNT;i++){
			stt_GetBinaryConfig()->m_binIn[i].nSelect = m_pIntellTerminalParas->m_binIn[i].nSelect;
		}

		for (int i=0;i<MAX_BINARYOUT_COUNT;i++){
			stt_GetBinaryConfig()->m_binOut[i].nState = m_pIntellTerminalParas->m_binOut[i].nState;
		}

		for (int i=0;i<MAX_ExBINARY_COUNT;i++){
			stt_GetBinaryConfig()->m_binInEx[i].nSelect = m_pIntellTerminalParas->m_binInEx[i].nSelect;
		}

		for (int i=0;i<MAX_ExBINARY_COUNT;i++){
			stt_GetBinaryConfig()->m_binOutEx[i].nState = m_pIntellTerminalParas->m_binOutEx[i].nState;
		}
	}
	else
	{
		m_pIntellTerminalParas->m_nBinLogic = stt_GetBinaryConfig()->m_nBinLogic;
		for (int i=0;i<MAX_BINARYIN_COUNT;i++){
			m_pIntellTerminalParas->m_binIn[i].nSelect = stt_GetBinaryConfig()->m_binIn[i].nSelect;
		}

		for (int i=0;i<MAX_BINARYOUT_COUNT;i++){
			m_pIntellTerminalParas->m_binOut[i].nState = stt_GetBinaryConfig()->m_binOut[i].nState;
		}

		for (int i=0;i<MAX_ExBINARY_COUNT;i++){
			m_pIntellTerminalParas->m_binInEx[i].nSelect = stt_GetBinaryConfig()->m_binInEx[i].nSelect;
		}

		for (int i=0;i<MAX_ExBINARY_COUNT;i++){
			m_pIntellTerminalParas->m_binOutEx[i].nState = stt_GetBinaryConfig()->m_binOutEx[i].nState;
		}
	}
}
...
void QSttMacroParaEditViewIntelligentTerminal::UpdateTestResource(BOOL bCreateChMaps)
{
	...
	m_pIntellTerminalWidget->SetData(stt_GetSttTestResource(), m_pIntellTerminalParas, 0);
