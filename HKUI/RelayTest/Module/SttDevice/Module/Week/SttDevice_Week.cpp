...
int g_nL336DWeekType = STT_MODULE_WEEK_L336D_ECTEVT;
...
unsigned int L336D_ECTEVT_Tap[8][2] =
{
	{0, 0},
	{0, 0},
	{0, 0},
		...
		if(pChsAdjust != NULL)
		{
			if(nChannel > -1 && nChannel < pModule->m_oModuleAttr.m_nChannelNum)
			{
				if(nHarmIndex < pChsAdjust[nChannel].nHarmCount)
				{
					...
					if(pChWaveAdj != NULL)
					{
						if(pChWaveAdj[nHarmIndex].m_nGearCount <= 1)
						{
							fCoef = pChWaveAdj[nHarmIndex].m_oChGearAdj[0].m_oAdjItem.m_fCoef;
						}
						else
						{
							for(int nGearIndex = 0; nGearIndex < pChWaveAdj[nHarmIndex].m_nGearCount; nGearIndex++)
							{
...
void TransAnalogDCToDrv_Week_PIA48S(float fAmp, int nChannel, int nModuleIndex, long *pDrvData, BOOL bUseDcZero)
{
	...
	if(pChsAdjust != NULL)
	{
		...
		if(pChWaveAdj != NULL)
		{
			...
			{
				pAdjItem = &pChWaveAdj[0].m_oChGearAdj[nGearCount - 1].m_oAdjItem;
			...
			else
			{
				...
				for(long nIndex = 0; nIndex < nGearCount; nIndex++)
				{
					...
					{
						pAdjItem = &pChWaveAdj[0].m_oChGearAdj[nIndex].m_oAdjItem;
						fCoef = pAdjItem->m_fCoef;
					}
				}
			}
	...
	if(pChsAdjust != NULL)
	{
		...
		if(pChWaveAdj != NULL)
		{
			if(nHarm < pChsAdjust->nHarmCount)
			{
				...
				{
					pAdjItem = &pChWaveAdj[nHarm].m_oChGearAdj[nGearCount - 1].m_oAdjItem;
					fCoef = pAdjItem->m_fCoef;
				}
				else
				{
					...
					for(long nIndex = 0; nIndex < nGearCount; nIndex++)
					{
						...
						{
							pAdjItem = &pChWaveAdj[nHarm].m_oChGearAdj[nIndex].m_oAdjItem;
							fCoef = pAdjItem->m_fCoef;
							break;
						}
					}
								  ...
								  int nModuleIndex, long *pnDrvData)
{
	...
	if(pChsAdjust != NULL)
	{
		...
		if(pChWaveAdj != NULL)
		{
			...
			{
				PSTT_ADJUST_ITEM pAdjItem;
				long nGearCount = pChWaveAdj[nHarm + 1].m_nGearCount;

				if(fabs(fAmp) >= pChWaveAdj[nHarm + 1].m_oChGearAdj[nGearCount - 1].m_fGearValue)
				{
					pAdjItem = &pChWaveAdj[nHarm + 1].m_oChGearAdj[nGearCount - 1].m_oAdjItem;
				...
				else
				{
					...
					for(long nIndex = 0; nIndex < nGearCount; nIndex++)
					{
						if(fabs(fAmp) <= pChWaveAdj[nHarm + 1].m_oChGearAdj[nIndex].m_fGearValue)
						{
							pAdjItem = &pChWaveAdj[nHarm + 1].m_oChGearAdj[nIndex].m_oAdjItem;
							faddPh = pAdjItem->m_fAngle;
							break;
						}
					}
...
void Manual_Week_PIA48S_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nHarm, int nIndex,
						BOOL bIsDc)
{
...
void Manual_Week_PN_Main_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nHarm, int nIndex,
						 BOOL bIsDc)
{
	int nModuleOffset = nIndex;
	nModuleOffset += g_theDeviceSystemParameter->m_nModuleCount_U;
	...
	if(bIsDc)
	{
		for(int k = 0; k < 12; k++)
		{
			...
			{
				pTmt_channel = &pAnalogModule->m_pCurrChannel[nCurChanCnt];
				nCurChanCnt++;
			}
	...
	else
	{
		...
		pDrv->oDrv_HarmParam[nModuleOffset].nDWcnt = 12 + 3 * 12;
	}
}
	...
	if(pChsAdjust != NULL)
	{
		...
		if(pChWaveAdj != NULL)
		{
			...
			else
			{
				...
				for(long nIndex = 0; nIndex < nGearCount; nIndex++)
				{
					if(fabs(fAmp) <= (pChWaveAdj[0].m_oChGearAdj[nIndex].m_fGearValue + EPSINON))
					{
						pAdjItem = &pChWaveAdj[0].m_oChGearAdj[nIndex].m_oAdjItem;
						fCoef = pAdjItem->m_fCoef;
						break;
...
void Manual_Week_PIA12DS_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nIndex)
{
...
void TransAnalogDCToDrv_Week_EVTECT(float fAmp, int nChannel, int nModuleIndex, long *pDrvData, BOOL bUseDcZero)
{
	...
	if(pChsAdjust != NULL)
	{
		...
		if(pChWaveAdj != NULL)
		{
			...
			if(fChMax > (pChWaveAdj[0].m_oChGearAdj[nGearCount - 1].m_fGearValue + EPSINON))
			{
				pAdjItem = &pChWaveAdj[0].m_oChGearAdj[nGearCount - 1].m_oAdjItem;
				fCoef = pAdjItem->m_fCoef;
			}
			else
			{
				...
				for(long nIndex = 0; nIndex < nGearCount; nIndex++)
				{
					if(fChMax < (pChWaveAdj[0].m_oChGearAdj[nIndex].m_fGearValue + EPSINON))
					{
						pAdjItem = &pChWaveAdj[0].m_oChGearAdj[nIndex].m_oAdjItem;
						fCoef = pAdjItem->m_fCoef;
						break;
	...
	if(pChsAdjust != NULL)
	{
		...
		if(pChWaveAdj != NULL)
		{
			if(nHarm < pChsAdjust->nHarmCount)
			{
				...
				if(fChMax > (pChWaveAdj[nHarm].m_oChGearAdj[nGearCount - 1].m_fGearValue + EPSINON))
				{
					pAdjItem = &pChWaveAdj[nHarm].m_oChGearAdj[nGearCount - 1].m_oAdjItem;
					fCoef = pAdjItem->m_fCoef;
				...
				else
				{
					...
					for(long nIndex = 0; nIndex < nGearCount; nIndex++)
					{
						if(fChMax < (pChWaveAdj[nHarm].m_oChGearAdj[nIndex].m_fGearValue + EPSINON))
						{
							pAdjItem = &pChWaveAdj[nHarm].m_oChGearAdj[nIndex].m_oAdjItem;
							fCoef = pAdjItem->m_fCoef;
							fZero = pAdjItem->m_fZero;
...
void Manual_Week_EVTECT_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nHarm, int nIndex,
						BOOL bIsDc)
{
...
void Manual_Week_EVTECT_Ex1_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nIndex)
{
		...
		if(nTmtPos >= 0)
		{
			...
			if(ui.Ramp[0].nIndex > -1)
			{
				...
				if(g_nTmtParaMode == 0)
				{
					...
					if(bIsVolt)
					{
						fStart = fStart*1000*g_theSystemConfig->GetPTRate(nTmtPos/3);
						fEnd = fEnd*1000*g_theSystemConfig->GetPTRate(nTmtPos/3);
						fStep = fStep*g_theSystemConfig->GetPTRate(nTmtPos/3);
					}
					else
					{
						fStart = fStart*g_theSystemConfig->GetCTRate(nTmtPos/3);
						fEnd = fEnd*g_theSystemConfig->GetCTRate(nTmtPos/3);
						fStep = fStep*g_theSystemConfig->GetCTRate(nTmtPos/3);
					}
				}
				...
				if(pChsAdjust != NULL)
				{
					...
					{
						PSTT_CHANNEL_WAVE_ADJUST pChWaveAdj = pChsAdjust[k].m_pChWaveAdj;
						if(pChWaveAdj != NULL)
						{
							if(pChWaveAdj[0].m_nGearCount > 0)
							{
								fCoef = pChWaveAdj[0].m_oChGearAdj[pChWaveAdj[0].m_nGearCount - 1].m_oAdjItem.m_fCoef;
							}
						}
					}
				}
...
void Manual_Week_DC6U10V_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nIndex)
{
...
void Manual_Week_DC6I20mA_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nIndex)
{
...
void Manual_Week_PN_DC6U6I_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nIndex)
{
...
void TransAnalogDCToDrv_Week_L336D(float fAmp, int nChannel, int nModuleIndex, long *pDrvData, BOOL bUseDcZero)
{
	...
	if(g_nL336DWeekType == STT_MODULE_WEEK_L336D_ECTEVT)
	{
		if(nChannel < 4)
		{
			//Volt
					   ...
					   BOOL bIsDc, BOOL bIsModuleOutput)
{
	...
	if(bIsDc)
	{
		...
		if(g_nL336DWeekType = STT_MODULE_WEEK_L336D_ECTEVT)
		{
			g_nWeekTap = GetTapCode_L336D_ECTEVT(fDcMaxOut, FALSE, bTapDc, WeekTap[nIndex]);
		}
	...
	else
	{
		...
		if(nHarm == 1)
		{
			if(g_nL336DWeekType = STT_MODULE_WEEK_L336D_ECTEVT)
			{
				g_nWeekTap = GetTapCode_L336D_ECTEVT(fMaxOut, FALSE, bTapDc, WeekTap[nIndex]);
			}
		}
...
int GetTapCode_L336D_ECTEVT(float *fMaxOut, BOOL bStateTest, BOOL *bTapDc, int *nTap)
{
	//ECTEVT类型
	int nRetTap = 0;
	float nValue;
	...
	for(int i = 0; i < 4; i++)
	{
		switch(i)
		{
			...
			case 2:
			{
				nRetTap |= L336D_ECTEVT_Tap[i][0];
				nTap[i] = 0;//fVoltMax = 4.0
			}
			break;
	...
	if(pChsAdjust != NULL)
	{
		...
		if(pChWaveAdj != NULL)
		{
			if(nHarm < pChsAdjust->nHarmCount)
			{
				...
				if(fAmp > (pChWaveAdj[nHarm].m_oChGearAdj[nGearCount - 1].m_fGearValue + EPSINON))
				{
					pAdjItem = &pChWaveAdj[nHarm].m_oChGearAdj[nGearCount - 1].m_oAdjItem;
					fCoef = pAdjItem->m_fCoef;
					fZero = pAdjItem->m_fZero;
				...
				else
				{
					...
					for(long nIndex = 0; nIndex < nGearCount; nIndex++)
					{
						if(fAmp < (pChWaveAdj[nHarm].m_oChGearAdj[nIndex].m_fGearValue + EPSINON))
						{
							pAdjItem = &pChWaveAdj[nHarm].m_oChGearAdj[nIndex].m_oAdjItem;
							fCoef = pAdjItem->m_fCoef;
							fZero = pAdjItem->m_fZero;
