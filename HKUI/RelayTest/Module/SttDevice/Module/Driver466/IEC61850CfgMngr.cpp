	...
	for(int k = 1; k < 16; k++)
	{
		for(int i = 0; i < g_theDeviceSystemParameter->m_nModuleCount; i++)
		{
					...
					|| pModule->m_oModuleAttr.m_nModuleType == STT_MODULE_TYPE_ADMU)
			{
				...
				if(pModule->m_oModuleAttr.m_nFiberPortNum_STSend > 0)
				{
				...
				{
					m_pIECConfigFt3->item[nModuleIndex].nSTSendBeginIdx = -1;
					m_pIECConfigFt3->item[nModuleIndex].nSTSendEndIdx = -1;
				}
				nSTSendBeginIdx += pModule->m_oModuleAttr.m_nFiberPortNum_STSend;

				if(pModule->m_oModuleAttr.m_nFiberPortNum_STRecv > 0)
				{
				...
				{
					m_pIECConfigFt3->item[nModuleIndex].nSTRecvBeginIdx = -1;
					m_pIECConfigFt3->item[nModuleIndex].nSTRecvEndIdx = -1;
				}
...
void CIEC61850CfgMngr::InitIEC61850Para()
{
	...
	if(m_nUseSmv92 != SMVTYPE_92)
	{
		m_p6044MngrBase->m_strSendProtocolID = m_oIecCfgDatasMngr.GetSmvMngr()->m_strFT3_ProtocolID;
		m_p6044MngrBase->m_nSendCRCType = m_oIecCfgDatasMngr.GetSmvMngr()->m_nFT3CRCType;
	}
	...
	if(m_nUseSmv92 != SMVTYPE_92)
	{
		m_p6044MngrBase->m_strRecvProtocolID = m_oIecCfgDatasMngr.GetSmvInMngr()->m_strFT3_ProtocolID;
		m_p6044MngrBase->m_nRecvCRCType = m_oIecCfgDatasMngr.GetSmvInMngr()->m_nFT3CRCType;
	}
	...
	CLogPrint::LogFormatString(XLOGLEVEL_DEBUG, _T("SMVCnt:%d, GOOSECnt:%d, FT3Cnt:%d"),
...
void CIEC61850CfgMngr::AfterInitIEC61850()
{
	...
	if(m_nUseSmv92 == SMVTYPE_92)
	{
		for(int index = 0; index < m_o92Mngr.data_92.count(); index++)
		{
			...
			if((nBoard < 0) || (nBoard >= Digital_MODULE_MAX))
			{
	...
	else
	{
		...
		for(int index = 0; index < n6044Count; index++)
		{
			...
			for(int i = 0; i < nByteCount && i < 4; i++)
			{
				ba.append((char)0);
			}
	...
	else
	{
		for(int i = 0; i < m_pIECConfigFt3->mdCnt; i++)
		{
			if(m_pIECConfigFt3->item[i].nSTSendEndIdx >= 0
					&& m_pIECConfigFt3->item[i].nSTSendBeginIdx >= 0)
			{
				//没有配置FT3发送口的模块不发送0x1003报文
				//例如PNS330的m_pIECConfigFt3配置了2个模块，
				//一个纯发送，一个纯接收，发两次0x1003可能会覆盖正确的信息
			}
			else
			{
				continue;
			...
			if(m_p6044MngrBase->m_nSendCRCType == 0
					|| m_p6044MngrBase->m_nSendCRCType == 1
					|| m_p6044MngrBase->m_nSendCRCType == 2)
			{
				confFt3->baud |= m_p6044MngrBase->m_nSendCRCType<<15;
			}

			if(m_p6044MngrBase->m_nSendSampleLen == 1
					|| m_p6044MngrBase->m_nSendSampleLen == 2
					|| m_p6044MngrBase->m_nSendSampleLen == 3)
			{
				// confFt3->baud |= m_p6044MngrBase->m_nSendSampleLen<<18;
			}
...
BOOL CIEC61850CfgMngr::DealSmvAbnPacket(StructComAbn *pSvItem, tmt_StateAbnormalSMV oTmtAbnPara, tmt_sv_92 &oTMtSmvAbn, unsigned int nQuality[50])
{
	...
	int nPort = oTMtSmvAbn.port;
	int nBoard = nPort / 8;
	int nSvIndex = -1;
	if(nBoard >= Digital_MODULE_MAX)
	{
		return FALSE;
	}
	...
	oTMtSmvAbn.m_FactorList.clear();

	for(int i = 0; i < oTMtSmvAbn.m_ChanValueList.size(); i++)
	{
		strLowQuality = QString("%1").arg(nQuality[i], 4, 16, QChar('0'));
		oTMtSmvAbn.m_FactorList.append(strHighQuality + strLowQuality);
	}

	if(oTmtAbnPara.m_nAbnormalType == 6)
	{
		oTMtSmvAbn.m_DelayTimeValue = QString::number(oTmtAbnPara.m_nDelayChangeValue);
	}

	m_o92Mngr.Create92ByteArray(oTMtSmvAbn, ba);
...
BOOL CIEC61850CfgMngr::DealGseAbnConfig(StructComAbn *pGSItem, tmt_StatePara  *pTmt_paraState, tmt_StatePara  *pTmt_paraPreState)
{
	...
	tmt_StateAbnormalGOOSE *pPreTmtAbnPara = NULL;
	if(pTmt_paraPreState != NULL)
	{
		pPreTmtAbnPara = &pTmt_paraPreState->m_oAbnormalGOOSE;
	}
	unsigned int nGroup = oTmtAbnPara.m_nGroupIndex;

	if(nGroup < m_oGooseMngr.data_gseout.size() && nGroup >= 0)
	{
	}
	else
	{
	...
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_MacSrc = QString(pPreTmtAbnPara->m_strSrcMac).replace("-", "").trimmed();
			oTMt.m_MacSrc = oTMt.m_MacSrc.rightJustified(12, QChar('0'));
		}
	}
	...
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_MacDesc = QString(pPreTmtAbnPara->m_strDestMac).replace("-", "").trimmed();
			oTMt.m_MacDesc = oTMt.m_MacDesc.rightJustified(12, QChar('0'));
		}
	}
	...
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_GcRef = QString(pPreTmtAbnPara->m_strGocbRef);
		}
	}
	...
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_GoID = QString(pPreTmtAbnPara->m_strGoID);
		}
	}
	...
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_DataSetRef = QString(pPreTmtAbnPara->m_strDataSet);
		}
	}
	...
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_AppID = QString::number(pPreTmtAbnPara->m_nAppID, 16);
		}
	}
	...
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_NdsCom = QString::number(pPreTmtAbnPara->m_nNdsCom);
		}
	}
	...
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_TestMode = QString::number(pPreTmtAbnPara->m_nTestFlag);
		}
	}

	if(oTmtAbnPara.m_bStNumSet_Sel)
	{
		oTMt.m_StNum = QString("%1").arg(oTmtAbnPara.m_nStNum, 8, 16, QChar('0'));
	}
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_StNum = QString("%1").arg(pPreTmtAbnPara->m_nStNum, 8, 16, QChar('0'));
		}
	}

	if(oTmtAbnPara.m_bSqNumSet_Sel)
	{
		oTMt.m_SqNum = QString("%1").arg(oTmtAbnPara.m_nSqNum, 8, 16, QChar('0'));
	}
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_SqNum = QString("%1").arg(pPreTmtAbnPara->m_nSqNum, 8, 16, QChar('0'));
		}
	}
	...
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_TTL = QString::number(pPreTmtAbnPara->m_nTimeAllowToLive);
		}
	}
	...
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_VLanID = QString::number(pPreTmtAbnPara->m_nVlanID);
		}
	}
	...
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_NetType = QString("%1").arg(pPreTmtAbnPara->m_nNetType & 0xFFFF, 4, 16, QChar('0'));
		}
	}
	...
	else
	{
		if(pPreTmtAbnPara != NULL)
		{
			oTMt.m_ConfRev = QString::number(pPreTmtAbnPara->m_nConfRecv);
		}
	}
...
void CIEC61850CfgMngr::SetFT3SubMap()
{
	...
	oSubCrcs.nCrcType = m_p6044MngrBase->m_nRecvCRCType;
	oSubCrcs.nModuleCnt = m_pIECConfigFt3->mdCnt;
	for(int i = 0; i < m_pIECConfigFt3->mdCnt && i < 4; i++)
	{
		if(m_pIECConfigFt3->item[i].nSTRecvBeginIdx > -1)
		{
			oSubCrcs.ft3SubCrc[i].nModule = m_pIECConfigFt3->item[i].module;
		}
	}
