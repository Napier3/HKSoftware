...
L:
	...
	int nsize = 0;
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	nsize = read(m_nfd, (char *)pszBuf, 0x210);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
void CSttDeviceComm::tmt_to_drv_LtReplay(tmt_ReplayParas *pTmt, char *pszReplayCoef)
{
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	ioctl(m_nfd, 0x354, 0);
	write(m_nfd, (char *)&oModuleSamples, sizeof(Drv_LtReplayModuleSamples));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	if(oModuleChMaps.ChMapCount > 0)
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		ioctl(m_nfd, 0x354, 1);
		write(m_nfd, (char *)&oModuleChMaps, sizeof(Drv_LtReplayModuleChMaps));
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
...
void CSttDeviceComm::SetFT3SubCRC(char *pszBuff)
{
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	ioctl(m_nfd, 0x312, 0);
	write(m_nfd, pszBuff, sizeof(Ft3SubCrcs));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
