...
long CPpSttIotDevice::X_SendIotCmd(CSttIotCmd &oIotCmd)
{
	...
	long nRet = m_pSttIotClient->SendIotCmd(oIotCmd);
	CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("-----for debug------>CPpSttIotDevice--->SendIotCmd  id=%s  len=%d"), oIotCmd.m_strID.GetString(), nRet);
