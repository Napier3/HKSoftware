...
void CComtradeBinaryData::InsertNormalCyclePoints(long nInsertPoints, long nDataFileType, int nInsertLocation)
{
...
// 		CLogPrint::LogFormatString(XLOGLEVEL_ERROR,_T("当前通道数据缓存长度为0,无法进行插值操作"));
	...
	{
		memcpy(pBuffer, pOldBuffer, nBufferSize * sizeof(unsigned short));
	}
	...
	{
		for (int nIndex = nInsertPoints; nIndex < nAfterBufferSize; nIndex++)//设置在正常波形前需要添加的点值
		{
			m_pnAttachBuffer[nIndex] = m_pnAttachBuffer[nInsertPoints];
		}
	}
