...
{
	SetMinMaxVal(dXMin, dXMax, 0);
}

//lijunqing 2024-6-20
void CRulerAxis::LogicPointToDevicePoint(double *pfBuff, CPoint *pPointBuf, long nDatasCount, long nAxisIndex)
{
	int k=0;
	double dVal = 0;
	...
	if(m_nRulerType == Ruler_Type_Horz)
	{
		...
		{
			dVal = pfBuff[k] * m_dOneHorGapValue + m_nCenterLogInDv;
	...
	else
	{
		...
		{
			dVal = - pfBuff[k] * m_dOneHorGapValue + m_nCenterLogInDv;
			pPointBuf[k].y = FloatValToInt(dVal);
		}
	}
}
...
// 	double f=0;
// 	
// 	if (Ruler_Type_Vert == m_nRulerType)
// 	{
