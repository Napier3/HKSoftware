...
CBinRcdFrame* C61850ParseFrameList::GetBinRcdFrame()
{
    if (m_pBinRcdFrame != NULL)
	{
        return m_pBinRcdFrame;
	}

    m_pBinRcdFrame = new CBinRcdFrame();
    AddNewChild(m_pBinRcdFrame);
    return m_pBinRcdFrame;
}
...
CFrameBase* C61850ParseFrameList::Parse_Binary(PEPBUFFERPOS pBufferPos)
{
	BYTE *pBuffer = Ep_GetBufferCurr(pBufferPos);
	PEPCAPBUFFERITEM pItemData = (PEPCAPBUFFERITEM)pBufferPos->pItemData;
	memset(pItemData->addr_61850.srcMac, 0, 6);
	...
	pItemData->addr_61850.APPID = 0;
	pBufferPos->pDatas->APPID = 0;
	pItemData->nProtocol = PROTOCOL_61850_TYPE_BINARY;
    GetBinRcdFrame();
    pBufferPos->pDatas->nParseState = m_pBinRcdFrame->ParseOwn(pBufferPos);
    return m_pBinRcdFrame;
}
