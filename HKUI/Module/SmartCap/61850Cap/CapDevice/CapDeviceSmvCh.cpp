...
{
	//2024-9-4 wuxinyi 修改乱码
	if (m_nChType == CAPDEVICE_CHTYPE_T)
	{
		m_strName = /*_T("额定延时")*/g_sLangTxt_Iec_RatedDelay.GetString();
		return;
	}
	CString strRelay = g_sLangTxt_Iec_Relay.GetString();/*_T("保护")*/
	CString strMeasure = g_sLangTxt_Iec_Measure.GetString();/*_T("测量")*/

	if (m_strName.Find(strRelay) != -1)
	{
		m_strName = strRelay;
	}
	else if (m_strName.Find(strMeasure) != -1)
	{
		m_strName = strMeasure;
	}
	else
	{
		m_strName = _T("");
	...
	{
		strChType = g_sLangTxt_Iec_Current.GetString();/*_T("电流")*/
		m_strName += strChType;
	}
	else if (m_nChType == CAPDEVICE_CHTYPE_U)
	{
		strChType = g_sLangTxt_Iec_Voltage.GetString();/*_T("电压")*/
		m_strName += strChType;
	} 
	else
	{
	...
	CString strGnd = g_sLangTxt_Iec_Gnd.GetString();/*_T("相")*/
	CString strZeroSeq = g_sLangTxt_Iec_ZeroSeq.GetString();/*_T("零序")*/
