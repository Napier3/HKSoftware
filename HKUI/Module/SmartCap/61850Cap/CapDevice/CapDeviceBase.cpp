...
{
	//2024-10-24 wuxinyi 修改探测多组9-2或goose控制块，输入的id相同，无法创建多个group导致只能探测到一个控制块
	//CDataGroup *pNew = pSttParas->AddNewGroup("Subscribe", "Subscribe", "Subscribe");

	CDataGroup *pNew = new CDataGroup();
	pNew->m_strID = _T("Subscribe");
	pNew->m_strName = _T("Subscribe");
	pNew->m_strDataType = _T("Subscribe");
	pSttParas->AddNewChild(pNew);
