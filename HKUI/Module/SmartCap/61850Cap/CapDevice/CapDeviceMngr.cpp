...
CCapDeviceBase* CCapDeviceMngr::AddCapDevice(PEPCAPBUFFERITEM pItemData)
{
	...
	case PROTOCOL_61850_TYPE_60448:
		...
		pNew->Iinit(pItemData);
		break;
	case PROTOCOL_61850_TYPE_BINARY:
		pNew = new CCapDeviceRecord();
		AddNewChild(pNew);
		pNew->Iinit(pItemData);
	default:
		break;
...
// 			pNew->m_strPkgDetect.GetString(),strPkgDetect.GetString(),pNew->m_nFiberIndex,nPort);
	...
	else if (strType == XPARA_ID_TYPE_BINARY)
	{
		CCapDeviceRecord *pCapDeviceBinary = new CCapDeviceRecord();
		pNew = pCapDeviceBinary;
		pNew->m_nType = PROTOCOL_61850_TYPE_BINARY;
	}
