...
int GetTimeGap_EpcapBufferItem(EPCAPBUFFERITEM *pCurrItem,EPCAPBUFFERITEM *pPrevItem,BOOL bUSec)
{
	...
	int nSecondGap = tmCurr.tv_sec - tmPrev.tv_sec;
	int nUsec = tmCurr.tv_usec - tmPrev.tv_usec;
...
unsigned long GetTimeGap_EpcapBufferItem(__int64 u64_CurrTime,__int64 u64_PrevTime,BOOL bUSec)
{
	struct timeval tmCurr,tmPrev;
...
bool stt_save_bin_record_file_text(const CString &strSrcAt02dFile, const char *pszSaveTxtFile)
{
	CFile oAt02dFile;
	...
	// 打开目标文本文件进行写入
	FILE *pTxtFile = fopen(pszSaveTxtFile, "w");
	...
	// 假设 g_bytePcapHead_ns 是预定义的二进制头部数据
	// 将头部数据以文本形式写入目标文件
//	fprintf(pTxtFile, "预定义头部数据:\n");
//	for (size_t i = 0; i < 24; ++i)
//	{
//		fprintf(pTxtFile, "%02X ", g_bytePcapHead_ns[i]);
//	}
//	fprintf(pTxtFile, "\n");

	BYTE pszReadDatas[CAP_RCD_BUFFER_MAX_LEN * 2];
	size_t szRead = 0;
	DWORD dwHeadFrm = 0, dwAt02dPkgLenth = 0, dwTimeValue = 0, dwChValue = 0;
	DWORD dwProtoType = 0, dwChIndex = 0, dwModulePos = 0, dwResourceType = 0, dwResourceIndex = 0;
	long nCurrParseLen = 0, nCurrParseLen_Head = 0, nValidLenth = 0; // 上一次解析的长度
	szRead = oAt02dFile.Read(pszReadDatas + nCurrParseLen_Head, CAP_RCD_BUFFER_MAX_LEN * sizeof(BYTE));

	while (szRead > 0)
	{
		nValidLenth = nCurrParseLen_Head + szRead; // 当前有效的长度
		nCurrParseLen = 0;
		dwHeadFrm = Global_BufferGetDWORD(pszReadDatas);
		...
		if (dwHeadFrm != STT_AT02_FIXED_HEAD)
		{
			fprintf(stderr, "错误: 当前读取长度 (%u) 未找到帧头。源文件 (%s) 可能有问题。\n", szRead, strSrcAt02dFile.GetString());
			break;
		}
		...
		// 处理数据包
		while (((nCurrParseLen_Head + dwAt02dPkgLenth + 4) <= nValidLenth) && (dwAt02dPkgLenth < nValidLenth))
		{
			nCurrParseLen += 4; // 跳过数据包头部
			dwTimeValue = Global_BufferGetDWORD(pszReadDatas + nCurrParseLen);
//			fprintf(pTxtFile, "时间戳: %u\n", dwTimeValue);
			nCurrParseLen += 4;
			dwTimeValue = Global_BufferGetDWORD(pszReadDatas + nCurrParseLen);
//			fprintf(pTxtFile, "时间戳纳秒: %u\n", dwTimeValue);
			nCurrParseLen += 4;
			dwAt02dPkgLenth -= 24; // 跳过自定义数据结构部分

//			fprintf(pTxtFile, "报文类型: ");
			for (size_t i = 0; i < 2; ++i)
			{
//				fprintf(pTxtFile, "%02X ", pszReadDatas[nCurrParseLen + i]);
			}
			nCurrParseLen += 4;//跳过中间保留的2字节
//			fprintf(pTxtFile, "硬件采样通道序号: ");
//			fprintf(pTxtFile, "%02X ", pszReadDatas[nCurrParseLen]);
			nCurrParseLen++;//序号一个字节
//			fprintf(pTxtFile, "模块位置: ");
//			fprintf(pTxtFile, "%02X ", pszReadDatas[nCurrParseLen]);
			nCurrParseLen++;//位置一个字节
			
//			fprintf(pTxtFile, "\n");
			nCurrParseLen += 6;//跳过保留6字节
			dwChValue = Global_BufferGetDWORD(pszReadDatas + nCurrParseLen);
			long nChIndex = 0;
			float fValue = 0;
			while (dwChValue != STT_AT02_FIXED_HEAD)
			{
				nChIndex++;
//				fprintf(pTxtFile, "%u\t", dwChValue);
//				fprintf(pTxtFile, "通道%d值: %u\n", nChIndex, dwChValue);
...
//				fValue = fValue / 54.4105;
		...
		if (nCurrParseLen_Head == 0)
		{
			fprintf(stderr, "错误: 当前源文件 (%s) 帧长度 (%ld) 超出范围。\n", strSrcAt02dFile.GetString(), dwAt02dPkgLenth);
			break;
		}
