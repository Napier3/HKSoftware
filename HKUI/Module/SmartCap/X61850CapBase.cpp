...
void CX61850CapBase::UpdateCapDevicesRecordSmvChRates()
{
	...
	while (pos_dev != NULL)
	{
		...
		//2024-10-12 zhouyangyong 开入量录波也更新变比（这里的实际作用是放大初始值，录波计算的初始值是long型，取不到小数点之后的数，之后也会再乘回去）
		if ((pCapDeviceBase->GetClassID() == CPDCLASSID_DEVICE92 || pCapDeviceBase->GetClassID() == CPDCLASSID_DEVICERECORD) && (pCapDeviceBase->m_bSelect))
		{
...
void SetErrorStatisValues_ADMU(CDvmValue *pDvmValue,float fActualValue,double fCurrValue,long nErrorType,long nAccurLevel,const CString &strChDataType)
{
	...
	if (fabs(fCurrValue)<fabs(dMinValue))
	{
	...
	CDvmValue *pAverageValue = dvm_FindAddByID(pDvmValue,strTmpID,pDvmValue->m_strValue);
...
// 	}
	...
	double dMaxActualValue = fActualValue;
...
void CX61850CapBase::UpdateDvmDeviceDatas_Analysis_HarmBase(CDvmValue *pDvmValue,float fActualValue,float fOutputValue,float fErrorValue)
{
	if (pDvmValue == NULL)
	{
		return;
	...
	//误差
	//double dErrorValue = fabs(1 - fActualValue/fOutputValue) * 100; 
//	double dErrorValue = fErrorValue;
//	strTmpID = pDvmValue->m_strID + _T("$ErrorValue");
//	CDvmValue *pErrorValue = dvm_FindAddByID(pDvmValue,strTmpID,pDvmValue->m_strValue);
//	dErrorValue = CString_To_double(pErrorValue->m_strValue);
// 	if (fErrorValue > dErrorValue)
// 	{
// 		dErrorValue = fErrorValue;
// 		pErrorValue->m_strValue.Format(_T("%3f"),dErrorValue);
// 	}
	...
	while (pos != NULL)
	{
		...
		while(posIn != NULL)
		{
			...
			else if(pDvmValue->m_strID.Find(STT_RcdInit_DL_RatError) > 0)//比差
			{
				...
				if (m_bIsFT3ADMU)
				{
					SetErrorStatisValues_ADMU(pDvmValue,fMag,fValue,MUAccuracy_ErrorType_RatError ,pDstData->m_nIndex,pDstData->m_strDataType);
				}
				else
				{
...
//当前通道的第一个点的实际相位值 - 过零点通道时刻过后到当前第一个点的时刻的相对相位 == 当前通道在参考通道过零点时刻的相位
	...
	{
		//更新角差
		UpdatePhaseDeltaValue(dAngDeltaValue,  nChnelCnt);
...
// 更新角差的值 2020-9-17 mayangming add 
void CX61850CapBase::UpdatePhaseDeltaValue(double * pValue,  int nChnelCnt)
{
	if (m_pDataset_MUTestCB == NULL)
		return;
	...
	while (pos != NULL)
	{
		...
		while(posIn != NULL)
		{
			...
			if(pDvmValue->m_strID.Find(STT_RcdInit_DL_AngError/*"$AngError"*/) > 0)
			{
				if (m_bIsFT3ADMU)
				{
					SetErrorStatisValues_ADMU(pDvmValue,fPhase,dCompValue[nChnelIndex],MUAccuracy_ErrorType_AngError ,pDstData->m_nIndex,pDstData->m_strDataType);
				}
				else
				{
...
// 		strChDesc = _T("通道");
		pCh->m_strValue = pSrcData->m_strName;
		//2024-9-3 wuxinyi 修改乱码
		//pCh->m_strValue.AppendFormat(_T("[通道%ld]"),nIndex+1);
		pCh->m_strValue += _T("[");
		pCh->m_strValue += g_sLangTxt_Channel2;
		pCh->m_strValue.AppendFormat("%d", nIndex + 1);
		pCh->m_strValue += _T("]");
	} 
	else
	{
...
void CX61850CapBase::StateValueChangedRlt_AddDataset()
{
	...
	pLogicDevice->AddNewChild(pDataset);
	CDvmData *pData = pDataset->AddNewData(_T("开出变位次数"),_T("BoutValueChangedCount"),_T("long"),_T("10"));
	pData->m_nChange = 1;
	pData = pDataset->AddNewData(_T("开入变位次数"),_T("StateValueChangedCount"),_T("long"),_T("10"));
	pData->m_nChange = 1;
	pData = pDataset->AddNewData(_T("结论"),_T("StateValueChangedResult"),_T("string"),_T("合格"));
	pData->m_nChange = 1;
