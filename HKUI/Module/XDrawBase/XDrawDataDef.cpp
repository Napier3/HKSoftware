...
bool CXDrawDataBase::IsPointIn(double x, double y)
{
	return false;
}
...
CXDrawDataBase* CXDrawDatas::PickShortestPoint(double x, double y)
{
	CXDrawData_Point *p = NULL;
	POS pos = GetHeadPosition();
	CXDrawData_Point *pSel = (CXDrawData_Point*)GetNext(pos);
	double dLenMin = xdraw_CalLengthPoints(x, y, pSel->m_fX, pSel->m_fY);
	double dLen = 0;

	while (pos != NULL)
	{
		p = (CXDrawData_Point *)GetNext(pos);
		dLen = xdraw_CalLengthPoints(x, y, p->m_fX, p->m_fY);
...
//////////////////////////////////////////////////////////////////////////
//CXDrawData_Line
bool CXDrawData_LineBase::IsPointIn(double x, double y)
{
	double d = m_fK * x + m_fB;

	return math_IsEqual(d, y);
}

void CXDrawData_LineBase::AngleTo360()
{
...
void CXDrawData_Line::Init_B_by_k_b()  //根据K、起点计算B
{
	if (math_IsEqual(m_fAngle, 90) || math_IsEqual(m_fAngle, 270))
	{
		m_fB = m_fXb;
...
bool CXDrawData_Line::IsPointIn(double x, double y)
{
	if (m_nType == CURVE_LINE_TYPE_LINE_SEGMENT)
	{
		return IsPointIn_Segment(x, y);
	}
	else if (m_nType == CURVE_LINE_TYPE_LINE_RAY)
	{
		return IsPointIn_Ray(x, y);
	}
	else if (m_nType == CURVE_LINE_TYPE_BEELINE)
	{
		return IsPointIn_Beeline(x, y);
	}
	else
	{
		return IsPointIn_Beeline(x, y);
...
bool CXDrawData_Line::IsPointIn_Beeline(double x, double y)
{//直线
	if (IsLineVert())
	{
		if (x != m_fXb)
...
bool CXDrawData_Line::IsPointIn_Segment(double x, double y)
{//线段
    if (math_IsEqual(m_fXb, m_fXe) )
	{
        if ( ! math_IsEqual(x , m_fXb) )
		{
			return false;
		}

		if ( (y - m_fYb) * (y - m_fYe) > 0)
		{
			return false;
		}
...
bool CXDrawData_Line::IsPointIn_Ray(double x, double y)
{//射线
	if (IsLineVert())
	{
		if (x != m_fXb)
    ...
    if (math_IsEqual(m_fAngle, 0))
    {//
        return x >= m_fXb;
    }

    if (math_IsEqual(m_fAngle, 180))
    {//
        return x <= m_fXb;
    }
...
bool CXDrawData_Line::IsPointLeft(double x, double y)
{
	double dx = x - m_fXb;
	double dy = y - m_fYb;
	CComplexNumber c1, c2;
	c1.SetValue(dx, dy);
	c2.SetValueAngle(1, m_fAngle);
	double dAngle = c2.CalAngle(c1);

	if (-180 < dAngle && dAngle < 0)
	{
		return false;
	}
	else
...
bool CXDrawData_Line::IsPointRight(double x, double y)
{
	return !IsPointLeft(x, y);
}

void CXDrawData_Line::Rotate(double dAngle)
{
	m_fAngle += dAngle;
	Init_K_by_Angle();
}
...
void CXDrawData_Circle::CalRadius(double x, double y)
{
	m_fXRadius = sqrt( (x-m_fXCenter) * (x-m_fXCenter) + (y-m_fYCenter) * (y-m_fYCenter) );
	m_fYRadius = m_fXRadius;
}

bool CXDrawData_Circle::IsPointIn(double x, double y)  //点是否在绘图对象上  2024-9-25 lijunqing
{
	double d = xdraw_CalLengthPoints(x, y, m_fXCenter, m_fYCenter);

	return math_IsEqual(d, m_fXRadius);
}
...
bool CXDrawData_Arc::IsPointIn(double x, double y)  //点是否在绘图对象上  2024-9-25 lijunqing
{
	if (! CXDrawData_Circle::IsPointIn(x, y))
	{
		return false;
	}

	return IsPointOnArc(x, y);
}
...
//////////////////////////////////////////////////////////////////////////
//
long xdraw_SolveEquation(CXDrawDataBase *pDrawData1, CXDrawDataBase *pDrawData2, CXDrawDatas &listIntersect, bool bJdgDir)
{
	...
	case XDRAW_DATA_LINE:
		...
		{
		case XDRAW_DATA_LINE:
			xdraw_SolveEquation((CXDrawData_Line*)pDrawData1, (CXDrawData_Line*)pDrawData2, listIntersect, bJdgDir);
			break;
		case XDRAW_DATA_ARC:
			xdraw_SolveEquation((CXDrawData_Line*)pDrawData1, (CXDrawData_Arc*)pDrawData2, listIntersect, bJdgDir);
			break;
		case XDRAW_DATA_CIRCLE:
			xdraw_SolveEquation((CXDrawData_Line*)pDrawData1, (CXDrawData_Circle*)pDrawData2, listIntersect, bJdgDir);
			break;
		default:
			break;
		}
	...
	case XDRAW_DATA_ARC:
		...
		{
		case XDRAW_DATA_LINE:
			xdraw_SolveEquation((CXDrawData_Arc*)pDrawData1, (CXDrawData_Line*)pDrawData2, listIntersect, TRUE, bJdgDir);
			break;
		case XDRAW_DATA_ARC:
			xdraw_SolveEquation((CXDrawData_Arc*)pDrawData1, (CXDrawData_Arc*)pDrawData2, listIntersect, bJdgDir);
			break;
		case XDRAW_DATA_CIRCLE:
			xdraw_SolveEquation((CXDrawData_Arc*)pDrawData1, (CXDrawData_Circle*)pDrawData2, listIntersect, bJdgDir);
			break;
		default:
			break;
		}
	...
	case XDRAW_DATA_CIRCLE:
		...
		{
		case XDRAW_DATA_LINE:
			xdraw_SolveEquation((CXDrawData_Circle*)pDrawData1, (CXDrawData_Line*)pDrawData2, listIntersect, bJdgDir);
			break;
		case XDRAW_DATA_ARC:
			xdraw_SolveEquation((CXDrawData_Circle*)pDrawData1, (CXDrawData_Arc*)pDrawData2, listIntersect, bJdgDir);
			break;
		case XDRAW_DATA_CIRCLE:
			xdraw_SolveEquation((CXDrawData_Circle*)pDrawData1, (CXDrawData_Circle*)pDrawData2, listIntersect, bJdgDir);
			break;
		default:
			break;
		}
	...
	if (listIntersect.GetCount() == 0 && listIntersect.m_bAddPtWhenHasNone)
	{
...
long xdraw_SolveEquation(CXDrawData_Circle *pCircle, CXDrawData_Line *pLine, CXDrawDatas &listIntersect, bool bJdgDir)
{
	int iIntersetNum=0;
	double fX0,fY0;	//圆心坐标
...
long xdraw_SolveEquation(CXDrawData_Line *pLine, CXDrawData_Circle *pCircle, CXDrawDatas &listIntersect, bool bJdgDir)
{
	return xdraw_SolveEquation(pCircle, pLine, listIntersect, bJdgDir);
}

long xdraw_SolveEquation(CXDrawData_Arc *pArc, CXDrawData_Line *pLine, CXDrawDatas &listIntersect, bool bJdgDir)
{
	return xdraw_SolveEquation((CXDrawData_Circle*)pArc, pLine, listIntersect, bJdgDir);
}

long xdraw_SolveEquation(CXDrawData_Arc *pArc, CXDrawData_Line *pLine, CXDrawDatas &listIntersect, BOOL bIner, bool bJdgDir)
{
	if (bIner)
	{
		CXDrawDatas listTemp;
		xdraw_SolveEquation(pArc, pLine, listTemp, bJdgDir);//解方程,获取圆弧与直线的交点
	...
	{
		xdraw_SolveEquation(pArc, pLine, listIntersect, bJdgDir);
	}

	return listIntersect.GetCount();
}

long xdraw_SolveEquation(CXDrawData_Line *pLine, CXDrawData_Arc *pArc, CXDrawDatas &listIntersect, bool bJdgDir)
{
...
long xdraw_SolveEquation(CXDrawData_Line *pLine1, CXDrawData_Line*pLine2,CXDrawDatas &listIntersect, bool bJdgDir)
{
	double fx,fy;
	int nNum = 0;
...
long xdraw_SolveEquation(CXDrawData_Circle &c1,CXDrawData_Circle &c2, CXDrawDatas &listIntersect, bool bJdgDir)
{
	long nCpunt = 0;
...
long xdraw_SolveEquation(CXDrawData_Circle *pCircle1,CXDrawData_Circle *pCircle2, CXDrawDatas &listIntersect, bool bJdgDir)
{
	return xdraw_SolveEquation(*pCircle1, *pCircle2, listIntersect, bJdgDir);
}

//2024-9-26 lijunqing 求取Line与曲线的交点
long xdraw_SolveIntersect(CXDrawDatas &listSrc, CXDrawData_Line *pLine, CXDrawDatas &listIntersect)
{
	POS pos = listSrc.GetHeadPosition();
	CXDrawDataBase *p = NULL;
	CXDrawDatas listTemp;
	listTemp.m_bAddPtWhenHasNone = false;

	while (pos != NULL)
	{
		p = (CXDrawDataBase *)listSrc.GetNext(pos);

		if (p->GetDrawDataType() == XDRAW_DATAS)
		{
			CXDrawDatas *pDatas = (CXDrawDatas*)p;
			xdraw_SolveIntersect(*pDatas, pLine, listIntersect);
		}
		else
		{
			listTemp.DeleteAll();
			xdraw_SolveEquation(pLine, p, listTemp, true);
			
			POS pos2 = listTemp.GetHeadPosition();
			CXDrawData_Point *point = NULL;

			while (pos2 != NULL)
			{
				point = (CXDrawData_Point*)listTemp.GetNext(pos2);

				if (p->IsPointIn(point->m_fX, point->m_fY) && pLine->IsPositiveDirPoint(point))
				{//在图源对象的区域中，并且在正方向
					listIntersect.AddTail(point);
					listTemp.Remove(point);
				}
			}
		}
	}
