...
CDataGroup* CDataGroup::AddNewGroup(const CString &strName, const CString &strID, const CString &strDataType, BOOL bInitNameAndId)
{
	...
	if (bHasCreateNew)//zhouhj 2024.7.27 只有才创建新对象时才添加到子节点
	{
...
void CDataGroup::InitDataValues(CDataGroup *pSrc, BOOL bCloneWhenNotFind, BOOL bUseFormatFirst)
{
	if (pSrc == NULL)
	{
		return;
	...
	while (pos != NULL)
	{
		...
		if (nClassID != pFind->GetClassID())
		{
			CLogPrint::LogFormatString(XLOGLEVEL_INFOR, _T("参数组参数更新错误，检测到参数【%s】，类型不一致"), p->m_strID.GetString());
			continue;
		}

		if (nClassID == DTMCLASSID_CDATAGROUP)
		{
			((CDataGroup*)pFind)->InitDataValues((CDataGroup*)p, bCloneWhenNotFind, bUseFormatFirst);
		}
		else if (nClassID == DVMCLASSID_CDVMDATA)
		{
			CDvmData *pSrcData = (CDvmData *)p;
			CDvmData *pDstData = (CDvmData *)pFind;

			//2024-10-16 取消关联定制，会将format属性清空。所以当属性为空时，也需要更新
			if (bUseFormatFirst/* && pSrcData->m_strFormat.GetLength() > 0*/)
			{
				if (pSrcData->m_strFormat.GetLength() > 0)
				{
					pDstData->m_strValue = pSrcData->m_strFormat;
				}
				else
				{
				...
				pDstData->m_strFormat = pSrcData->m_strFormat;
			}
			else
			{
				pDstData->m_strValue = pSrcData->m_strValue;
		...
		{
			CShortData *pSrcData = (CShortData *)p;
			CShortData *pDstData = (CShortData *)pFind;
