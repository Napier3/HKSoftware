...
//////////////////////////////////////////////////////////////////////////
void CDvmMapTool::CmpDvmFormat(CExBaseList *pMap)//20231031 luozibing 遍历查找格式不规范的CDvmDataset
{
	POS pos = pMap->GetHeadPosition();
	while(pos != NULL)
	{
		CExBaseObject *tmp = pMap->GetNext(pos);
		if(tmp->GetClassID() == DVMCLASSID_CDVMDATASET)
		{
			CDvmDataset *TmpData = CanonicalFormat((CDvmDataset*)tmp);
			if(TmpData != NULL)
			{
				pMap->AddTail(TmpData);
				TmpData->SetParent(pMap);
				pMap->Remove(tmp);
			}
		}
		else
		{	
			CmpDvmFormat((CExBaseList *)tmp);
		}
	}
}

CDvmDataset * CDvmMapTool::CanonicalFormat(CDvmDataset *pMap)//20231031 luozibing 改变格式
{
	BOOL ShowDvm = FALSE;
	CDvmDataset * tmpObj;
	CDvmDataset * pDvmOldMap;
	CDvmData* ChildTmp;

	tmpObj = (CDvmDataset *)pMap->Clone();
	tmpObj->DeleteAll();
	pDvmOldMap = pMap;

	POS pos = pDvmOldMap->GetHeadPosition();
	while(pos != NULL)
	{
		ChildTmp = (CDvmData*)pDvmOldMap->GetNext(pos);
		if(ChildTmp->GetHead() == NULL)
		{
			ShowDvm = TRUE;
			if(tmpObj == NULL)
			{
				CDvmData *ChildObj = (CDvmData *)ChildTmp->Clone();
				tmpObj->AddNewChild(ChildObj);
			}
			CExBaseList* ValTmp = (CExBaseList*)tmpObj->FindByName(ChildTmp->m_strName);
			if(ValTmp == NULL)
			{
				CDvmData *ChildObj = (CDvmData *)ChildTmp->Clone();
				tmpObj->AddNewChild(ChildObj);

				CDvmValue *ValObj = new CDvmValue();
				ValObj->m_strName = ChildTmp->m_strName;
				ValObj->m_strID =  ChildTmp->m_strID;
				ChildObj->AddNewChild(ValObj);
			}
			else
			{
				CDvmValue *ValObj = new CDvmValue();
				ValObj->m_strName = ChildTmp->m_strName;
				ValObj->m_strID =  ChildTmp->m_strID;
				ValTmp->AddNewChild(ValObj);
			}
		}
	}
	if(ShowDvm)
	{
		m_OldDest.AddTail(pDvmOldMap);
		return tmpObj;
	}
	else
	{
