...
void CExBaseList::InitNameAndIDForPaste(CString &strChildObjName, CString &strChildObjID)
{
	if ((FindByName(strChildObjName) == NULL) && (FindByID(strChildObjID) == NULL))
	{
		return;
	}

	CString strName = strChildObjName;
	CString strID = strChildObjID;
...
//nAddPos：添加位置。0=添加为子项目；1=当前节点之前；2=当前节点之后
void CExBaseList::AddChildByPos(CExBaseObject *pObj, long nAddPos, BOOL bSetParent)
{
	switch (nAddPos)
	{
	case 0:
	...
	case 1:
		{
			...
			POS pos = pParent->Find(this);
			pParent->InsertBefore(pos, pObj);
