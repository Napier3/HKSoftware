...
public:
	...
	void DecryptAnyLengthHex(char* _srcBytes, int nLen, unsigned int keyN);
	...
	char* GetCiphertextAnyLength_Hex();
...
private:
	...
	char szFCiphertextAnyLengthHex[8192];//任意长度密文
	char szFPlaintextAnyLength[8192];//任意长度明文字符串
	char szFPlaintextAnyLengthHex[8192];//任意长度明文字符串
...
public:
	CString Encrypt(const CString &strText);
	CString Decrypt(const CString &strText);

	long Encrypt(const CString &strText, char *pszBuffer);
    long Encrypt(char *strText, char *pszBuffer);
    long Encrypt2(char *strText, char *pszBuffer);
    long Decrypt(const CString &strText, char *pszBuffer);
    long Decrypt(char *strText, char *pszBuffer);
    long Decrypt2(char *strText, char *pszBuffer);
    long Decrypt(char *strSrcText, CString &strDest);
};
