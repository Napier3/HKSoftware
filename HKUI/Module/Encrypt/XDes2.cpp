...
CXDes2::CXDes2()
{
	...
	if (g_nEncryptKeyID == 0)
	{
		InitializeKey("#power-sky-lijq##power-sky-lijq#", 0);
	}
	else if (g_nEncryptKeyID == 1)
	{
		InitializeKey("#ponovo-lijq#$ponovo-lijq$", 0);
	}
}
...
long CXDes2::Encrypt(char *strText, char *pszBuffer)
{
    long nLen = strlen(strText);

    EncryptAnyLength(strText, nLen, 0);
...
long CXDes2::Encrypt2(char *strText, char *pszBuffer)
{
    long nLen = strlen(strText);
    ...
    char pszLen[8];
    nLen = nLen % 999;
    sprintf(pszLen, "%03d", nLen);
    strcpy(pszBuffer, pszLen);
    strcat(pszBuffer, szFCiphertextAnyLengthHex);
...
long CXDes2::Decrypt2(char *strText, char *pszBuffer)
{
    char pszLen[8];
    pszLen[0] = strText[0];
    pszLen[1] = strText[1];
    ...
    DecryptAnyLengthHex(strText+3,nLen, 0);
    strcpy(pszBuffer, szFPlaintextAnyLength);
