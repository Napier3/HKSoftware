...
void CEpExpression::identificator_var(int &poz, PNOD &nod, PNOD &nod2, PNOD &result, PNOD &pResultTemp)
{
	...
	else if (id == "TG" || id == "TAN")		// Functia sinus CString	//20240821 huangliang TAN和TG在函数vexp中相同处理，添加TAN
	{
		nod2 = factor();
		nod = _newnode();
		nod->left = nod2;
	...
	else if (id == "ATG" || id == "ATAN")		// Functia sinus CString	//20240821 huangliang ATG和ATAN在函数vexp中相同处理，添加ATAN
	{
		nod2 = factor();
		nod = _newnode();
		nod->left = nod2;
