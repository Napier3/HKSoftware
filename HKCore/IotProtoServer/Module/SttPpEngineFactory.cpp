...
CSttPpEngineFactory::CSttPpEngineFactory()
{
...
*/
CIotPxEngineDeviceBase* CSttPpEngineFactory::CreateDevice(CIotPxCommChannelInterface *pPxCommChn, CSttIotCmdDriver &oDrvIotCmd)
{
	CSttIotCmd *pIotCmd = oDrvIotCmd.SttIotCmd();
	CSttPpEngineDevice *pNew = new CSttPpEngineDevice();
	pNew->SetParent(this);
	...
	pNew->m_pPxiDevice = g_thePxiEngine->CreateDevice(strPpXmlFile, strDvmFile, strDeviceSN, pNew);
	pNew->Process_Cmd_IOT_CreateDevice(pPxCommChn, oDrvIotCmd);

	return pNew;
}

void CSttPpEngineFactory::CloseDevice(CIotPxCommChannelInterface *pPxCommChn, CSttIotCmdDriver &oDrvIotCmd)
{
	//在实际的对象中实现Close
}
