...
class CFaultCalc_Diff :
	public FaultCalc_Diff
{
public:

	CFaultCalc_Diff(void)
	{
		InitStruct();
	}
	~CFaultCalc_Diff(void)
...
public:
	...
	//结构体初始化
	void InitStruct(FaultCalc_Diff *pSrc=NULL)
	{
		int i;
	...
	//结构体复制
	void CopyStructTo(FaultCalc_Diff *pDst)
	{
		CopyStruct(this,pDst);
	}

	//结构体复制
	void CopyStruct(FaultCalc_Diff *pSrc, FaultCalc_Diff *pDst)
	{
		pDst=pSrc;
	}
