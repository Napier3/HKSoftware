...
//负序过流保护定值校验计算: 固定定电流模型, 
//计算结果: 三个状态 (故障前, 故障, 故障转换=故障)
class CFaultCalc_NegSeqCurr :
	public FaultCalc_NegSeqCurr
{
public:

	CFaultCalc_NegSeqCurr(void)
	{
		InitStruct();
	}
	~CFaultCalc_NegSeqCurr(void)
...
public:
	...
	//结构体初始化
	void InitStruct(FaultCalc_NegSeqCurr *pSrc=NULL)
	{
		int i;
	...
	//结构体复制
	void CopyStructTo(FaultCalc_NegSeqCurr *pDst)
	{
		CopyStruct(this,pDst);
	}

	//结构体复制
	void CopyStruct(FaultCalc_NegSeqCurr *pSrc, FaultCalc_NegSeqCurr *pDst)
	{
		pDst=pSrc;
	}
