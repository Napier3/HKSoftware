...
CDrawTimeCursor::CDrawTimeCursor()
{
	...
	m_nXAOld		= 200;
	m_nXBOld		= 300;
	m_crATimeCursorColor = RGB(0,0,0);
	m_crBTimeCursorColor = RGB(0,0,0);
	...
	CString str;
	str.Format(_T("%sBitmaps\\%s"), _P_GetConfigPath(), g_pszKeyZoomBmp);
...
void CDrawTimeCursor::Draw(CDC *pDC) 
{	
	if (m_nShow == 1)
	{
		DrawACursor(pDC);
...
void CDrawTimeCursor::Move(long nHor, long nVer) 
{
...
void CDrawTimeCursor::DrawACursor(CDC *pDC)
{
	...
	pDC->MoveTo(m_nXA+m_nCursorWidth,m_rcBorder.top);
	pDC->LineTo(m_nXA+m_nCursorWidth,m_rcBorder.bottom);
...
void CDrawTimeCursor::ChangeCursorPosition(long& nPosition,long nCurrentX)
{
	...
	else if (nCurrentX > (m_rcBorder.right-m_nCursorWidth) )
	{
		nPosition = m_rcBorder.right - m_nCursorWidth;
	}
	else
	{
		nPosition = nCurrentX;
	...
	case USETIMECURSORA:
		{
			...
			rcOld.right = rcOld.left + m_nCursorWidth + 8 /*m_nXAOld + WIDTHTOCENTER + 2*/ ;
			rcOld.bottom = m_rcBorder.bottom;

			rcNew.left = m_nXA - WIDTHTOCENTER - 1 ;
			rcNew.top = m_rcBorder.top;
			rcNew.right = rcNew.left + m_nCursorWidth + 8/*m_nXA + WIDTHTOCENTER + 2*/ ;
