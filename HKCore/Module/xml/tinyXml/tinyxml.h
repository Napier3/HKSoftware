...
inline void CString_to_TIXML_STRING(TIXML_STRING &strDest, const CString &_value)
{
#ifndef _PSX_IDE_QT_

#ifdef UNICODE
	const wchar_t *pwszSrc = _value.GetString();
	int nLen = wcslen(pwszSrc);
	char *pszDest = new char[nLen*2+2];
...
#else
#ifdef TIXML_USE_STL
    strDest = _value.toLocal8Bit().constData();
#else
    strDest = _value;
#endif
#endif
...
public:
	...
	virtual void GetXml( std::string &strXML, int depth ) const = 0;//zhouhj 用于保存xml文本
...
public:
    ...
    {
        CString_to_TIXML_STRING(m_value, _value);
	}				
	...
	*/
	...
	virtual void GetXml( std::string &strXML, int depth ) const
	{
		GetXml( strXML, depth, 0 );
	}
		//zhouhj 用于保存xml文本
	void GetXml( std::string &strXML, int depth , TIXML_STRING* str) const;//zhouhj 用于保存xml文本
...
public:
	...
	virtual void Print( FILE* cfile, int depth ) const;
	virtual void GetXml( std::string &strXML, int depth ) const;//zhouhj 用于保存xml文本
...
public:
	...
	virtual void GetXml( std::string &strXML, int depth, TIXML_STRING* str ) const;
	virtual void GetXml( std::string &strXML, int depth ) const {
		GetXml( strXML, depth, 0 );
	}
...
public:
	...
	char *GetFileBuf( FILE*, TiXmlEncoding encoding = TIXML_DEFAULT_ENCODING );//获取文件中的文本内容，返回指针，使用完后 需要把内存释放
	...
	*/
	...
	virtual void GetXml( std::string &strXML, int depth = 0 ) const;//zhouhj 用于保存xml文本
	// [internal use]
