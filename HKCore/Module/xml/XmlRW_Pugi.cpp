...
BOOL CXmlRWNode_Pugi::xml_SetElementOwnText(const BSTR strAttrName, CXmlRWDocBase *pXMLDoc, const CString &strAttrVal)
{
	wchar_t *pstr = strAttrVal.AllocSysString();
	m_oPugiXmlNode.text().set(pstr);
	//   ::SysFreeString(pstr);
...
long CXmlRWDoc_Pugi::GetXml(char **ppszRetBuffer, long nOffset, bool bUtf8)
{
    if (m_oPugiXmlDoc == NULL)
    {
        *ppszRetBuffer = NULL;
