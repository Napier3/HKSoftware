...
//***************************************************************************
	...
	void SpectrumAnalyze(double *pre, double *pim, double Fs, int N, int index,
		double& _Mag, double& _Ang, double& _Fre)
	{
		...
		//计算主瓣与K的偏移量
		maxModu = sqrt(pre[index]*pre[index] + pim[index]*pim[index]);
		double y1 = sqrt(pre[index-1] * pre[index-1] + pim[index-1] * pim[index-1]);
		double y2 = sqrt(pre[index+1] * pre[index+1] + pim[index+1] * pim[index+1]);
		double v;
		...
		//频率
		_Fre = (index + delta) * Fs / N;

		//幅值
        CComplexNumber oComplex(pre[index], pim[index]);
		_Mag = 4 * FFT_PI * delta * (1 - delta * delta)
                * oComplex.GetModulus()  //abs(complex<double>(pre[index], pim[index]))
                / sin(FFT_PI * delta) / N;

		//相位
        _Ang = oComplex.GetRadianAngle() //arg(complex<double>(pre[index], pim[index]))
                - delta * FFT_PI;
	}
	//***************************************************************************
	//*	函数名:	SpectrumAnalyze
	...
	//***************************************************************************
	long SpectrumAnalyze(double *pre, double *pim, double Fs, int N,
		double& _Mag, double& _Ang, double& _Fre)
	{
		double maxModu = 0.0;
		int maxN = 0;
		...
		//幅值
        CComplexNumber oComplex(pre[maxN], pim[maxN]);
		_Mag = 4 * FFT_PI * delta * (1 - delta * delta)
              * oComplex.GetModulus()  //abs(complex<double>(pre[maxN], pim[maxN]))
                / sin(FFT_PI * delta) / N;

		//相位
        _Ang = oComplex.GetRadianAngle()  //arg(complex<double>(pre[maxN], pim[maxN]))
                - delta * FFT_PI;

		return maxN;  
	}

    void SpectrumAnalyze(double *pre, double *pim, double Fs, long N,
		double *pArrMag, double *pArrAng, double *pArrFre, long nHarmCount)
	{
		double Mag, Ang, Fre;
		int nBase = SpectrumAnalyze(pre, pim, Fs, N, Mag, Ang, Fre);
		pArrMag[nBase] = Mag;
		pArrAng[nBase] = Ang;
		pArrFre[nBase] = Fre;
		...
		long nIndex = 0;
        long nCount = N / 2;
        nCount = nHarmCount > nCount ? nCount : nHarmCount;//min(nHarmCount, nCount);
		
		for (nIndex=nBase + 1; nIndex < nCount; nIndex++)
		{
			SpectrumAnalyze(pre, pim, Fs, N, nIndex, pArrMag[nIndex], pArrAng[nIndex], pArrFre[nIndex]);
		}
	}
	...
	{
        nCount = nCount >FFT_PTCOUNT ? FFT_PTCOUNT : nCount ;//min(nCount, (int)FFT_PTCOUNT);
		InitFFTData(nCount);
	...
	void CalcFFT(double *pPoints, int nCount, double Fs, 
		double *pArrMag, double *pArrAng, double *pArrFre, long nHarmCount)
	{
        nCount = nCount > FFT_PTCOUNT ? FFT_PTCOUNT : nCount;  //min(nCount,FFT_PTCOUNT);
		InitFFTData(nCount);
		double temp[FFT_PTCOUNT];
		ZeroMemory(temp,sizeof(temp));
		FFT(pPoints, temp, m_win, m_rev, nCount);
		SpectrumAnalyze(pPoints, temp, Fs, nCount, pArrMag, pArrAng, pArrFre, nHarmCount);

		long nIndex = 0;

		for (nIndex=0; nIndex<nHarmCount; nIndex++)
		{
			pArrAng[nIndex] *= 180.0 / FFT_PI;
		}
	}
public:
...
{
	theStaticFFTData.CalcFFT(pSrcPoints, nPoints, SampleRate, pArrMag, pArrAng, pArrFre, nHarmCount);

	long nIndex = 0;

	for (nIndex=0; nIndex<nHarmCount; nIndex++)
	{
		pArrMag[nIndex] /= 1.414159265;
	}
}
