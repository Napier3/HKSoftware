...
// 		CLogPrint::LogFormatString(XLOGLEVEL_ERROR,_T("当前通道数据缓存长度为0,无法进行删除操作"));
	...
	if ((nEndPos + 1) == m_nTotalPoints)//zhouhj 2025.3.15 如果删除的结束位置，刚好是波形结束位置,则只需要改变当前点的数量即可
		{
		m_nTotalPoints -= (nEndPos - nBeginPos); // 更新总点数
			return;
		}

	long nDeleteLength = nEndPos - nBeginPos;
	long nCopyLenth = m_nTotalPoints - nEndPos;

	//zhouhj 2025.3.15 删除点时,无需重新创建内存,只需要将后面的点拷贝到前面即可

	//从删除的起始位置往后开始拷贝
	if (nDataFileType == COMTRADE_DATAFILE_TYPE_ASCII && IsAnalogData())
	{
		int *pBuffer_int = (int *)pBuffer;
		memcpy(&pBuffer_int[nBeginPos], &pBuffer_int[nEndPos], nCopyLenth * sizeof(int));
	}
	else
		{
		memcpy(&pBuffer[nBeginPos], &pBuffer[nEndPos], nCopyLenth * sizeof(unsigned short));
	}
