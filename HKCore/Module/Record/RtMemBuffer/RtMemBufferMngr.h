...
public:
	...
	void Free();
	long GetRecordDataLength()//得到录波缓存数据长度
	{
...
//	BOOL m_bTestFomat ;
	...
	CMemBuffer_UShort   *m_pAttrTimeBuffer;//m_pAttrTimeBuffer记录了每一个属性数据的点数
...
public:
	...
	CMemBufferInterface* GetSpyMemBuffer(long nChannelIndex);//根据通道索引获取通道的监视缓存的管理对象
	CMemBufferInterface* GetMemBuffer(long nChannelIndex);//根据通道索引获取通道的录波缓存的管理对象
	
	//缓冲区的查找
	CExBaseObject* FindByBufferID(const CString &pszBufferID);//根据缓存ID,返回通道缓存的管理对象
	unsigned short* FindUShortByBufferID(const CString & pszBufferID);//根据缓存ID,返回通道缓存
	long GetMemIndexByBufferID(const CString & pszBufferID);//根据缓存ID获得缓存索引
	long GetAttrMemIndexByBufferID(const CString & pszBufferID);//根据缓存ID获得属性缓存索引
	long* FindLongByBufferID(const CString & pszBufferID);//根据缓存ID,返回复合通道的缓存
	float* FindFloatByBufferID(const CString & pszBufferID);//同上
	double* FindDoubleByBufferID(const CString & pszBufferID);//同上
