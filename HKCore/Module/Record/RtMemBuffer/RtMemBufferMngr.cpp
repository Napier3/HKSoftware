// RtMemBuffer.cpp: implementation of the CRtMemBufferMngr class.
//
//////////////////////////////////////////////////////////////////////
...
CExBaseObject* CRtMemBufferMngr::FindBufferByBufferID(const char *pszBufferID)
{
	return FindByBufferID(pszBufferID);
}
...
CRtMemBufferMngr::CRtMemBufferMngr()
{
	...
	m_n64SpyHeadIndex    = 0;
	m_n64CurrCalIndex    = 0;
	m_pHdMemBuffer = new CMemBuffer_UShort();
	m_pAttrTimeBuffer = new CMemBuffer_UShort();
...
CMemBufferInterface* CRtMemBufferMngr::GetMemBuffer(long nChannenIndex)
{
...
long CRtMemBufferMngr::ReadOwnXml(CXmlRWNodeBase &oNode)
{
	return 0;
}
...
void CRtMemBufferMngr::Init(long nMaxSpyBuffer, long nMaxRecordBuffer, long nMaxAttrBuffer, long nMaxHdBuffer)
{
	...
	//hd buffer
	long nTemp = nMaxHdBuffer % m_nFIFOUnitSize;
	nMaxHdBuffer = nMaxHdBuffer + m_nFIFOUnitSize - nTemp;
	m_nMaxHdBuffer = nMaxHdBuffer;
	ASSERT (nMaxHdBuffer % m_nFIFOUnitSize == 0);
...
void CRtMemBufferMngr::AllocBuffers()
{
	...
	CDebugDataOnline *pDebugData = g_theRecordApp.CreateOnlineDebugData(0, 11);
...
CMemBufferInterface* CRtMemBufferMngr::GetSpyMemBuffer(long nChannenIndex)
{
	CMemBufferInterface* pBuffer = NULL;
	CRtMemBufferChannel* pChannel = NULL;
...
long CRtMemBufferMngr::AppendHDBufferData(unsigned short *pHDData, long nLength, DWORD nSampleGap)
{
	long nPos =m_pHdMemBuffer->CopyBuffer(pHDData, nLength);
	CopyHdBufferToSpyBuffer();
	CopySpyBufferToRecordBuffer(nSampleGap);
...
void CRtMemBufferMngr::CopyHdBufferToSpyBuffer()
{
	...
	{//此次拷贝只需要拷贝一次缓冲区
		...
		//nLenCopy = nLenCopy - nLenCopy % m_nFIFOUnitSize ;     //去掉不完整的FIFO数据，得到此次需要拷贝的数据长度 //????
		nLenCopy = nLenCopy - nLenCopy % m_nFIFOUnitSize;     //得到此次需要拷贝的数据长度，通道的倍数
		CopyHdBufferToSpyBuffer(m_nCurrHdBufferCopyPos, nLenCopy + m_nCurrHdBufferCopyPos );
		m_nCurrHdBufferCopyPos += nLenCopy;
	}
	else
	{//此次拷贝需要拷贝两次缓冲区
		//第一次，当前拷贝位置到HdBuffer缓冲区末尾，因为HdBuffer缓冲区分配为m_nFIFOUnitSize的整数倍
		//所以，不需要再去掉不完整的FIFO数据，但调试的时候，增加着一个限制的判断
		nLenCopy = nHdBufferLength - m_nCurrHdBufferCopyPos;
		ASSERT (nLenCopy % m_nFIFOUnitSize == 0);  //调试的时候，增加着一个限制的判断
		CopyHdBufferToSpyBuffer(m_nCurrHdBufferCopyPos, nLenCopy + m_nCurrHdBufferCopyPos);
		
		//第二次拷贝，缓冲区头到HdBuffer填充的位置
		nLenCopy = nHdDataFillBeginPos;//总长度
		nLenCopy = nLenCopy - nLenCopy % m_nFIFOUnitSize ;
...
void CRtMemBufferMngr::CopyHdBufferToSpyBuffer(long nHdBufferBegin, long nHdBufferEnd)
{
	...
	long nPos2Begin = -1;
	long nPos2End = -1;
	long nLengthCopy = (nHdBufferEnd - nHdBufferBegin) / m_nFIFOUnitSize;

	CMemBufferInterface *pSpyMemBuffer = m_ppRtMemBufferMngr[0]->GetSpyMemBuffer();

	long nSpyBufferFillBeginPos1 = pSpyMemBuffer->GetDataFillBeginPosition();
	
	//获得拷贝数据的起始位置和终止位置,第三个参数是FLASE是由于我们只是获得地址而不是改变地址
	pSpyMemBuffer->CalCopyBuffer(nLengthCopy, 1, FALSE, nPos1Begin, nPos1End, nPos2Begin, nPos2End);
	...
	for (nChannel=0; nChannel<MAX_MR1200_CHANNELS; nChannel++)
	{
		...
		pHdTemp = pHdBuffer + nHdBufferBegin + m_pChannelFIFOIndex[nChannel];
		...
		if (m_pChannelFIFOIndex[nChannel] == 12 || m_pChannelFIFOIndex[nChannel] == 13)
		{
	...
	for (nChannel=0; nChannel<MAX_MR1200_CHANNELS; nChannel++)
	{
		...
		{
			pSpyMemBuffer->CalCopyBuffer(nLengthCopy, 1, TRUE, nPos1Begin, nPos1End, nPos2Begin, nPos2End);
		}
	}
	...
	for (nChannel=0; nChannel<MAX_MR1200_CHANNELS; nChannel++)
	{
		...
		{
			pLSpyMemBuffer->CalCopyBuffer(nLengthCopy, 1, FALSE, nPos1Begin, nPos1End, nPos2Begin, nPos2End);
			pLSpyMemBuffer->ExecScript(nPos1Begin, nPos1End, nPos2Begin, nPos2End);
			pLSpyMemBuffer->CalCopyBuffer(nLengthCopy, 1, TRUE, nPos1Begin, nPos1End, nPos2Begin, nPos2End);
		}
	}
}
...
void CRtMemBufferMngr::CopySpyBufferToRecordBuffer(long nGap)
{
	...
	CMemBufferInterface* pRecordMemBuffer = NULL;
...
//缓冲区的查找
CExBaseObject* CRtMemBufferMngr::FindByBufferID(const CString & pszBufferID)
{
...
unsigned short* CRtMemBufferMngr::FindUShortByBufferID(const CString & pszBufferID)
{
	CExBaseObject *pFind = NULL;
...
long CRtMemBufferMngr::GetMemIndexByBufferID(const CString & pszBufferID)
{
	CExBaseObject *pFind = NULL;
...
long CRtMemBufferMngr::GetAttrMemIndexByBufferID(const CString & pszBufferID)
{
	CExBaseObject *pFind = NULL;
	...
	for (nIndex=0; nIndex<MAX_MR1200_VARIABLE_ATTR_BUFFERS; nIndex++)
	{
		if (m_ppVariableAttrBuffer[nIndex] != NULL)
		{
			if (_stricmp(m_ppVariableAttrBuffer[nIndex]->m_strBufferID.GetString(), pszBufferID.GetString()) == 0)
			{
				nFind = nIndex;
				break;
			}
...
long* CRtMemBufferMngr::FindLongByBufferID(const CString & pszBufferID)
{
	CExBaseObject *pFind = NULL;
	pFind = FindByBufferID(pszBufferID);
	...
	{
		return ((CMemBuffer_Long*)pFind)->GetBuffer_long();
	}
	else
	{
		return NULL;
...
float* CRtMemBufferMngr::FindFloatByBufferID(const CString & pszBufferID)
{
	CExBaseObject *pFind = NULL;
	pFind = FindByBufferID(pszBufferID);
	...
	{
		return ((CMemBuffer_Float*)pFind)->GetBuffer_float();
	}
	else
	{
		return NULL;
...
double* CRtMemBufferMngr::FindDoubleByBufferID(const CString & pszBufferID)
{
	CExBaseObject *pFind = NULL;
	pFind = FindByBufferID(pszBufferID);
...
CRtMemBufferChannel* CRtMemBufferMngr::_AddMR1200BinaryChannel(UINT nID, BOOL bUseRecordBuf, const CString &strRecordBufID, BOOL bUseSpyBuf, const CString &strSpyBufID)
{
	...
	rcd_GetBinaryChannelID(nID, strChannelID);
