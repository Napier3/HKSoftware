...
CMemBuffer_CPoint::CMemBuffer_CPoint()
{
	...
	m_pBufferX      = NULL;
	m_pBufferY		= NULL;
	m_pBufferX_Long      = NULL;
	m_pBufferY_Long		= NULL;
	m_nCurrUseLength  = 0;
	m_dCoefY	      = 0;
	m_dOffSetY		  = 0;
	m_dCoefX	      = 0;
...
void CMemBuffer_CPoint::GetBeginAndEndPosition(long nBufferLength, long nDataLength, long nDataFillBeginPos
											   ...
											   , long &nSrcBeginPos2, long &nSrcEndPos2)
{
	...
	{//缓冲区被填满
		...
		{//从最早的数据开始，往前推nSpace的数据
			nSrcBeginPos = nDataFillBeginPos;
		}
		else
		{
		}
...
void CMemBuffer_CPoint::GetBeginAndEndPosition(CMemBuffer_UShort *pBuffer
												, long nSrcBeginPos, long nSrcLength
												, long &nSrcBeginPos1, long &nSrcEndPos1
												, long &nSrcBeginPos2, long &nSrcEndPos2)
{
	...
	GetBeginAndEndPosition(nBufferLength, nDataLength, nDataFillBeginPos
		, nSrcBeginPos, nSrcLength
		, nSrcBeginPos1, nSrcEndPos1
		, nSrcBeginPos2, nSrcEndPos2);
}


void CMemBuffer_CPoint::GetBeginAndEndPosition(CMemBuffer_Long *pBuffer
											   , long nSrcBeginPos, long nSrcLength
											   , long &nSrcBeginPos1, long &nSrcEndPos1
											   , long &nSrcBeginPos2, long &nSrcEndPos2)
{
...
void CMemBuffer_CPoint::AttachBufferX(CMemBuffer_UShort *pBufferX)
{
	m_pBufferX = pBufferX;
}

void CMemBuffer_CPoint::AttachBufferY(CMemBuffer_UShort *pBufferY)
{
	m_pBufferY = pBufferY;
}

void CMemBuffer_CPoint::AttachBuffer(CMemBuffer_UShort *pBufferX, CMemBuffer_UShort *pBufferY)
{
	m_pBufferX = pBufferX;
	m_pBufferY = pBufferY;
}
...
void CMemBuffer_CPoint::InitValueX(double fCoef, double fOffset, CMemBuffer_UShort *pBufferX, long nSrcBeginPos , long nSrcLength , long nGap)
{
	m_dCoefX = fCoef;
	m_dOffSetX = fOffset;
	...
	unsigned short *pSrcBufferX = pBufferX->GetBuffer();
	unsigned short *pSrcTemp = NULL;
	unsigned short *pSrcTempEnd = NULL;
	long nIndex = 0;
	long nOffset = 0;
...
void CMemBuffer_CPoint::InitValueY(double fCoef, double fOffset, CMemBuffer_UShort *pBufferY, long nSrcBeginPos, long nSrcLength, long nGap)
{
	m_dCoefY = fCoef;
	m_dOffSetY = fOffset;
...
void CMemBuffer_CPoint::InitValueX(double fCoef, double fOffset, CMemBuffer_Long *pBufferX, long nSrcBeginPos , long nSrcLength , long nGap)
{
	...
	CPoint *pPoint = m_pBuffer ;
	long *pSrcBufferX = pBufferX->GetBuffer_long();
	long *pSrcTemp = NULL;
...
void CMemBuffer_CPoint::InitValueY(double fCoef, double fOffset, CMemBuffer_Float *pBufferY, long nSrcBeginPos, long nSrcLength, long nGap)
{
	...
	CPoint *pPoint = m_pBuffer;
	float *pSrcBufferY = pBufferY->GetBuffer_float();
	float *pSrcTemp = NULL;
...
void CMemBuffer_CPoint::InitValueX(double fCoef, double fOffset, CMemBuffer_Float *pBufferX, long nSrcBeginPos, long nSrcLength, long nGap)
{
	...
	CPoint *pPoint = m_pBuffer;
	float *pSrcBufferX = pBufferX->GetBuffer_float();
	float *pSrcTemp = NULL;
	float *pSrcTempEnd = NULL;
	long nIndex = 0;
	long nOffset = 0;
...
void CMemBuffer_CPoint::InitValueXY(double fCoefX, double fOffsetX, CMemBuffer_UShort *pBufferX, double fCoefY, double fOffsetY, CMemBuffer_UShort *pBufferY, long nSrcBeginPos, long nSrcLength, long nGap)
{
...
void CMemBuffer_CPoint::InitValueX(double fCoef, double fOffset, long nSrcBeginPos, long nSrcLength, long nGap)
{
	...
	if (m_pBufferX_Long != NULL)
	{
		InitValueX(fCoef,fOffset,m_pBufferX_Long,nSrcBeginPos,nSrcLength,nGap);	
	}
}
...
void CMemBuffer_CPoint::InitValueY(double fCoef, double fOffset, long nSrcBeginPos, long nSrcLength, long nGap)
{
	...
	if (m_pBufferY_Long != NULL)
	{
		InitValueY(fCoef,fOffset,m_pBufferY_Long,nSrcBeginPos,nSrcLength,nGap);	
	}
}
...
void CMemBuffer_CPoint::CalValueY(double fCoef, double fOffset,CMemBuffer_UShort *pSrc,long nBuffer1Begin,long nBuffer1End,long nBuffer2Begin ,long nBuffer2End , long nGap)
{
	ASSERT (nGap > 0);
	if (nBuffer1Begin < 0)
	{//没有需要转换的缓冲区
	...
	CPoint *pPoint = m_pBuffer;
	unsigned short *pSrcBufferY = pSrc->GetBuffer();
	unsigned short *pSrcTemp = NULL;
	unsigned short *pSrcTempEnd = NULL;
	long nIndex = 0;
	long nOffset = 0;
	...
	if (nBuffer1Begin >= 0)
	{
		...
		{
			//short nVal = *pSrcTemp;
			pPoint->y = (long)( ( fCoef * (short)*pSrcTemp)+ fOffset);
...
void CMemBuffer_CPoint::CalValueY(double fCoef, double fOffset,CMemBuffer_Long *pSrc,long nBuffer1Begin,long nBuffer1End,long nBuffer2Begin ,long nBuffer2End , long nGap)
{
	...
	CPoint *pPoint = m_pBuffer;
	long *pSrcBufferY = pSrc->GetBuffer_long();
	long *pSrcTemp = NULL;
	long *pSrcTempEnd = NULL;
	long nIndex = 0;
	long nOffset = 0;
...
void CMemBuffer_CPoint::CalValueY(double fCoef, double fOffset,CMemBuffer_Float *pSrc,long nBuffer1Begin,long nBuffer1End,long nBuffer2Begin ,long nBuffer2End , long nGap)
{
	...
	CPoint *pPoint = m_pBuffer;
	float *pSrcBufferY = pSrc->GetBuffer_float();
	float *pSrcTemp = NULL;
	float *pSrcTempEnd = NULL;
	long nIndex = 0;
	long nOffset = 0;
