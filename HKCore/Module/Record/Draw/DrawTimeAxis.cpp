...
long TIMEAXIS_RANGESTATEAXIS_HEIGHT =  (RANGESTATEAXIS_BLOCK_WIDTH+4);
...
void CDrawTimeAxis::SetBorder(const CRect &rect, BOOL bUseClipRect)
{
	...
	CRect rcBoder = GetBorder();
	m_oTimeAxis.SetBorder(rcBoder);

	if (m_bUseRangeAxis)
	{
		CRect rcRular = m_oTimeAxis.GetBorder();
		CRect rcRangeAxisBorder = rcBoder;
...
void CDrawTimeAxis::Draw(CDC *pDC, double fZoomRatio,const CPoint &ptOffset, DWORD dwDrawFlag)
{
	m_oTimeAxis.DrawIt(pDC);
}

void CDrawTimeAxis::DrawSubject(CDC *pDC, double fZoomRatio,const CPoint &ptOffset, DWORD dwDrawFlag) 
{	
	if (m_bUseRangeAxis)
	{
		m_oRangeAxis.Draw(pDC, fZoomRatio, ptOffset, dwDrawFlag);
	}
}
...
void CDrawTimeAxis::SetMaxMinTime(double dMax,double dMin)
{
	...
	m_oRangeAxis.SetTimeRange(dMin, dMax);
...
void CDrawTimeAxis::TimeGetAxisRange(double &dCoefLeft, double &dCoefRight)
{
	m_oRangeAxis.GetAxisRange(dCoefLeft, dCoefRight);
}

void CDrawTimeAxis::TimeSetAxisRange(double dCoefLeft, double dCoefRight)
{
	m_oRangeAxis.SetAxisRange(dCoefLeft, dCoefRight);
}

void CDrawTimeAxis::SetAxisCurrTime(double dMax,double dMin)
{
	m_dTimeMin = dMin;
	m_dTimeMax = dMax;
	double dCoefLeft = dMin / m_dOldTimeMax;
	double dCoefRight = dMax / m_dOldTimeMax;
	m_oTimeAxis.SetMinMaxVal(m_dTimeMin, m_dTimeMax, 0);
	m_oRangeAxis.SetAxisRange(dCoefLeft, dCoefRight);
}
