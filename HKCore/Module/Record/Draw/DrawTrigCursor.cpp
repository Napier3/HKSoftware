...
void CDrawTrigCursor::Move(long nHor, long nVer , BOOL bUseAcc)
{
	...
	if (nHor != 0 || nVer != 0) 
	{
		...
		if (m_ptTrig[1].y <= m_rcClipRect.top)
		{
			CRect rect;
			rect.left = m_ptTrig[2].x - 4;
			rect.right = m_ptTrig[2].x + 4;
			rect.top = m_rcClipRect.top ;
			rect.bottom = m_rcClipRect.top + 10;
			m_pParentWnd->InvalidateRect(rect);
		}
		else if (m_ptTrig[2].y >= m_rcClipRect.bottom)
		{
			CRect rect;
			rect.left = m_ptTrig[2].x - 4;
			rect.right = m_ptTrig[2].x + 4;
			rect.top = m_rcClipRect.bottom - 10;
			rect.bottom = m_rcClipRect.bottom;
			m_pParentWnd->InvalidateRect(rect);
		}
		else
		{
...
void CDrawTrigCursor::Draw(CDC *pDC, ULONG nDrawStyle) 
{
	if (m_nShow == 1)
	{
		...
		if (m_ptTrig[1].y <= m_rcClipRect.top)
		{
			pDC->MoveTo(m_ptTrig[2].x,m_rcClipRect.top);
			pDC->LineTo(m_ptTrig[2].x,m_rcClipRect.top + 10);
			
			pDC->MoveTo(m_ptTrig[2].x,m_rcClipRect.top);
			pDC->LineTo(m_ptTrig[2].x - 3,m_rcClipRect.top + 3);
		...
		{
			pDC->MoveTo(m_ptTrig[2].x,m_rcClipRect.bottom);
			pDC->LineTo(m_ptTrig[2].x,m_rcClipRect.bottom - 10);
			
			pDC->MoveTo(m_ptTrig[2].x,m_rcClipRect.bottom);
			pDC->LineTo(m_ptTrig[2].x - 3,m_rcClipRect.bottom - 3);
		...
		CRgn rgn;
		rgn.CreateRectRgn(m_rcClipRect.left, m_rcClipRect.top, m_rcClipRect.right, m_rcClipRect.bottom);
...
void CDrawTrigCursor::SetCursorPosition(const CPoint &point) 
{
	...
	m_ptTrig[3].x = point.x + 10;
	m_ptTrig[3].y = point.y;

	m_ptTrig[1].x = point.x;
	m_ptTrig[1].y = point.y + 10;

	m_ptTrig[0].x = point.x - 10;
	m_ptTrig[0].y = point.y + 10;
}
