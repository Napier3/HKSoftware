...
void CDrawRangeStateAxis::ModifyBlock_Move(long &x, long &y)
{
	...
	if (m_pClipRect != NULL)
	{//有限制的区域
		//如果Head1、Head2都Frozen锁定，则限制中心轴
		if (m_bFrozenHead1 && m_bFrozenHead2)
		{
			if (m_nRangeStateAxisType == RANGESTATEAXIS_TYPE_HORZ)
			{//水平坐标
				if (m_ptBlockCenter.x < m_pClipRect->left)
				{//超出左边范围，移动到限制区域最左边
					x = m_pClipRect->left - m_ptBlockCenter.x;
				}
				
				if (m_ptBlockCenter.x > m_pClipRect->right)
				{//超出右边范围，移到限制区域最右边
					x = m_pClipRect->right - m_ptBlockCenter.x;
				}
			}
			else
			{//纵向座标
				if (m_ptBlockCenter.y < m_pClipRect->top)
				{//超出上边范围，移动到限制区域最上边
					y = m_pClipRect->top - m_ptBlockCenter.y;
				}
				
				if (m_ptBlockCenter.y > m_pClipRect->bottom)
				{//超出下边范围，移动到限制区域最下边
					y = m_pClipRect->bottom - m_ptBlockCenter.y;
				}
			}
...
void CDrawRangeStateAxis::ModifyBlock_MinRect()
{
	...
	if (m_nRangeStateAxisType == RANGESTATEAXIS_TYPE_HORZ)
	{
		if (m_nWidth < RANGESTATEAXIS_MIN_LENGTH)
		{
			if (m_ptTopLeft.x == m_pClipRect->left)
			{
				m_nWidth = RANGESTATEAXIS_MIN_LENGTH;
			}
			
			if (m_ptTopLeft.x + m_nWidth == m_pClipRect->right)
			{
				m_ptTopLeft.x = m_pClipRect->right - RANGESTATEAXIS_MIN_LENGTH;
			}
		}
	}
	else
	{
		if (m_nHeight < RANGESTATEAXIS_MIN_LENGTH)
		{
			if (m_ptTopLeft.y == m_pClipRect->top)
			{
				m_nHeight = RANGESTATEAXIS_MIN_LENGTH;
			}
			
			if (m_ptTopLeft.y + m_nHeight == m_pClipRect->bottom)
			{
				m_ptTopLeft.y = m_pClipRect->bottom - RANGESTATEAXIS_MIN_LENGTH;
			}
		}
	}
}
...
LONG CDrawRangeStateAxis::LButtonDown(UINT nFlags, CPoint &point)
{
	...
	{
		m_nBlockID = RANGESTATEAXIS_BLOCK_NONE;
		if (!MrDraw::PointInRect(point, m_ptTopLeft, m_nWidth, m_nHeight))
		{
			return DRAW_NONE_OBJECT_SELECTED;
		}
	}
...
BOOL CDrawRangeStateAxis::IsInClipRect()
{
	...
	if (! MrDraw::PointInRect(*m_pClipRect, m_ptTopLeft))
	{
		return FALSE;
	}
	
	if (! MrDraw::PointInRect(*m_pClipRect, CPoint(m_ptTopLeft.x + m_nWidth, m_ptTopLeft.y + m_nHeight)))
	{
		return FALSE;
	}
...
void CDrawRangeStateAxis::Draw(CDC *pDC, double fZoomRatio,const CPoint &ptOffset, DWORD dwDrawFlag)
{
	if (m_pAttachDraw == NULL)
	{
		return;
	...
	DrawRangeStateAxis(pDC);//, m_rcBlockHead1, m_rcBlockHead2, m_rcBlockCenter, m_nRangeStateAxisType, m_crForeColor);
}
...
void CDrawRangeStateAxis::DrawRangeStateAxis(CDC *pDC)//, const CRect rcHead1, const CRect rcHead2, const CRect rcCenter, long nType, COLORREF crColor)
{
	...
	pDC->LineTo(m_ptBlockHead2);
 	...
 	DrawBlock(pDC, brush, brushFrozen, m_rcBlockHead1, m_bFrozenHead1);
 	DrawBlock(pDC, brush, brushFrozen, m_rcBlockHead2, m_bFrozenHead2);
 	DrawBlock(pDC, brush, brushFrozen, m_rcBlockCenter, m_bFrozenCenter);
...
void CDrawRangeStateAxis::DrawBlock(CDC *pDC, CBrush &brush, CBrush &brushFronzen, CRect &rcBlock, BOOL bFrozen)
{
...
void CDrawRangeStateAxis::AttachDrawObject(CXDrawElement *pVariable)
{ 
	m_pAttachDraw = pVariable;  

	if (pVariable != NULL) 
		SetColor(pVariable->GetColor());
}
...
void CDrawRangeStateAxis::InitBlockRect()
{
	...
	else
	{
		...
		m_rcBlockHead2.bottom = m_ptBlockHead2.y + RANGESTATEAXIS_BLOCK_HEIGHT2;
...
void CDrawRangeStateAxis::AdjustRangeStateAxis()
{
	...
	CPoint ptCenter = CenterPoint();
	
	if (m_nRangeStateAxisType == RANGESTATEAXIS_TYPE_HORZ)
	{
		m_ptBlockHead1.x = m_ptTopLeft.x;
		m_ptBlockHead1.y = ptCenter.y;

		m_ptBlockHead2.x = m_ptTopLeft.x + m_nWidth;
	...
	else
	{
		...
		m_ptBlockHead1.y = m_ptTopLeft.y;
		
		m_ptBlockHead2.x = ptCenter.x;
		m_ptBlockHead2.y = m_ptTopLeft.y + m_nHeight;
...
void CDrawRangeStateAxis::AdjustRangeStateAxisByCoef(double dLeftCoef,double dRightCoef)
{
	...
	if (m_nRangeStateAxisType == RANGESTATEAXIS_TYPE_HORZ)
	{
		nWidth = m_ptOriginBlockHead2.x - m_ptOriginBlockHead1.x/* + 1*/;//20230112 zhouhj  去除后面加1,,会导致后续计算时初始化后左侧按钮无法拖动
	}
	else
	{
...
CRect CDrawRangeStateAxis::GetInvalidateRect()
{
	...
	rc.left = min(rc.left ,m_ptTopLeft.x);
    rc.right = max((long)rc.right ,  (long)(m_ptTopLeft.x + m_nWidth));
	rc.top = min(rc.top ,m_ptTopLeft.y);
    rc.bottom = max((long)rc.bottom ,(long)m_ptTopLeft.y + (long)m_nHeight);
