...
#define GVET_BINARYEDIT			0x08000000  //zhouhj 20201208增加用于二进制显示，如61850数据对象中的品质位
...
class CXGridCtrlBase_MsgFunc
{
public:
	virtual CFont* msg_OnGetFont(WPARAM hFont=NULL, LPARAM lParam=0) = 0;
	virtual void msg_OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult) = 0;
	virtual void msg_OnEndPointEdit(NMHDR* pNMHDR, LRESULT* pResult) = 0;
	virtual void msg_OnEndInplaceComboBoxEdit(NMHDR* pNMHDR, LRESULT* pResult) = 0;

	virtual LRESULT msg_OnFillComboBox(WPARAM nColumn, LPARAM pListBox) = 0;
	virtual LRESULT msg_OnFillList(WPARAM nColumn, LPARAM pListBox) = 0;
	virtual LRESULT msg_OnFillTree(WPARAM nColumn, LPARAM pListBox) = 0;
	virtual LRESULT msg_OnFillTimeCtrl(WPARAM nColumn, LPARAM pListBox) = 0;
};
