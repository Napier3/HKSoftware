...
void CGridComboBox::EndEdit(BOOL bFromEdit)
{
	if (m_bEndEdit)
	{
		return;
	...
	if (nSel != CB_ERR)
	{
		...
		{
			LRESULT nResullt = 0;
			m_pMsgFunc->msg_OnEndInplaceComboBoxEdit((NMHDR*)&dispinfo, &nResullt);
		}
		else
		{
			CWnd* pOwner = GetOwner();
