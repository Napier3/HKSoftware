...
//	if (!CellRange.IsValid())
	...
	if (IsReadOnly())
	{
		TextBkClr = RGB(240,240,240);
		TextClr = RGB(192,192,192);
	}
    ...
    if (state & GVIS_FOCUSED && !pDC->IsPrinting()) 
    {
		...
		rcCell.right++; 
		rcCell.bottom++;    // FillRect doesn't draw RHS or bottom
        pDC->FillSolidRect(rcCell, pGrid->m_crHighLight);
        rcCell.right--; 
		rcCell.bottom--;
    ...
    else if (state & GVIS_SELECTED && !pDC->IsPrinting()) 
    {
        ...
        rcCell.right--; 
		rcCell.bottom--;
        pDC->SetTextColor(pGrid->m_crHighLightText);
    } 
	else
	{
        rcCell.right++; 
...
BOOL CGridCell::DrawFixedCell(CDC* pDC, CRect rect, CXGridCtrlBase *pGrid, int nRow, int nCol, BOOL bCalMerge, BOOL bEraseBk)
{
    ...
    {
        CPen lightpen(PS_SOLID, 1,  pGrid->m_cr3DHighLight),
			darkpen(PS_SOLID,  1, pGrid->m_cr3DDKShadow),
