...
class CXGridCtrlBase : public CXGridCtrlBase_MsgFunc
{
...
public:
	virtual CFont* msg_OnGetFont(WPARAM hFont=NULL, LPARAM lParam=0);
	virtual void msg_OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult);
	virtual void msg_OnEndPointEdit(NMHDR* pNMHDR, LRESULT* pResult);
	virtual void msg_OnEndInplaceComboBoxEdit(NMHDR* pNMHDR, LRESULT* pResult);

	virtual LRESULT msg_OnFillComboBox(WPARAM nColumn, LPARAM pListBox);
	virtual LRESULT msg_OnFillList(WPARAM nColumn, LPARAM pListBox);
	virtual LRESULT msg_OnFillTree(WPARAM nColumn, LPARAM pListBox);
	virtual LRESULT msg_OnFillTimeCtrl(WPARAM nColumn, LPARAM pListBox);
...
public:
	...
	virtual void InvalidateGrid(BOOL b=TRUE);
	virtual CWnd* GetParentWnd();

	virtual void AttachViewWindow(CXDrawBaseInterface_ViewWindow *pViewWindow);

	//lijq 2020-2-26  
	virtual CString GetGridClassID()	{	return _T("GridCtrl");	}
	virtual BOOL ChangeRowPosition(long nRow1, long nRow2);
...
protected:
	CWnd *m_pGridRefWnd;
	CXDrawBaseInterface_ViewWindow *m_pViewWindowPos;
...
public:
	...
	COLORREF GetHighLightColor()					{	return  m_crHighLight;	}
	void     SetHighLightColor(COLORREF clr)           { m_crHighLight = clr;             }
	COLORREF GetHighLightTextColor()				{	return  m_crHighLightText;	}
	void     SetHighLightTextColor(COLORREF clr)           { m_crHighLightText = clr;             }

	COLORREF Get3DHighLightColor()					{	return  m_cr3DHighLight;	}
	void     Set3DHighLightColor(COLORREF clr)           { m_cr3DHighLight = clr;             }
	COLORREF Get3DDKShadowColor()					{	return  m_cr3DDKShadow;	}
	void     Set3DDKShadowColor(COLORREF clr)           { m_cr3DDKShadow = clr;             }
	...
	void SetReadOnly(BOOL bReadOnly = TRUE)  { m_bIsReadOnly = bReadOnly;   }
    ...
    BOOL   SetItemValue(int nRow, int nCol, DWORD dwValue);
    DWORD GetItemValue(int nRow, int nCol) const;
...
public:
	...
	//20210624 xxy 根据窗口大小调整列宽
	void ExpandColumnsToFitEx();
...
public:
	...
	virtual void SelectRowEx(int nRow)  
	{  
		m_SelectionStartCell.row = nRow; 
		m_idCurrentCell.row = nRow;
		m_idCurrentCell.col = m_nFixedCols;
...
protected:
	...
	virtual BOOL IsReadOnly(CGridCell *pCell);
...
protected:
	...
	//2021-6-3  lijunqing 
	COLORREF    m_crHighLight, m_crHighLightText;
	COLORREF    m_cr3DHighLight, m_cr3DDKShadow;
...
public:
	...
	virtual CPoint GetPosTopLeft()	{	return m_posTopLeft;	}
	virtual void   SetPosTopLeft(CPoint pos)	{	m_posTopLeft = pos;	}
