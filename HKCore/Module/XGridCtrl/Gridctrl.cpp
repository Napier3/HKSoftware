...
BOOL CXGridCtrlBase::SetRowCount(int nRows)
{
	ASSERT(nRows >= 0);
...
BOOL CXGridCtrlBase::AutoSizeColumn(int nCol)
{
	ASSERT(nCol >= 0 && nCol < m_nCols);
	...
	case GVET_EDITBOX:
		{
			...
			m_pInplaceEditWnd = new CGridEdit(m_pGridRefWnd, rect, dwStyle, IDC_INPLACE_EDIT, nRow, nCol, Item.szText, nChar, nFormat);
	...
	case GVET_HEXEDIT:
		{
			...
			CGridHexEdit *pCGridHexEdit = new CGridHexEdit(m_pGridRefWnd, rect, dwStyle, IDC_INPLACE_EDIT
				, nRow, nCol, Item.szText, nChar, nMaxLen);
