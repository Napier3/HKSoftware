...
static CSftpCleint::GWI_SFTPClient_ErrorCode stuSftpError[] = {\
...
// 	sprintf_s(username, 300, "root");
// 	sprintf_s(password, 300, "12345678");

	if (!AfxSocketInit())
	{
		CLogPrint::LogString(XLOGLEVEL_TRACE, _T("AfxSocketInit 失败"));
		m_bConnect = FALSE;
		return FALSE;
	}

	CString_to_char(CSFtpServerConfig::GetServerIP(), &charIP);
	CString_to_char(CSFtpServerConfig::GetSftpUser(), &username);
	CString_to_char(CSFtpServerConfig::GetSftpPsw(), &password);
	nRet = sftpOpen(charIP, CSFtpServerConfig::GetSftpPort(), username, password);
	delete charIP;
	delete username;
	delete password;
	...
	if (nRet != 0)
	{
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("创建SFTP连接失败，nRet = %d"), nRet);
		m_bConnect = FALSE;
		return FALSE;
	}

	m_bConnect = TRUE;
	CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("创建SFTP连接成功，ip = %s, port=%d"), CSFtpServerConfig::GetServerIP(), CSFtpServerConfig::GetSftpPort());
	return TRUE; 
}
...
*/
int CSftpCleint::SFTPDirExist(LIBSSH2_SFTP *sftp_session, const char *sftppath)
{
...
* 返回值：0成功；-1失败
*/
int CSftpCleint::makeSFTPDir(LIBSSH2_SFTP *sftp_session, const char *sftppath)
{
...
void CSftpCleint::getGWI_SFTPClient_ErrorMsg_By_Errorcode(int errorcode,char* errormsg)
{
...
/************************************************************************
	 ...
	 * 返回   ： 0 成功；1 其它错误
	************************************************************************/
int CSftpCleint::sftpOpen(const char *ip,int port,const char *username, const char *password)
{
	int ret = 0;
	int auth_pw = 0;
	int rc;
...
int CSftpCleint::preSftpDownloadFileDir(const char sftpDir[MAX_PATH],const char localDir[MAX_PATH],const char extendDir[MAX_PATH])
{
	int rc = -1;
	int iSt = 0;
	LIBSSH2_SFTP *sftp_session;
		...
		if(rc > 0) {
			...
			if (!pcTmp)//不是带后缀的文件
			{
				...
				if (sftp_handle) { //没有后缀的文件
					...
					iSt = downloadFile(sftpDirAdd,localDirAdd);			
				}
				else{  //文件夹
			...
			else //带后缀的文件
			{
				...
				char localDirAdd[MAX_PATH] = {};
				sprintf_s(localDirAdd,MAX_PATH,"%s%s\\\\%s",localDir,extendDir,tmpbuffer);
				iSt = downloadFile(SftpFilePath,localDirAdd);
			}
...
返回：0 成功  其它失败
************************************************************************/
int CSftpCleint::sftpDwnloadFileDir(const char *sftpdir, const char *slocaldir)
{
	return preSftpDownloadFileDir(sftpdir,slocaldir,"");
}

int CSftpCleint::preSftpUpdateFileDir(const char localDir[MAX_PATH],const char sftpDir[MAX_PATH],const char externDir[MAX_PATH])      
{  
	...
	{   
		...
		{  
			...
			{  
				...
				ret = uploadFile(sftpDirAdd,csLocalDirAdd);
				if(ret)
				{
					FindClose(hFind);  
					return ret;
...
/* 返回：0 成功  其它失败
/************************************************************************/
int CSftpCleint::sftpUpdateFileDir(const char *sftpdir,const char *slocaldir)
{
...
/************************************************************************/
int CSftpCleint::sftpCd(const char* path)
{
	int ret = 0;
	LIBSSH2_SFTP* sftp_session = libssh2_sftp_init(session);
