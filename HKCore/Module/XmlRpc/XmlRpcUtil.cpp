...
void XmlRpcUtil::log(int level, const char* fmt, ...)
{
  if (level <= XmlRpcLogHandler::getVerbosity())
  {
	...
	XmlRpcLogHandler* pHandle = XmlRpcLogHandler::getLogHandler();
    pHandle->log(level, buf);
  }
}
...
void XmlRpcUtil::error(const char* fmt, ...)
{
  ...
  //xxy20190520
#ifdef _DEBUG
 //  XmlRpcLogHandler::getLogHandler()->log(0, buf);
#endif
}
