...
CXFcenterTcpBase::CXFcenterTcpBase(void)
{
...
CString CXFcenterTcpBase::GetError(DWORD error)
{
	CString strError;
...
UINT CXFcenterTcpClient::TcpClientThread(LPVOID cs)
{
	char buffer[255] = {0};
	int numrcv;
...
long CXFcenterTcpClient::OnReceive()
{
	m_nReceiveLen = 0;
...
BOOL CXFcenterTcpClient::IsCmmChannelExit()
{
	if (m_pThread == NULL)
	{
		if (m_hSocket != NULL)
...
BOOL CXFcenterTcpClient::TerminateChannel()
{
	return TRUE;
}
...
BEGIN_MESSAGE_MAP(CXFcenterTcpClient, CWnd)
...
LRESULT CXFcenterTcpClient::OnTCPIP(WPARAM wParam, LPARAM lParam)
{
