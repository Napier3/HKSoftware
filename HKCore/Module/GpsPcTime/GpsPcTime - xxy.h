...
public:
	...
	virtual ~CGpsPcTime();
	BOOL FromString(const CString &strTime);
	BOOL FromStringEx(const CString &strTime);
...
public:
	...
	CGpsPcTime& operator-= (const CGpsPcTimeSpan &ts);
	CGpsPcTime& operator+= (const CGpsPcTimeSpan &ts);
...
inline CGpsPcTime operator+ (const CGpsPcTime& tm, const CGpsPcTimeSpan& ts)
{
	CGpsPcTime t = tm;
	t += ts;
	return t;
