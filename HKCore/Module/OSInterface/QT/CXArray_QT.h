...
#ifndef __CXArray_QT_H__
#define __CXArray_QT_H__
...
// Operations
	...
	void SetAt(int nIndex, wchar_t* newElement);
	...
	// Potentially growing the array
	void SetAtGrow(int nIndex, wchar_t* newElement);

	void SetAtGrow(int nIndex, const CString& newElement);

	int Add(wchar_t* newElement);
	...
	// Operations that move elements around
	void InsertAt(int nIndex, wchar_t* newElement, int nCount = 1);
...
inline const CString& CStringArray::GetAt(int nIndex) const
	{ ASSERT(nIndex >= 0 && nIndex < m_nSize);
		if( nIndex < 0 || nIndex >= m_nSize )
			ThrowInvalidArgException();
		return m_pData[nIndex]; }
inline void CStringArray::SetAt(int nIndex, wchar_t* newElement)
	{ ASSERT(nIndex >= 0 && nIndex < m_nSize);
		if( nIndex < 0 || nIndex >= m_nSize )
			ThrowInvalidArgException();
		m_pData[nIndex] = newElement; }
...
inline int CStringArray::Add(wchar_t* newElement)
	{ int nIndex = m_nSize;
		SetAtGrow(nIndex, newElement);
		return nIndex; }
