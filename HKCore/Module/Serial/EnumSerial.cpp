			...
			if (bOk) {
				...
				if (bOk) {
						...
						(PBYTE)locinfo, sizeof(locinfo), NULL))
					{
						...
						#else
							bUsbDevice = (strncmp(locinfo, _T("USB"), 3)==0);
						#endif
					}
					if (bSuccess) {
...
void EnumPortsW9x(CArray<SSerInfo,SSerInfo&> &asi)
{
	...
	try {
			...
			NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
		{
			...
			BOOL bUsbDevice = (lstrcmp(acSubEnum,_T("USBPORTS"))==0);
			...
			{
				BOOL bMatch = (lstrcmp(acSubSubEnum,_T("*PNP0500"))==0 ||
					lstrcmp(acSubSubEnum,_T("*PNP0501"))==0 ||
					bUsbDevice);
				if (bMatch) {
					HKEY hkSubSubEnum = NULL;
