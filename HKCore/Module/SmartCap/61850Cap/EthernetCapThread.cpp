...
{
	m_oCapBufferMngr.InitXSmMemBufferMngr();
	m_pCapBufferMngr = &m_oCapBufferMngr;
...
void CEthernetCapThread::packet_handler_r(const struct pcap_pkthdr *header, const u_char *pkt_data)
{
	...
	if (eth->ether_type == EtherType_Priority || eth->ether_type == EtherType_Smv || eth->ether_type == EtherType_Goose)
	{
		...
		double dTime = header->ts.tv_sec + 1E-6 * header->ts.tv_usec;
		pCapBuffer = m_oCapBufferMngr.AddFrameBuffer(header, pkt_data);
		//m_pProtocol->Parse(pCapBuffer, 0);  //lijq 2020-4-23  在线程中进行解析
	}
	else
	{
		return;
...
long CEthernetCapThread::StartWinPcap()
{
	...
	CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("启动网卡%d监视......"), nNetCardIndex+1);
	...
	if (pNetCard == NULL)
	{
		...
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("不能打开网络适配器,请在网卡设置中进行设置"), m_strName);
	...
	if (adhandle == NULL)
	{
		CLogPrint::LogString(XLOGLEVEL_TRACE, _T("不能打开网络适配器,请在网卡设置中进行设置"));
		CStNetCardInforList::Release();
		return 0;  
	}
	...
	{
		CLogPrint::LogString(XLOGLEVEL_ERROR,  _T("该程序仅支持以太网设备"));
		CStNetCardInforList::Release();
		return 0;
	}
	...
	{
		CLogPrint::LogString(XLOGLEVEL_ERROR,  _T("编译过滤字符串失败"));
...
// 		m_packet_filter = NULL;
	...
	{
		CLogPrint::LogString(XLOGLEVEL_ERROR, _T("设置过滤器失败") );
// 		pcap_freealldevs(alldevs);
// 		delete m_packet_filter;
// 		m_packet_filter = NULL;
		CStNetCardInforList::Release();
...
// 	strText.Format(_T("------开始监听设备 { %s }------"), CString(d->description));
	CLogPrint::LogString(XLOGLEVEL_TRACE, _T("------开始监听设备------"));
...
// 		pThis->m_packet_filter = NULL;
		...
		CLogPrint::LogString(XLOGLEVEL_TRACE, _T("启动监视监视线程失败"));
		return 0;
	}
...
// 		CTickCount32 oTick;
		...
		//adhandle后不调用pcap_loop，而用下面的方法：
 		while(TRUE)
 		{
			...
			if (g_nCapConstFrameBufferIndex == 0)
			{
				nCount++;
...
// 			{
// 				CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("接受到%d字节"), nLenCap);
// 				nLenCap = 0;
...
// 	pThis->m_packet_filter = NULL;
	...
	pThis->m_pCaptureThread = NULL;
	CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("监视线程（%s）结束"), pThis->m_strName);
