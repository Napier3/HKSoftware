...
double epx_n64_t0_double(__int64 n64)
{
	double dTime = n64 & 0xFFFFFFFF;
	dTime /= 1000000000;
	dTime +=  n64 >> 32;
	return dTime;
}
...
BOOL CCapListCtrlEx::GetItemText(CFrameMemBuffer *pCapBuffer, long nSubItem, CString &strText)
{
	...
	case 1:
		{
// 			double dTime = epx_n64_t0_double(pCapBuffer->m_n64TimeSnamp);
// 			dTime -= m_dBeginTime;
// 
// 			if (dTime >= 0)
// 			{
// 				strText.Format(_T("%f"), dTime);;
// 			}
// 			else
// 			{
// 				strText.Format(_T("%f [%016I64X] "), dTime, pCapBuffer->m_n64TimeSnamp);;
// 				//CLogPrint::LogString(XLOGLEVEL_TRACE, strText);
// 			}
			ep_cap_timeval_to_string_ns(pCapBuffer->m_n64TimeSnamp, strText);
		}
...
/*
			double dTime = epx_n64_t0_double(pCapBuffer->m_n64TimeSnamp);
			COleDateTime tm(pCapBuffer->m_n64TimeSnamp);
			SYSTEMTIME tmSys;
			tm.GetAsSystemTime(tmSys);
			dTime = pCapBuffer->m_n64TimeSnamp & 0xFFFFFFFF;
			dTime /= 1000000000;
			strText.Format(_T("%d-%d-%d %d:%d:%d %f"), tmSys.wYear, tmSys.wMonth, tmSys.wDay
				, tmSys.wHour, tmSys.wMinute, tmSys.wSecond, dTime);
*/
		}
...
LRESULT CCapListCtrlEx::OnShowWatch(WPARAM wParam, LPARAM lParam)
{
	...
	if (m_nViewCurrIndex == nBufferIndex)
	{
		return 0;
	}
	...
	if (m_bFirstTime )
	{
		m_n64BeginTime = m_pFrameMemBufferMngr->GetCapBeginTime64();
		m_dBeginTime = epx_n64_t0_double(m_n64BeginTime);
		m_bFirstTime = FALSE;
	}
...
void CCapListCtrlEx::SetFrameMemBufferMngr(CXSmMemBufferMngr *pFrameMemBufferMngr)
{
...
void CCapListCtrlEx::PostItemSelChangeMsg(BOOL bCreateAndShow)
{
	...
	CXSmMemBuffer *pBuffer = m_pFrameMemBufferMngr->GetFrameBuffer(nSel);
	GetParent()->PostMessage(WM_CAPLIST_ITEM_SEL, bCreateAndShow, (DWORD)pBuffer);
}
