...
// mym add 2020-7-15 为9-2报文解析过程中出现报文出错的情况，特创建此函数。
inline DWORD Ep_BufferGetAccurateDWord2(BYTE &byteTag, PEPBUFFERPOS pBufferPos, long nLen)
{
	if (nLen > 4 || nLen < 0)
		nLen = 4;
...
inline DWORD Ep_BufferAccurateReadDWord2(BYTE &byteTag, PEPBUFFERPOS pBufferPos, long nLen)
{
	DWORD val = Ep_BufferGetAccurateDWord2(byteTag,  pBufferPos, nLen);
	Ep_OffsetBufferCurr(pBufferPos, nLen);
	return val;
}
