...
void CParaSortGroup::AddToTail(CParaSortGroup *pGroup)
{
...
void CParaSortGroup::AddToTail(CParaSortObject *pData)
{
	...
	{
		AddToTail((CParaSortGroup*)pData);
	}
}	
...
//		rect.bottom = nTop + (nIndex + 1) * nPerHeight - 1 ;
		pVariable->SetParaRectTop(nTop + nIndex * nPerHeight);
		pVariable->SetParaRectBottom(nTop + (nIndex + 1) * nPerHeight-1);
...
void CParaSortGroup::CalParaGroupRect()
{
	...
	m_rcPara.left = pVariable->m_oParas.m_rcBorder.left;
	m_rcPara.right = pVariable->m_oParas.m_rcBorder.right;
	
	while (pos != NULL)
	{
		pObj = GetNext(pos);
		pVariable = pObj->m_pDrawVaraible;
		nTotalHeight += pVariable->m_oParas.m_rcBorder.Height() + 1;//让各个通道的参数有1的间隔
...
CParaSortMngr::CParaSortMngr(CDrawBaseList *pDrawVariables, const CRect &rcClip)
{
	...
	while(pos!=NULL)
	{
		...
		//if (pObj->GetClassID() == DRAWCLASSID_ANALOGVARIABLE)
		{
...
CParaSortGroup* CParaSortMngr::CreateGroup(CParaSortObject* pFirst,CParaSortObject* pCurr)
{
	...
	pGroup->AddToTail(pFirst);
	pGroup->AddToTail(pCurr);
