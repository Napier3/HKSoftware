...
void CStNetCardInforList::InsertToComboBox(CComboBox *pComboBox)
{
	ASSERT (g_pNetCardList != NULL);

	if (g_pNetCardList == NULL)
	{
		return;
	}

	POS pos = g_pNetCardList->GetHeadPosition();
	...
	while (pos != NULL)
	{
		pCard = g_pNetCardList->GetNext(pos);
		strText.Format(_T("Íø¿¨%d:%s(%s)"), nIndex, CString(pCard->m_strInfor), CString(pCard->m_strIP));
		nIndex = pComboBox->AddString(strText);
		pComboBox->SetItemData(nIndex, (DWORD)pCard);
	}
