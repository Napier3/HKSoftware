...
void CCapSmDb::Write(CFrameMemBufferMngr *pBufferMngr)
{
	if (pBufferMngr->GetCount(m_nCurrWriteIndex) == 0)
	{
		CLogPrint::LogString(XLOGLEVEL_TRACE, _T("没有需要写的数据........"));
		return;
	}
...
void CCapSmDb::SetWriteFlag(CFrameMemBufferMngr *pBufferMngr)
{
	pBufferMngr->SetState(m_nCurrWriteIndex, EBS_HASWRITE);
	m_nCurrWriteIndex = m_nCurrIndex_MemBufMngr;
}
...
// 	}
// 
	if (!m_pCapFrameSmDbFile->IsAreaOpenSucc())
	{
		return FALSE;
	}
...
long CCapSmDb::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
	long dSize = 0;
	long dView = 0;
...
{
	if (m_pCapFrameSmDbFile == NULL)
	{
		return;
	}

	m_pCapFrameSmDbFile->ReadBuffer(pIndex->n64CapFramePos, pIndex->nFrameLen, pBuffer);
}
