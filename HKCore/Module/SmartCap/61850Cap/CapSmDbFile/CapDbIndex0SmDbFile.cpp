...
BYTE* CCapDbIndex0SmDbArea::MapAreaView()
{
	...
	BYTE *pBuffer = (BYTE*)m_pSmFileClient->MapView(m_n64AreaBeginPos, dwMapLength);

	ASSERT(pBuffer != NULL);

	if (pBuffer != NULL)
	{
		m_pSmFileClient->LocatePos(0);
	}
...
{
	m_pSmFileClient->LocatePos(m_nPosIndex0Mngr);
	*m_pSmFileClient >> m_oIndex0Mngr.nCurrIndex;  //当前的有效索引
	*m_pSmFileClient >> m_oIndex0Mngr.nIndexCount;  //有效索引的个数
	*m_pSmFileClient >> m_oIndex0Mngr.nIndexMaxCount;  //最大索引数量
	
// 	BYTE* pBuffer = (BYTE*)m_pSmFileClient->GetCurrPos();
...
void CCapDbIndex0SmDbArea::InitIndex0Mngr()
{
	...
	m_pSmFileClient->LocatePos(nPos);
...
{
	LPVOID pBuffer = m_pSmFileClient->LocatePos(0);
...
void CCapDbIndex0SmDbArea::WriteIndexoMngrEx()
{
	...
	*m_pSmFileClient << m_oIndex0Mngr.nIndexMaxCount;  //最大索引数量
	...
	m_pSmFileClient->WriteBuffer(m_oIndex0Mngr.pIndex0s, nSize);
}
...
void CCapDbIndex0SmDbFile::CreateCapDbIndex0SmDbServer(volatile __int64 nDataSize, volatile __int64 nDataView, const CString &strPath)
{
	...
	CreateSmDb(strFile, g_strCapSmDbIndex0ID, nDataSize);
