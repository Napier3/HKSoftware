...
TiXmlNode* TiXmlNode::FindNode(const char *strNodeID)
{
	TiXmlNode* node = firstChild;
	TiXmlNode* pFind = NULL;

	while ( node )
	{
		if (strcmp(node->Value(), strNodeID) == 0)
		{
			pFind = node;
			break;
		}
		node = node->FirstChild(strNodeID);
	}
...
bool TiXmlDocument::LoadFile( const CString &_filename, TiXmlEncoding encoding )
{
	...
	TIXML_STRING filename( pszConvertedAnsiString);
#else
	TIXML_STRING filename( _filename );
#endif
	value = filename;
