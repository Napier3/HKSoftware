...
BOOL CCmmTcpServerChannel::Create(CExBaseObject *pConfig)
{
	...
	CCmmChannelCreateMngr::RegisterTcpServer(this);
...
// {
// 	CLogPrint::LogString(LOGLEVEL_TRACE, L"CCmmTcpServerChannel::OnClose");
// 
// 	CloseOwnSocket();
// }
...
BOOL CCmmTcpServerChannel::RemoveClient(CCmmTcpServerClientChannel *pChannel)
{
	...
	while (pos != NULL)
	{
		...
		if (p == pChannel)
		{
			CLogPrint::LogFormatString(LOGLEVEL_TRACE, _T("Remove TCP Client %X  %d"), pChannel->m_nIPAddr, pChannel->m_nSocketPort);
			m_oTcpServerClientChannels.RemoveAt(posPrev);
			p->Close();
			delete p;
		}
