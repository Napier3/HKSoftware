...
UINT DynDlgGetNewUniqueID(void)   
{   
	static UINT glb_nNextID = 0x00004000;  // We have to start somewhere...    
	glb_nNextID++;   
	return glb_nNextID;   
}   
...
DLGITEMTEMPLATECONTROLS CDynDialogItemEx::GetClassTypeByName(const CString & lpszClassName)   
{   
...
{   
	...
	if (nID == 0) 
	{   
		m_ControlID = ::DynDlgGetNewUniqueID();   
	}   
	else 
	{   
		m_ControlID = nID;   
...
UINT CDynDialogItemEx::InitDialogItem(const CString & lpszClassName,   
									  DWORD dwStyle,   
									  DWORD dwExtendedStyle,   
									  LPRECT pRect,   
									  const CString & lpszCaption,   
...
{   
	...
	*(WORD*)pdest = (USHORT)m_eTypeControl;    // atom value for the control    
	...
	nChars = wcslen(bstrCaption);
#endif

	pchCaption = new WCHAR[nChars];   
	ZeroMemory(pchCaption, nChars *sizeof(WCHAR));
	nActualChars = nChars - 1;//m_strCaption.GetLength();//MultiByteToWideChar(CP_ACP, 0, m_strCaption, -1, pchCaption, nChars);   
