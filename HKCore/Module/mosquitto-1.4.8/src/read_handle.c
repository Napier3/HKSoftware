...
HWND g_hMosquittoMsgWnd = NULL; 
MOSQUITTO_TOPIC_PROCESS *g_MosquittoTopicProcess = NULL;
...
int mqtt3_handle_publish(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	//xxy
#ifdef MOSQUITTO_SVR_EX
	ServerPocess(&topic,&payload,&payloadlen);
#endif
	
// 	if (strcmp(topic,"123") == 0)
// 	{
// 		_mosquitto_free(topic);
// 		_mosquitto_free(payload);
// 
// 		topic = _mosquitto_calloc(4, 1);
// 		payload = _mosquitto_calloc(10, 1);
// 		payloadlen = 8;
// 		strcpy(topic,"456");
// 		strcpy(payload,"abcdefgh");
// 	}
// 	else if (strcmp(topic,"456") == 0)
// 	{
// 		_mosquitto_free(topic);
// 		_mosquitto_free(payload);
// 
...
// 	}
	...
	//yzj 在这里进行4个模块的主题过滤
	//如果需要进行内部主题发送也在这里进行
	if (g_MosquittoTopicProcess != NULL)
	{//2022-8-1  lijunqing
		g_MosquittoTopicProcess(topic, payload);
	}
//		if(!Process_Ats_Publish(topic, payload))
//		{
//
//		}	
