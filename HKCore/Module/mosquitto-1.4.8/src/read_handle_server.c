...
int mqtt3_handle_connect(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	//yzj 发送客户端的连接消息
	if(g_hMosquittoMsgWnd)
	{
		char* strID =  (char*)malloc(strlen(context->id) + 1);
		char* strAddress = (char*)malloc(strlen(context->address) + 1);
		strcpy_s(strAddress, strlen(context->address) + 1, context->address);
		strcpy_s(strID, strlen(context->id) + 1, context->id);
		PostMessage(g_hMosquittoMsgWnd, WM_MQTT_CONNECT, (WPARAM)strAddress, (LPARAM)strID);
	}
...
int mqtt3_handle_subscribe(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	while(context->in_packet.pos < context->in_packet.remaining_length){
		...
		if(sub){
			...
			if(g_hMosquittoMsgWnd)
			{
				char* strSub = (char*)malloc(strlen(sub) + 1);
				char* strID =  (char*)malloc(strlen(context->id) + 1);
				strcpy_s(strSub, strlen(sub) + 1, sub);
				strcpy_s(strID, strlen(context->id) + 1, context->id);
				PostMessage(g_hMosquittoMsgWnd, WM_MQTT_SUBSCRIBE, (WPARAM)strSub, (LPARAM)strID);
			}
	...
	while(context->in_packet.pos < context->in_packet.remaining_length){
		...
		if(sub){
			...
			if(g_hMosquittoMsgWnd)
			{
				...
				strcpy_s(strSub, strlen(sub) + 1, sub);
				strcpy_s(strID, strlen(context->id) + 1, context->id);
				PostMessage(g_hMosquittoMsgWnd, WM_MQTT_UNSUBSCRIBE, (WPARAM)strSub, (LPARAM)strID);
			}
