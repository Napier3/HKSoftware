...
int _mosquitto_log_printf(struct mosquitto *mosq, int priority, const char *fmt, ...)
{
	va_list va;
	int rc;
	int nLen = strlen(fmt) + 500;
	char* strMsg = _mosquitto_malloc(nLen*sizeof(char));

	va_start(va, fmt);
	rc = _mosquitto_log_vprintf(mosq, priority, fmt, va);
	
	vsnprintf(strMsg, nLen, fmt, va);
	switch(priority)
	{
	case MOSQ_LOG_WARNING:
	case MOSQ_LOG_ERR:
	...
	case MOSQ_LOG_DEBUG:
	case MOSQ_LOG_NOTICE:
		if (g_hMosquittoMsgWnd != NULL)
		{
			PostMessage(g_hMosquittoMsgWnd, WM_MQTT_LOG, priority, (LPARAM)strMsg);
		}
		break;
	case MOSQ_LOG_NONE:
	case MOSQ_LOG_INFO:
