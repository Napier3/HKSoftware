...
// {16C5278D-06DC-4117-A420-57C105B29FA0}
static const IID IID_IPolarAxis =
{ 0x16c5278d, 0x6dc, 0x4117, { 0xa4, 0x20, 0x57, 0xc1, 0x5, 0xb2, 0x9f, 0xa0 } };

/*
static const IID IID_IPolarAxisInterfaceEx = 
{ 0xb0b78981, 0x68ba, 0x483e, { 0xa5, 0x2, 0xe8, 0x56, 0xce, 0x15, 0x92, 0xf5 } };
*/
...
BEGIN_INTERFACE_MAP(CPolarAxis, CCmdTarget)
	...
	INTERFACE_PART(CPolarAxis, IID_IAxisBaseInterface,AxisBaseInterface)
END_INTERFACE_MAP()

// {91300EAE-559E-4DC0-90E0-4B7F5B161456}
IMPLEMENT_OLECREATE(CPolarAxis, PROGID_IPolarAxis/*"Axis.PolarAxis"*/, 0x91300eae, 0x559e, 0x4dc0, 0x90, 0xe0, 0x4b, 0x7f, 0x5b, 0x16, 0x14, 0x56)
...
void CPolarAxis::DrawAxisTitle(CDC *pDC,float fZoomRatio,CPoint ptOffset)
{
	...
	char cName[128];
	lstrcpy(cName,m_strPolarAxisTitle);
	CmDrawTextRect(pDC,(LPSTR)cName,m_crPolarAxisTitleFont,&rect,RGB(0,0,0),
...
void CPolarAxis::DrawAxisCenter(CDC*pDC)
{
	...
	CmDrawTextRect(pDC,cStr,m_crAngleUnitFontColor,rect,RGB(0,0,0),m_lfAngleUnitFont,TRANSPARENT,DT_VCENTER|DT_LEFT,0);

	//±180
	strcpy(cStr,"±180");
	rect.right=m_rectWorkArea.left-iGap;
	rect.left=rect.right-iLen;
	CmDrawTextRect(pDC,cStr,m_crAngleUnitFontColor,rect,RGB(0,0,0),m_lfAngleUnitFont,TRANSPARENT,DT_VCENTER|DT_RIGHT,0);
	...
	CmDrawTextRect(pDC,cStr,m_crAngleUnitFontColor,rect,RGB(0,0,0),m_lfAngleUnitFont,TRANSPARENT,DT_BOTTOM|DT_CENTER,0);
	
	//-90
	strcpy(cStr,"-90");
	rect.top=m_rectWorkArea.bottom;
	rect.bottom=rect.top+15*m_lZoomCoefficient;
	CmDrawTextRect(pDC,cStr,m_crAngleUnitFontColor,rect,RGB(0,0,0),m_lfAngleUnitFont,TRANSPARENT,DT_TOP|DT_CENTER,0);
}	
...
{
	float fAngleUnit = GetAngleUnit();
	fAngleUnit *= fRate;
	m_iAngleUnit = fAngleUnit;
	if(m_iAngleUnit<=5) m_iAngleUnit = 0;
	if(m_iAngleUnit>=90) m_iAngleUnit = 90;
	
	m_iCircleGridCounts++;
	if (m_iCircleGridCounts<=0) m_iCircleGridCounts = 1;
	if (m_iCircleGridCounts>10) m_iCircleGridCounts = 10;
...
*/
	pThis->m_strPolarAxisTitle = bstrValue;
...
{
	METHOD_PROLOGUE(CPolarAxis, PolarAxisInterface);
	CString strPolarAxisTitle;
	strPolarAxisTitle = pThis->m_strPolarAxisTitle;
	*pbstrValue = strPolarAxisTitle.AllocSysString();
...
{
	METHOD_PROLOGUE(CPolarAxis, PolarAxisInterface);
	pThis->m_lCircleUnitLineWidth = nValue;
...
{
	METHOD_PROLOGUE(CPolarAxis, PolarAxisInterface);
	pThis->SetCircleGridCounts(nValue);
...
{
	METHOD_PROLOGUE(CPolarAxis, PolarAxisInterface);
	*nValue = pThis->GetCircleGridCounts();
...
{
	METHOD_PROLOGUE(CPolarAxis, PolarAxisInterface);
	pThis->GetWorkArea(left,top,right,bottom);
	return 0;
}
...
STDMETHODIMP_(long) CPolarAxis::XPolarAxisInterface::GetTheVector(double fRadius1,double fAngle1,double fRadius2,double fAngle2,double *fRadiusL,double *fAngleL)
{
	METHOD_PROLOGUE(CPolarAxis, PolarAxisInterface);
	pThis->GetTheVector(fRadius1,fAngle1,fRadius2,fAngle2,fRadiusL,fAngleL);
...
////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////
STDMETHODIMP_(ULONG) CPolarAxis::XAxisBaseInterface::AddRef()
{
	METHOD_PROLOGUE(CPolarAxis, AxisBaseInterface);
	return pThis->ExternalAddRef();
}

STDMETHODIMP_(ULONG) CPolarAxis::XAxisBaseInterface::Release()
{
	METHOD_PROLOGUE(CPolarAxis, AxisBaseInterface);
	return pThis->ExternalRelease();
}

STDMETHODIMP CPolarAxis::XAxisBaseInterface::QueryInterface(REFIID iid,LPVOID* ppvObj)
{
	METHOD_PROLOGUE(CPolarAxis, AxisBaseInterface);
	return pThis->ExternalQueryInterface(&iid, ppvObj);
}

STDMETHODIMP_(long) CPolarAxis::XAxisBaseInterface::GetBoder(long *left, long *top, long *right, long *bottom)
{
	METHOD_PROLOGUE(CPolarAxis, AxisBaseInterface);
	CRect rectBorder;
...
STDMETHODIMP_(long) CPolarAxis::XAxisBaseInterface::GetWorkArea(long *left, long *top, long *right, long *bottom)
{
	METHOD_PROLOGUE(CPolarAxis, AxisBaseInterface);
	CRect rectWorkArea;
...
STDMETHODIMP_(long) CPolarAxis::XAxisBaseInterface::LogicPointToDevicePoint(double x,double y,long FAR*ptx,long FAR*pty)
{
	METHOD_PROLOGUE(CPolarAxis, AxisBaseInterface);
...
STDMETHODIMP_(long) CPolarAxis::XAxisBaseInterface::DevicePointToLogicPoint(long ptx,long pty ,double *x,double *y)
{
	METHOD_PROLOGUE(CPolarAxis, AxisBaseInterface);
	pThis->DevicePointToLogicPoint(ptx,pty,x,y);
...
STDMETHODIMP_(long) CPolarAxis::XAxisBaseInterface::GetMinMaxVal(double *xMin,double *yMin,double *xMax,double *yMax,long nReserved)
{
	METHOD_PROLOGUE(CPolarAxis, AxisBaseInterface);
