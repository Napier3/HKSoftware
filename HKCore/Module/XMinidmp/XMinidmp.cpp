...
{
#ifdef _PSX_IDE_QT_
	HMODULE hKernel32 = LoadLibrary(L"kernel32.dll"); //zhouhj 20211008 增加用于Qt下的使用
#else
...
LONG WINAPI UnhandledExceptionFilterEx(struct _EXCEPTION_POINTERS *pException)
{
	...
	strFile += _T(".dmp");
#ifdef _PSX_IDE_QT_
	wchar_t *pzfPath = NULL;//zhouhj 20211008 增加用于Qt下的使用
	CString_to_wchar(strFile,&pzfPath);
	CreateMiniDump(pException,(LPCTSTR)pzfPath);
	delete pzfPath;
#else
	CreateMiniDump(pException,strFile);
#endif
