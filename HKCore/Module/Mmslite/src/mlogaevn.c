...
ST_VOID m_log_evt_tod (MMS_BTOD *btod);
  ...
  switch (info->evtime_tag)
    ...
    case (1)  :
      MLOG_CALWAYS1 ("Time Sequence Id: %lu", info->evtime.time_seq_id);
      break;
...
ST_VOID m_log_evt_tod (MMS_BTOD *info)
  ...
  MLOG_CALWAYS1 ("   Milliseconds since midnight: %ld", info->ms);
  if ( info->form == MMS_BTOD6)
    {
    MLOG_CALWAYS1 ("   Days since 1/1/1984        : %ld", info->day);
    }
  else
    {
