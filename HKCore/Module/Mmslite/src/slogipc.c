...
#ifdef SLOGIPC_SEAL
    if (ctx->sealMode != SSEAL_MODE_NONE)
      {
    /* OK, check the seal ... */
      rc = _slogIpcCheckSeal (ctx->lc, (SLOGIPC_SMART_MSG_HDRS *) sHdr);
      if (rc != SD_SUCCESS)
        {
  ...
  if (appId)
    ...
    if (ctx->smartMode != SD_TRUE)
      ...
      slogIpc_DataBufAlloc (pSock, 
                             ctx->hdrSize + (appIdLen + 1), 
                             &sockData);

     /* Add the data following the header */
      p = (ST_CHAR *) (sockData->data + ctx->hdrSize);
      strcpy (p, appId);
      ...
      if (options != 0)
        {
        slogIpc_DataBufAlloc (pSock, 
                              ctx->hdrSize + _SLOGIPC_MAX_OPTIONS_LEN + 1, 
                              &sockData);
    ...
    /* All others can get passed to the application */
      ...
      default:
        if (ctx->lc->ipc.slog_ipc_cmd_fun != NULL)
          {
          (*ctx->lc->ipc.slog_ipc_cmd_fun) (pSock, sockData, msgType, msgDataLen, dataBuf);
          }
...
/* Make sure this is the right log control */
  if (ctx->lc != lc)
    return (SD_FAILURE);
  ...
  while (conCtrl)
    ...
    if (conCtrl->conState == SLOGIPC_CONNECTED)
        ...
        if (conCtrl->txEnable == SD_TRUE)
	  {
	    ...
	    {
            slogDataBufLen = ctx->hdrSize +	/* IPC CMD Headers	*/ 
            ...
            strncat_maxstrlen (p, (ST_CHAR *) buf, slogDataBufLen - ctx->hdrSize-1);
	    logMsgLen = strlen (p);
	  ...
	  else /* Not first time, just send duplicate */
	    {
            ...
            p = (ST_CHAR *) sockData->data + ctx->hdrSize;
	    *p = 0;
            strncat_maxstrlen (p, logMsg, slogDataBufLen - ctx->hdrSize-1);
	    }
...
SLOGIPC_SV_SLOG   svSlog;
  ...
  slogIpc_DataBufAlloc (pSock, (ctx->hdrSize + dataLen), &sockData);

  /* Add the data */
  memcpy (sockData->data+ctx->hdrSize, data, dataLen);
  ...
  /* See if there is room at in the queue */
    ...
    /* Take a look at the last one, see if it is a 'missed' message */
      ...
      /* Increment the missed count to 1 */
        pUl = (ST_UINT32 *) (sockData->data + ctx->hdrSize);
        ...
        *pUl = htonl (missedMsgCount);
        ...
        {
        slogIpc_DataBufAlloc (conCtrl->conSock, (ctx->hdrSize + sizeof (ST_UINT32)), &sockData);
...
/* If we have room on the que, alloc a ctrl struct for this command */
  if (list_get_sizeof (slogIpcCmdList) < slogIpcMaxCmdPend)
    { /* Add a byte for NULL termination of strings (for clone, for instance) */
    slogIpcCmd = (SLOGIPC_CMD *)chk_calloc (1, sizeof (SLOGIPC_CMD) + msgDataLen +1);
...
SLOGIPC_CTX *ctx  = &lc->slogIpcCtx;
  ...
  if (lc->slogIpcCtx.smartMode == SD_FALSE)
    ctx->hdrSize = 1;			 /* set to 1 to drain the socket rcv buf */
  else
    {
    sockCfg->hdrSize     = ctx->hdrSize; /* our header size in smart mode */
    ctx->sealMode = lc->ipc.sealMode;

#ifdef SLOGIPC_SEAL
    if (lc->ipc.sealMode == SSEAL_MODE_NONE)
      {
      lc->slogIpcCtx.magicNumber   = SLOGIPC_MAGIC_NUMBER_NOSEAL;
      lc->slogIpcCtx.hdrSize       = sizeof (SLOGIPC_SMART_MSG_HDR);
      }
    else
      {
      lc->slogIpcCtx.magicNumber   = SLOGIPC_MAGIC_NUMBER_SEAL;
      lc->slogIpcCtx.hdrSize       = sizeof (SLOGIPC_SMART_MSG_HDRS);
      }
#else
    lc->slogIpcCtx.magicNumber   = SLOGIPC_MAGIC_NUMBER_NOSEAL;
    lc->slogIpcCtx.hdrSize       = sizeof (SLOGIPC_SMART_MSG_HDR);
  ...
  sockCfg->hdrSize      = lc->slogIpcCtx.hdrSize;
  sockCfg->hdrAllocSize = lc->slogIpcCtx.hdrSize;
...
SLOGIPC_SMART_MSG_HDRS *sHdrS;
  ...
  sockData->dataLen = ctx->hdrSize + dataLen;
  ...
  if (ctx->sealMode != SSEAL_MODE_NONE)
    ...
    sHdrS->magicNumber = htonl (ctx->magicNumber);
  ...
  sHdr->magicNumber = htonl (ctx->magicNumber);
