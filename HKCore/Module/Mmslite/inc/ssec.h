...
/* --------  ---  ------   -------------------------------------------  */
/* 07/01/08  EJV    87     Chg S_SEC_VERSION_STR to 1.20 (for Win Vista)*/
/* 05/14/08  EJV    86     Merged with non-Marben version.		*/
/* 03/27/08  EJV    85     Use S_MAX_PATH instead of MAX_PATH.		*/
/* 02/18/08  EJV    84	   Changed to V2.00 for Sun Solaris.		*/
/* 02/01/08  EJV    83	   Added tags to typedef structs		*/
/* 12/04/07  EJV    82	   Added updateCRLists, ssleLoadCRLists	(sh lib)*/
/* 11/21/07  EJV    81	   Added ssleSetCertStorePath			*/
/* 09/05/07  EJV    80     Port to SNAP-Lite and LEAN-T stack. Cleanup.	*/
/*			   cfgId is now CRC-32 computed on S_CERT_ID.	*/
/*			   Merged S_SEC_LOC_AR and S_SEC_REM_AR into	*/
/*			     S_SEC_LOC_REM_AR struct.			*/
/*			   Merged sslLocalCertList and sslRemoteCertList*/
/*			     into locRemCertList in S_SEC_CFG.		*/
/*			   Del secManGetDebugSel (secman.dll eliminated)*/
/* 03/06/07  EJV    79	   Moved extern ssle_debug_sel from sslelog.h.	*/
/*			   Changed to V1.9950 for MMS-SECURE-145-015.	*/
/* 04/08/08  JRB    79     Chg () to (ST_VOID) in several prototypes.	*/
/* 08/28/06  EJV    78     MMS-LITE SECURE: chg version to 1.10.	*/
/* 03/02/06  EJV    77     Chg S_SEC_VERSION_STR to 1.10 (VS .NET 2005)	*/
/* 01/04/06  EJV    76     Added DLLs incompatibility comments.		*/
/* 10/25/05  EJV    75     Chg S_SEC_LITESECURE_NAME to fit identify	*/
/* 09/22/05  EJV    74     Any new build use S_SEC_VERSION_STR 1.04	*/
...
#endif /* !defined(MMS_LITE) || defined(MAP30_ACSE) */

#if defined(_WIN32)
  #if defined(MMS_LITE)
    #define SISCO_REGISTRY_SECURITY	"SOFTWARE\\SISCO\\Secure MMS-EASE Lite\\CurrentVersion"
...
#define S_SEC_ERR_VALIDITY_TOOSOON             0xA001	/* 40961 */
#define S_SEC_ERR_VALIDITY_EXPIRED             0xA002	/* 40962 */
#define S_SEC_ERR_CNAME_TYPE_UNKNOWN           0xA003	/* 40963 */
#define S_SEC_ERR_CNAME_MISSING                0xA004	/* 40964 */
#define S_SEC_ERR_CNAME_EXTRA                  0xA005	/* 40965 */
#define S_SEC_ERR_CNAME_MISMATCH               0xA006	/* 40966 */
#define S_SEC_ERR_TIME_SEAL_INVALID            0xA007	/* 40967 */
#define S_SEC_ERR_TIME_SEAL_TIME_WINDOW        0xA008	/* 40968 */
#define S_SEC_ERR_READ_INSTALL_PATH            0xA009	/* 40969 */
/* DEBUG: these codes are not appearing anywhere */
...
#define S_CERT_NAME_MAX_LEN			255
...
typedef struct  S_SEC_LOC_AR_tag
  {
  DBL_LNK l;
...
typedef struct  S_SEC_REM_AR_tag
  {
  DBL_LNK l;
  ST_CHAR   *arName;		/* Local or Remote AR Name 		*/
...
typedef struct
  ...
  /* Flags */
  ST_BOOLEAN certAuthCalling;	/* verify that peer certificate is configured	*/
  ST_BOOLEAN certAuthCalled;	/* verify that peer certificate is configured	*/
...
#else /* !defined(MMS_LITE) || defined(MAP30_ACSE) */
/* ssecusr.c functions */
...
ST_RET  ssleLoadCRLists(ST_VOID *ssl_ctx, ST_BOOLEAN *newCrlOut);
...
/* Set log masks */
ST_UINT* secManGetDebugSel(ST_VOID);
...
/*     __declspec(dllexport) ST_VOID WINAPI secManCfgChange(ST_VOID);*/
#endif
#else  /* MMS_LITE */
...
ST_VOID secManPrintSrlNum (S_CERT_ID *certId, ST_CHAR *outBuf, ST_INT outBufLen);
