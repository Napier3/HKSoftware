...
/* --------  ---  ------   -------------------------------------------	*/
/* 10/22/08  JRB    12     TPDU must be at least 2 bytes to decode.	*/
/*			   Chg tp_decode_er to really decode ER TPDU.	*/
/*			   tp_decode_ak(_dc): add pdu_len arg.		*/
/* 			   Delete tp_decode_rj call (not used in TP4).	*/
...
TPDU_ER  tpdu_er_rx;
  ...
  while (!done && len_proc + 2 <= n_unitdata->data_len)
    {
    ...
    switch (type)
      ...
      case TP_PDU_TYPE_DC:
        if ((len = tp_decode_dc (&tpdu_dc_rx, pdu_ptr, pdu_len)) > 0)
          {
          len_proc += len;
          /* set the rem_mac and rem_nsap pointers */
      ...
      case TP_PDU_TYPE_AK:
        if ((len = tp_decode_ak (&tpdu_ak_rx, pdu_ptr, pdu_len)) > 0)
          {
          len_proc += len;
          /* set the rem_mac and rem_nsap pointers */
      ...
      case TP_PDU_TYPE_ER:
        /* NOTE: the last arg indicates this is Transport Class 4.	*/
        if ((len = tp_decode_er (&tpdu_er_rx, pdu_ptr, pdu_len, 4)) > 0)
          {
  ...
  else if (len_proc > n_unitdata->data_len)
    {
    /* Should NEVER happen	*/
    TP_LOG_ERR2 ("Decoded past end of NSDU (NSDU length = %u, length processed = %u)",
      n_unitdata->data_len, len_proc);
    ret = SD_FAILURE;
    }
