...
/************************************************************************/
/*			clpp_event (LEAN-T Version)			*/
/* The user calls this function to retrieve events from the stack	*/
/* RETURN:	TRUE or FALSE						*/
/* NOTES:								*/
/*   Only one function is allowed to call clnp_read, so if TP4_ENABLED	*/
/*   is defined, this function MUST NOT be called UNLESS there is a	*/
/*   CLNP Read Thread running.						*/
/*   Instead, "copp_event" must be called to get packets from CLNP.	*/
/*   The TP4 function "tp_decode_nsdu" will then call "cltp_decode_nsdu"*/
/*   in this module to pass up CLTP packets.				*/
/************************************************************************/
ST_BOOLEAN clpp_event ()
  {
N_UNITDATA *n_unitdata;

  n_unitdata = (*clnp_read_fun_cl) ();	/* Read NSDU from CLNP	*/

  if (n_unitdata == NULL)
    return (SD_FALSE);		/* N-Unitdata NOT received	*/

  cltp_decode_nsdu (n_unitdata);	/* Decode it.	*/

  clnp_free (n_unitdata);			/* Free it.	*/

  return (SD_TRUE);		/* N-Unitdata received	*/
  }
