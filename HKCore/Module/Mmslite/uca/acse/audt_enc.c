...
static ST_INT clpp_init_called;
ST_UINT audt_msgsize;
static ST_UCHAR *audt_buf;
...
/************************************************************************/
/* audt_set_msgsize							*/
/* Change audt_msgsize.  Check if allowed first.			*/
/************************************************************************/
ST_RET audt_set_msgsize (ST_UINT len)
  {
  if (!clpp_init_called)	/* Don't allow after clpp_initialize.	*/
    {
    audt_msgsize = len;
    return (SD_SUCCESS);
    }
...
/************************************************************************/
/*			clpp_initialize					*/
/* This function is called by the user to get interface initialized	*/
/* and started								*/
/************************************************************************/

ST_RET clpp_initialize ()
  {
  if (audt_msgsize == 0)	/* If user never set, set to default.	*/
    audt_msgsize = 1024;
  audt_buf = (ST_UCHAR *) M_MALLOC (MSMEM_STARTUP, audt_msgsize);
  clpp_init_called = SD_TRUE;

#define AUDT_OVERHEAD_SIZE	200	/* DEBUG: is this enough?	*/
  if (clnp_init (audt_msgsize + AUDT_OVERHEAD_SIZE))
    return (SD_FAILURE);

#ifdef TP4_ENABLED
  /* If TP4 is enabled, then it will receive all CLNP PDUs.		*/
  /* Set decode funct ptr, so TP4 can pass CLTP PDUs to us.		*/
  cltp_decode_nsdu_fun = cltp_decode_nsdu;
#endif
...
/************************************************************************/
/*			clpp_terminate					*/
/* This function is called by the user to terminate clpp interface 	*/
/************************************************************************/

ST_RET clpp_terminate ()
  {
  M_FREE (MSMEM_STARTUP, audt_buf);		/* Free the global encode buffer.	*/
  clnp_end ();
  return (SD_SUCCESS);
  }
...
ST_UINT pdulen;
  ...
  curptr = a_unit_data_enc (audt_apdu);

  if (curptr == NULL)
    return (E_ACSE_ENC_ERR);

  /* Compute the PDU length	*/
  pdulen = audt_buf+audt_msgsize-curptr;
