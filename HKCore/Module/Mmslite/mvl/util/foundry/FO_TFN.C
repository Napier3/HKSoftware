...
static ST_INT bufSize;		      
  ...
  if (fun == NULL)
    {
    fun = chk_calloc (MAX_FUN_SIZE, sizeof (ST_CHAR)); /* This will be realloc'd as needed */
  ...
  while (bCallStateMachine)
	...
	if (bNewline == SD_TRUE) /* new line read in */
	  ...
	  while (funSize + (ST_INT) strlen (tmpPtr) > bufSize)
	    {
	    fun = chk_realloc (fun, bufSize + (MAX_FUN_SIZE * sizeof (ST_CHAR)));
