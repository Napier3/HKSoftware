...
#include "SttXftp\VC\SttXftpClientSocket.h"
...
void CSttFileTool::Free()
{
	...
	{
#ifdef _USE_WINBASE_SOCKET
		m_pSocket->SttScoketClose();
#endif
		delete m_pSocket;
		m_pSocket = NULL;
	}
...
void CSttFileTool::OnSendCmdOverTime(LPVOID pSocketRef, DWORD nCmdType, char *pszCmdID)
{
	...
	if (m_pProgress != NULL)
	{
		m_pProgress->TransferError(NULL,0);
	}
...
{
	m_pSocket = new CSttXftpClientSocket;
	m_bIsSocketRef = FALSE;
...
BOOL CSttFileTool::SendCurrFile()
{
	...
	m_pSocket->SendFile(pFile->m_strLocalFile, pFile->m_strRemoteFile, m_nDestID, g_nFileCmdTimeOut,FALSE);
...
{
	CSttXftpFile *pFile = (CSttXftpFile*)GetAtIndex(m_nCurrSendIndex);
	m_pSocket->DownloadFile(pFile->m_strLocalFile, pFile->m_strRemoteFile, m_nDestID, g_nFileCmdTimeOut);
	SetRetResult_Proccessing();
