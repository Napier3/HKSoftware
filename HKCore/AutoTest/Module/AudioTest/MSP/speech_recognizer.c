...
static void end_sr_on_vad(struct speech_rec *sr)
{
	...
	{
		sr->state = SR_STATE_INIT;
		sr->notif.on_speech_end(1);
		return;
	}
	while(sr->rec_stat != MSP_REC_STATUS_COMPLETE )
	{
		if(sr->rec_stat == MSP_REC_STATUS_NO_DEL_TEXT)
		{
			int nTempState = MSP_REC_STATUS_SUCCESS;
			rslt = QISRGetResult(sr->session_id, MSP_REC_STATUS_SUCCESS, 0, &errcode);
			if (rslt && sr->notif.on_result)
			{
				//sr->state = SR_STATE_INIT;
				sr->notif.on_result(rslt, MSP_REC_STATUS_NO_DEL_TEXT);
			}
			/*if(rslt == NULL)
			{
				sr->notif.on_result("", MSP_REC_STATUS_NO_DEL_TEXT);
			}*/
	...
	if(sr->state != SR_STATE_INIT)
	{
		sr->state = SR_STATE_INIT;
	}

	errcode = QISRSessionEnd(sr->session_id, "VAD Normal");
	sr->session_id = NULL;
	if (/*sr->session_id &&*/ !errcode) 
	{
		sr->notif.on_speech_end(END_REASON_VAD_DETECT);
	}
	else
...
int sr_init(struct speech_rec * sr, const char * session_begin_params, enum sr_audsrc aud_src, int devid, struct speech_rec_notifier * notify)
{
	...
	memset(sr->session_begin_params, 0, param_size);
...
int sr_stop_listening(struct speech_rec *sr)
{
	...
	if (sr->state < SR_STATE_STARTED) {
		...
		////
		QISRSessionEnd(sr->session_id, "start record fail");
		sr->session_id = NULL;
		sr->state = SR_STATE_INIT;
		sr->notif.on_speech_end(1);
		//////
	...
	if (sr->aud_src == SR_MIC) {
		...
		//wait_for_rec_stop(sr->recorder, 10000);
	}
...
{
	//CLogPrint::LogString(XLOGLEVEL_INFOR,"sr_write_audio_data");
	const char *rslt = NULL;
	int ret = 0;
	if (!sr )
		return -E_SR_INVAL;
	...
	*/
	...
	if(MSP_REC_STATUS_NO_DEL_TEXT == sr->rec_stat)
	{
		int nTempState = MSP_REC_STATUS_SUCCESS;
		ret = QISRAudioWrite(sr->session_id, data, len, sr->audio_status, &sr->ep_stat,MSP_REC_STATUS_SUCCESS);
	}
	else
	{
		ret = QISRAudioWrite(sr->session_id, data, len, sr->audio_status, &sr->ep_stat, &sr->rec_stat);
	...
	{ 
		int nTempState = MSP_REC_STATUS_SUCCESS;
		rslt = QISRGetResult(sr->session_id, MSP_REC_STATUS_SUCCESS, 0, &ret);
