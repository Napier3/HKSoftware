...
void clear_rec_buffer(WAVEHDR* pBufheader)
{
	if(pBufheader == NULL)
	{
		return 0;
	...
	pBufheader->dwBytesRecorded = 0;
	pBufheader->dwUser = 0;
	pBufheader->dwFlags = 0;
	pBufheader->dwLoops = 1;
	pBufheader->lpNext = NULL;


	memset(pBufheader->lpData, 0, (sizeof(char))*(pBufheader->dwBufferLength));
