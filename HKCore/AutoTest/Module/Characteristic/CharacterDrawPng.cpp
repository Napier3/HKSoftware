...
CCharacterDrawPng::CCharacterDrawPng()
{
	//初始化属性
...
void CCharacterDrawPng::UpdateCurrTestPoint(double dX, double dY)
{
...
void CCharacterDrawPng::FreeCurrTestPoint()
{
	m_bDrawCurrTestPoint = FALSE;
}

//创建绘图对象：整个特性曲线的绘图对象
DWORD CCharacterDrawPng::XCreateElement(CExBaseObject *pData, CWnd *pParentWnd, BOOL bViewTestLines, BOOL bCalCharTemp)
{
...
//创建绘图对象：整个特性曲线的绘图对象
DWORD CCharacterDrawPng::XCreateElement(CExBaseObject *pData, CWnd *pParentWnd)
{
	if (m_pXDrawMngr_Axis != NULL)
	{
...
void CCharacterDrawPng::CreateXDrawMngr(CCharacteristic *pCharTemp)
{
	CreateXDrawMngr(pCharTemp->m_strAxis);
}

void CCharacterDrawPng::CreateXDrawMngr(const CString &strAxis)
{
	if (m_pXDrawMngr_Axis != NULL)
	{
		return;
...
void CCharacterDrawPng::InitCharDrawView(CCharacteristic *pCharTemp, CWnd *pParentWnd)
{
	if (m_bCalCharTemp)
	{
	...
	else
	{
		...
		{
#ifndef NOT_USE_XLANGUAGE
            CLogPrint::LogFormatString(XLOGLEVEL_TRACE, CXLanguageResourceAtsBase::g_sLangTxt_CoordinatesConflict.GetString()/*_T("特性曲线的坐标系“%s”与与图形管理模块的坐标系“%s”冲突")*/
#else
...
void CCharacterDrawPng::Draw(CDC *pDC, double fZoomRatio,const CPoint &ptOffset)
{
	if (m_pXDrawMngr_Axis != NULL)
	{
...
//2021-9-7  lijunqing
void CCharacterDrawPng::GetAxisInterface(CAxisInterface **ppAxisInterface)
{
	m_pXDrawMngr_Axis->GetAxisInterface(ppAxisInterface);
}

CSize CCharacterDrawPng::Layout(CPoint &ptTopLeft)
{
	return CSize(m_nWidth, m_nHeight);
}

CSize CCharacterDrawPng::CalLayout(CSize &szDraw)
{
	//计算总的Size
    CPoint pt(0, 0);
    szDraw = Layout(pt);
...
BOOL CCharacterDrawPng::HasDefChar()
{
	if (m_pXDrawMngr_Axis == NULL)
	{
		return FALSE;
...
BOOL CCharacterDrawPng::HasDefChar(CCharacteristic *pCharTemplate)
{
	if (m_pXDrawMngr_Axis == NULL)
	{
		return FALSE;
...
void CCharacterDrawPng::SetBorder(const CRect &rcBorder, CDC *pDC)
{
	if (m_pXDrawMngr_Axis == NULL)
	{
		return;
...
CRect CCharacterDrawPng::GetBorder()
{
	if (m_pXDrawMngr_Axis == NULL)
	{
...
BOOL char_view_WriteBitmapFile(CCharacteristic *pCharTemplate, long nWidth, long nHeight, const CString &strFile)
{
	...
	CCharacterDrawPng *pBmpView = new CCharacterDrawPng();
...
//////////////////////////////////////////////////////////////////////////
//
void CCharacterDrawPng::xdraw_TestSolvingEquation()
{
...
long CCharacterDrawPng::DevicePointToLogicPoint (long ptx,long pty ,double *x,double *y)
{
	if (m_pXDrawMngr_Axis == NULL)
	{
		return 0;
...
void CCharacterDrawPng::SetAxisName(const CString &strXAxisName, const CString &strYAxisName, const CString &strAxisTitle)
{
	if (m_pXDrawMngr_Axis == NULL)
	{
		return;
...
void CCharacterDrawPng::ZoomIn(double dCoef)
{
	CAxisInterface *pAxisInterface = NULL;
	m_pXDrawMngr_Axis->GetAxisInterface(&pAxisInterface);
...
void CCharacterDrawPng::ZoomOut(double dCoef)
{
	CAxisInterface *pAxisInterface = NULL;
	m_pXDrawMngr_Axis->GetAxisInterface(&pAxisInterface);
...
void CCharacterDrawPng::CalAxisMinMax(double &dXMin, double &dYMin, double &dXMax, double &dYMax)
{
	if (m_pXDrawMngr_Axis != NULL)
	{
...
void CCharacterDrawPng::ZoomReset(double dCoef)
{
	CAxisInterface *pAxisInterface = NULL;
	m_pXDrawMngr_Axis->GetAxisInterface(&pAxisInterface);
...
void CCharacterDrawPng::MoveLeft(double dCoef)
{
	CAxisInterface *pAxisInterface = NULL;
	m_pXDrawMngr_Axis->GetAxisInterface(&pAxisInterface);
...
void CCharacterDrawPng::MoveRight(double dCoef)
{
	CAxisInterface *pAxisInterface = NULL;
	m_pXDrawMngr_Axis->GetAxisInterface(&pAxisInterface);
...
void CCharacterDrawPng::MoveUp(double dCoef)
{
	CAxisInterface *pAxisInterface = NULL;
	m_pXDrawMngr_Axis->GetAxisInterface(&pAxisInterface);
...
void CCharacterDrawPng::MoveDown(double dCoef)
{
	CAxisInterface *pAxisInterface = NULL;
	m_pXDrawMngr_Axis->GetAxisInterface(&pAxisInterface);
...
void CCharacterDrawPng::MouseMove_SetBegin(CPoint &ptBegin)
{
	m_ptMouseMoveBegin = ptBegin;
}

void CCharacterDrawPng::MouseMove_MoveTo(CPoint &point)
{
