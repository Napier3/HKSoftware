// AtsTitleTipWnd.cpp : implementation file
//
...
CAtsTitleTipWnd::CAtsTitleTipWnd()
{
	WNDCLASS wndcls;
...
#else
	strcpy(m_logFont.lfFaceName,_T("Tahoma"));
#endif
...
BEGIN_MESSAGE_MAP(CAtsTitleTipWnd, CWnd)
	//{{AFX_MSG_MAP(CAtsTitleTipWnd)
	ON_WM_MOUSEMOVE()
	ON_WM_KILLFOCUS()
	ON_WM_TIMER()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CAtsTitleTipWnd message handlers


BOOL CAtsTitleTipWnd::Create(CWnd * pParentWnd)
{
...
void CAtsTitleTipWnd::Show(CRect rectTitle, LPCTSTR lpszTitleText, int xoffset /*=0*/)
{
	...
	//根据Item状态改变字体颜色 2010-07-16
	CAtsUiTestStatusButton* pParent = (CAtsUiTestStatusButton*)m_pParentWnd;
	dc.SetTextColor(pParent->GetTipColor());
...
void CAtsTitleTipWnd::SetLogFont(LOGFONT logFont)
{
	m_bUseParentFont = FALSE;
	m_logFont = logFont;
}

void CAtsTitleTipWnd::CalTipRectAngle(LPCTSTR lpszTitleText,long nMaxWidth,CSize &sizeTip)
{
	HWND hWnd = GetSafeHwnd();
	if (!::IsWindow(hWnd))
	{
...
BOOL CAtsTitleTipWnd::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
{
	//SetTimer(124,500,NULL);
...
BOOL CAtsTitleTipWnd::PreTranslateMessage(MSG* pMsg) 
{
	CWnd *pWnd;
	int hittest;
...
void CAtsTitleTipWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void CAtsTitleTipWnd::OnKillFocus(CWnd* pNewWnd) 
{
	Hide();
	CWnd::OnKillFocus(pNewWnd);

	TRACE("CAtsTitleTipWnd::OnKillFocus");
}


void CAtsTitleTipWnd::OnTimer(UINT nIDEvent) 
{
	if (! ::IsWindow(m_hWnd))
	{
		return;
