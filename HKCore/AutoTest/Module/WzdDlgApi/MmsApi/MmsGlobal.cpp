...
void mms_RegisterDataChangedMsg(tagDataChangeDef  *ptagDataChangeDef, ACSI_NODE *pDevice, const CString &strPath, ULONG nHwnd, ULONG nMsgID)
{
	...
	{//如果是路径，则根据路径提取对象
		...
		if (ptagDataChangeDef->pRefDA == NULL)
		{
			CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("RegisterDataChangedMsg(%s)失败"));
		}
		else
		{
			CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("RegisterDataChangedMsg(%s)成功"));
		}
	}
	else
	{
...
// 			if (ptagDataChangeDef->pRefDA == NULL)
// 			{
// 				CLogPrint::LogFormatString(XLOGLEVEL_TRACE, CXLanguageResourcePp_Mms::g_sLangTxt_RegistDataFail, strPath);//_T("RegisterDataChangedMsg(%s)失败")
// 				break;
// 			}
// 			else
// 			{
// 				CLogPrint::LogFormatString(XLOGLEVEL_TRACE, CXLanguageResourcePp_Mms::g_sLangTxt_RegistDataSuc, strPath);//_T("RegisterDataChangedMsg(%s)成功")
// 			}
// 
// 			CString_to_char(strPath, ptagDataChangeDef->PathName);
// 			ptagDataChangeDef->hMsgWnd = nHwnd;
...
BOOL mms_FireCmdMsg(ACSI_NODE *pDevice, ST_INT nCmdID, ST_INT nCmdIndex)
{
	...
	while (ptagDataChangeDef < ptagDataChangeDefEnd)
	{
		...
		if (ptagDataChangeDef->nCmdID == nCmdID && ptagDataChangeDef->nCmdState== nCmdIndex)
		{
			if (!::IsWindow((HWND)ptagDataChangeDef->hMsgWnd))
			{
				CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("mms_FireCmdMsg(%x, %x,  %s, %d, %d)失败：目标窗口不存在")
