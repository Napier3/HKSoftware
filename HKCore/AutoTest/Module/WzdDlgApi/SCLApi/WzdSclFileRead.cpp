...
void CSclFileRead::ReadAllIedAddr()
{
		...
		scl_subnet = (SCL_SUBNET *)list_find_prev((DBL_LNK *)m_SclInfo.subnetHead, (DBL_LNK *)scl_subnet))
	{
			...
			scl_cap = (SCL_CAP *)list_find_prev((DBL_LNK *)scl_subnet->capHead, (DBL_LNK *)scl_cap))
		{
			...
			//20230621 huangliang 如果IP已经找到就不再查找
			if(pIed->m_strIP != "")
			{
				continue;
			}
				...
				scl_ln = (SCL_LN *)list_find_prev((DBL_LNK *)scl_ld->lnHead, (DBL_LNK *)scl_ln))
			{
				...
				if (fcdalnName == lnName)
				{
						...
						scl_dai = (SCL_DAI *)list_find_prev((DBL_LNK *)scl_ln->daiHead, (DBL_LNK *)scl_dai))
					{
						...
						else if (0 == strcmp(mms_string(&scl_dai->flattened), fcdadoiName))
						{//fcda do desc
							if (scl_dai->Val)
							{
		...
		scl_lntype = (SCL_LNTYPE *)list_find_prev((DBL_LNK *)sclinfo->lnTypeHead, (DBL_LNK *)scl_lntype))
	{
		...
		if (lntype == mms_string(&scl_lntype->id))
		{
			...
			for (scl_do = (SCL_DO *)list_find_last((DBL_LNK *)scl_lntype->doHead);
				...
				scl_do = (SCL_DO *)list_find_prev((DBL_LNK *)scl_lntype->doHead, (DBL_LNK *)scl_do))
			{
				if (0 == strcmp(mms_string(&scl_do->name), doName))
				{
...
#endif
{
	...
	else
	{
		...
		else
		{
			...
			{
#ifdef SCL_USE_MMS_STRING_
				if (0 == strcmp(mms_string(&scl_dotype->id), dotype))
				{
...
#endif
{
	...
	else
	{
		...
		else
		{
			...
			{
#ifdef SCL_USE_MMS_STRING_
				if (0 == strcmp(mms_string(&scl_datype->id), datype))
				{
					CString daFstName = _T("");
					FindDelStr(fcdadodaName, daFstName);
						...
						if (0 == stricmp(lnName, _T("LLN0")))
						{
							...
							if (bGsOut)//ClickType_GOOSE_Outputs == m_iClickType)
							{
								FindGSOutControl(sclinfo, scl_ln, mms_string(&scl_ied->name), 
									apName.GetBuffer(), mms_string(&scl_ld->inst),scl_ld->desc, lnName, pIed->GetGsOuts());//new单个的控制块，并将相关参数MAC、AppID等等写入;
							} 
							/*else*/ 
							if (bSmvOut)//ClickType_SMV_Outputs == m_iClickType)
							{
								FindSVOutControl(sclinfo, scl_ln, mms_string(&scl_ied->name), 
									apName.GetBuffer(), mms_string(&scl_ld->inst),scl_ld->desc, lnName, pIed->GetSmvOuts());//new单个的控制块，并将相关参数MAC、AppID等等写入;
							} 
#else
							if (bGsOut)//ClickType_GOOSE_Outputs == m_iClickType)
							{
								FindGSOutControl(sclinfo, scl_ln, scl_ied->name, 
									apName.GetBuffer(), scl_ld->inst,scl_ld->desc, lnName, pIed->GetGsOuts());//new单个的控制块，并将相关参数MAC、AppID等等写入;
							} 
							/*else*/ 
							if (bSmvOut)//ClickType_SMV_Outputs == m_iClickType)
							{
								FindSVOutControl(sclinfo, scl_ln, scl_ied->name, 
									apName.GetBuffer(), scl_ld->inst,scl_ld->desc, lnName, pIed->GetSmvOuts());//new单个的控制块，并将相关参数MAC、AppID等等写入;
							} 
#endif
							/*else */
			...
			scl_ap = (SCL_AP *)list_find_prev((DBL_LNK *)scl_ied->apHead, (DBL_LNK *)scl_ap))
		{
				...
				scl_ld = (SCL_LD *)list_find_prev((DBL_LNK *)scl_ap->ldHead, (DBL_LNK *)scl_ld))
			{
					...
					scl_ln = (SCL_LN *)list_find_prev((DBL_LNK *)scl_ld->lnHead, (DBL_LNK *)scl_ln))
				{
					...
					{//GOOSE in
#ifdef SCL_USE_MMS_STRING_
						FindGSINControl(sclinfo, scl_ln, mms_string(&scl_ied->name), scl_ied->desc, apName.GetBuffer(), 
							mms_string(&scl_ld->inst),scl_ld->desc, lnName, listied, listextref, (CSclCtrlsGsIn*)pCtrls, scl_path);
					} 
					else  if (scl_IsSmvIns(pCtrls))
					{//sv in (ClickType_SMV_Inputs == m_iClickType)
						FindSVINControl(sclinfo, scl_ln, mms_string(&scl_ied->name), scl_ied->desc, apName.GetBuffer(), 
...
void CSclFileRead::FindGSINControl(SCL_INFO *sclinfo, SCL_LN *sclln, char *iedName, 
								   char *iedDesc, char *apName, char *ldName,char *ldDesc, char *lnName, 
...
void CSclFileRead::FindGSOutControl(SCL_INFO *sclinfo, SCL_LN *sclln, char *iedName, 
									char *apName, char *ldName,char *ldDesc, char *lnName, CSclCtrlsGsOut *pGsOuts)
{
	SCL_GCB		*scl_gcb = NULL;
	SCL_DATASET	*scl_dataset = NULL;
...
void CSclFileRead::FindSVOutControl(SCL_INFO *sclinfo, SCL_LN *sclln, char *iedName, 
									char *apName, char *ldName,char *ldDesc, char *lnName, CSclCtrlsSmvOut *pSmvOuts)
{
	SCL_SVCB	*scl_svcb = NULL;
	SCL_DATASET *scl_dataset = NULL;
...
// 		}
			...
			scl_ap = (SCL_AP *)list_find_prev((DBL_LNK *)scl_ied->apHead, (DBL_LNK *)scl_ap))
		{
			...
			if ((ipos = apStr.Find(_T("--"))) >= 0)
			{
				apStr = apStr.Left(ipos);
#ifdef SCL_USE_MMS_STRING_
				if (apStr != mms_string(&scl_ap->name))// != strncmp(domName, dataSet.GetBuffer(), strlen(domName)))
#else
				if (apStr != scl_ap->name)// != strncmp(domName, dataSet.GetBuffer(), strlen(domName)))
#endif
				{
					continue;
				}
...
//				pCtrlBase->m_strLDdesc = scl_ld->desc;//zhow 增加LD的desc属性2017.07.31
				...
				//??????????????
#ifdef SCL_USE_MMS_STRING_
				if (pCtrlBase->ldName != mms_string(&scl_ld->inst))// != strncmp(domName, dataSet.GetBuffer(), strlen(domName)))
#else
...
#ifdef SCL_USE_MMS_STRING_
		if (pCtrlBase->m_strDataSet != mms_string(&scl_dataset->name))//strncmp(scl_dataset->name, dsStr.GetBuffer(), dsnLen))
#else
		if (pCtrlBase->m_strDataSet != scl_dataset->name)//strncmp(scl_dataset->name, dsStr.GetBuffer(), dsnLen))
#endif
		{
			continue;
		}
					...
					if (0 == stricmp(lnName, _T("LLN0")))
					{
							...
							scl_inputs = (SCL_INPUTS *)list_find_prev((DBL_LNK *)scl_ln->inputsHead, (DBL_LNK *)scl_inputs))
						{
								...
								scl_extref = (SCL_EXTREF *)list_find_prev((DBL_LNK *)scl_inputs->extrefHead, (DBL_LNK *)scl_extref))
							{
								...
								if ((strcmp(pCtrlBase->GetInIedExtName() ,mms_string(&scl_extref->domName))  == 0)&&(pCtrlBase->ldName == mms_string(&scl_extref->ldInst)))
								{
									CString extrefname(_T(""));
									extrefname.Format(_T("%s%s/%s%s%s"), mms_string(&scl_extref->domName), 
										mms_string(&scl_extref->ldInst),	mms_string(&scl_extref->prefix), 
				...
				scl_ld = (SCL_LD *)list_find_prev((DBL_LNK *)scl_ap->ldHead, (DBL_LNK *)scl_ld))
			{
					...
					scl_ln = (SCL_LN *)list_find_prev((DBL_LNK *)scl_ld->lnHead, (DBL_LNK *)scl_ln))
				{
						...
						scl_dataset = (SCL_DATASET *)list_find_prev((DBL_LNK *)scl_ln->datasetHead, (DBL_LNK *)scl_dataset))
					{
						...
						//CString dsStr = dataSet.Mid(dsLen - dsnLen);
#ifdef SCL_USE_MMS_STRING_
						if (pCtrlBase->m_strDataSet != mms_string(&scl_dataset->name))// 0 != strncmp(scl_dataset->name, dsStr.GetBuffer(), dsnLen))
#else
						if (pCtrlBase->m_strDataSet != scl_dataset->name)// 0 != strncmp(scl_dataset->name, dsStr.GetBuffer(), dsnLen))
#endif
						{
							continue;
						}
...
CString CSclFileRead::GetSelStationName()
{
	...
	return m_pSclStation->m_strName;
}
