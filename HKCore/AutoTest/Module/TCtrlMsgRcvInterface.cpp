...
long CTCtrlMsgRcvInterfaceMngr::OnConnectFailed(CExBaseObject *pTestControl)
{
	POS pos = GetHeadPosition();
	CTCtrlMsgRcvInterface *p = NULL;
	...
	{
		p = GetNext(pos);
		p->OnConnectFailed(pTestControl);
	}

	return 0;
}

long CTCtrlMsgRcvInterfaceMngr::OnEngineEvent(CExBaseObject *pTestControl, const CString &strEventID)
{
	POS pos = GetHeadPosition();
	CTCtrlMsgRcvInterface *p = NULL;
	...
	{
		p = GetNext(pos);
		p->OnEngineEvent(pTestControl, strEventID);
	}
