...
//计算时间间隔
long GBS_CalGpsPcTimeGap(const char *pszTimeID1, const char *pszTimeID2)
{
...
long GBS_CalTimeGap(const char *pszTime1, const char *pszTime2)
{
	CGpsPcTime tm1, tm2;
...
long GBS_CalTimeGapCurr(const char *pszTime1)
{
	CGpsPcTime tm1, tm2;
	...
	{
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("[%s] - [%s] = %d ms"), strTime1, strTime, nMs);
	}
...
long GBS_CreateGpsTime(const char *pszTimeID, long nHour, long nMinute, long nS, long nMs, long nUs)
{
	SYSTEMTIME tmSys;
	::GetLocalTime(&tmSys);
	return GBS_CreateGpsDateTime(pszTimeID, tmSys.wYear, tmSys.wMonth, tmSys.wDay, nHour, nMinute, nS, nMs, nUs);
}
