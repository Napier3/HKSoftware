...
BOOL CRptTemplate_GenXmlTool::GenRpt_Xml_Doc_ReportTemplateDef_Head(CRptTemp_TestMacroUiRptDef *pRptDef, CExBaseList *pParent_RptTemp, CGbItemBase *pItemBase)
{
	...
	while (pos != NULL)
	{
		...
		else if (nClassID == MNGRCLASSID_CRPTTEMP_NEWPAGE)
		{
			//换页：2025-2-20
			CRptTemp_NewPage *pNewPage = (CRptTemp_NewPage *)pObj;
			GenRpt_Xml_NewNewPage(pNewReportTemplate);
		}
	}
...
BOOL CRptTemplate_GenXmlTool::GenRpt_Xml_Doc_TextDef_Head(CRptTemp_TextDef *pTextDef, CRptTemp_ReportTemplate *pParent, CGbItemBase *pItemBase)
{
	...
	pNewText->m_strHAlignType = pTextDef->m_strHAlignType;
...
BOOL CRptTemplate_GenXmlTool::GenRpt_Xml_ProcessRptValue(CString &strRptValue, const CString &strDataID, CReport *pReport)
{
	...
	if (pFind == NULL && pRptData == NULL && strDataID.Find(_T("$")) >= 0)
	{
		pValue = (CDvmValue *)pReport->m_pRptDvmDataset->SelectData(strDataID);
	}

	if (pFind == NULL && pRptData == NULL && pValue == NULL)
	{
		if (m_bCurrentItems_Ain)
		{
			...
			if (pMap != NULL)
			{
				...
				if (pFind == NULL)
				{
					pRptData = (CDvmData *)pReport->m_pRptDvmDataset->FindByID(pMap->m_strValue);//此处暂不考虑嵌套的数据，即含$，从CDvmValue取值的
...
CRptTemp_NewPage* CRptTemplate_GenXmlTool::GenRpt_Xml_NewNewPage(CExBaseList *pParent)
{
	CRptTemp_NewPage* pNew = new CRptTemp_NewPage();
	pParent->AddNewChild(pNew);
	return pNew;
}
