...
//	CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("--------------报文缓冲{%d}----------------"), oBufPos.nBuffer1EndPos);
	...
	while (TRUE)
	{
		...
		{//解析成功
			...
			{//解析包含完整的报文，此处的目的在于考虑串口，一次性没有接受全全部的报文
				...
				{
                    CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("解析帧：[%s][%s]    (%d-%d ; %d-%d  : nIndex=%d)")
	...
	{
		//2020-7-14  lijunqing注释，南自发现的问题：文件传输到917761字节，程序崩溃
		//复位数据填充起始位置，却没有复位解析位置，导致从解析位置开始的数据无效
...
void CPpEngineThreadBase::LogErrorParse(long nBeginPos, long nLength, PACKAGEBUFFERPOS *pBufPos)
{
    ...
    CLogPrint::LogFormatString(XLOGLEVEL_DEBUG
             , g_sLangTxt_ErrorMessage.GetString()/*_T("错误报文:(Begin=%d      Len=%d)")*/	, nBeginPos	, nLength);
}
	...
	{//遍历过程临时链表，添加过程
		...
		{//如果有相同的过程在运行，则不处理
            CLogPrint::LogFormatString(XLOGLEVEL_ERROR
                      , g_sLangTxt_ProcessRun.GetString()/*L"过程\"%s\"已经运行"*/
		...
		{//遍历运行过程
			...
			{//过程是否可以运行，即进行过程优先级处理
				if (p->IsProcedureExit())
				{
					...
					{
                    CLogPrint::LogFormatString(XLOGLEVEL_ERROR
                              , g_sLangTxt_ProcessClose.GetString()/*L"----------过程\"%s\"结束----------"*/
...
CCmmChannel* CPpEngineThreadBase::GetCmmChannel(CPpNode *pNode)
{
	...
	if (pCmmChannel == NULL)
	{
        ...
        CLogPrint::LogFormatString(XLOGLEVEL_ERROR
                 , g_sLangTxt_CommunPathUnExist3.GetString()/*L"节点\"%s\"定义的通信通道\"%s\"不存在"*/
		...
		{
            CLogPrint::LogFormatString(XLOGLEVEL_ERROR
                     , g_sLangTxt_CommunPathUnExist2.GetString()/*L"模板定义的通信通道\"%s\"不存在"*/
			...
			{
                CLogPrint::LogFormatString(XLOGLEVEL_ERROR
                         , g_sLangTxt_CommunPathUnExist.GetString()/*L"模板定义的通信通道不存在"*/);
			}
		}
	}
...
void AFX_MSG_CALL CPpEngineThreadBase::OnChannelReceive(WPARAM wParam, LPARAM lParam)
{
	...
	{//2025-3-13 lijunqing 如果不是正常通信状态，则不处理任何报文
		return;
	}
