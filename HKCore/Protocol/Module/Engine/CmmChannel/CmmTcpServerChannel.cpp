...
{
	CLogPrint::LogString(XLOGLEVEL_TRACE, _T("CCmmTcpServerChannel::DisConnect"));
...
BOOL CCmmTcpServerChannel::RemoveClient(CCmmTcpServerClientChannel *pChannel)
{
	...
	while (pos != NULL)
	{
		...
		if (p == pChannel)
		{
			CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("Remove TCP Client %d")
				, pChannel->m_sockaddr.sin_port);
			m_oTcpServerClientChannels.RemoveAt(posPrev);
