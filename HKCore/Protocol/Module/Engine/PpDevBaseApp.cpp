...
CPpDevBaseApp *g_thePpDevBaseApp = NULL;
...
#ifndef _PSX_IDE_QT_
#ifndef not_use_PpDeviceEditLib   //2023-01-01  lijunqing
    if (!LoadPpDeviceEditLib())
	{
		CLogPrint::LogString(XLOGLEVEL_ERROR, g_sLangTxt_PointListEditor/*_T("点表编辑库PpDeviceModelEdit.dll不存在!")*/);
	}
#endif
#endif
...
// 	}
// 
// 	return pEngineData->RunScript(pScript);
// // 	ASSERT (FALSE);
// // 	// 首先做互斥处理
...
// 	}
// 
// 	return pEngineData->CompileScript(pScript);
// }
...
void CPpDevBaseApp::CalExpression(CString &strValue,const CString& strExpr, CDvmDataset *pDataset)
{
	double dValue = 0.0;
	...
	if (oEpExp.ChangeEpExpression(strExpr) == -1)
	{//表达式错误，返回
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("Expression [%s] error"), strExpr.GetString());
		return;
	}
	...
	while (pos != NULL)
	{
		...
		else
		{
			if (pDataset != NULL)
			{
				pFind = (CDvmData *)pDataset->FindByID(pExpVar->m_strVarID);
...
#else
					pExpVar->m_dValue = atof(pFind->m_strValue.GetString());
#endif
				}
				else
				{//变量不存在，返回，保持数据不变
