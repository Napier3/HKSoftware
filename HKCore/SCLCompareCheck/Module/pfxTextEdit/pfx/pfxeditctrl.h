/// *************************************************************************
///                              使用说明
///     这份源代码是本人所写的MFC控件库 (PFX pushall studio MFC foundation
/// classes) 中的一部分，它最早被用于本人所写的一个共享编辑软件LiteEdit中，
/// 该软件得到了很多朋友的肯定，本人对支持 LiteEdit的朋友表示深深的感激。应
/// 很多朋友的要求以及考虑到答谢大家对 LiteEdit的支持，本人现在公布LiteEdit
/// 所使用的编辑控件PfxEditCtrl的源代码，如果您要使用这份代码，您需要同意以
/// 下使用协议，如果你不同意请不要使用这份代码。
///                                                     by 陈建初2005-11-07
///                                                 Email: pushall@21cn.com
/// *************************************************************************
///                              许可协议
/// 1) 本人保留签名权并始终在文件中保留本段内容。
/// 2) 目前这份代码的版本本人称为 1.2版，如果本人编写了这份代码的新版本，则
///        新版本同样不适用于本协议，本人没有责任要公开新版本的代码，同时新
///        版本的版权在任何情况下都归本人所有，除非本人主动放弃。
/// 3) 本人所写的软件LiteEdit虽然也使用了这个控件但LiteEdit不受本协议限制。
/// 4) 如果因为使用这份代码而给您带来了任何的损失，本人不负任何责任。
/// 5) 你可以任意复制、使用、修改、传播这份代码。
/// 6) 你可以将这份代码用于你的个人的、公司／企业的、免费的、商业的等各种形
///       式的应用中。
/// 7) 如果你修改了这份代码，你可以不公开，但如果你公开，则你修改后的版本的
///       版权归大家所有。
/// *************************************************************************
...
#define PFX_EC_CLR_LINE_NUMBER                (COLORREF)RGB(0,0,0)/*::GetSysColor(COLOR_WINDOWTEXT)*///zhow20170627
#define PFX_EC_CLR_TEXT                       (COLORREF)RGB(176,48,96)/*::GetSysColor(COLOR_WINDOWTEXT)*///zhow20170627
...
enum EPfxEditPredefinedKeyClass
{
        ...
        kcPredefinedKeyClassNumber = 90,//原来18个
        kcKeyClassArraySize = 180,//原来36个
...
public:
        ...
        // 逻辑列。如果是主行，它等于零，如果是子行，它等于该子行第一个字符
        // 到主行行首的逻辑偏移量（包含TAB扩展）。
        LONG           nFirstChar;
        ...
        // 如果是主行它是整行（包括子行）的长度，如果是子行它是到主行的行偏
        // 移量。
        LONG           nActLen;
        
        // 一行中选择文本的起始和终点位置，nSelTo可能超过本行长度，表明选择
        // 内容包括换行符。
        LONG           nSelFrom;
        LONG           nSelTo;
...
public:
        ...
        // 如果是插入操作，它是缓冲区所在内存块的指针，否则应该为NULL。
        // 注意：它仅由内存管理模块使用，正常情况下你不应该使用。
        LPVOID         pMemBlock;
};
...
protected:
        ...
        public:
				...
				BOOL LoadStringAutoUtfToMulti(const CString &strText);
...
protected:
	...
	virtual BOOL   OnOpenDocFromStringAutoUtfToMulti(const CString &strText);
...
inline CPfxEditLineInfo* CPfxEditDoc::TextGetLineAt(LONG nIndex) const { ASSERT(nIndex >= 0 && nIndex < m_clsLine.nli); return &m_clsLine.pli[nIndex]; }
...
public:
        ...
        // 查找与当前编辑位置所在符号配对的符号，例如匹配括号功能，标识符对
        // 由CPfxEditTypeInfo中的 m_pBraceStrings指定，一般由语法配置文件读
        // 入。
        BOOL           EditMatchBrace();
...
public:
        // 键盘输入处理函数组，也可以作通用接口模拟键盘输入，所有操作都相对
        // 于当前位置进行。
