...
/////////////////////////////////////////////////////////////////////////////
namespace CPfx { // Pushall Studio MFC Foundation classes - by ChenJianChu
...
{
        m_bDrawHilighted       = true;
        m_bDrawRuler           = true;
        m_bDrawLineNumber      = true;
        m_bDrawLineBreak       = false;
        m_bDrawWhitespace      = false;
...
#ifdef UNICODE
	m_nMappingSize = strText.GetLength(); // 宽字符的长度;
...
BOOL CPfxEditDoc::CPfxEditDocFile::LoadStringAutoUtfToMulti(const CString &strText)
{
	Close();
	...
	long nIndex = strText.GetLength()*1.1 + 4;
	unsigned short* wszGBK = new unsigned short[nIndex];
	m_pMappingAddress = new char[nIndex];
	memset(wszGBK, 0, nIndex);
	iTextLen = MultiByteToWideChar(CP_UTF8, 0, (LPCTSTR)strText, -1, NULL,0); 
	memset(wszGBK, 0, iTextLen * 2 + 2); 
	MultiByteToWideChar(CP_UTF8, 0, (LPCTSTR)strText, -1, (LPWSTR)wszGBK, iTextLen); 
	iTextLen = WideCharToMultiByte(CP_ACP, 0, (LPWSTR)wszGBK, -1, NULL, 0, NULL, NULL); 
	WideCharToMultiByte(CP_ACP, 0, (LPWSTR)wszGBK, -1, (LPSTR)/*(LPCSTR)*/m_pMappingAddress, iTextLen, NULL, NULL); 
	delete []wszGBK;
	m_nMappingSize=strlen((char*)m_pMappingAddress); 
#endif
...
void CPfxEditDoc::OnChangedViewList() 
{
        ...
        // 一定不要将它放在前面，因为它可能会“delete this”从而令到上面的
        // 代码非法操作。
...
BOOL CPfxEditDoc::OnOpenDocFromStringAutoUtfToMulti(const CString &strText)
{
	DeleteContents();
	CPfxEditDocFile file;
	file.LoadStringAutoUtfToMulti(strText);
	FileLoadAnsi(file);
...
//     CException* e - 异常指针。
// 返回：
//     如果该异常无法识别，返回PFX_EE_UNKNOWN_ERROR;
// 备注：
/////////////////////////////////////////////////////////////////////////////
...
// 参数：
// 返回：
//     如果当前行已经是第一个一主行内的行反回PFX_EC_INVALID_LINE，否则返回上
//     一主行行序号。
// 备注：
/////////////////////////////////////////////////////////////////////////////
...
// 参数：
// 返回：
//     如果当前行已经是最后一主行内的行反回PFX_EC_INVALID_LINE，否则返回下一
//     主行行序号。
// 备注：
/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
// CPfxEditView...
IMPLEMENT_DYNCREATE(CPfxEditView, PfxEditViewPRentClass)
...
BOOL CPfxEditView::OnOpenDocFromStringAutoUtfToMulti(const CString &strText)
{
	ASSERT (m_pdb != NULL);

	m_pdb->OnOpenDocFromStringAutoUtfToMulti(strText);
	Invalidate(FALSE);
...
{
        // 如果下面的代码修改了视图的私有数据，无论新的字体据柄是否与旧的相
        // 同都要更新。
        if (hFont == NULL)
        {
...
//                               CPfxEditView::CPfxEditViewPaintContext& pc)
// 作用：
//     分析一给定的字符串，如果它是关键字就用相应的颜色输出，否则用普通的文
//     本颜色输出。
// 参数：
//     LPCTSTR pData - 字符串地址。
//     LONG nLen     - 字符串长度。
...
//     CPfxEditViewPaintContext& pc - 与显示相关的参数。
// 返回：
//     返回NULL表示不需要继续分析和显示，返回非NULL表示应该从该地址起继续分
//     析下面的内容。
// 备注：
/////////////////////////////////////////////////////////////////////////////
                                          ...
                                          CPfxEditView::CPfxEditViewPaintContext& pc)
{
        ...
        // 如果选择部分的第一个字符不是本行的第一个字符，就要计算选择部分第
        // 一个字符所在的水平坐标，这个坐标用于绘制，另外还要计算与该字符对
        // 应的逻辑列数。
...
// 参数：
// 返回：
//     返回与指定行中某列相对应的文本坐标，该坐标从文本影像的水平方向起点开
//     始计算。
// 备注：
...
// 参数：
//     LONG yTextPos - 是相对于文本影像垂直方向起始点的坐标。
// 返回：
//     返回相应的行序号。
// 备注：
/////////////////////////////////////////////////////////////////////////////
...
// 参数：
//     LONG nLine     - 行序号。
//     LONG rxTextPos - 是相对于文本影像水平方向起始点的坐标，函数返回后该地
//                      方存放与返回值（列序号）相对应的相对于文本影像水平方
//                      向起始点的坐标，该坐标通常用于定位插入符。
// 返回：
...
/////////////////////////////////////////////////////////////////////////////
// 使用空格或TAB填充指定的缓冲区。
// LPTSTR pBuffer - 该缓冲区必须足够大，不小于
//                  FillBufferWithWhitespacesCalcBufferSize()返回的数值大小。
...
#else
        LPCTSTR pCur,pEnd;
		for (pCur = pData, pEnd = pData + cch; pCur < pEnd; )
        {
                if (!PFX_EC_IS_LEAD_BYTE(*pCur))
                {
                        pCur++;
...
//     LONG& rnLogChar - 与第一个字符对应的逻辑列数，这个数值主要是用来计
//                       算制表符的宽度。
//     CDC* pDC        - CDC* 指针。
// 返回：
//     成功返回该字符串的宽度，否则返回零。
...
// 作用：
//     将给定的行中的的逻辑字符位置（TAB扩展）转换成绝对字符位置（指针读写
//     位置）。
// 参数：
//     LONG nLine          - 物理行索引。
...
// 作用：
//     给定一行号和列数，检查该指定的列是否在一个MBCS字符的后半部分，如果是
//     则返回下一个字符的位置。
// 参数：
// 备注：
/////////////////////////////////////////////////////////////////////////////
...
//     以指定字符数计算换行，并实施换行操作。
// 参数：
//     LONG nWidth - 以逻辑字符数指出换行宽度，逻辑字符数是指将制表符扩展后
//                   的字符数。
// 备注：
/////////////////////////////////////////////////////////////////////////////
...
//     LONG cchFindWhat      - 查找对象的长度，如果等于-1表示由函数自动计算
//                             长度，这时应以NULL结尾。
...
BOOL CPfxEditView::MatchBraceGetHint(CPfxEditView::CPfxEditViewBraceHint& hint)
{
        ...
        // 一个单词进行比较。
		LONG i;
        for (i = 0; !bResult && (i < PFX_EC_SIZEOF(arrBOOL)); i++)
        {
