...
static BSTR G_BSTRKEY_inst  = L"inst";
static BSTR G_BSTRKEY_desc  = L"desc";
static BSTR G_BSTRKEY_LDEVICE  = L"ldevice";
static BSTR G_BSTRKEY_DEVICE_MODEL  = L"device-model";
static BSTR G_BSTRKEY_NODE  = L"node";
...
long mms_XmlWrite(ACSI_NODE *pDevice, CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oParent, BOOL bOblyGetLd)
{
	...
	//oXMLDoc.AppendChild(*oElement);  //2023-1-30  lijunqing  参考：CBaseObject::SaveXmlFile
...
{
	CXmlRWElementBase* oElement = NULL;
    oElement = oXMLDoc.CreateElement(strElement,&oParent);
	...
	for (int i=0; i<nNum; i++)
	{
        ...
        oSubElement = oXMLDoc.CreateElement(G_BSTRKEY_DATA,oElement);
...
long mms_XmlWrite(const CString &strElement,tagDA *pDA,CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oParent)
{
    ...
    //xml_SetAttributeValue((X_TO_BSTR)("name-utf8"), oElement, strName);
	strType = pDA->DataType;
    xml_SetAttributeValue(G_BSTRKEY_DATATYPE, *oElement, strType);

	strText = mms_ParseUnit(pDA->DAUnite.Value.i);
    xml_SetAttributeValue(G_BSTRKEY_UNIT, *oElement, strText);
#ifdef _use_mms_string
	strText = mms_string(&pDA->DAUnite.mmsName);
#else
	strText = pDA->DAUnite.mmsName;
#endif
    xml_SetAttributeValue(G_BSTRKEY_UNITPATH, *oElement, strText);
	long i = pDA->DAUnite.Value.i;
    xml_SetAttributeValue(G_BSTRKEY_UNITVALUE, *oElement, i);

	strText = mms_GetDAValue(pDA);
    xml_SetAttributeValue(G_BSTRKEY_VALUE, *oElement, strText);
    xml_SetAttributeValue(G_BSTRKEY_DEFVALUE, *oElement, strText);
...
#endif
    xml_SetAttributeValue(G_BSTRKEY_VALUEPATH/*(X_TO_BSTR)("value-path")*/, *oElement, strText);
...
// 			xml_SetAttributeValue((X_TO_BSTR)("sub-value-num"), oElement, lData);
// 
// 			CString str;
...
// 				xml_SetAttributeValue((X_TO_BSTR)(str), oElement, strText);
...
long mms_XmlWrite_SubDA(const CString &strElement,tagDA *pDA,CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oParent)
{
    ...
    xml_SetAttributeValue(G_BSTRKEY_DATATYPE, *oElement, strText);
	...
	strText = pDA->DAValue.mmsName;
#endif
    xml_SetAttributeValue(G_BSTRKEY_ID, *oElement, strText);
...
#endif
    xml_SetAttributeValue(G_BSTRKEY_NAME, *oElement, strText);
	...
	strText = pDA->DAValue.mmsName;
#endif
    xml_SetAttributeValue(G_BSTRKEY_PATH, *oElement, strText);
...
// 	docPtr.CreateInstance(__uuidof(MSXML2::DOMDocument));
#ifndef _PSX_IDE_QT_
    //2022-2-26  lijunqing LINUX, 调用函数会导致文件死锁
	ClearFileReadOnlyAttr(strFile);
#endif
...
// 	ASSERT(pRCB!=NULL);
	...
	{
		oNode = oNodes->GetChild(nIndex);
        xml_GetAttibuteValue(G_BSTRKEY_NAME/*(X_TO_BSTR)("name")*/, *oNode, strText);
        strcpy(pRCB[nIndex].rcbName,strText.GetString());

		strText.Empty();
        xml_GetAttibuteValue(G_BSTRKEY_ID/*(X_TO_BSTR)("name")*/, *oNode, strText);
        strcpy(pRCB[nIndex].RptID,strText.GetString());
	}
...
//	ASSERT (pNode!=NULL);
	...
	strName.TrimRight();
#ifdef _use_mms_string
    mms_str_set2(&pJournal->logRef, strName.GetString(), strName.GetLength());
#else
...
//	mms_str_set2(&pJournal->Descri, strID, strID.GetLength());
    mms_str_set2(&pJournal->Name, strID.GetString(), strID.GetLength());
#else
...
#ifdef _use_mms_string
    mms_str_set2(&pJournal->Dataset, strText.GetString(), strText.GetLength());
#else
...
long mms_XmlRead(LD_NODE *pLDevice, CXmlRWNodeBase &pNode)
{
    ...
    strcpy(pLDevice->LDName, strName.GetString());

    xml_GetAttibuteValue(G_BSTRKEY_NAME/*(X_TO_BSTR)("name")*/, pNode, strName);
    strcpy(pLDevice->LDDesc, strName.GetString());
...
// 		CString strText;
// 		xml_GetAttibuteValue((X_TO_BSTR)("id"),child, strText);
// 		strcpy(pLD_INFO->DataSetID,strText);
// 		//xml_GetAttibuteValue((X_TO_BSTR)("name"), pLD_INFO->DaSetName);
// 		pLDevice->numDaSet++;
// 	}
...
long mms_XmlReadData(LD_DATASET_INFO *pLD_INFO, CXmlRWNodeBase &pNode)
{
    ...
    strcpy(pLD_INFO->DataSetID, strText.GetString());

    xml_GetAttibuteValue(G_BSTRKEY_NAME/*(X_TO_BSTR)("name")*/,pNode, strText);
    strcpy(pLD_INFO->DaSetDesc, strText.GetString());
	...
	for (nIndex=0; nIndex<nCount; nIndex++)
	{
        ...
        if (strNodeName ==  G_BSTRKEY_DATA/*G_BSTRKEY_DATA*/)
		{
			nDataCount++;
		}
	}
	...
	for (nIndex=0; nIndex<nCount; nIndex++)
	{
        ...
        if (strNodeName ==  G_BSTRKEY_DATA/*(X_TO_BSTR)("data")*/)
		{
        ...
        else if (strNodeName == G_BSTRKEY_BRCB/*(X_TO_BSTR)("brcb")*/)
		{
...
// 			continue;
// 
// 		(X_TO_BSTR) bstr = pElement->GettagName();
...
//	ASSERT (pNode!=NULL);
	...
	strName.TrimRight();
#ifdef _use_mms_string
    mms_str_set2(&pDA->DADescri.Value.string, strName.GetString(), strName.GetLength());
#else
	memcpy(pDA->DADescri.Value.string, strName, strName.GetLength());
#endif

    xml_GetAttibuteValue(G_BSTRKEY_ID/*(X_TO_BSTR)("id")*/, pNode, strID);
#ifdef _use_mms_string
    mms_str_set2(&pDA->DAValue.mmsName, strID.GetString(), strID.GetLength());
#else
	...
	{
#ifdef _use_mms_string
        mms_str_set2(&pDA->DADescri.mmsName, strID.GetString(), strID.GetLength());
#else
		memcpy(pDA->DADescri.mmsName, strID, strID.GetLength());
#endif
	}

    xml_GetAttibuteValue(G_BSTRKEY_UNITPATH/*(X_TO_BSTR)("unit-path")*/, pNode, strText);
#ifdef _use_mms_string
    mms_str_set2(&pDA->DAUnite.mmsName, strText.GetString(), strText.GetLength());
#else
	memcpy(pDA->DAUnite.mmsName, strText, strText.GetLength());
#endif

	long lData = 0;
    xml_GetAttibuteValue(G_BSTRKEY_UNITVALUE/*(X_TO_BSTR)("unit-value")*/, pNode, lData);
	pDA->DAUnite.Value.i = lData;

    xml_GetAttibuteValue(G_BSTRKEY_DATATYPE/*(X_TO_BSTR)("data-type")*/, pNode, strType);
    ...
    memcpy(pDA->DataType, strType.GetString(), strType.GetLength());
	mms_InitDA_DataType(pDA);
    xml_GetAttibuteValue(G_BSTRKEY_VALUE/*(X_TO_BSTR)("value")*/, pNode, strValue);
    xml_GetAttibuteValue(G_BSTRKEY_DEFVALUE/*(X_TO_BSTR)("default-value")*/, pNode, strText);
	...
	strText.Empty();
	strStep.Empty();
    xml_GetAttibuteValue(G_BSTRKEY_MAX/*(X_TO_BSTR)("max")*/, pNode, strMax);
    xml_GetAttibuteValue(G_BSTRKEY_MIN/*(X_TO_BSTR)("min")*/, pNode, strMin);
    xml_GetAttibuteValue(G_BSTRKEY_STEPSIZE/*(X_TO_BSTR)("min")*/, pNode, strStep);
...
long mms_XmlRead_SubDA(tagDA *pDA, CXmlRWNodeBase &pNode)
{
    ...
    xml_GetAttibuteValue(G_BSTRKEY_DATATYPE/*(X_TO_BSTR)("data-type")*/, pNode, strType);
    strcpy(pDA->DataType,strType.GetString());
	mms_InitDA_DataType(pDA);

	//path和ID是同样的值
    xml_GetAttibuteValue(G_BSTRKEY_ID/*(X_TO_BSTR)(_T("path"))*/, pNode, strTemp);
#ifdef _use_mms_string
    mms_str_set(&pDA->DAValue.mmsName, strTemp.GetString());
#else
	strcpy(pDA->DAValue.mmsName, strTemp);
#endif

    xml_GetAttibuteValue(G_BSTRKEY_NAME/*(X_TO_BSTR)(_T("name"))*/, pNode, strTemp);
#ifdef _use_mms_string
    mms_str_set(&pDA->DADescri.Value.string, strTemp.GetString());
#else
	strcpy(pDA->DADescri.Value.string, strTemp);
#endif

    xml_GetAttibuteValue(G_BSTRKEY_VALUE/*(X_TO_BSTR)(_T("value"))*/, pNode, strTemp);
	...
	if (XmlRootptr != NULL)
	{
		...
		//while (childList->raw_nextNode(&child)==S_OK)
		{
			...
			{				
				...
				if (strName.IsEmpty())
                    xml_GetAttibuteValue(G_BSTRKEY_inst, *child, strName);
                xml_GetAttibuteValue(G_BSTRKEY_desc, *child, strDesc);
				...
				for (int i=0;i<pDevice->numLD;i++)
				{
                    ...
                    if (strstr(pLD->LDName,strName.GetString()))
					{	
						bMap = TRUE;
						break;
					}
				...
				{	// 匹配逻辑设备描述
                    ...
                    strcpy(pDevice->devName, strDesc.GetString());
					break;
				}
			}
		}	
...
{
	CXmlRWNodeBase* nodeptr;
    nodeptr = pNode.GetChildNode((CString)(LDNODE_PATH));
	...
	//while (childList->raw_nextNode(&child)==S_OK)
	{
		...
		if (strText==_T("LDevice"))
		{
			...
			for (int i=0;i<pDevice->numLD;i++)
			{
				LD_NODE *pLD = &pACSINode->LD[i];

				// 匹配逻辑设备类型
                if (strstr(pLD->LDName,strName.GetString()))
				{
                    strcpy(pLD->LDDesc,strDesc.GetString());
                    nodeptr = child->GetChildNode((CString)(DATASET_PATH));
					if (nodeptr)
					{
	...
	//while (childList->raw_nextNode(&child)==S_OK)
	{
		...
		if (strText==_T("DataSet"))
		{
			...
			for (int i=0;i<pLD->numDaSet;i++)
			{
                ...
                if (strstr(pLD_INFO->DataSetID,strName.GetString()))
				{
                    strcpy(pLD_INFO->DaSetDesc,strDesc.GetString());
...
BOOL mms_GetXml(LD_NODE *pLDevice, CString &bstrXml)
{
	...
	if (pRWDoc != NULL)
	{
        ...
        oElement = pRWDoc->CreateElement(G_BSTRKEY_LDEVICE,oRootElement);
...
BOOL mms_GetXml(LD_DATASET_INFO *pDataset, CString &bstrXml)
{
	...
	if (oXMLDoc != NULL)
	{
        //oElement = oXMLDoc->createElement((X_TO_BSTR)("dataset"));
		//oXMLDoc->appendChild(oElement);
...
void mms_XmlWriteNode(ACSI_NODE *pDevice, CXmlRWDocBase &oXMLDoc,CXmlRWElementBase &oParent)
{
    ...
    oElement = oXMLDoc.CreateElement(G_BSTRKEY_DEVICE_MODEL,&oParent);
	oXMLDoc.AppendChild(*oElement);
...
{
	CXmlRWElementBase* oElement = NULL;
    oElement = oXMLDoc.CreateElement(G_BSTRKEY_LDEVICE,&oParent);
...
{
	CXmlRWElementBase* oElement = NULL;
    oElement = oXMLDoc.CreateElement(G_BSTRKEY_NODE,&oParent);
