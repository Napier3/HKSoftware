...
void Global_LogString( const char *strFormat, ...)
{
	...
	int nLenth = 0;
	utf8_to_gbk((const unsigned char*)g_strLogGloalDest_CharGSP,strlen(g_strLogGloalDest_CharGSP),&pTmpStringGBK,&nLenth);
...
ST_RET CDLT860Api::Client61850_StartDevRpt(CFuncParamEx *pFuncParaEx)
{
	...
	*rcbItem = 0;
	...
	{
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("启动报告：报告控制块【%s】已经启动"),CString(pLD_INFO->RCB[pLD_INFO->curRcbIndex-1].rcbName));
		return SD_SUCCESS;
	}
	...
	if (bStartRpt)
	{
		if (bRet)
		{
			CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("启动报告[%s]完成"), str_rcbItem);
			mms_UpdateDatasetState(pLD_INFO);
		}
		else
		{
			CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("启动报告[%s]失败"), str_rcbItem);
		}
	}
	else
	{
...
ST_RET CDLT860Api::Client61850_UploadFile(CFuncParamEx *pFuncParaEx)
{
	...
	{
		CLogPrint::LogFormatString (XLOGLEVEL_TRACE, "文件路径 %s 不存在;",strLocalFile);
		return SD_FAILURE;
	}
...
ST_RET CDLT860Api::Client61850_WriteData(CFuncParamEx *pFuncParaEx)
{
	...
	for (k=0; k<nDaNum; k++)
	{
		...
		if (!pRemoteDev->WriteSEValue(pLD->LDName,pDA))
		{
			...
			strInfor.Format(_T("写数据【%s】【%s】=【%s】失败."), mms_GetDADesc(pDA), mms_string(&pDA->DAValue.mmsName), mms_GetDAValue(pDA));
									  ...
									  , ST_INT bEnd, ST_INT nEndType, CTime &tmEnd, ST_INT nEntryNum)
{
	...
	if (pDevice == NULL)
	{
 	...
 	{
 		CLogPrint::LogFormatString(XLOGLEVEL_ERROR,_T("初始化读日志数目出错,对应日志索引未查找到,nDeviceIndex=%ld;nLDeviceIndex=%ld;strJournalName=%s;"),nDeviceIndex,nLDeviceIndex,strJournalName);
 	}
