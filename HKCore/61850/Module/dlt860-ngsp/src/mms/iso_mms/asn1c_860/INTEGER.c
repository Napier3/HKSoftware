...
int
asn_imax2INTEGER_All(INTEGER_t *st, intmax_t value ,int bSigned) {//参数3指定是否包含符号  zhouhj 20220107
    uint8_t *buf, *bp;
    uint8_t *p;
    uint8_t *pstart;
    uint8_t *pend1;
    ...
    buf = (uint8_t *)(long *)MALLOC(sizeof(value));//创建8字节内存空间
    if(!buf) return -1;

    if(*(char *)&littleEndian) {//小端在前的情况下
        pstart = (uint8_t *)&value + sizeof(value) - 1;//pstart为最后一个字节
        pend1 = (uint8_t *)&value;//pend1为第一个字节
        add = -1;//反向查找
    } else {
    ...
    for(p = pstart; p != pend1; p += add) {
        if(*p == 0x00){//从最高位开始查找,找到第一个不为0的字节
                continue;
        }
        break;
    }
	...
	if (bSigned)//如果为有符号数据
	{
		if (((*p)>127)&&(p != pstart))//zhouhj20220107 第一个不为0的字节的最高位为1时,需要往前扩充一位
		{
			(*bp++) = 0x0;
		}
	}

    for(pend1 += add; p != pend1; p += add)//从第一个不为0的字节开始拷贝内容,一直拷贝到最低位
        *bp++ = *p;
...
asn_imax2INTEGER(INTEGER_t *st, intmax_t value) {
    return asn_imax2INTEGER_All(st,value,1);
}
...
asn_ulong2INTEGER(INTEGER_t *st, unsigned long value) {
    return asn_imax2INTEGER_All(st, value,0);
}
