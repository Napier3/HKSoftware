...
ST_RET My_Pare_RptID_LD(LD_NODE *ptLD, ST_INT *numDA, tagDA **pTagDA, ST_INT *nDsIndex, const char *pzfRptID)
{
	...
	for (i=0;i<ptLD->numDaSet;i++)
	{
		...
		for (k=0; k<pLD_INFO->numRcb;k++)
		{
            ...
            const char *pszRptName = strRptName.GetString();

            if (strstr(pzfRptID, pszRptName) || strstr(pszRptName, pzfRptID))
			{
            ...
            const char *pszRptID = strRptID.GetString();

            if (strcmp(pzfRptID, pszRptID) == 0)
			{
				*numDA = ptLD->DataInfo[i].numData;
				*pTagDA = ptLD->DataInfo[i].DataSet;
				*nDsIndex = i;
...
BOOL GetMmsDataRef(const char *dataItemRefs,const char *fc,CString &strMmsDataRef)
{
    ...
    strMmsDataRef.Format(_T("%s$%s$%s"),strTmp2.Left(nIndex).GetString(),fc,strTmp2.Mid(nIndex+1).GetString());
...
//		ASSERT(GspMmsValue_getType(pActDataItem) == Data_PR_int32u);
	...
	else if (strcmp(pTagDA->DataType ,_T("BIT_STR")) == 0)
	{
        ...
        mms_str_set(&pTagDA->DAValue.Value.string, strTmp.GetString());
	}
...
BOOL Gsp_SetRptValue(tagDA *pTagDA,long nNumDA,GspMmsValue *dataItem,const char *dataItemRefs,const char *fc,unsigned reasonCode,ST_INT &nDA_Index)
{
	...
	{
        if (strcmp(mms_string(&pTagDA[nIndex].DAValue.mmsName),strMmsDataRef.GetString()) == 0)
		{
...
// 				printf("********************** 报告条目 %d *****************************\n", dataID);
                ...
                if (!Gsp_SetRptValue(pTagDA,numDA,dataItem,strTmpRef.GetString(),fc,reasonCode,nDA_Index))
				{
					continue;
				}
        ...
        PostMessage((HWND)(ptACSINode->hRptMsgWnd), ptACSINode->nRptMsg, wParam, lParam);
	}
