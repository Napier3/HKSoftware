...
#include "../61850ClientAPI/mms_global_include.h"
#include "../MmsApi.h"  //ok
...
void* CMSCallBackProc_Data(void* lpParameter)
#else
static DWORD WINAPI CMSCallBackProc_Data(void* lpParameter)
#endif
{
...
CRemoteDev* CDLT860Api::GetRemoteDev(const long nDevIndex)
{
    ...
    return (CRemoteDev*)pRemoteDevs->GetAtIndex(nDevIndex);
}
...
#ifdef _PSX_QT_LINUX_
        pthread_create(&pFuncParaEx->m_hThread_Proc_Data, 0, CMSCallBackProc_Data, (void*)pFuncParaEx);
#else
        HANDLE h = CreateThread(NULL,1024, CMSCallBackProc_Data, pFuncParaEx, 0,NULL);
...
void CDLT860Api::ProgDlg_StopProgress(long nDeviceIndex)
{
    ...
    for (nLDIndex=0; nLDIndex<nLdCount; nLDIndex++)
    {
        ...
        for (int i=0; i<pLD->numDaSet; i++)
        {
            ...
            if (!CheckCurSelIsSetting(nDeviceIndex,nLDIndex,i))
			   ...
			   mms_SetReportFlag(&pDevice[nDeviceIndex],nLDIndex,i,TRUE);
        }
    }
}
...
void CDLT860Api::InitCallReportFuncParas(ST_INT nDeviceIndex, ST_INT nLDIndex, ST_INT nDsIndex, BOOL bStart)
{
    ...
    if (mms_GetDsDaDatasNum(pRemoteDev->GetAcsiNode(), nLDIndex, nDsIndex) <= 0)
    {
