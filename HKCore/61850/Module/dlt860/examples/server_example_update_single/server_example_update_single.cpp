...
int main(int argc, char** argv) {
    ...
    while (running) {
        ...
        try{
            ...
            auto singleItemValue = GspMmsValue_newBoolean(flag);
            milliseconds ms = duration_cast< milliseconds >( system_clock::now().time_since_epoch());
            auto t = GspMmsValue_newUtcTimeByMsTime(ms.count());

            // 要模拟的DA的信息
            const char* singleRefs = "P_L2211DPROT/LLN0.FuncEna1.stVal";
            const char* singleFC = "ST";
            GspIedServer_updateReferValue(iedServer, singleRefs, singleFC, singleItemValue);
            std::cout << "单点数据变动测试:" << singleRefs << ":" << singleFC << " 数据为: " << flag << std::endl;

            // 休眠时间
            GspThread_sleep(3000);

            // 资源释放
            GspMmsValue_delete(singleItemValue);
            GspMmsValue_delete(q);
            GspMmsValue_delete(t);
