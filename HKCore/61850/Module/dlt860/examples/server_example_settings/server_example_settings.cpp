...
int main(int argc, char** argv) {
    ...
    // 编辑定值组切换回调
    GspIedServer_setEditSettingGroupChangedHandler(iedServer, nullptr,
                                                   [](void* parameter, GspSettingGroupControlBlock* sgcb, uint8_t newEditSg, GspClientConnection connection){
        printf("这里处理定值组改变，处理正常返回true，处理异常返回false\n");
        // connection 为null 说明是服务端自身触发的回调，可以酌情忽略
        return true;
    },
    (void *)"custom param");

    // 激活定值组回调
    GspIedServer_setActiveSettingGroupChangedHandler(iedServer, nullptr,
                                                     [](void* parameter, GspSettingGroupControlBlock* sgcb, uint8_t newActSg, GspClientConnection connection)
    {
        printf("这里处理激活定值组，处理正常返回true，处理异常返回false\n");
        // connection 为null 说明是服务端自身触发的回调，可以酌情忽略
    ...
    // 编辑定值回调
    GspIedServer_setEditSettingGroupConfirmationHandler(iedServer, nullptr,
                                                        [](void* parameter, GspSettingGroupControlBlock* sgcb, uint8_t editSg){
        printf("这里处理编辑定值组确认回调，处理正常返回true，处理异常返回false\n");
        // connection 为null 说明是服务端自身触发的回调，可以酌情忽略
        return true;
    },
    (void *)"customm param"
