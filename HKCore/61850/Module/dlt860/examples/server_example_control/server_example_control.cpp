...
int main(int argc, char** argv) {
    ...
    // 设置控制回调
    GspServer_setFunctionHookFunc(iedServer, [](GspIedServer server, GspClientConnection clientConn, void* request, int type, void* param, void **response){
        switch (type)
        {
        case GSP_Select:
        {
            auto requestData = static_cast<SelectRequestPDU_t *>(request);
            printf("refer is %s\n", (const char *)requestData->reference.buf);
        }
            /* code */
            break;
        case GSP_SelectWithValue:
        {
            auto requestData = static_cast<SelectWithValueRequestPDU_t *>(request);
            printf("refer is %s\n", (const char *)requestData->reference.buf);
        ...
        case GSP_Cancel:
        {
            auto requestData = static_cast<CancelRequestPDU_t *>(request);
            printf("refer is %s\n", (const char *)requestData->reference.buf);
        ...
        case GSP_Operate:
        {
            auto requestData = static_cast<OperateRequestPDU_t *>(request);
            printf("refer is %s\n", (const char *)requestData->reference.buf);
        }
            break;
        case GSP_CommandTermination:
        {
            auto requestData = static_cast<CommandTerminationRequestPDU_t *>(request);
            printf("refer is %s\n", (const char *)requestData->reference.buf);
        }
            break;
        case GSP_TimeActivatedOperate:
        {
            auto requestData = static_cast<TimeActivatedOperateRequestPDU_t *>(request);
            printf("refer is %s\n", (const char *)requestData->reference.buf);
        }
            break;
        case GSP_TimeActivatedOperateTermination:
        {
            auto requestData = static_cast<TimeActivatedOperateTerminationRequestPDU_t *>(request);
            printf("refer is %s\n", (const char *)requestData->reference.buf);
        }
            break;
        default:
