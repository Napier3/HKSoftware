...
void CSttDevice::InitDevice()
{
	...
	for(int nModuleUIndex=0;nModuleUIndex<g_theDeviceSystemParameter->m_nModuleCount_U;nModuleUIndex++)
	{
		...
		g_theSttDeviceStatus->m_oModule[g_theSttDeviceStatus->m_nAnalogCount].m_nModuleIndex=oModule->m_oModuleAttr.m_nModuleIndex;
...
void CSttDevice::OnTimer()
{
	...
	for(int nModuleIndex=0;nModuleIndex<g_theDeviceSystemParameter->m_nModuleCount;nModuleIndex++)
	{
		...
		if(pModuleAttr->m_nModuleType==STT_MODULE_TYPE_VOLT)
			AnalysisDrvSysInfo(m_pSysInfo->stBack[nModulePos-1],nModulePos,TRUE);
		else if(pModuleAttr->m_nModuleType==STT_MODULE_TYPE_CURRENT)
			AnalysisDrvSysInfo(m_pSysInfo->stBack[nModulePos-1],nModulePos);
	}
}
