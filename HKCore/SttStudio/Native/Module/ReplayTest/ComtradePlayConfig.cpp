...
void CComtradePlayConfig::GetMax92OutputValue( double &fUMax,double &fIMax )
{
...
// 	fUMax = g_oLocalSysPara.m_fAC_VolMax  * SQRT2;//120万V;
// 	fIMax = g_oLocalSysPara.m_fAC_CurMax  * SQRT2;//12万A

	float fUMaxRateValue = 0.0f , fIMaxRateValue = 0.0f;

	if(pIecCfgSysParas->m_nPrimParas == 0)//二次值
	{
		CStringArray strUPrimRate,strIPrimRate;

		strUPrimRate.Add(_T("U1-abc") );
		strUPrimRate.Add(_T("U2-abc") );

		strIPrimRate.Add(_T("I1-abc") );
		strIPrimRate.Add(_T("I2-abc") );
		float fValue = 0.0f;

		for(int nIndex = 0; nIndex < strUPrimRate.GetSize(); nIndex++)
		{
			strPrimRate = strUPrimRate.GetAt(nIndex);
			CIecCfgPrimRate *pIecCfgPrimRate = (CIecCfgPrimRate *)pIecCfgPrimRates->FindByID(strPrimRate);
			fValue = pIecCfgPrimRate->m_fPrimValue * 1000 / pIecCfgPrimRate->m_fSecondValue;

			if(fValue > fUMaxRateValue )
			{
				fUMaxRateValue = fValue;
			}
		}

		fValue = 0.0f;
		for(int nIndex = 0; nIndex < strIPrimRate.GetSize(); nIndex++)
		{
			strPrimRate = strIPrimRate.GetAt(nIndex);
			CIecCfgPrimRate *pIecCfgPrimRate = (CIecCfgPrimRate *)pIecCfgPrimRates->FindByID(strPrimRate);
			fValue = pIecCfgPrimRate->m_fPrimValue / pIecCfgPrimRate->m_fSecondValue;

			if(fValue > fIMaxRateValue )
			{
				fIMaxRateValue = fValue;
			}
		}

		fUMax = fUMax/fUMaxRateValue;
		fIMax = fIMax/fIMaxRateValue;
	}
