...
#define OVERLOAD_FAULT_TYPE_Ia1                                                    0
#define OVERLOAD_FAULT_TYPE_Ib1                                                    1
#define OVERLOAD_FAULT_TYPE_Ic1                                                    2
#define OVERLOAD_FAULT_TYPE_Ia2                                                    3
#define OVERLOAD_FAULT_TYPE_Ib2                                                    4
#define OVERLOAD_FAULT_TYPE_Ic2                                                    5

#define OVERLOAD_TESTNAME_LEN	64

typedef struct tmt_overload_paras 
{
	...
	char m_pszTestName[OVERLOAD_TESTNAME_LEN];
...
public:
	virtual void init()
	{
		...
		m_nFaultCurChannel=OVERLOAD_FAULT_TYPE_Ia1;
		m_fFaultCur = 0;
		...
		memset(m_pszTestName, 0, OVERLOAD_TESTNAME_LEN);
	} 
}tmt_OverLoadParas;


typedef struct tmt_overload_paras_ex :public tmt_OverLoadParas
{
public:
...
typedef struct tmt_overload_results
{
	float m_fTripTime;
	long m_nAct;
...
}tmt_OverLoadTestResults;

typedef struct tmt_overload_test : public TMT_PARAS_HEAD
{
public:
	tmt_OverLoadParasEx   m_oOverLoadParas ;
	tmt_OverLoadTestResults m_oOverLoadTestResults;
	double m_fTsetCal;

	virtual void init()
	{
		init_head(); 
		strcpy(m_pszMacroID, STT_MACRO_ID_OverLoadTest);
	...
	tmt_overload_test()	{init();}
	virtual ~tmt_overload_test(){}
}tmt_OverLoadTest;
