...
{
	memset(&pParas, 0, sizeof(tmt_DC_OverLoadParas));
	pParas->init();
}

void stt_init_paras_ex(tmt_DC_OverLoadParasEx *pParas)
{
	memset(&pParas, 0, sizeof(tmt_DC_OverLoadParasEx));
	pParas->init();
}

void stt_init_results(tmt_DC_OverLoadTestResults *pResults)
{
	memset(&pResults, 0, sizeof(tmt_DC_OverLoadTestResults));
	pResults->init();
}

void stt_xml_serialize_binary_out(tmt_DC_OverLoadParas *pParas, CSttXmlSerializeBase *pXmlSierialize)
{
#ifdef NOT_USE_XLANGUAGE
...
void stt_xml_serialize(tmt_DC_OverLoadParas *pParas, CSttXmlSerializeBase *pXmlSierialize)
{
	...
	pXmlSierialize->xml_serialize(/* "热过负荷时间常数" */ g_sLangTxt_Native_HotOverloadTimeConst.GetString(), "_Time", "", "number", pParas->m_fTime);
	pXmlSierialize->xml_serialize(/* "热过负荷基数电流" */ g_sLangTxt_Native_HotOverloadBaseCurrent.GetString(), "BaseCur", "A", "number", pParas->m_fBaseCur);
	pXmlSierialize->xml_serialize(/* "热过负荷动作定值" */ g_sLangTxt_Native_HotOverloadActionValue.GetString(), "BaseActionSet", "", "number", pParas->m_fBaseActionSet);
	pXmlSierialize->xml_serialize(/* "过负荷电流通道" */ g_sLangTxt_Native_OverloadCurrChannel.GetString(), "FaultCurChannel", "无", "UIRampChannel", pParas->m_nFaultCurChannel);
	pXmlSierialize->xml_serialize(/* "过负荷电流" */ g_sLangTxt_Native_OverloadCurrent.GetString(), "FaultCur", "A", "number", pParas->m_fFaultCur);
	pXmlSierialize->xml_serialize(/* "动作时间定值" */ g_sLangTxt_Native_OpTimeSet.GetString(), "SettingTime", "s", "number", pParas->m_fFaultSettingTime);
	pXmlSierialize->xml_serialize(/* "常态时间" */ g_sLangTxt_Native_NormTime.GetString(), "Normaltime", "s", "number", pParas->m_fNormaltime);
	pXmlSierialize->xml_serialize(/* "故障时间裕度" */ g_sLangTxt_Native_FaultTimeMargin.GetString(), "FaultMarginTime", "s", "number", pParas->m_fFaultMarginTime);
	...
	pXmlSierialize->xml_serialize(/* "电流变化始值" */ g_sLangTxt_Native_CurrentChangeStartValue.GetString(), "fStartCur", "", "number", pParas->m_fStartCur);
	pXmlSierialize->xml_serialize(/* "电流变化终值" */ g_sLangTxt_Native_CurrentChangeEndValue.GetString(), "EndCur", "", "number", pParas->m_fEndCur);
	pXmlSierialize->xml_serialize(/* "电流变化步长" */ g_sLangTxt_Native_CurrentChangeStep.GetString(), "StepCur", "", "number", pParas->m_fStepCur);
