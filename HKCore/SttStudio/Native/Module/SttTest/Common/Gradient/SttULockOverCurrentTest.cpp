	...
	if(nType == 0)
	{
		...
		else
		{
			...
			case 0:
			{
				...
				oRtdata.m_oFaultVolCh.m_fMag = m_oGradientParas.m_uiVOL[0][0].Harm[1].fAmp;
				oRtdata.m_oFaultVolCh.m_fAng = m_oGradientParas.m_uiVOL[0][0].Harm[1].fAngle;
			}
				break;
			case 1:
			{
				oRtdata.m_oFaultCurCh.m_fFreq = oRtdata.m_oCurCh[0].m_fFreq;
				oRtdata.m_oFaultCurCh.m_fMag = m_oGradientParas.m_uiCUR[0][0].Harm[1].fAmp;
				oRtdata.m_oFaultCurCh.m_fAng = m_oGradientParas.m_uiCUR[0][0].Harm[1].fAngle;
			}
				break;
			case 2:
			{
				oRtdata.m_oFaultCurCh.m_fFreq = oRtdata.m_oCurCh[1].m_fFreq;
				oRtdata.m_oFaultCurCh.m_fMag = m_oGradientParas.m_uiCUR[0][1].Harm[1].fAmp;
				oRtdata.m_oFaultCurCh.m_fAng = m_oGradientParas.m_uiCUR[0][1].Harm[1].fAngle;
			}
				break;
			case 3:
			{
				oRtdata.m_oFaultCurCh.m_fFreq = oRtdata.m_oCurCh[2].m_fFreq;
				oRtdata.m_oFaultCurCh.m_fMag = m_oGradientParas.m_uiCUR[0][2].Harm[1].fAmp;
				oRtdata.m_oFaultCurCh.m_fAng = m_oGradientParas.m_uiCUR[0][2].Harm[1].fAngle;
			}
				break;
			}
		}
...
void CSttULockOverCurrentTest::CalStepValue(tmt_GradientParas *pTmt_GradientParas,float fCurValue,int nHarmIndex)
{
	...
	case ULOCK_FAULT_TYPE_NEG_SEQ_VOL:
		fPhi=(float)(atan2(3.0*pTmt_GradientParas->m_fEDU-6.0*fCurValue,sqrt(3.0)*pTmt_GradientParas->m_fEDU)*180.0f/pi);
		fV1 = p1.polar((float)((3.0f*pTmt_GradientParas->m_fEDU-6.0f*fCurValue)/sqrt(3.0f)/2.0f/sin(fPhi*pi/180.0f)),-90.0+fPhi);
		fV2 = p1.polar((float)((3.0f*pTmt_GradientParas->m_fEDU-6.0f*fCurValue)/sqrt(3.0f)/2.0f/sin(fPhi*pi/180.0f)),-90.0-fPhi);
	...
	case ULOCK_FAULT_TYPE_LOW_CURRENT_A:
//		fPhi=fabs(atan2(m_oULockOverCurrentParas.m_fSCVoltage,pTmt_GradientParas->m_fEDU))*180.0/pi;
		fPhi=atan2(m_oULockOverCurrentParas.m_fSCVoltage,pTmt_GradientParas->m_fEDU)*180.0/pi;
		fV=m_oULockOverCurrentParas.m_fSCVoltage;
		fV2 = p1.polar(fV/2.0/sin(fPhi*pi/180.0),-90.0+fPhi);
		fV3 = p1.polar(fV/2.0/sin(fPhi*pi/180.0),-90.0-fPhi);
...
//		fPhi=fabs(atan2(m_oULockOverCurrentParas.m_fSCVoltage,pTmt_GradientParas->m_fEDU))*180.0/pi;
		...
		fV=m_oULockOverCurrentParas.m_fSCVoltage;
		fV3 = p1.polar(fV/2.0/sin(fPhi*pi/180.0),-90.0+fPhi);
		fV1 = p1.polar(fV/2.0/sin(fPhi*pi/180.0),-90.0-fPhi);
