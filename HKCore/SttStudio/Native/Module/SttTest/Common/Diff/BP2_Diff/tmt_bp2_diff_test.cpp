...
void stt_xml_serialize_diff_search_results(tmt_BP2DiffParas *pParas,tmt_BP2DiffResults *pResults, CSttXmlSerializeBase *pXmlSierialize)
{
	...
	pXmlSierialize->xml_serialize("制动电流","Ir","","number",pParas->m_fIr_Id);
	pXmlSierialize->xml_serialize("差动电流","Id","","number",pParas->m_fIdiff);//20230428 zhouhj 增加用于谐波制动搜索
#else
  ...
  pXmlSierialize->xml_serialize("I1A", "TripI1A", "In/A", "number", pResults->m_fI1);
  pXmlSierialize->xml_serialize("I2A", "TripI2A", "In/A", "number", pResults->m_fI2);
  pXmlSierialize->xml_serialize(/* "制动电流" */ g_sLangTxt_Native_ControlCurrent.GetString(), "Ir", "", "number", pParas->m_fIr_Id);
  pXmlSierialize->xml_serialize(/* "差动电流" */ g_sLangTxt_Native_DiffCurrents.GetString(), "Id", "", "number", pParas->m_fIdiff);
#endif
}
...
void stt_xml_serialize_common_sys_paras(tmt_BP2DiffParas *pParas, CSttXmlSerializeBase *pXmlSierialize)
{
	...
	pXmlSierialize->xml_serialize("Uc幅值","UcMag","V","number",pParas->m_fUmag[2]);
  ...
  pXmlSierialize->xml_serialize(/* "Uc幅值" */ g_sLangTxt_Native_UcAmplitude.GetString(), "UcMag", "V", "number", pParas->m_fUmag[2]);
...
{
#ifdef NOT_USE_XLANGUAGE
	pXmlSierialize->xml_serialize("比率制动系数类别(0-低值 1-高值)","KAType","","number",pParas->m_nKAType);
	pXmlSierialize->xml_serialize("差动电流Id(表达式)","IdExp","","string",pParas->m_pszId);
	pXmlSierialize->xml_serialize("复式制动电流(Ir-Id)(表达式)","(Ir-Id)Exp","","string",pParas->m_pszIrId);
#else
  pXmlSierialize->xml_serialize(/* "比率制动系数类别(0-低值 1-高值)" */ g_sLangTxt_Native_BrkCoeffType.GetString(), "KAType", "", "number", pParas->m_nKAType);
  pXmlSierialize->xml_serialize(/* "差动电流Id(表达式)" */ g_sLangTxt_Native_DiffCurrentIds.GetString(), "IdExp", "", "string", pParas->m_pszId);
  pXmlSierialize->xml_serialize(/* "复式制动电流(Ir-Id)(表达式)" */ g_sLangTxt_Native_CompBrakeCurr.GetString(), "(Ir-Id)Exp", "", "string", pParas->m_pszIrId);
#endif
}
