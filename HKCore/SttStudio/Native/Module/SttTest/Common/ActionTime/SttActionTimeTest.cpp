...
void CSttActionTimeTest::TranslateToStateParas()
{
	...
	case ACTION_TIME_FAULT_TYPE_AB:
		...
		{
			fPhiTemp=atan2(3.0*fV-6.0*fVf,sqrt(3.0)*fV)*180.0/pi;
			fVTemp=(3.0*fV-6.0*fVf)/sqrt(3.0);
		}
		else
		{
			fPhiTemp=atan2(fVf,fV)*180.0/pi;
			fVTemp=fVf;
		}
		...
		fVa=p1.polar(fVTemp/2.0/sin(fPhiTemp*pi/180.0),-90.0+fPhiTemp);
		fVb=p1.polar(fVTemp/2.0/sin(fPhiTemp*pi/180.0),-90.0-fPhiTemp);
	...
	case ACTION_TIME_FAULT_TYPE_BC:
		...
		fVb=p1.polar(fVTemp/2.0/sin(fPhiTemp*pi/180.0),-90.0+fPhiTemp);
		fVc=p1.polar(fVTemp/2.0/sin(fPhiTemp*pi/180.0),-90.0-fPhiTemp);
