...
void  CFaultCalculat::VI0Cal(float Vx1, float Phx1, float Vx2, float Phx2, float Vx3, float Phx3, float *Vx, float *Phx)
{
	*Vx=(float)(float)hypot(Vx1*(float)cos(Phx1*(float)PI/180.0f)+Vx2*(float)cos(Phx2*(float)PI/180.0f)+Vx3*(float)cos(Phx3*(float)PI/180.0f),Vx1*(float)sin(Phx1*(float)PI/180.0f)+Vx2*(float)sin(Phx2*(float)PI/180.0f)+Vx3*(float)sin(Phx3*(float)PI/180.0f))/3.0f;

	float f1 = Vx1*(float)sin(Phx1*(float)PI/180.0f)+Vx2*(float)sin(Phx2*(float)PI/180.0f)+Vx3*(float)sin(Phx3*(float)PI/180.0f);
	float f2 = Vx1*(float)cos(Phx1*(float)PI/180.0f)+Vx2*(float)cos(Phx2*(float)PI/180.0f)+Vx3*(float)cos(Phx3*(float)PI/180.0f);

	if(fabs(f1)<=0.00005 && fabs(f2) <= 0.00005)
	{
		*Phx = 0;
		return;
	}

	if(fabs(f1) == 0 && fabs(f2) <= 0.00005)
	{
		*Phx = 0;
		return ;
	}

	if(fabs(f1) <= 0.00005 && fabs(f2) == 0)
	{
		*Phx = 0;
		return;
	}

	*Phx=(float)((float)atan2(f1,f2)*180.0f/(float)PI);
}
