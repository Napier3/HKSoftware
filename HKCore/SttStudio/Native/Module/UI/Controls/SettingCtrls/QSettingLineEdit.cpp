...
void QSettingLineEdit::paintEvent(QPaintEvent *event)
{
	QPalette palette = this->palette();

	if (!this->isEnabled())	//20241114 huangliang 设置定值后颜色，新增测试中设置灰色
	{
		if (!IsSetting())
			palette.setColor(QPalette::Text, Qt::gray); // 设置色
		else
			palette.setColor(QPalette::Text, Qt::gray); // 设置色
	}
	else
	{
		if (!IsSetting())
			...
			palette.setColor(QPalette::Text, Qt::blue /*g_SettingColor*/); // 设置色
	}
...
void QSettingLineEdit::connectSetting()
{
	...
	//QObject::connect(m_pDlgSetting,SIGNAL(signal_ClearAll()),this,SLOT(slot_ClearAll()), Qt::UniqueConnection);
...
void QSettingLineEdit::Disconnect()
{
	...
	//QObject::disconnect(m_pDlgSetting,SIGNAL(signal_ClearAll()),this,SLOT(slot_ClearAll()));
