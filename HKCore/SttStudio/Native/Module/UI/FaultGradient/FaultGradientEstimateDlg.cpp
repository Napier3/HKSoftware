...
void QFaultGradientEstimateGrid::InitGrid()
{
	...
	lstTabCol << /*"名称"*/g_sLangTxt_Name << /*g_sLangTxt_State_SettingValue*/g_sLangTxt_SetValue/*"整定值"*/ << /*"误差类型"*/g_sLangTxt_StateEstimate_ErrorType <</* "相对误差(%)"*/g_sLangTxt_StateEstimate_RelError + "(%)" << /*"绝对误差"*/g_sLangTxt_StateEstimate_AbsError;
...
void QFaultGradientEstimateGrid::addTabRow(int nRow,QString strValue,int nComType,long l1Value,float f2Value,float f3Value,float f4Value)
{
	...
	item(nRow, GRADIENTESTIMATE_COL_NAME)->setTextAlignment(Qt::AlignVCenter | Qt::AlignLeft);
	...
	((QSettingItem*)item(nRow, GRADIENTESTIMATE_COL_RELERROR))->setText(QString::number(f2Value));//UpdateStructText(CVariantDataAddress(&f2Value));
	...
	((QSettingItem*)item(nRow, GRADIENTESTIMATE_COL_ABSERROR))->setText(QString::number(f3Value));//UpdateStructText(CVariantDataAddress(&f3Value));
	...
	((QSettingItem*)item(nRow, GRADIENTESTIMATE_COL_SETVALUE))->setText(QString::number(f4Value));//UpdateStructText(CVariantDataAddress(&f4Value));
...
void CFaultGradientEstimateDlg::initTab()
{
	...
	strText.replace("°", /*QString::fromLocal8Bit(STT_MATH_degree)*/g_sLangTxt_AxisUnitAng);
	...
	strText.replace("Ω", /*QString::fromLocal8Bit(STT_MATH_ohm)*/g_sLangTxt_AxisUnitOmega);
	...
	strText.replace("°", /*QString::fromLocal8Bit(STT_MATH_degree)*/g_sLangTxt_AxisUnitAng);
	ui->tableWidget->addTabRow(5,/*"最大灵敏角(°)"*/g_sLangTxt_Gradient_MaxSenAngle, 0, m_pCurFGTest->m_oFaultGradientParas.m_nMaxAngle_ErrorLogic, m_pCurFGTest->m_oFaultGradientParas.m_fMaxAngle_RelErr, m_pCurFGTest->m_oFaultGradientParas.m_fMaxAngle_AbsErr, m_pCurFGTest->m_oFaultGradientParas.m_fMaxAngleSet);
	ui->tableWidget->addTabRow(6,/*"返回系数"*/g_sLangTxt_Gradient_ReCoefficient, 1, m_pCurFGTest->m_oFaultGradientParas.m_nRetCoef_ErrorLogic, m_pCurFGTest->m_oFaultGradientParas.m_fRetCoef_RelErr, m_pCurFGTest->m_oFaultGradientParas.m_fRetCoef_AbsErr, m_pCurFGTest->m_oFaultGradientParas.m_fRetCoefSet);
...
void CFaultGradientEstimateDlg::slot_btnOK_Clicked()
{
	...
	m_pCurFGTest->m_oFaultGradientParas.m_fMaxAngle_RelErr = ui->tableWidget->item(5, GRADIENTESTIMATE_COL_RELERROR)->text().toFloat();
	...
	m_pCurFGTest->m_oFaultGradientParas.m_fMaxAngle_AbsErr = ui->tableWidget->item(5, GRADIENTESTIMATE_COL_ABSERROR)->text().toFloat();
	m_pCurFGTest->m_oFaultGradientParas.m_fRetCoef_AbsErr = ui->tableWidget->item(6, GRADIENTESTIMATE_COL_ABSERROR)->text().toFloat();
	...
	{
	m_pCurFGTest->m_oFaultGradientParas.m_nMaxAngle_ErrorLogic = ((QComboBox*)ui->tableWidget->cellWidget(5, GRADIENTESTIMATE_COL_ERRORTYPE))->currentIndex();
	}
	...
	{
	m_pCurFGTest->m_oFaultGradientParas.m_nRetCoef_ErrorLogic = ((QComboBox*)ui->tableWidget->cellWidget(6, GRADIENTESTIMATE_COL_ERRORTYPE))->currentIndex();
	}
	...
	m_pCurFGTest->m_oFaultGradientParas.m_fMaxAngleSet = ui->tableWidget->item(5, GRADIENTESTIMATE_COL_SETVALUE)->text().toFloat();
	m_pCurFGTest->m_oFaultGradientParas.m_fRetCoefSet = ui->tableWidget->item(6, GRADIENTESTIMATE_COL_SETVALUE)->text().toFloat();
	accept();
}
