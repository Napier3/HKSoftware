...
//		"QHeaderView::section { background-color: lightblue; }");
	...
	for(int i = 0; i<4; i++)
	{
		...
		QTableWidgetItem *pItem = ui->m_tableWidget->item(i, 0);
...
void QSwingSysDiagramWidget::SetDatas()
{
	...
	pItem = new QTableWidgetItem(QString("%1%2").arg(QString::number(m_dZoscmin, 'f', 3), /*QString::fromLocal8Bit("Ω")*/g_sLangTxt_AxisUnitOmega));
    pItem->setTextAlignment(Qt::AlignCenter | Qt::AlignVCenter);
    pItem->setFont(*g_pSttGlobalFont);
    ui->m_tableWidget->setItem(2, 1, pItem);

	pItem = new QTableWidgetItem(QString("%1%2").arg(QString::number(m_dPhioscmax, 'f', 3), /*QString::fromLocal8Bit("°")*/g_sLangTxt_AxisUnitAng));
    pItem->setTextAlignment(Qt::AlignCenter | Qt::AlignVCenter);
    pItem->setFont(*g_pSttGlobalFont);
	...
	{
		ui->m_tableWidget->setRowHeight(i, 40);
		QTableWidgetItem *pItem = ui->m_tableWidget->item(i, 1);
		pItem->setFlags(pItem->flags() & ~Qt::ItemIsEditable);
	}
