...
m_bIsrBtnAmpAngleChecked =FALSE;
...
void QSttMacroParaEditViewCBOperate::InitUI()
{
    ...
    ui->m_pAmpAngleRadio->setChecked(true);
...
void QSttMacroParaEditViewCBOperate::InitFont()
{
    ...
    ui->m_pAmpAngleRadio->setFont(*g_pSttGlobalFont);
    ui->m_pRXRadio->setFont(*g_pSttGlobalFont);
...
void QSttMacroParaEditViewCBOperate::InitConnections()
{
	...
	connect(ui->m_pAmpAngleRadio,SIGNAL(toggled(bool)),this,SLOT(slot_ImpedanceChanged(bool)));
	//connect(this,SIGNAL(sig_updataRXParas(bool)), this, SLOT(slot_updateUIParas(bool)),Qt::UniqueConnection);
...
void QSttMacroParaEditViewCBOperate::UpdateSettingAttachUI()
{
	m_pResultAssessWidget = new CBOperateResultAssessGrid(m_oCBOperateParas, this);
	m_pResultAssessWidget->UpdataResultAssess();
	m_pCommonParas = new CBOperateCommonParasDlg(m_oCBOperateParas, this);
	m_pCommonParas->slot_BtnOKCommonParas();
}

void QSttMacroParaEditViewCBOperate::CopyBinaryConfig(BOOL b /*= TRUE*/)
{
	if (b)
	{
		if (m_oCBOperateParas->m_nBinSelect == 0)
		{
			for (int i = 0; i < MAX_BINARYIN_COUNT; i++)
			{
				if (i < 4)
				{
					 g_theTestCntrFrame->GetBinaryConfig()->m_binIn[i].nSelect = 1;
				}
				else
				{
					 g_theTestCntrFrame->GetBinaryConfig()->m_binIn[i].nSelect = 0;
				}
			}
		}

		if (m_oCBOperateParas->m_nBinSelect == 1)
		{
			for (int i = 0; i < MAX_BINARYIN_COUNT; i++)
			{
				if (i >= 4)
...
void QSttMacroParaEditViewCBOperate::Initdates()
{
	...
	if (ui->m_pAmpAngleRadio->isChecked())
	{
		...
		ui->m_pEditRZ->setText(QString::number(m_oCBOperateParas->m_fImpedance[1], 'f',3));
		ui->m_pEditRImpedanceAngle->setText(QString::number(m_oCBOperateParas->m_fImpAngle[1], 'f',3));
	}
	else
	{
		ui->m_pEditZ->setText(QString::number(m_oCBOperateParas->m_fResistance[0], 'f',3));
		ui->m_pEditImpedanceAngle->setText(QString::number(m_oCBOperateParas->m_fReactance[0], 'f',3));

		ui->m_pEditRZ->setText(QString::number(m_oCBOperateParas->m_fResistance[1], 'f',3));
		ui->m_pEditRImpedanceAngle->setText(QString::number(m_oCBOperateParas->m_fReactance[1], 'f',3));
	}
...
void QSttMacroParaEditViewCBOperate::slot_EditZ()
{
	...
	{
		m_oCBOperateParas->m_fResistance[0] = fv;
	}
...
void QSttMacroParaEditViewCBOperate::slot_EditImpedanceAngle()
{
	...
	{
		m_oCBOperateParas->m_fReactance[0] = fv;
	}
...
void QSttMacroParaEditViewCBOperate::slot_EditRZ()
{
	...
	{
		m_oCBOperateParas->m_fResistance[1] = fv;
	}
    return ;
}
...
void QSttMacroParaEditViewCBOperate::slot_EditRImpedanceAngle()
{
	...
	{
		m_oCBOperateParas->m_fReactance[1] = fv;
	}
...
void QSttMacroParaEditViewCBOperate::startInit()
{
    ...
    ui->m_pRXRadio->setEnabled(false);
    ui->m_pAmpAngleRadio->setEnabled(false);
}
...
void QSttMacroParaEditViewCBOperate::stopInit()
{
    ...
    ui->m_pRXRadio->setEnabled(true);
    ui->m_pAmpAngleRadio->setEnabled(true);
...
void QSttMacroParaEditViewCBOperate::slot_ImpedanceChanged(bool bChecked)
{
	if (bChecked)
	{
		QString strEdit1 = GetImpedanceByXY("Amp", ui->m_pEditZ->text(), ui->m_pEditImpedanceAngle->text());
		QString strEdit2 = GetImpedanceByXY("Angle", ui->m_pEditZ->text(), ui->m_pEditImpedanceAngle->text());
	...
	{
		QString strEdit1 = GetXYByImpedance("X", ui->m_pEditZ->text(), ui->m_pEditImpedanceAngle->text());
		QString strEdit2 = GetXYByImpedance("Y", ui->m_pEditZ->text(), ui->m_pEditImpedanceAngle->text());
		ui->m_pEditZ->setText(strEdit1);
		ui->m_pEditImpedanceAngle->setText(strEdit2);
		...
		m_bIsrBtnAmpAngleChecked = TRUE;
	}

	QTimer::singleShot(10, this, SLOT(slot_updateUIParas()));

}
void QSttMacroParaEditViewCBOperate::slot_updateUIParas()
{
	...
	else
	{
		...
		{
			ui->m_pLabRImpedanceAngle->setText(g_sLangTxt_Gradient_ImpAngle);
		}
