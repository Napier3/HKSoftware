...
//#endif
	...
	if (!isVisible()) 
	{
		...
		{
			ui->m_ChkShowTips->setCheckState( Qt::Checked);
		}
		else
		{
			ui->m_ChkShowTips->setCheckState( Qt::Unchecked);
...
void QSttAssistMenuWidget::on_m_ChkShowTips_clicked()
{
	...
	bool bShowTips = ( Qt::Checked == ui->m_ChkShowTips->checkState());

	if (g_oSttSystemConfig.IsAssist() != bShowTips)
	{
		g_oSttSystemConfig.SetAssist(bShowTips);
	}
