...
#else
	m_pHtmlBuffer->AppendString((/*"<h1>零漂测试</h1>"*/"<h1>"+g_sLangTxt_Native_ZeroDriftTest+"</h1>").toStdString().c_str());
#endif
}
...
//	Gen_Table_th("结论");
	...
	Gen_Table_th(/*"额定值（V/A）"*/ (g_sLangTxt_Report_RatedValue +"（V/A）").toStdString().c_str() );
	Gen_Table_th(/*"最大零漂(dB)"*/(g_sLangTxt_Native_MaximumZeroDrif +"(dB)" ).toStdString().c_str());
	Gen_Table_th(/*"最小零漂(dB)" */(g_sLangTxt_Native_MinimumZeroDrif +"(dB)" ).toStdString().c_str() );
	Gen_Table_th(/*"平均零漂(dB)"*/(g_sLangTxt_Native_AverageZeroDrif + "(dB)").toStdString().c_str());
