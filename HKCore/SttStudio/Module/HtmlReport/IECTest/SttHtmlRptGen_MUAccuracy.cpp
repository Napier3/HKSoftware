...
#else
	//m_pHtmlBuffer->AppendString((/*"<h1>准确度测试</h1>"*/"<h1>"+g_sLangTxt_Native_MUAccuracyTest+"</h1>").toStdString().c_str());

	m_pHtmlBuffer->AppendString((/*"<h1>准确度测试</h1>"*/"<h1>"+g_sLangTxt_Native_MUAccuracyTest+"</h1>").toLocal8Bit());
...
#else
	GenMUAccuracy_h2(/*"比差测试"*/g_sLangTxt_Native_DifferentialTest);
#endif
...
#else
	GenMUAccuracy_h2(/*"角差测试"*/g_sLangTxt_Native_AngleDifferenceTest);
#endif
...
#else
	GenMUAccuracy_h2(/*"复合误差测试"*/g_sLangTxt_Native_CompositeErrorTest);
#endif
...
#else
	GenMUAccuracy_h2(/*"通道延时测试"*/g_sLangTxt_Native_ChannelDelayTest);
#endif
...
void CSttHtmlRptGen_MUAccuracy::GenMUAccuracy_h2(const CString &strName)		//生成子表标题
{
 ...
 void CSttHtmlRptGen_MUAccuracy::GenModule_RootDiv(const char *strModuleID)
 {
 	m_pHtmlBuffer->AppendString("<div id=\"");
...
void CSttHtmlRptGen_MUAccuracy::GenMUAccuracy_Table_th_RatError(const CString &strTypeID)  //生成子表头
{
		...
		Gen_Table_th(/*"通道名称"*/g_sLangTxt_channelname.toStdString().c_str());
		Gen_Table_th(/*"输出幅值(V/A)"*/(g_sLangTxt_Native_OutputAmp+"(V/A)").toStdString().c_str());
		Gen_Table_th(/*"最大比差(%)"*/ (g_sLangTxt_Native_MaximumRatioDifference+"(%)").toStdString().c_str());
		Gen_Table_th(/*"最小比差(%)"*/(g_sLangTxt_Native_MinimumRatioDifference+"(%)").toStdString().c_str());
		Gen_Table_th(/*"平均比差(%)"*/(g_sLangTxt_Native_AverageRatioDifference +"(%)").toStdString().c_str());
		Gen_Table_th(/*"准确级"*/g_sLangTxt_IEC_AccuratyLevel.toStdString().c_str());
...
void CSttHtmlRptGen_MUAccuracy::GenMUAccuracy_Table_th_AngError(const CString &strTypeID)  //生成子表头
{
	...
	Gen_Table_th(/*"序号"*/g_sLangTxt_SerNumber.toStdString().c_str());
	Gen_Table_th(/*"通道名称"*/g_sLangTxt_channelname.toStdString().c_str());
	Gen_Table_th(/*"输出角度(°)" */(g_sLangTxt_Native_OutputAngle +"(°)").toStdString().c_str());
	Gen_Table_th(/*"最大角差(′)"*/(g_sLangTxt_Native_MaximumAngleDifference + "(′)").toStdString().c_str());
	Gen_Table_th(/*"最小角差(′)"*/ (g_sLangTxt_Native_MinimumAngleDifference+ "(′)" ).toStdString().c_str());
	Gen_Table_th(/*"平均角差(′)"*/ (g_sLangTxt_Native_AverageAngleDifference + "(′)" ).toStdString().c_str());
	Gen_Table_th(/*"准确级"*/g_sLangTxt_IEC_AccuratyLevel.toStdString().c_str());
	Gen_Table_th(/*"结论"*/g_sLangTxt_Conclusion.toStdString().c_str());
...
void CSttHtmlRptGen_MUAccuracy::GenMUAccuracy_Table_th_CompError(const CString &strTypeID)  //生成子表头
{
	...
	Gen_Table_th(/*"序号"*/g_sLangTxt_SerNumber.toStdString().c_str());
	Gen_Table_th(/*"通道名称"*/g_sLangTxt_channelname.toStdString().c_str());
	Gen_Table_th(/*"输出幅值(A/V)"*/(g_sLangTxt_Native_OutputAmp+"(V/A)").toStdString().c_str());
	Gen_Table_th(/*"输出角度(°)" */(g_sLangTxt_Native_OutputAngle + "(°)").toStdString().c_str());
	Gen_Table_th(/*"最大误差(%)" */(g_sLangTxt_Native_MaximumError +"(%)").toStdString().c_str() );
	Gen_Table_th(/*"最小误差(%)"*/ (g_sLangTxt_Native_MinimumError +"(%)").toStdString().c_str());
	Gen_Table_th(/*"平均误差(%)"*/ (g_sLangTxt_Native_AverageError + "(%)").toStdString().c_str());
	Gen_Table_th(/*"结论"*/g_sLangTxt_Conclusion.toStdString().c_str());
#endif
	m_pHtmlBuffer->AppendString("</tr>");
}
...
#else
	Gen_Table_th(/*"序号"*/g_sLangTxt_SerNumber.toStdString().c_str());
	Gen_Table_th(/*"控制块描述"*/g_sLangTxt_Native_ControlBlockDescription.toStdString().c_str());
	Gen_Table_th(/*"通道延时(μs)"*/(g_sLangTxt_Gradient_ChanDelay+"(μs)").toStdString().c_str());
	Gen_Table_th(/*"最大延时(μs)"*/ (g_sLangTxt_Native_MaximumDelay + "(μs)").toStdString().c_str());
	Gen_Table_th(/*"最小延时(μs)"*/(g_sLangTxt_Native_MinimumDelay + "(μs)").toStdString().c_str());
	Gen_Table_th(/*"平均延时(μs)" */(g_sLangTxt_Native_AverageDelay + "(μs)").toStdString().c_str());
	Gen_Table_th(/*"结论"*/g_sLangTxt_Conclusion.toStdString().c_str());
#endif
	m_pHtmlBuffer->AppendString("</tr>");
}
