...
void QSttReportViewHtml::InitReportView()
{
	...
	m_nCurrIndex = 0;
	m_bCanExecCmd = FALSE;//dingxy 20250117 此变量重新置为FALSE，防止资源未能加载成功就加载报告
	OpenHtmlFile(STT_REPORT_HEAD_HTML_FILE);   //打开报告头文件
	//填充报告
#ifndef STT_NOT_TESTCNTRFRAME
	...
	//g_theTestCntrFrame->UpdateRptHtml();//dingxy 20240923 show应该根据当前资源重新更新报告内容
...
void QSttReportViewHtml::slot_UpdateRptData()
{
	...
	{
	UpdateReportHead_StartEndTestTime(TRUE, TRUE);//更新第一次点击报告前的测试开始结束时间
...
void QSttReportViewHtml::UpdateReportHead_StartEndTestTime(BOOL bUpdateStartTime,BOOL bUpdateEndTime)
{
	...
	if (bUpdateStartTime)
	{
		...
		m_strStartTestTime = strCurrTestTime;//dingxy 20240924 记录测试时间
#endif
#endif
