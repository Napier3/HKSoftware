...
void QImpedanceManuWidget::slot_CmbFirstModeIndexChanged(int index)
{
	...
	else//除变化时间外，其他三个值变换单位为Ω
	{
		...
		else
		{
			CString strtemp;
			strtemp += _T("(");
			strtemp += g_sLangTxt_AxisUnitAng;
			strtemp += _T("):");
			ui->m_pStepLabel->setText(strStep +/*QString::fromLocal8Bit("(°):")*/strtemp);
			ui->m_pStartLabel->setText(strStart +/*QString::fromLocal8Bit("(°):")*/strtemp);
			ui->m_pEndLabel->setText(strEnd +/*QString::fromLocal8Bit("(°):")*/strtemp);
		}
