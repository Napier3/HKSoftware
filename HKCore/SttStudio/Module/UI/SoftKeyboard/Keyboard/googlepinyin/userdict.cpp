...
//#ifdef _WIN32
static int custom_gettimeofday(struct timeval *tp, void *) {//zhouhj 2024.4.16
	//是否使用自定义的时间对象
#ifdef USE_Custom_CDateTime
	const qint64 current_msecs_since_epoch = CDateTime::CurrentMSecsSinceEpoch();
#else
	const long long current_msecs_since_epoch = QDateTime::currentMSecsSinceEpoch();
#endif
...
#define DEBUG_PERF_BEGIN \
    do { \
      custom_gettimeofday(&_tv_start_, NULL); \
    } while (0)
#define DEBUG_PERF_END \
    do { \
      custom_gettimeofday(&_tv_end_, NULL); \
...
bool UserDict::load_dict(const char *file_name, LemmaIdType start_id,
  ...
  custom_gettimeofday(&load_time_, NULL);
...
bool UserDict::close_dict() {
  ...
  if (load_time_.tv_sec > g_last_update_.tv_sec ||
    ...
    write_back();
    custom_gettimeofday(&g_last_update_, NULL);
  }
