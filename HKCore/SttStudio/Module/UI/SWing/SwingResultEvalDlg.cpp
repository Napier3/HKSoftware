...
void QSwingResultEvalDlg::InitUI()
{
    ...
    ui->m_tableWidget->item(0,0)->setText(g_sLangTxt_Native_ActionTime);
    ...
    ui->m_tableWidget->setColumnWidth(2, 270);
...
void QSwingResultEvalDlg::UpdateSettingText()
{
    if (m_editRelErr->inherits(STT_SETTING_ITEM_ClassID/*QSettingItem*/))
    {
        if (m_editRelErr->IsSetting())
        {
            m_editRelErr->UpdateStructText(CVariantDataAddress(&m_pCurTest->m_oSwingParas.m_fTtripRelErr), 3);
        }
    }

    if (m_editAbsErr->inherits(STT_SETTING_ITEM_ClassID/*QSettingItem*/))
    {
        if (m_editAbsErr->IsSetting())
        {
            m_editAbsErr->UpdateStructText(CVariantDataAddress(&m_pCurTest->m_oSwingParas.m_fTtripAbsErr), 3);
        }
    }

    if (m_editAbsErrNeg->inherits(STT_SETTING_ITEM_ClassID/*QSettingItem*/))
    {
        if (m_editAbsErrNeg->IsSetting())
        {
            m_editAbsErrNeg->UpdateStructText(CVariantDataAddress(&m_pCurTest->m_oSwingParas.m_fTtripAbsErr_Neg), 3);
...
void QSwingResultEvalDlg::on_m_tableWidget_itemChanged(QTableWidgetItem *item)
{
    ...
    {
        /*绝对误差(-)*/
        m_editAbsErrNeg->setText(SwingTestValueCheck(m_editAbsErrNeg->text(),TTRIPABSERR_MAX, TTRIPABSERR_MIN, 3));
    }
