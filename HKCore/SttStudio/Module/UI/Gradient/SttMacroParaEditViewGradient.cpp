	...
	ui(new Ui::QSttMacroParaEditViewGradient)
{
	...
	InitLanuage();
	debug_time_long_log("ViewGradient InitLanuage", true);
...
// 	m_strDefaultParaFile.append("Gradient.grtxml");
	...
	debug_time_long_log("ViewGradient SetData", true);

	m_pOriginalSttTestResource = g_theTestCntrFrame->GetSttTestResource();
	g_theTestCntrFrame->InitTestResource();
	debug_time_long_log("ViewGradient InitTestResource", true);
	if (g_oSystemParas.m_nHasDigital)//zhouhj 在打开测试模板前先初始化当前GOOSE发布数据
	{
		InitGoosePubDataGroups();
		InitFT3PubDataGroups();
		debug_time_long_log("ViewGradient InitGoosePubFT3Pub", true);
	}
	OpenTestTestMngrFile(m_strDefaultParaFile);
	debug_time_long_log("ViewGradient OpenTestTestMngrFile", true);
	
	m_pCurPara = &m_pCurTest->m_oGradientParasEx;
   
 	SetDatas(0);
    m_pCurPara->m_nSttTestMode = 0;
	debug_time_long_log("ViewGradient SetDatas", true);

	InitUI();
	debug_time_long_log("ViewGradient InitUI", true);
	InitParas();
	debug_time_long_log("ViewGradient InitParas", true);
	InitMannualparas();
	debug_time_long_log("ViewGradient InitMannualparas", true);
	InitConnections();
	InitStyle();
	debug_time_long_log("ViewGradient InitStyle", true);

	g_theTestCntrFrame->UpdateButtonStateByID(STT_CMD_TYPE_ATS_ManuTriger,false,false);//QSttTestCntrFrameLinux::UpdateButtonsStateByID()重复刷新
...
void QSttMacroParaEditViewGradient::UpdatePrimParaSetUI()
{
	...
	if (m_pCurPara->m_nType == amplitude_type)
	{
		...
		else if (str.contains(tr("U")))
		{
			...
			disconnect(ui->m_editInit, SIGNAL(editingFinished ()), this, SLOT(slot_Para_StartChanged()));
			{
				...
				{
					emit sig_YRangeChanged(amplitude_type, m_strVoltage + "(kV)",fdown/FRADIO,fup*FRADIO);
...
// 				ui->m_labVarV2->setDisabled(false);
			...
			{
				if (IsUseSecondParaSet() == 0)
				{
					ui->m_labInit->setText(strInit + "(kV):");
					ui->m_labFinish->setText(strFinish + "(kV):");
