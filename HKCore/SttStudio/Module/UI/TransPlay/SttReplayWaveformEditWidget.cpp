...
// 	long nNewValue = CString_To_long(strValue);
	...
	{
		nNewValue = nCurValue;
		CLogPrint::LogFormatString(XLOGLEVEL_WARNING, /*_T("波形最小点号为1，请重新输入！")*/g_sLangTxt_ReplayTest_ReEnterWavePoint.GetString());
	}
	else if(nNewValue > g_theBigComtradeTransPlay->m_oBigComtradeFileRead.GetTotalPointsNum())
	{
		...
		CLogPrint::LogFormatString(XLOGLEVEL_WARNING, /*_T("输入的起始点号/终点点号不能超出波形总点数[%d]，请重新输入！")*/g_sLangTxt_ReplayTest_CheckInputWavePoint.GetString(),g_theBigComtradeTransPlay->m_oBigComtradeFileRead.GetTotalPointsNum());
	}
...
// 	long nNewValue = CString_To_long(strValue);
	...
	{
		nNewValue = g_theBigComtradeTransPlay->m_oBigComtradeFileRead.GetTotalPointsNum();
		CLogPrint::LogFormatString(XLOGLEVEL_WARNING, /*_T("输入的起始点号/终止点号不能超出波形总点数[%d]，请重新输入！")*/g_sLangTxt_ReplayTest_CheckInputWavePoint.GetString(),g_theBigComtradeTransPlay->m_oBigComtradeFileRead.GetTotalPointsNum());
	}
	else if(nBegin > nNewValue)
	{
		nNewValue = nBegin;
		CLogPrint::LogFormatString(XLOGLEVEL_WARNING, /*_T("输入的终止点号不能小于输入的起始点号[%d]，请重新输入！")*/g_sLangTxt_ReplayTest_CheckBeginAndEndPoint.GetString(),nBegin);
	}
...
void SttReplayWaveformEditWidget::slot_Insert_CycleIndexChanged()
{
	...
	{
		dNewValue = dCurValue;
		CLogPrint::LogFormatString(XLOGLEVEL_WARNING,/* _T("循环次数不能少于1次，请重新输入！")*/g_sLangTxt_ReplayTest_CycleNumCheck.GetString());
	}
 	else
 	{
...
void SttReplayWaveformEditWidget::slot_Insert_OK()
{
	...
	{	
		...
		if(dEnd <= dBegin)
		{
			CXMessageBox::information(this,/*_T("提示")*/g_sLangTxt_Native_LangTip,/*_T("插入起点不能小于或等于终止点号，请重新输入!")*/g_sLangTxt_ReplayTest_InsertBeginTimeError,QMessageBox::Yes);
			return;
		}		
	...
	else//点号
	{
			if(nEnd <= nBegin)
			{
				CXMessageBox::information(this,/*_T("提示")*/g_sLangTxt_Native_LangTip,/*_T("插入起始点号不能小于或等于终止点号，请重新输入!")*/g_sLangTxt_ReplayTest_InsertBeginPointError,QMessageBox::Yes);
				return;
			}
		}	
	}
...
void SttReplayWaveformEditWidget::slot_Delete_OK()
{
	...
	if(dEnd <= dBegin)
	{
		CXMessageBox::information(this,/*_T("提示")*/g_sLangTxt_Native_LangTip,/*_T("删除起点不能小于或等于终点，请重新输入!")*/g_sLangTxt_ReplayTest_DeleteTimeError,QMessageBox::Yes);
		return;
	}	
...
void SttReplayWaveformEditWidget::slot_Delete_ZeroChanged()
{
	...
	if(!CheckMaxDataTime(dNewValue))
	{
		...
		dNewValue = dCurValue;
		CLogPrint::LogFormatString(XLOGLEVEL_WARNING, /*_T("输入的起点/终点不能超出总波形长度[%.3fs]，请重新输入！")*/g_sLangTxt_ReplayTest_CheckInputWaveTime.GetString(),dRecordDataTimeLength);
	}
...
void SttReplayWaveformEditWidget::slot_Delete_DestinationChanged()
{
	...
	{
		dNewValue = dBegin;
		CLogPrint::LogFormatString(XLOGLEVEL_WARNING, /*_T("输入的终点不能小于输入的起点[%.3fs]，请重新输入！")*/g_sLangTxt_ReplayTest_CheckBeginAndEndTime.GetString(),dBegin);
	}
	else
	{
...
// 	dNewValue *= Ratio;
	...
	{
		CLogPrint::LogFormatString(XLOGLEVEL_ERROR,/*_T("当前输入的实际常态波形电压值(%f),大于测试仪最大输出电压(%f).")*/g_sLangTxt_ReplayTest_InsertWaveVolOver.GetString(),dChangeValue,g_oLocalSysPara.m_fAC_VolMax);
		dNewValue = dCurValue;
	}
	else
	{
...
// 	dNewValue *= Ratio;
	...
	{
		CLogPrint::LogFormatString(XLOGLEVEL_ERROR,/*_T("当前输入的实际常态波形电流值(%f),大于测试仪最大输出电流(%f).")*/g_sLangTxt_ReplayTest_InsertWaveCurOver.GetString(),dChangeValue,g_oLocalSysPara.m_fAC_CurMax);
		dNewValue = dCurValue;
	}
	else
	{
...
void SttReplayWaveformEditWidget::slot_InsertNormal_OutTimeChanged()
{
	...
	{
		dNewValue = dCurValue;
		CLogPrint::LogFormatString(XLOGLEVEL_WARNING, /*_T("当前输出时间不能超出[%.3fs]，请重新输入！")*/g_sLangTxt_ReplayTest_InsertWaveTimeOver.GetString(),dTime);
	}
	else
	{
...
BOOL SttReplayWaveformEditWidget::VaildWaveOverlap()
{
	...
	{
		CXMessageBox::warning(this,/* "错误"*/g_sLangTxt_Native_error, /*"插入与删除时间段重叠，请重新选择时间！"*/g_sLangTxt_ReplayTest_CheckOverlapTime);
		return FALSE;
	}
	return TRUE;
}
...
void SttReplayWaveformEditWidget::slot_Insert_EndTimeChanged()
{
	...
	if(!CheckMaxDataTime(dNewValue))
	{
		...
		dNewValue = dCurValue;
		CLogPrint::LogFormatString(XLOGLEVEL_WARNING, /*_T("输入的起点/终点不能超出总波形长度[%.3fs]，请重新输入！")*/g_sLangTxt_ReplayTest_CheckInputWaveTime.GetString(),g_pReplayTestMain->m_pWaveGraphWidget->m_dRecordDataTimeLength);
	}
	else if(dBegin > dNewValue)
	{
		dNewValue = dBegin;
