...
QSttMacroParaEditViewOverCurrTable::QSttMacroParaEditViewOverCurrTable( int rows, int columns,QWidget *parent ): QTableWidget(rows, columns,parent)
{
...
QSttMacroParaEditViewOverCurrTable::~QSttMacroParaEditViewOverCurrTable()
{

}


void QSttMacroParaEditViewOverCurrTable::InitUI()
{
...
void QSttMacroParaEditViewOverCurrTable::InitTable()
{
	setHorizontalHeaderLabels(QStringList()<< _T("选择") <<  _T("区段")<< _T("整定值")  << _T("整定时间")  << _T("0.95倍(A)") << _T("1.05倍(A)"));
...
void QSttMacroParaEditViewOverCurrTable::slot_OnCellChanged( int row,int col )
{
	QStringList zoneCheckItems ,iSetingItems ,tSetingItems; 
	zoneCheckItems << _T("Zone1_Check") <<  _T("Zone2_Check") << _T("Zone3_Check");
	iSetingItems << _T("ISetingZone1") <<  _T("ISetingZone2")<< _T("ISetingZone3");
	tSetingItems << _T("TSetingZone1")<< _T("TSetingZone2")<< _T("TSetingZone3");
...
void QSttMacroParaEditViewOverCurrTable::InitData( CDataGroup *pData )
{
	m_pData = pData;
	CString strText; 
	...
	pItem->setTextAlignment(Qt::AlignHCenter|Qt::AlignVCenter);
	pItem->setFlags(pItem->flags() & (~Qt::ItemIsEditable));
	//GetTableWidgetItemValue(pItem,_T("Zone3_Check"),true);
	setItem(2, 0, pItem); 
	...
	pItem = new QTableWidgetItem();
	pItem->setTextAlignment(Qt::AlignHCenter|Qt::AlignVCenter);
	//GetTableWidgetItemValue(pItem,_T("ISetingZone3"),false);
	setItem(2, 2, pItem);
	...
	pItem = new QTableWidgetItem();
	pItem->setTextAlignment(Qt::AlignHCenter|Qt::AlignVCenter);
	//GetTableWidgetItemValue(pItem,_T("TSetingZone3"),false);
	setItem(2, 3, pItem);
...
void QSttMacroParaEditViewOverCurrTable::GetTableWidgetItemValue( QTableWidgetItem * pItem,const CString strID ,bool bCheck)
{
...
void QSttMacroParaEditViewOverCurrTable::UpdateTable()
{
	QTableWidgetItem *pItem = NULL;
	CString strItemId;
