...
void QSttMacroParaEditViewMuAutoTest::InitUI()
{
	...
	CString strButton;
	strButton = /*_T("全选")*/g_sLangTxt_SelAll;
	m_pSelectall->setText(strButton);
	m_pSelectall->adjustSize();
	strButton = /*_T("清除")*/g_sLangTxt_Clear;
	m_pClear->setText(strButton);
	m_pClear->adjustSize();
	strButton = /*_T("其他参数")*/g_sLangTxt_Gradient_OtherSet;
	m_pOtherparameters->setText(strButton);
	m_pOtherparameters->adjustSize();
	strButton = /*_T("刷新")*/g_sLangTxt_Manual_Refresh;
	m_pRefresh->setText(strButton);
	m_pRefresh->adjustSize();
	CString strGroupbox = /*_T("参数设置")*/g_sLangTxt_Gradient_ParaSet;
    m_pParameterSet->setTitle(strGroupbox);
	CString strLabel;
	strLabel = /*_T("PT变比")*/g_sLangTxt_PT;
    m_pPTRatio->setText(strLabel);
	strLabel = /*_T("CT变比")*/g_sLangTxt_CT;
	m_pCTRatio->setText(strLabel);
	strLabel = /*_T("测试次数")*/g_sLangTxt_numbertest;
