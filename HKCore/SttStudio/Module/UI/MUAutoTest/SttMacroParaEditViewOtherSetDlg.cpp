...
{  
	...
	CString strTitle = /*_T("其他设置")*/g_sLangTxt_State_Othersettings;
	this->setWindowTitle(strTitle);
	CString strGroupbox;
	strGroupbox = /*_T("参数设置")*/g_sLangTxt_Gradient_ParaSet;
    m_pGroupBox_L->setTitle(strGroupbox);
	strGroupbox = /*_T("PPS设置")*/g_sLangTxt_State_PPSsettings;
    m_pGroupBox_R->setTitle(strGroupbox);
	CString strLabel;
	strLabel = /*_T("接收报文类型")*/g_sLangTxt_Manual_MeasType;
    m_pReceivLabel->setText(strLabel);
	strLabel = /*_T("测试方法")*/g_sLangTxt_Manual_TestMethod;
    m_pTestLabel->setText(strLabel);
	strLabel = /*_T("B码输出逻辑")*/g_sLangTxt_Manual_BCode;
    m_pBOutputLabel->setText(strLabel);	

	m_pRCombox->addItem(_T("IEC-61850-9-2"));
	m_pRCombox->addItem(_T("IEC-60044-8(FT3)"));
	m_pTCombox->addItem(/*_T("同步法")*/g_sLangTxt_State_Synchronousmethod);
	m_pTCombox->addItem(/*_T("插值法")*/g_sLangTxt_State_Interpolation);
	m_pBCombox->addItem(/*_T("正逻辑")*/g_sLangTxt_Manual_PositiveLogic);
	m_pBCombox->addItem(/*_T("负逻辑")*/g_sLangTxt_Manual_NegativeLogic);
	
	CString strRButton;
	strRButton = /*_T("无PPS")*/g_sLangTxt_State_NOPPS;
    m_pNoPPSRButton->setText(strRButton);
	strRButton = /*_T("上升沿有效")*/g_sLangTxt_State_Risingedgeeffective;
	m_pRiseRButton->setText(strRButton);
	strRButton = /*_T("下降沿有效")*/g_sLangTxt_State_Downedgeeffective;
	m_pFallRButton->setText(strRButton);
	CString strButton;
	strButton = /*_T("确认")*/g_sLangTxt_Native_Confm;
