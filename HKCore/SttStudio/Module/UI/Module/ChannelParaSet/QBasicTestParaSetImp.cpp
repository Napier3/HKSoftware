...
void QBasicTestParaSetImp::SetParaSetSecondValue(int nParaSetSecondValue)
{
	...
	int num;
	strheader.clear();
	CString strAmp = g_sLangTxt_Native_Amplitude.GetString();
	if (m_nParaSetSecondValue == V_Primary)//一次值
	{
		strAmp += "(kV)";
		strheader<</*tr("通道")*/g_sLangTxt_Native_Channel<<strAmp<</*tr("相位(°)")*/g_sLangTxt_Gradient_DPhase<</*tr("频率(Hz)")*/g_sLangTxt_Frequency;
	...
	if(isHarm())
	{
		...
		if (m_nParaSetSecondValue == V_Primary)//一次值
		{
			...
			strheader<</*tr("通道")*/g_sLangTxt_Native_Channel<<strAmp<</*tr("相位(°)")*/g_sLangTxt_Gradient_DPhase<</*tr("谐波设置")*/g_sLangTxt_Harm_Set;
