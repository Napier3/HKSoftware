...
void CIecCfgSMVFT3DCOtherSetGrid::InitGridTitle()
{
	...
	{ 
		...
		{
			strIECPrimtRatedType = /*_T("二次额定电压(V)")*/g_sLangTxt_IEC_SecondRatedCur.GetString();
		}
		else
		{
	...
	else
	{
		...
		{
			strIECPrimtRatedType = /*_T("二次额定电流(A)")*/g_sLangTxt_IEC_SecondRatedType.GetString();
		}
	else
	{
...
void CIecCfgProtMaesCurrCodeSetGrid::InitGridTitle()
{
	...
	xlang_GetLangStrByFile(strName,"sName");
	strProtCodeValue = /*"保护码值"*/g_sLangTxt_Native_Protection_CodeValue.GetString();
	strMeasCodeValue = /*"测量码值"*/g_sLangTxt_Native_Measure_CodeValue.GetString();
