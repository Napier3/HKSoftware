...
// 	m_pPkgTypeSelCombox->ShowBaseList(&m_oPkgTypeList);
	...
	if (g_oSystemParas.m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
	{
		...
		m_pCRC_ComboBox->insertItem(STT_IEC_SMVFT3_CRC_16BYTE_VERIFICATION,/*"标准16字节校验"*/g_sLangTxt_IEC_16ByteVerification.GetString());
		m_pCRC_ComboBox->insertItem(STT_IEC_SMVFT3_CRC_END_VERIFICATION,/*"报文末尾校验"*/g_sLangTxt_IEC_EndVerification.GetString());
...
CIecCfgDataBase* QSttIecSMVFT3InWidget::AddNewIecCfgData()
{
	...
	while(pos)
	{
		...
		if (pCfgData->GetClassID() == CFGCLASSID_CIECCFG6044COMMONINDATA)
		{
			...
			{
				QMessageBox::information(this, /*tr("提示")*/g_sLangTxt_Message,/*tr("光口已全部占用，当前无可选择的光口！")*/g_sLangTxt_Native_OpticalPort_Occupation.GetString());
				return NULL;
			}
		}
	}
