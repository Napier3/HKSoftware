...
BOOL CSttMulticastClientSocket::Create(const CString &strMultiIP, long nPort,int nType)
{
	...
	if (bRet)
	{
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE, /*_T("组播初始化[%s][%d]成功")*/g_sLangTxt_Native_MulticastInitSuccess.GetString(), pszMultiIP, nPort);
	}
	else
	{
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE, /*_T("组播初始化[%s][%d]失败")*/g_sLangTxt_Native_MulticastInitFailed.GetString(), pszMultiIP, nPort);
	}
#else
	if (bRet)
	{
...
BOOL CSttMulticastClientSocket::Create(const CString &strMultiIP,long nMultiPort,const CString strLocalIP, long nLocalPort)
{
	...
	{
#ifndef NOT_USE_XLANGUAGE
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE, /*_T("组播服务初始化[%s][%d]失败")*/g_sLangTxt_Native_MulticastInitFailed.GetString(), pszLocalIP, nLocalPort);
#else
...
// 	setsockopt(m_hSocket,SOL_SOCKET,SO_BROADCAST,(char *)&bEnable,sizeof(BOOL));
	...
	CreateThead();
#ifndef NOT_USE_XLANGUAGE
	CLogPrint::LogFormatString(XLOGLEVEL_TRACE, /*_T("组播初始化[%s][%d]成功，绑定本地IP[%s]")*/g_sLangTxt_Native_MulticastInitBindSuccess.GetString(), pszMultiIP, nMultiPort,pszLocalIP);
#else
