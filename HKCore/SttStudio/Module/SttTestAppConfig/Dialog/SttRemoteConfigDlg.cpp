...
BOOL CSttRemoteConfigDlg::OnInitDialog()
{
	...
	{
		CreateStatic(m_stcCloudIP,/*_T("云端IP：")*/g_sLangTxt_Native_CloudIP, CRect(5,20,120,45), STT_STC_CLOUDIP, this);
	}
	else
		CreateStatic(m_stcCloudIP,/*_T("云端IP：")*/g_sLangTxt_Native_CloudIP + _T(":"), CRect(5,20,120,45), STT_STC_CLOUDIP, this);
	...
	if (!g_sLangTxt_Native_CloudPort.IsEmpty() && (g_sLangTxt_Native_CloudPort.Right(1) == _T(":")))
		CreateStatic(m_stcCloudPort,/*_T("云端Port：")*/g_sLangTxt_Native_CloudPort, CRect(300,20,380,45), STT_STC_CLOUDPORT, this);
	else
		CreateStatic(m_stcCloudPort,/*_T("云端Port：")*/g_sLangTxt_Native_CloudPort + _T(":"), CRect(300,20,380,45), STT_STC_CLOUDPORT, this);
	...
	if (!g_sLangTxt_Native_Connect.IsEmpty() && (g_sLangTxt_Native_Connect.Right(1) == _T(":")))
		CreateButton(m_btnConnect,/*_T("连接")*/g_sLangTxt_Native_Connect, CRect(560,20,630,45), STT_BTN_CONNECT, this);
	else
		CreateButton(m_btnConnect,/*_T("连接")*/g_sLangTxt_Native_Connect + _T(":"), CRect(560,20,630,45), STT_BTN_CONNECT, this);

	if (!g_sLangTxt_Native_TestInstrumentSerialNumber.IsEmpty() && (g_sLangTxt_Native_TestInstrumentSerialNumber.Right(1) == _T(":")))
		CreateStatic(m_stcSN,/*_T("测试仪序列号：")*/g_sLangTxt_Native_TestInstrumentSerialNumber, CRect(5,100,120,125), STT_STC_SN, this);
	else
		CreateStatic(m_stcSN,/*_T("测试仪序列号：")*/g_sLangTxt_Native_TestInstrumentSerialNumber + _T(":"), CRect(5,100,120,125), STT_STC_SN, this);
	...
	if (!g_sLangTxt_Native_Password.IsEmpty() && (g_sLangTxt_Native_Password.Right(1) == _T(":")))
		CreateStatic(m_stcPassword,/*_T("密码：")*/g_sLangTxt_Native_Password, CRect(300,100,380,125), STT_STC_PASSWORD, this);
	else
		CreateStatic(m_stcPassword,/*_T("密码：")*/g_sLangTxt_Native_Password + _T(":"), CRect(300,100,380,125), STT_STC_PASSWORD, this);
	...
	if (!g_sLangTxt_Unit.IsEmpty() && (g_sLangTxt_Unit.Right(1) == _T(":")))
		CreateStatic(m_stcCompany,/*_T("单位：")*/g_sLangTxt_Unit, CRect(5,215,60,240), STT_STC_COP, this);
	else
		CreateStatic(m_stcCompany,/*_T("单位：")*/g_sLangTxt_Unit + _T(":"), CRect(5,215,60,240), STT_STC_COP, this);
	CreateEdit(m_editCompany,   CRect(60,215,230,240), STT_EDIT_COP, this);

	if (!g_sLangTxt_User2.IsEmpty() && (g_sLangTxt_User2.Right(1) == _T(":")))
		CreateStatic(m_stcUser,/*_T("用户：")*/g_sLangTxt_User2, CRect(250,215,305,240), STT_STC_USER, this);
	else
		CreateStatic(m_stcUser,/*_T("用户：")*/g_sLangTxt_User2 + _T(":"), CRect(250,215,305,240), STT_STC_USER, this);

	CreateEdit(m_edtUser,   CRect(305,215,385,240), STT_EDIT_USER, this);
	CreateButton(m_btnCancel,/*_T("取消")*/g_sLangTxt_Cancel, CRect(500,215,560,240), STT_BTN_CANCEL_2, this);
	CreateButton(m_btnBind,/*_T("绑定")*/g_sLangTxt_Native_Bind, CRect(570,215,630,240), STT_BTN_BIND, this);
#else
...
void CSttRemoteConfigDlg::OnBtnClickConnect()
{
	...
	if(!m_bConnect)
	{
		...
		CLogPrint::LogString(XLOGLEVEL_TRACE, /*_T("连接云端失败")*/g_sLangTxt_Native_CloudConnectionFailed);
#else
