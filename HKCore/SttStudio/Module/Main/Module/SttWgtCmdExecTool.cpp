...
//	if(g_pQSttWgtCmdExecTool->m_nType == LINUX)
	{
		...
		else if (strType == _T("Refresh_file")) //刷新表格
		{
			RefreshGbxmlFile(strSrcData);
		}
	}
// 	else
// 	{
...
void QSttWgtCmdExecTool::OnOpenFile( const CString& strDataSrc , const CString &strArgv)
{
	...
	QList<QTableWidgetItem*> pSelectedItems = pWgt->selectedItems();
	if (pSelectedItems.isEmpty())
	{
		return;
	}
	int nSelectRow = pSelectedItems.first()->row();
	QTableWidgetItem* pNameItem = pWgt->item(nSelectRow, 0);
	...
	strFileName = pNameItem->text();	
	strFileName = QString("%1%2").arg(strFileName).arg(".gbxml");
	...
	g_pQSttWgtCmdExecTool->m_oThread.m_listCmd.clear();
	g_pQSttWgtCmdExecTool->m_oThread.m_listCmd << ARGV_CMD_ID_OPENFILE;
	g_pQSttWgtCmdExecTool->m_oThread.m_listCmd << strFilePath;
...
void QSttWgtCmdExecTool::RefreshGbxmlFile( const CString& strDataSrc )
{
	if(strDataSrc.GetLength() <= 1)
	{
		return;
	...
	QSttWgtFileMngrGrid* pWgt = m_gQSttWgtNativeMainLinux->findChild<QSttWgtFileMngrGrid*>(strDataSrc);
	pWgt->RefreshFiles();
