...
typedef struct tmt_pik100_DAValue
{
    ...
    unsigned int nDelayTime;//0-no,1-5
    unsigned int nStopTestAfterCheck;
	void init()
	{
		StateStart = 0;//
		K1=0;//
...
typedef struct tmt_pick100_result
{
public:
    long m_nResUseful;
    unsigned int m_nKR[12];
    unsigned int m_nKC[12];
public:
    void init()
    {
        m_nResUseful = 0;
...
public:
    ...
    tmt_Pik100Result  m_oPik100Result;
	void init()
	{
		m_oPik100Para.init();
        m_oPik100Result.init();
	}
	tmt_pik100_test() {}
}tmt_Pik100Test;
