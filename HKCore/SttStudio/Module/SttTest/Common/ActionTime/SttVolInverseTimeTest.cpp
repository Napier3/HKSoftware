...
void CSttVolInverseTimeTest::TranslateToStateParas()
{
	...
	float fFaultVolLN = m_fFaultVol;
 	...
 	case VOL_INVERSE_FAULT_TYPE_AB:
		fPhi=atan2(m_fFaultVol,(double)m_fPreFaultVol)*180.0/pi;
		fV=m_fFaultVol;
		oComplexUI[0][0] = p1.polar(fV/2.0/sin(fPhi*pi/180.0),-90.0+fPhi);
		oComplexUI[0][1] = p1.polar(fV/2.0/sin(fPhi*pi/180.0),-90.0-fPhi);
 	...
 	case VOL_INVERSE_FAULT_TYPE_BC:
		...
		fV=m_fFaultVol;
		oComplexUI[0][1] = p1.polar(fV/2.0/sin(fPhi*pi/180.0),-90.0+fPhi);
		oComplexUI[0][2] = p1.polar(fV/2.0/sin(fPhi*pi/180.0),-90.0-fPhi);
