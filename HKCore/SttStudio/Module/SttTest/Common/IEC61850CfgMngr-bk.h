...
typedef struct
{
	...
	QByteArray m_DataSetList;//??????е?????,use for packet
...
//	void SetGooseValue(tmt_StateParas &oStateParas,int nIndex,drv_state *pDrvState);
	void GetGooseDataPosAndValue(CIecCfgGooseDataBase *gsein_data,int nIndex,unsigned long& nlastByte,unsigned long& nTrueValue,unsigned long& nFalseValue);
