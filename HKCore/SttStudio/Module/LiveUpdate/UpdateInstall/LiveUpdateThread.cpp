...
void CLiveUpdateThread::OnUpdateVersion(WPARAM wParam,LPARAM lParam)
{
	...
	//dingxy 改文件名
 	strFileName = GetPathFileNameFromFilePathName(strSvrZipFile);
 	int nPos = strFileName.Find(_T('.'));
 	if (nPos != -1)
 	{
 		strFileName = g_theLiveUpdateBaseApp->m_pAts_System->m_strID + "_" + strFileName;
 		CString strLocalFile;
 		strLocalFile.Format(_T("%s%s"), g_theLiveUpdateBaseApp->m_strLocalZipFile, strFileName);
 		g_theLiveUpdateBaseApp->m_strLocalZipFile = strLocalFile;
 	}
 	else
 	{
