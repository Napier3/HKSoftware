...
BOOL CLiveUpdateBaseApp::InitLiveUpdateBaseApp()
{
	_P_InitSystemPath();
#ifndef NOT_USE_XLANGUAGE
	CXLanguageResourceBase *pLanguage = new CXLanguageResourceBase();
...
BOOL CLiveUpdateBaseApp::UpdateTestApp_SendFileOffline()
{
	...
	//	g_strFtpRootFile = STTNATIVE_PATH_ROOT;	//底层更新设置
	 return m_oSttLiveUpdateEngine.m_pSttClientSocket->SttSendFile(m_strOffLineFilePath.GetString(),strVerRemoteRelPath.GetString(),0,m_pProgress);
}
...
{
	if (!UpdateTestApp_ConnectOfflineUpdate(m_strTestAppIP))
	{
		return FALSE;
	}
...
{
	CString strFileName = GetPathFileNameFromFilePathName(strSvrZipFile); //dingxy 根据当前选择的系统ID,修改压缩包文件名
	int nPos = strFileName.Find(_T('.'));
	if (nPos != -1)
	{
		strFileName = m_pAts_System->m_strID + "_" + strFileName;
	}
...
// #endif
    ...
    if (!ret)
	{
		//当前测试仪版本不匹配
 		...
 		CString strText = _T("当前升级软件包版本不匹配，更新操作结束");
 		while(true)
 		{
...
#else
        strInfo.Format(_T("测试仪更新：当前升级软件包【%s】版本不匹配，更新操作结束"),strLocalZipFile.GetString());
        SendUpdateInfo(strInfo);
#endif
		strInfo.Format(_T("m_strVertempPath = %s, m_strVerBkPath = %s"), m_strVertempPath.GetString(), m_strVerBkPath.GetString());
		SendUpdateInfo(strInfo);
		DeleteAllDirectorys(m_strVertempPath,FALSE);
...
BOOL CLiveUpdateBaseApp::CompareSystem()
{
	CSttNativeCfgMngr oSttNativeCfgMngr;
	CSttNativeCfgLiveUpdate* pCfgLiveUpdate;
	CString strInfo;
	CString strSystemId;
	UINT nClassID = 0;
	CString strSttNativeCfgFile = _P_GetSystemPath();
	strSttNativeCfgFile += LIVEUPDATE_PATH_UNZIP;
	strSttNativeCfgFile += _T("Stt/Config/");
	strSttNativeCfgFile += LIVEUPDATE_CFG_NATIVE;
	strInfo.Format(_T("FILE = %s "), strSttNativeCfgFile.GetString());
#ifdef _PSX_IDE_QT_
    SendUpdateInfo(strInfo);
#endif
	if (IsFileExist(strSttNativeCfgFile))
	{
		strInfo.Format(_T("FILE exit = %s "), strSttNativeCfgFile.GetString());
#ifdef _PSX_IDE_QT_
		SendUpdateInfo(strInfo);
 #endif
		BOOL ret = oSttNativeCfgMngr.OpenXmlFile(strSttNativeCfgFile, CSttNativeCfgXmlRWKeys::g_pXmlKeys);
		POS pos = oSttNativeCfgMngr.GetHeadPosition();
		CBaseObject *p = NULL;
		while(pos && ret)
		{
			p = oSttNativeCfgMngr.GetNext(pos);
			nClassID = p->GetClassID();
			if (nClassID == STTNATIVECFGCLASSID_LIVEUPDATE)
			{
				pCfgLiveUpdate = (CSttNativeCfgLiveUpdate*)p;
				if ((pCfgLiveUpdate != NULL) && (!pCfgLiveUpdate->m_strID_System.IsEmpty()))
				{
#ifdef _PSX_IDE_QT_
					CString str =  g_theLiveUpdateBaseApp->m_oNativeCfgMngr.GetCfgLiveUpdate()->m_strID_System;
					strSystemId = pCfgLiveUpdate->m_strID_System;
					if (str == strSystemId)
					{
						strInfo.Format(_T("当前设备id = %s, 压缩包中系统id = %s"), str.GetString(), strSystemId.GetString());
                        //CLogPrint::LogString(XLOGLEVEL_INFOR,strInfo.GetString());
                        SendUpdateInfo(strInfo);
						return TRUE;
					}
					...
					{
						strInfo.Format(_T("当前设备id = %s, 压缩包中系统id = %s,id不匹配"), str.GetString(), strSystemId.GetString());
                        SendUpdateInfo(strInfo);
						return FALSE;
					}
#endif
    ...
    {
        strInfo.Format(_T("FILE is not exit = %s "), strSttNativeCfgFile.GetString());
#ifdef _PSX_IDE_QT_
        SendUpdateInfo(strInfo);
 #endif
    }
