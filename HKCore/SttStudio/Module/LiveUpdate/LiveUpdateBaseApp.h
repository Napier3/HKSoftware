...
//TestApp
	...
	CString m_strOffLineFilePath;		        //接收离线升级包路径    20240607 xueyangfan
...
public:
	...
	BOOL UpdateTestApp_ConnectOfflineUpdate(const CString &strIPServer,long nPort=LIVEUPDATE_PORT,long nTimeOut=100); //离线升级 连接底层 20240607 xueyangfan
	BOOL UpdateTestApp_SendCmd_Request();
	BOOL UpdateTestApp_SendFile();
	BOOL UpdateTestApp_SendFileOffline();           //离线升级 下传更新包文件到测试仪 20240607 xueyangfan
	BOOL UpdateTestApp_SendFileOver();
	BOOL UpdateTestApp_SendFileOverOffline();       //离线升级 发送命令 20240607 xueyangfan
	...
	BOOL SendCmd_Local();
	BOOL SendCmd_Local(const CString &strOffLineFilePath);	//离线升级 发送命令 20240607 xueyangfan
	BOOL SendCmd_Cloud();
	...
	BOOL CompareSystem();//dingxy 20240628 比较测试仪设备中的id与当前离线升级中的压缩包id是否匹配
