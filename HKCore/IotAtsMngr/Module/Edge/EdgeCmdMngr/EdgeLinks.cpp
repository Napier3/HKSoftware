...
{
	if (nClassID == MNGRCLASSID_CEDGELINK)
	{
		return TRUE;
	}
...
CExBaseObject* CEdgeLinks::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
	...
	if (strClassID == pXmlKeys->m_strCEdgeLinkKey)
	{
		pNew = new CEdgeLink();
	}
