...
CIot_Device::CIot_Device()
{
	...
	m_nTest_State = DEVICE_TEST_STATE_IDLE;
	::GetLocalTime(&m_tmCreate_Time);
	m_nTest_Rslt = DEVICE_TEST_STATE_NORMAL;
	m_strDeviceType = DEVICE_TYPE_TEST;  //默认是被测样品
...
long CIot_Device::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
	...
	xml_GetAttibuteValue(pXmlKeys->m_strTestRsltKey, oNode, m_nTest_Rslt);
...
long CIot_Device::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
	...
	xml_SetAttributeValue(pXmlKeys->m_strTestRsltKey, oElement, m_nTest_Rslt);
...
long CIot_Device::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
	...
	if(oBinaryBuffer.IsCalSizeMode())
	{
		...
		BinarySerializeCalLen(oBinaryBuffer, m_nTest_Rslt);
	...
	else if(oBinaryBuffer.IsReadMode())
	{
		...
		BinarySerializeRead(oBinaryBuffer, m_nTest_Rslt);
	...
	else if(oBinaryBuffer.IsWriteMode())
	{
		...
		BinarySerializeWrite(oBinaryBuffer, m_nTest_Rslt);
...
BOOL CIot_Device::IsEqualOwn(CBaseObject* pObj)
{
	...
	if(m_nTest_Rslt != p->m_nTest_Rslt)
	{
		return FALSE;
	}
...
long CIot_Device::DB_ReadFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
	...
	pRecordset->GetFieldValue(pXmlKeys->m_strTestRsltKey, m_nTest_Rslt);
	pRecordset->GetFieldValue(pXmlKeys->m_strDeviceTypeKey, m_strDeviceType);
	return 0;
}
...
long CIot_Device::DB_WriteFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
	...
	pRecordset->SetFieldValue(pXmlKeys->m_strTestRsltKey, m_nTest_Rslt);
	pRecordset->SetFieldValue(pXmlKeys->m_strDeviceTypeKey, m_strDeviceType);
	return 0;
}

BOOL CIot_Device::QueryByDeviceSN(CXDbBaseLib *pXDbLib,const CString &strDeviceSN)
{
	...
	CString strDeviceTable,strSNKey,strSql;
	strDeviceTable = CIot_AtsMngrXmlRWKeys::CIot_DeviceKey();
	strSNKey = CIot_AtsMngrXmlRWKeys::SnKey();

	strSql.Format(_T("select * from %s where %s=\"%s\" limit 1"),strDeviceTable,strSNKey,strDeviceSN);

	return DB_Select_OwnEx(pXDbLib, CIot_AtsMngrXmlRWKeys::g_pXmlKeys, strSql);
}

BOOL CIot_Device::IsMonitorDevice()
{
	return (m_strDeviceType == DEVICE_TYPE_MONITOR);
}
