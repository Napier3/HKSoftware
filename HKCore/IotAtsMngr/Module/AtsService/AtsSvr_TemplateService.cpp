#include "StdAfx.h"
#include "AtsSvr_TemplateService.h"
...
#include "../Iot_AtsMngrDB/Iot_Template.h"
#include "../Iot_AtsMngrDB/Sys_User.h"
#include "../Iot_AtsMngrDB/Iot_TypeTaskStandard.h"
...
CAtsSvr_TemplateService::CAtsSvr_TemplateService(void)
{
}
...
long CAtsSvr_TemplateService::Process_Cmd_Ats(CXObjectRefDriver &oAtsCmdDrv, CSttProtocolInterface *pSttProtocol)
{
	...
	{
		//2023-6-6：按照最新的修改，导入任务信息，由网站解析写库。自动测试服务不处理该指令
		delete pszCmdID;
		return 0;
		//pSttProtocol->ReturnSysState_REPLY((CSttCmmOptrInterface *)pAtsCmd->GetSttCmmOptrInterface(), STT_CMD_ExecStatus_ACK, &oSysState, pszCmdID, STT_CMD_SYSSTATE_ATS);
...
long CAtsSvr_TemplateService::Process_Cmd_SysState(CXObjectRefDriver &oSysStateDrv, CSttProtocolInterface *pSttProtocol)
{
...
*/
long CAtsSvr_TemplateService::Process_Cmd_Ats_UploadTemplate(CSttAtsCmd *pAtsCmd)
{
...
*/
long CAtsSvr_TemplateService::Process_Cmd_Ats_ImportTaskInfo(CSttAtsCmd *pAtsCmd)
{
	//strFileDownLoad，下载后返回的本地全路径（带文件名）
...
*/
long CAtsSvr_TemplateService::Process_Cmd_Ats_ImportSampleInfo(CSttAtsCmd *pAtsCmd)
{
	/*
	//strFileDownLoad，下载后返回的本地全路径（带文件名）
...
long CAtsSvr_TemplateService::Process_Cmd_Ats_ExportReport(CSttAtsCmd *pAtsCmd, CSttProtocolInterface *pSttProtocol)
{
	BOOL bRet = TRUE;
...
long CAtsSvr_TemplateService::Process_Cmd_SysState_TestAppEvent(CSttSysState *pSysState)
{
...
long CAtsSvr_TemplateService::Process_Cmd_SysState_EngineEvent(CSttSysState *pSysState)
{
	CString strDeviceSN, strEventID;
	pSysState->GetParasDataValueByID(STT_CMD_PARA_DeviceSN, strDeviceSN);
	pSysState->GetParasDataValueByID(SYS_STATE_XEVENT_EVENTID, strEventID);
...
*/
//实时数据，设备和测试仪公用
long CAtsSvr_TemplateService::Process_Cmd_SysState_RTDATA(CSttSysState *pSysState)
{
	CString strDeviceSN, strEventID;
	pSysState->GetParasDataValueByID(STT_CMD_PARA_DeviceSN, strDeviceSN);
...
long CAtsSvr_TemplateService::Process_Cmd_SysState_RTDATA_TestApp(CSttSysState *pSysState, CIot_TestApp *pTestApp)
{
...
long CAtsSvr_TemplateService::Process_Cmd_SysState_RTDATA_Dev(CSttSysState *pSysState, CIot_TypeTaskDev *pDev)
{
...
BOOL CAtsSvr_TemplateService::Export_Report(CSttAtsCmd *pAtsCmd, CSttProtocolInterface *pSttProtocol, const CString &strDocRptFile, CIot_TypeTaskTests *pListTaskTests)
{
...
BOOL CAtsSvr_TemplateService::Export_Report(const CString &strDocRptFile, CIot_TypeTaskTest *pTaskTest)
{
...
BOOL CAtsSvr_TemplateService::DownLoadFile_Template_RptFile(const CString &strFileName, CString &strFileDownLoad)
{
	BOOL bRet = TRUE;
...
BOOL CAtsSvr_TemplateService::DownLoadFile_Template(CSttAtsCmd *pAtsCmd, CString &strFileDownLoad)
{
	CString strHttpUrlHead,strUrl; 
	CSttMqttClientEngineConfig::GetHttpUrlHead(strHttpUrlHead);
	CString strFile, strFile_s, strRootPath;
...
BOOL CAtsSvr_TemplateService::DownLoadFile_TaskInfo(CSttAtsCmd *pAtsCmd, CString &strFileDownLoad)
{
	CString strHttpUrlHead,strUrl; 
	CSttMqttClientEngineConfig::GetHttpUrlHead(strHttpUrlHead);
	CString strFile, strFile_s, strRootPath;
...
BOOL CAtsSvr_TemplateService::DownLoadFile_SampleInfo(CSttAtsCmd *pAtsCmd, CString &strFileDownLoad)
{
	CString strHttpUrlHead,strUrl; 
	CSttMqttClientEngineConfig::GetHttpUrlHead(strHttpUrlHead);
	CString strFile, strFile_s, strRootPath;
...
BOOL CAtsSvr_TemplateService::UploadFile(const CString &strLocalFilePath, CString &strFile_s)
{
...
BOOL CAtsSvr_TemplateService::UploadFile_DvmDevice(const CString &strFile, CString &strFile_s)
{
...
//strFile是绝对路径
BOOL CAtsSvr_TemplateService::UploadFile_RptFile(const CString &strFile, CString &strFile_s)
{
	return UploadFile(strFile, strFile_s);
}

BOOL CAtsSvr_TemplateService::OpenGbxmlFile(const CString &strFilePath, CExBaseList *pItemsList, CGuideBook *pGuideBook)
{
...
//为每一个测试大项，保存一个测试模板
BOOL CAtsSvr_TemplateService::SaveGbxmlFile_Items(const CString &strFileName, CGuideBook *pGuideBook, CExBaseList *pListItems)
{
	BOOL bRet = TRUE;
...
CDataGroup* CAtsSvr_TemplateService::ParseFile_TaskInfo(const CString &strTaskInfoFile)
{
...
CDataGroup* CAtsSvr_TemplateService::ParseFile_SampleInfo(const CString &strSampleInfoFile)
{
...
*/
//样品点表参数，保存模型文件
CString CAtsSvr_TemplateService::GenerateDevDvm(const CString &strTaskSN, CDataGroup *pRatedParameter, CDataGroup *pDevDvm)
{
...
CString CAtsSvr_TemplateService::GenerateDevDvm(const CString &strTaskSN, CIot_TypeTaskDevParases *pRatedParas, CIot_TypeTaskDevModels *pDevDvm)
{
	CString strFileName, strPath;
	strFileName = strTaskSN + _T(".xml");
	strPath = _P_GetInstallPath();
...
//根据用户ID，查询用户名
BOOL CAtsSvr_TemplateService::QueryUserByUserID(long nUser_ID, CString &strName_User)
{
	CSys_User oUser;
	return oUser.QueryUserByUserID(g_pTheMySqlApp, nUser_ID, strName_User);
}

BOOL CAtsSvr_TemplateService::DB_InsertTemplateItems(const CString &strFileName, CExBaseList *pListItems)
{
...
//模板关联的大项，写入数据库表iot_template_items
CIot_TemplateItems* CAtsSvr_TemplateService::DB_InsertTemplateItems_Items(long nIndex_Template, CExBaseList *pItems)
{
...
CIot_TemplateItem* CAtsSvr_TemplateService::DB_InsertTemplateItems_Item(long nIndex_Template, long nIndex_Items, CExBaseList *pItem)
{
...
//待处理
BOOL CAtsSvr_TemplateService::DB_Insert_IotItems(CExBaseList *pListItems)
{
	BOOL bRet = TRUE;
...
*/
//写数据库表iot_type_task，新建委托任务
CIot_TypeTask* CAtsSvr_TemplateService::DB_InsertTypeTask(CDataGroup *pTaskBasicInfo, const CString &strTaskFileName, const CString &strTaskFile_s, const CString &strID_User)
{
...
CIot_TypeTask* CAtsSvr_TemplateService::DB_UpdateTypeTask(long nIndex_TypeTask, CDataGroup *pTaskBasicInfo, const CString &strTaskFileName
...
*/
//写数据库表iot_type_task_standard
BOOL CAtsSvr_TemplateService::DB_InsertTypeTask_Standard(long nIndex_TypeTask, CDataGroup *pInspectionBases)
{
	//检验依据，允许为空
...
*/
//写数据库：任务样品表iot_type_task_dev
BOOL CAtsSvr_TemplateService::DB_InsertTypeTask_Devs(long nIndex_TypeTask, CDataGroup *pSampleDevices)
{
	if (pSampleDevices == NULL)
	{
		return FALSE;
...
BOOL CAtsSvr_TemplateService::DB_InsertTypeTask_RatedParameter(long nIndex_TypeTask, CDataGroup *pRatedParameter)
{
	if (pRatedParameter == NULL)
	{
		//样品额定参数，允许为空
...
BOOL CAtsSvr_TemplateService::DB_InsertTypeTask_DevDvm(long nIndex_TypeTask, CDataGroup *pDevDvm)
{
	//点表参数不允许为空
	if (pDevDvm == NULL)
	{
...
BOOL CAtsSvr_TemplateService::DB_InsertDeviceModel(const CString &strFile, const CString &strFile_s)
{
