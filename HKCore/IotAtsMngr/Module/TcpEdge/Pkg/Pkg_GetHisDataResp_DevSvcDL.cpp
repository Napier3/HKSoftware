...
//Pkg_GetHisDataResp_DevSvcDL.cpp  CPkg_GetHisDataResp_DevSvcDL


#include "stdafx.h"
#include "Pkg_GetHisDataResp_DevSvcDL.h"

CPkg_GetHisDataResp_DevSvcDL::CPkg_GetHisDataResp_DevSvcDL()
{
	//初始化属性
...
BOOL CPkg_GetHisDataResp_DevSvcDL::xml_GetAttibuteValue(const BSTR strAttrName, CXmlRWNodeBase &oNode, CString &strAttrVal)
{
	...
	{
		//值存在时检查类型是否正确
		strTemp = QString("【错误】【必需项错误】GetHisDataResp：[%1]=>[%2$%3$%4$%5]字段类型错误;\n").arg(PkgData_Node_Data).arg(PkgData_Node_Devices).arg(PkgData_Node_Services).arg(PkgData_Node_DataList).arg(strAttr);
		m_strErrorInfo += strTemp;
...
long CPkg_GetHisDataResp_DevSvcDL::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CPkg_GetHisDataResp_DevSvcDL::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
...
long CPkg_GetHisDataResp_DevSvcDL::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
void CPkg_GetHisDataResp_DevSvcDL::InitAfterRead()
{
}

BOOL CPkg_GetHisDataResp_DevSvcDL::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
	...
	CPkg_GetHisDataResp_DevSvcDL *p = (CPkg_GetHisDataResp_DevSvcDL*)pObj;
...
BOOL CPkg_GetHisDataResp_DevSvcDL::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
	...
	CPkg_GetHisDataResp_DevSvcDL *p = (CPkg_GetHisDataResp_DevSvcDL*)pDest;
...
CBaseObject* CPkg_GetHisDataResp_DevSvcDL::Clone()
{
	CPkg_GetHisDataResp_DevSvcDL *p = new CPkg_GetHisDataResp_DevSvcDL();
	Copy(p);
	return p;
}

long CPkg_GetHisDataResp_DevSvcDL::InitListCtrlColumns(CListCtrl *pListCtrl)
{
...
long CPkg_GetHisDataResp_DevSvcDL::UpdateListCtrl(CListCtrl *pListCtrl, long lItemIndex)
{
	CString strTemp;
...
BOOL CPkg_GetHisDataResp_DevSvcDL::CanPaste(UINT nClassID)
{
	return FALSE;
}

CExBaseObject* CPkg_GetHisDataResp_DevSvcDL::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
...
CExBaseObject* CPkg_GetHisDataResp_DevSvcDL::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
...
long CPkg_GetHisDataResp_DevSvcDL::DB_ReadFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
...
long CPkg_GetHisDataResp_DevSvcDL::DB_WriteFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
