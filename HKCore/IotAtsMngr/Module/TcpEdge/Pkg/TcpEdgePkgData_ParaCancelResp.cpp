...
//TcpEdgePkgData_ParaCancelResp.cpp  CTcpEdgePkgData_ParaCancelResp


#include "stdafx.h"
#include "TcpEdgePkgData_ParaCancelResp.h"

CTcpEdgePkgData_ParaCancelResp::CTcpEdgePkgData_ParaCancelResp()
{
	//初始化属性
...
BOOL CTcpEdgePkgData_ParaCancelResp::xml_GetAttibuteValue(const BSTR strAttrName, CXmlRWNodeBase &oNode, CString &strAttrVal)
{
	...
	if(!strAttrVal.GetLength())
	{
		...
		strTemp = QString("【错误】【必需项缺失】ParaCancelResp：[%1]=>[%2]必需字段缺失;\n").arg(strID).arg(strAttr);
	...
	{
		//值存在时检查类型是否正确
		strTemp = QString("【错误】【必需项错误】ParaCancelResp：[%1]=>[%2]字段类型错误;\n").arg(strID).arg(strAttr);
		m_strErrorInfo += strTemp;
...
long CTcpEdgePkgData_ParaCancelResp::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_ParaCancelResp::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_ParaCancelResp::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
void CTcpEdgePkgData_ParaCancelResp::InitAfterRead()
{
}

BOOL CTcpEdgePkgData_ParaCancelResp::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
	...
	CTcpEdgePkgData_ParaCancelResp *p = (CTcpEdgePkgData_ParaCancelResp*)pObj;
...
BOOL CTcpEdgePkgData_ParaCancelResp::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
	...
	CTcpEdgePkgData_ParaCancelResp *p = (CTcpEdgePkgData_ParaCancelResp*)pDest;
...
CBaseObject* CTcpEdgePkgData_ParaCancelResp::Clone()
{
	CTcpEdgePkgData_ParaCancelResp *p = new CTcpEdgePkgData_ParaCancelResp();
	Copy(p);
	return p;
}

long CTcpEdgePkgData_ParaCancelResp::InitListCtrlColumns(CListCtrl *pListCtrl)
{
...
long CTcpEdgePkgData_ParaCancelResp::UpdateListCtrl(CListCtrl *pListCtrl, long lItemIndex)
{
	CString strTemp;
...
BOOL CTcpEdgePkgData_ParaCancelResp::CanPaste(UINT nClassID)
{
	return FALSE;
}

CExBaseObject* CTcpEdgePkgData_ParaCancelResp::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
...
CExBaseObject* CTcpEdgePkgData_ParaCancelResp::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
...
long CTcpEdgePkgData_ParaCancelResp::DB_ReadFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_ParaCancelResp::DB_WriteFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
