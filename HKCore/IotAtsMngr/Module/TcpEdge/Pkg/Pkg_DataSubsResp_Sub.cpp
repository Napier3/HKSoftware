...
BOOL CPkg_DataSubsResp_Sub::xml_GetAttibuteValue(const BSTR strAttrName, CXmlRWNodeBase &oNode, CString &strAttrVal)
{
	...
	{
		//值存在时检查类型是否正确
		strTemp = QString("【错误】【必需项错误】DataSubsResp：[%1]=>[%2$%3]字段类型错误;\n").arg(PkgData_Node_Data).arg(PkgData_Node_Subs).arg(strAttr);
		m_strErrorInfo += strTemp;
...
long CPkg_DataSubsResp_Sub::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
//	CPxPkgDataBase::XmlReadOwn(oNode, pXmlRWKeys);
	...
	if (pDatasJson)
	{
		CPkg_DataSubsResp_SubSvcs* pDatas = new CPkg_DataSubsResp_SubSvcs;
		pDatas->m_strID = pDatas->m_strName = PkgData_Node_Services;
		AddNewChild(pDatas);
		CJSON* pDatasChild = pDatasJson->child;
		while (pDatasChild)
		{
			CPkg_DataSubsResp_SubSvc* pData = new CPkg_DataSubsResp_SubSvc;
...
long CPkg_DataSubsResp_Sub::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
//	CPxPkgDataBase::XmlWriteOwn(oXMLDoc, oElement, pXmlRWKeys);
	...
	CPkg_DataSubsResp_SubSvcs* pDatas = (CPkg_DataSubsResp_SubSvcs*)FindByID(PkgData_Node_Services);

	CJSON* pDatasJson = CJSON_CreateArray();
	POS pos = pDatas->GetHeadPosition();
	while (pos)
	{
		CPkg_DataSubsResp_SubSvc* pData = (CPkg_DataSubsResp_SubSvc*)pDatas->GetNext(pos);
...
long CPkg_DataSubsResp_Sub::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
void CPkg_DataSubsResp_Sub::InitAfterRead()
{
}

BOOL CPkg_DataSubsResp_Sub::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
	...
	CPkg_DataSubsResp_Sub *p = (CPkg_DataSubsResp_Sub*)pObj;
...
BOOL CPkg_DataSubsResp_Sub::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
	...
	CPkg_DataSubsResp_Sub *p = (CPkg_DataSubsResp_Sub*)pDest;

	p->m_strModel = m_strModel;
	return TRUE;
}

CBaseObject* CPkg_DataSubsResp_Sub::Clone()
{
	CPkg_DataSubsResp_Sub *p = new CPkg_DataSubsResp_Sub();
	Copy(p);
	return p;
}

long CPkg_DataSubsResp_Sub::InitListCtrlColumns(CListCtrl *pListCtrl)
{
...
long CPkg_DataSubsResp_Sub::UpdateListCtrl(CListCtrl *pListCtrl, long lItemIndex)
{
	CString strTemp;
...
BOOL CPkg_DataSubsResp_Sub::CanPaste(UINT nClassID)
{
	return FALSE;
}

CExBaseObject* CPkg_DataSubsResp_Sub::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
...
CExBaseObject* CPkg_DataSubsResp_Sub::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
...
long CPkg_DataSubsResp_Sub::DB_ReadFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
...
long CPkg_DataSubsResp_Sub::DB_WriteFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
