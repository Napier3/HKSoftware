...
//Pkg_DataReadDev.cpp  CPkg_DataReadDev
...
CPkg_DataReadDev::CPkg_DataReadDev()
{
	//初始化属性
...
long CPkg_DataReadDev::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
//	CPxPkgDataBase::XmlReadOwn(oNode, pXmlRWKeys);
	...
	if (pDatasJson)
	{
		CPkg_DataReadDevSvcs* pDatas = new CPkg_DataReadDevSvcs;
		pDatas->m_strID = pDatas->m_strName = PkgData_Node_Services;
		AddNewChild(pDatas);
		CJSON* pDatasChild = pDatasJson->child;
		while (pDatasChild)
		{
			CPkg_DataReadDevSvc* pData = new CPkg_DataReadDevSvc;
...
long CPkg_DataReadDev::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
//	CPxPkgDataBase::XmlWriteOwn(oXMLDoc, oElement, pXmlRWKeys);
	...
	CPkg_DataReadDevSvcs* pDatas = (CPkg_DataReadDevSvcs*)FindByID(PkgData_Node_Services);

	CJSON* pDatasJson = CJSON_CreateArray();
	POS pos = pDatas->GetHeadPosition();
	while (pos)
	{
		CPkg_DataReadDevSvc* pData = (CPkg_DataReadDevSvc*)pDatas->GetNext(pos);
...
long CPkg_DataReadDev::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
void CPkg_DataReadDev::InitAfterRead()
{
}

BOOL CPkg_DataReadDev::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
	...
	CPkg_DataReadDev *p = (CPkg_DataReadDev*)pObj;
...
BOOL CPkg_DataReadDev::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
	...
	CPkg_DataReadDev *p = (CPkg_DataReadDev*)pDest;

	p->m_strDeviceId = m_strDeviceId;
	return TRUE;
}

CBaseObject* CPkg_DataReadDev::Clone()
{
	CPkg_DataReadDev *p = new CPkg_DataReadDev();
	Copy(p);
	return p;
}

long CPkg_DataReadDev::InitListCtrlColumns(CListCtrl *pListCtrl)
{
...
long CPkg_DataReadDev::UpdateListCtrl(CListCtrl *pListCtrl, long lItemIndex)
{
	CString strTemp;
...
BOOL CPkg_DataReadDev::CanPaste(UINT nClassID)
{
	return FALSE;
}

CExBaseObject* CPkg_DataReadDev::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
...
CExBaseObject* CPkg_DataReadDev::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
...
long CPkg_DataReadDev::DB_ReadFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
...
long CPkg_DataReadDev::DB_WriteFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
...
CDvmDataset* CPkg_DataReadDev::ToDataset()
{
	...
	CTcpEdgePkgData::AddNewData(pDataset, CPkg_DataReadDevSvcs::ToDataset());

	return pDataset;
}

BOOL CPkg_DataReadDev::NewData(CDvmDatasetGroup* pArray)
{
	CPkg_DataReadDevSvc* pObj = new CPkg_DataReadDevSvc;
	AddNewChild(pObj);

	CDvmDataset* pDataset = CPkg_DataReadDevSvc::ToDataset();
	pDataset->m_strID = "";
