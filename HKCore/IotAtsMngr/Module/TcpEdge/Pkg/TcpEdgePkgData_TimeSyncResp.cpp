...
//TcpEdgePkgData_TimeSyncResp.cpp  CTcpEdgePkgData_TimeSyncResp
...
CTcpEdgePkgData_TimeSyncResp::CTcpEdgePkgData_TimeSyncResp()
{
	//初始化属性
...
BOOL CTcpEdgePkgData_TimeSyncResp::xml_GetAttibuteValue(const BSTR strAttrName, CXmlRWNodeBase &oNode, CString &strAttrVal)
{
	...
	if(!strAttrVal.GetLength())
	{
		...
		strTemp = QString("【错误】【必需项缺失】TimeSyncResp：[%1]=>[%2]必需字段缺失;\n").arg(strID).arg(strAttr);
	...
	{
		//值存在时检查类型是否正确
		strTemp = QString("【错误】【必需项错误】TimeSyncResp：[%1]=>[%2]字段类型错误;\n").arg(strID).arg(strAttr);
		m_strErrorInfo += strTemp;
...
long CTcpEdgePkgData_TimeSyncResp::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_TimeSyncResp::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_TimeSyncResp::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
void CTcpEdgePkgData_TimeSyncResp::InitAfterRead()
{
}

BOOL CTcpEdgePkgData_TimeSyncResp::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
	...
	CTcpEdgePkgData_TimeSyncResp *p = (CTcpEdgePkgData_TimeSyncResp*)pObj;
...
BOOL CTcpEdgePkgData_TimeSyncResp::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
	...
	CTcpEdgePkgData_TimeSyncResp *p = (CTcpEdgePkgData_TimeSyncResp*)pDest;
...
CBaseObject* CTcpEdgePkgData_TimeSyncResp::Clone()
{
	CTcpEdgePkgData_TimeSyncResp *p = new CTcpEdgePkgData_TimeSyncResp();
	Copy(p);
	return p;
}

CBaseObject* CTcpEdgePkgData_TimeSyncResp::CloneEx(BOOL bCopyOwn/*=TRUE*/, BOOL bCopyChildren/*=FALSE*/)
{
	CTcpEdgePkgData_TimeSyncResp *p = new CTcpEdgePkgData_TimeSyncResp();
...
BOOL CTcpEdgePkgData_TimeSyncResp::CanPaste(UINT nClassID)
{
	return FALSE;
}

CExBaseObject* CTcpEdgePkgData_TimeSyncResp::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
...
CExBaseObject* CTcpEdgePkgData_TimeSyncResp::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
...
long CTcpEdgePkgData_TimeSyncResp::DB_ReadFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_TimeSyncResp::DB_WriteFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
