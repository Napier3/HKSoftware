...
//TcpEdgePkgData_AppRemoveResp.cpp  CTcpEdgePkgData_AppRemoveResp
...
CTcpEdgePkgData_AppRemoveResp::CTcpEdgePkgData_AppRemoveResp()
{
	//初始化属性
...
BOOL CTcpEdgePkgData_AppRemoveResp::xml_GetAttibuteValue(const BSTR strAttrName, CXmlRWNodeBase &oNode, CString &strAttrVal)
{
	...
	if(!strAttrVal.GetLength())
	{
		...
		strTemp = QString("【错误】【必需项缺失】AppRemoveResp：[%1]=>[%2]必需字段缺失;\n").arg(strID).arg(strAttr);
	...
	{
		//值存在时检查类型是否正确
		strTemp = QString("【错误】【必需项错误】AppRemoveResp：[%1]=>[%2]字段类型错误;\n").arg(strID).arg(strAttr);
		m_strErrorInfo += strTemp;
...
long CTcpEdgePkgData_AppRemoveResp::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_AppRemoveResp::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_AppRemoveResp::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
void CTcpEdgePkgData_AppRemoveResp::InitAfterRead()
{
}

BOOL CTcpEdgePkgData_AppRemoveResp::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
	...
	CTcpEdgePkgData_AppRemoveResp *p = (CTcpEdgePkgData_AppRemoveResp*)pObj;
...
BOOL CTcpEdgePkgData_AppRemoveResp::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
	...
	CTcpEdgePkgData_AppRemoveResp *p = (CTcpEdgePkgData_AppRemoveResp*)pDest;
...
CBaseObject* CTcpEdgePkgData_AppRemoveResp::Clone()
{
	CTcpEdgePkgData_AppRemoveResp *p = new CTcpEdgePkgData_AppRemoveResp();
	Copy(p);
	return p;
}

CBaseObject* CTcpEdgePkgData_AppRemoveResp::CloneEx(BOOL bCopyOwn/*=TRUE*/, BOOL bCopyChildren/*=FALSE*/)
{
	CTcpEdgePkgData_AppRemoveResp *p = new CTcpEdgePkgData_AppRemoveResp();
...
BOOL CTcpEdgePkgData_AppRemoveResp::CanPaste(UINT nClassID)
{
	return FALSE;
}

CExBaseObject* CTcpEdgePkgData_AppRemoveResp::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
...
CExBaseObject* CTcpEdgePkgData_AppRemoveResp::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
...
long CTcpEdgePkgData_AppRemoveResp::DB_ReadFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_AppRemoveResp::DB_WriteFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
