...
//TcpEdgePkgData_ConRemoveResp.cpp  CTcpEdgePkgData_ConRemoveResp
...
CTcpEdgePkgData_ConRemoveResp::CTcpEdgePkgData_ConRemoveResp()
{
	//初始化属性
...
BOOL CTcpEdgePkgData_ConRemoveResp::xml_GetAttibuteValue(const BSTR strAttrName, CXmlRWNodeBase &oNode, CString &strAttrVal)
{
	...
	if(!strAttrVal.GetLength())
	{
		...
		strTemp = QString("【错误】【必需项缺失】ConRemoveResp：[%1]=>[%2]必需字段缺失;\n").arg(strID).arg(strAttr);
	...
	{
		//值存在时检查类型是否正确
		strTemp = QString("【错误】【必需项错误】ConRemoveResp：[%1]=>[%2]字段类型错误;\n").arg(strID).arg(strAttr);
		m_strErrorInfo += strTemp;
...
long CTcpEdgePkgData_ConRemoveResp::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_ConRemoveResp::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_ConRemoveResp::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
void CTcpEdgePkgData_ConRemoveResp::InitAfterRead()
{
}

BOOL CTcpEdgePkgData_ConRemoveResp::IsEqualOwn(CBaseObject* pObj)
{
	if(this == pObj)
	{
		return TRUE;
	...
	CTcpEdgePkgData_ConRemoveResp *p = (CTcpEdgePkgData_ConRemoveResp*)pObj;
...
BOOL CTcpEdgePkgData_ConRemoveResp::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
	...
	CTcpEdgePkgData_ConRemoveResp *p = (CTcpEdgePkgData_ConRemoveResp*)pDest;
...
CBaseObject* CTcpEdgePkgData_ConRemoveResp::Clone()
{
	CTcpEdgePkgData_ConRemoveResp *p = new CTcpEdgePkgData_ConRemoveResp();
	Copy(p);
	return p;
}

CBaseObject* CTcpEdgePkgData_ConRemoveResp::CloneEx(BOOL bCopyOwn/*=TRUE*/, BOOL bCopyChildren/*=FALSE*/)
{
	CTcpEdgePkgData_ConRemoveResp *p = new CTcpEdgePkgData_ConRemoveResp();
...
BOOL CTcpEdgePkgData_ConRemoveResp::CanPaste(UINT nClassID)
{
	return FALSE;
}

CExBaseObject* CTcpEdgePkgData_ConRemoveResp::CreateNewChild(const CString &strClassID, BOOL &bAddToTail, CXmlRWKeys *pXmlRWKeys)
{
...
CExBaseObject* CTcpEdgePkgData_ConRemoveResp::CreateNewChild(long nClassID/*, BOOL &bAddToTail*/)
{
	CExBaseObject *pNew  = NULL;
...
long CTcpEdgePkgData_ConRemoveResp::DB_ReadFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
...
long CTcpEdgePkgData_ConRemoveResp::DB_WriteFields(CXDbRecordsetBase *pRecordset, CXmlRWKeys *pXmlRWKeys)
{
